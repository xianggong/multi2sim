...
/* THIS FILE :
 * /home/yifan/Documents/hsa/multi2sim/samples/hsa/histogram/kernels.snackwrap.c
 */
/* INPUT FILE:
 * /home/yifan/Documents/hsa/multi2sim/samples/hsa/histogram/kernels.cl  */
...
/*
  ...
  (collectively, the
  Materials) pursuant to the terms and conditions of the Software License
  Agreement included with the
  Materials.  If you do not have a copy of the Software License Agreement,
  contact your AMD
  representative for a copy.

  You agree that you will not reverse engineer or decompile the Materials, in
  whole or in part, except for
  example code which is provided in source code form and as allowed by
  applicable law.
  ...
  PURPOSE, TITLE, NON-INFRINGEMENT, THAT THE SOFTWARE WILL RUN UNINTERRUPTED OR
  ERROR-
  FREE OR WARRANTIES ARISING FROM CUSTOM OF TRADE OR COURSE OF USAGE.  THE
  ENTIRE RISK
  ...
  OF SUCH
  DAMAGES.  In no event shall AMD's total liability to You for all damages,
  losses, and
  causes of action (whether in contract, tort (including negligence) or
  otherwise)
  exceed the amount of $100 USD.  You agree to defend, indemnify and hold
  harmless
  AMD and its licensors, and any of their directors, officers, employees,
  affiliates or
  agents from and against any and all loss, damage, liability and other expenses
  (including reasonable attorneys' fees), resulting from Your use of the
  Software or
  ...
  restrictions as set
  forth in FAR 52.227-14 and DFAR252.227-7013, et seq., or its successor.  Use
  of the
  Materials by the Government constitutes acknowledgement of AMD's proprietary
  rights in them.
...
enum status_t { STATUS_SUCCESS = 0, STATUS_UNKNOWN = 1 };
#ifdef __cplusplus
...
#define SNK_INIT_LPARM(X, Y)                     \
  snk_lparm_t* X;                                \
  snk_lparm_t _##X = {.ndim = 1,                 \
                      .gdims = {Y},              \
                      .ldims = {64},             \
                      .stream = -1,              \
                      .barrier = SNK_UNORDERED,  \
                      .acquire_fence_scope = 2,  \
                      .release_fence_scope = 2}; \
  X = &_##X;
...
status_t _kernels_InitContext() {
  ...
  err = hsa_agent_get_info(_kernels_Agent, HSA_AGENT_INFO_QUEUE_MAX_SIZE,
                           &queue_size);
  ...
  err = hsa_ext_program_add_module(_kernels_HsaProgram,
                                   (hsa_ext_module_t)_kernels_HSA_BrigMem);
  ...
  err =
      hsa_ext_program_finalize(_kernels_HsaProgram, isa, 0, control_directives,
  ...
  err = hsa_executable_load_code_object(_kernels_Executable, _kernels_Agent,
                                        code_object, "");
  ...
  hsa_agent_iterate_regions(_kernels_Agent, get_kernarg_memory_region,
                            &_kernels_KernargRegion);
  err = (_kernels_KernargRegion.handle == (uint64_t)-1) ? HSA_STATUS_ERROR
                                                        : HSA_STATUS_SUCCESS;
  ErrorCheck(Finding a kernarg memory region, err);

  /*  Create a queue using the maximum size.  */
  err = hsa_queue_create(_kernels_Agent, queue_size, HSA_QUEUE_TYPE_SINGLE,
                         NULL, NULL, UINT32_MAX, UINT32_MAX, &Sync_CommandQ);
  ErrorCheck(Creating the queue, err);

  /*  Create signal to wait for the dispatch to finish. this Signal is only used
...
void barrier_sync(int stream_num, snk_task_t* dep_task_list) {
  ...
  for (barrier_pkt_id = 0; barrier_pkt_id < barrier_pkt_count;
    ...
    barrier->header |= HSA_FENCE_SCOPE_SYSTEM
...
extern void stream_sync(int stream_num) {
  /* This is a user-callable function that puts a barrier packet into a queue
     where
     all former dispatch packets were put on the queue for asynchronous
     asynchrnous
     executions. This routine will wait for all packets to complete on this
     queue.
  */
  ...
  hsa_barrier_or_packet_t* barrier =
      &(((hsa_barrier_or_packet_t*)(queue->base_address))[index & queueMask]);
...
extern void HIST(int* input, int* output, int colors, int size,
  ...
  /* HSA 1.0F has a bug that serializes all queue operations when
     hsa_memory_allocate is used.
  ...
  hsa_kernel_dispatch_packet_t* this_aql = &(((
      hsa_kernel_dispatch_packet_t*)(this_Q->base_address))[index & queueMask]);
  ...
  this_aql->setup |= (uint16_t)lparm->ndim
  ...
  this_aql->group_segment_size =
      group_base; /* group_base includes static and dynamic group space */

  /*  Prepare and set the packet header */
  /* Only set barrier bit if asynchrnous execution */
  if (stream_num >= 0)
    this_aql->header |= lparm->barrier << HSA_PACKET_HEADER_BARRIER;
  this_aql->header |= lparm->acquire_fence_scope
                      << HSA_PACKET_HEADER_ACQUIRE_FENCE_SCOPE;
  this_aql->header |= lparm->release_fence_scope
                      << HSA_PACKET_HEADER_RELEASE_FENCE_SCOPE;
  __atomic_store_n((uint8_t*)(&this_aql->header),
...
extern status_t HIST_init(const int printStats) {
  ...
  /* printf("Kernel name HIST_: Looking for symbol
   * %s\n","__OpenCL_HIST_kernel"); */
  err = hsa_executable_get_symbol(_kernels_Executable, NULL,
                                  "&__OpenCL_HIST_kernel", _kernels_Agent, 0,
                                  &HIST_Symbol);
  ErrorCheck(Extract the symbol from the executable, err);

  /* Extract dispatch information from the symbol */
  err = hsa_executable_symbol_get_info(HIST_Symbol,
                                       HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT,
                                       &HIST_Kernel_Object);
  ErrorCheck(Extracting the symbol from the executable, err);
  err = hsa_executable_symbol_get_info(
      HIST_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_KERNARG_SEGMENT_SIZE,
      &HIST_Kernarg_Segment_Size);
  ErrorCheck(Extracting the kernarg segment size from the executable, err);
  err = hsa_executable_symbol_get_info(
      HIST_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_GROUP_SEGMENT_SIZE,
      &HIST_Group_Segment_Size);
  ErrorCheck(Extracting the group segment size from the executable, err);
  err = hsa_executable_symbol_get_info(
      HIST_Symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_PRIVATE_SEGMENT_SIZE,
      &HIST_Private_Segment_Size);
