...
struct cuda_device_t *cuda_device_create() {
  ...
  device->attributes[CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURECUBEMAP_LAYERED_WIDTH] =
      16384;
  device
      ->attributes[CU_DEVICE_ATTRIBUTE_MAXIMUM_TEXTURECUBEMAP_LAYERED_LAYERS] =
      2046;
  ...
  device->attributes[CU_DEVICE_ATTRIBUTE_MAXIMUM_SURFACECUBEMAP_LAYERED_WIDTH] =
      32768;
  device
      ->attributes[CU_DEVICE_ATTRIBUTE_MAXIMUM_SURFACECUBEMAP_LAYERED_LAYERS] =
      2046;
  ...
  if (device->fd < 0)
    fatal(
        "[CUDA Runtime] Could not open /dev/kepler\n\n"
