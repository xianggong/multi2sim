...
int elf_buffer_read(struct elf_buffer_t *buffer, void *ptr, int size) {
  ...
  /* Read bytes */
  if (ptr) memcpy(ptr, buffer->ptr + buffer->pos, size);
...
void elf_buffer_dump(struct elf_buffer_t *buffer, FILE *f) {
  ...
  if (size != buffer->size)
    fatal("%s: couldn't dump buffer contents", __FUNCTION__);
}
...
static void elf_file_read_symbol_section(struct elf_file_t *elf_file,
  ...
  symbol_names_section =
      list_get(elf_file->section_list, section->header->sh_link);
...
struct elf_symbol_t *elf_symbol_get_by_address(struct elf_file_t *elf_file,
  ...
  for (;;) {
    ...
    if (!prev_symbol || prev_symbol->value != symbol->value) break;
    symbol = prev_symbol;
  }

  /* Return the symbol and its address */
  if (offset_ptr) *offset_ptr = addr - symbol->value;
  return symbol;
}

struct elf_symbol_t *elf_symbol_get_by_name(struct elf_file_t *elf_file,
                                            char *name) {
  struct elf_symbol_t *symbol;
  int i;
...
int elf_symbol_read_content(struct elf_file_t *elf_file,
                            struct elf_symbol_t *symbol,
...
static void elf_file_read_elf_header(struct elf_file_t *elf_file) {
  ...
  if (count < EI_NIDENT) fatal("%s: not a valid ELF file\n", elf_file->path);
  ...
  elf_debug("  ehdr.e_ident: EI_CLASS=%d, EI_DATA=%d, EI_VERSION=%d\n",
            elf_header->e_ident[4], elf_header->e_ident[5],
...
static void elf_file_read_section_headers(struct elf_file_t *elf_file) {
  ...
  for (i = 0; i < elf_header->e_shnum; i++) {
    ...
    if (count < sizeof(Elf32_Shdr))
      fatal("%s: unexpected end of file while reading section headers",
            elf_file->path);
    ...
    if (section->header->sh_type != 8) {
      ...
      if (section->buffer.ptr + section->buffer.size >
        ...
        fatal(
            "section %d out of the ELF boundaries (offs=0x%x, size=%u, "
            "ELF_size=%u)",
            i, section->header->sh_offset, section->header->sh_size,
            buffer->size);
    }
  ...
  elf_file->string_table =
      list_get(elf_file->section_list, elf_header->e_shstrndx);
  assert(elf_file->string_table->header->sh_type == 3);
  for (i = 0; i < list_count(elf_file->section_list); i++) {
    section = list_get(elf_file->section_list, i);
    section->name =
        elf_file->string_table->buffer.ptr + section->header->sh_name;
  }
  ...
  for (i = 0; i < list_count(elf_file->section_list); i++) {
    ...
    elf_debug("%-3d %-4u %-5u 0x%-8x 0x%-8x %-9u %-8u %s\n", i,
              section->header->sh_type, section->header->sh_flags,
              section->header->sh_addr, section->header->sh_offset,
              section->header->sh_size, section->header->sh_link,
              section->name);
  }
  elf_debug("\n");
}
...
static void elf_file_read_program_headers(struct elf_file_t *elf_file) {
  ...
  if (!elf_header->e_phnum) return;

  /* Check program header size */
  if (elf_header->e_phentsize != sizeof(Elf32_Phdr))
    fatal("%s: program header size %d (should be %d)", elf_file->path,
          elf_header->e_phentsize, (int)sizeof(Elf32_Phdr));
  ...
  for (i = 0; i < elf_header->e_phnum; i++) {
    ...
    if (count < sizeof(Elf32_Phdr))
      fatal("%s: unexpected end of file while reading program headers",
            elf_file->path);
  ...
  elf_debug(
      "idx type       offset     vaddr      paddr      filesz    memsz     "
      "flags  align\n");
  for (i = 0; i < 80; i++) elf_debug("-");
  elf_debug("\n");
  for (i = 0; i < list_count(elf_file->program_header_list); i++) {
    ...
    elf_debug("%-3d 0x%-8x 0x%-8x 0x%-8x 0x%-8x %-9u %-9u %-6u %u\n", i,
              program_header->header->p_type, program_header->header->p_offset,
              program_header->header->p_vaddr, program_header->header->p_paddr,
...
static struct elf_file_t *elf_file_create_from_allocated_buffer(
    void *buffer, int size, const char *path) {
...
struct elf_file_t *elf_file_create_from_buffer(void *ptr, int size,
                                               char *name) {
  struct elf_file_t *elf_file;
  void *ptr_copy;
...
struct elf_file_t *elf_file_create_from_path(const char *path) {
  ...
  /* Get file size */
  if (stat(path, &st)) fatal("'%s': path not found", path);
  size = st.st_size;

  /* Open file */
  f = fopen(path, "rt");
  if (!f) fatal("'%s': cannot open file", path);

  /* Read file contents */
  buffer = xmalloc(size);
  count = fread(buffer, 1, size, f);
  if (count != size) fatal("'%s': error reading file contents", path);
...
void elf_file_read_header(char *path, Elf32_Ehdr *ehdr) {
  ...
  if (count != 1) fatal("%s: invalid ELF file", path);
