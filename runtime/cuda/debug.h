...
#define debug_tab_inc(category) \
  ((category) ? __debug_tab_inc((category)) : (void)0)
#define debug_tab_dec(category) \
  ((category) ? __debug_tab_dec((category)) : (void)0)
...
#define debug(category, ...) \
  ((category) ? __debug((category), __VA_ARGS__) : (void)0)
void __debug(int category, char* fmt, ...)
    __attribute__((format(printf, 2, 3)));

/* Dump a buffer */
#define debug_buffer(category, buffer_name, buffer, size)                   \
  ((category) ? __debug_buffer((category), (buffer_name), (buffer), (size)) \
              : (void)0)
