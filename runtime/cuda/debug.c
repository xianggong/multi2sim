...
void debug_done(void) {
  ...
  for (i = 0; i < list_count(debug_category_list); i++) {
    ...
    if (c->file_name) free(c->file_name);
    if (c->f && c->f != stdout && c->f != stderr) fclose(c->f);
    free(c);
  }
...
int debug_new_category(char *file_name) {
  ...
  if (!strcmp(file_name, "stdout")) {
    ...
    if (!c->f) fatal("%s: cannot open debug file", file_name);
  }
...
void __debug_tab_inc(int category, int space_count) {
  ...
  assert(c);
  c->space_count += space_count;
  if (c->space_count < 0) c->space_count = 0;
}
...
void __debug(int category, char *fmt, ...) {
  ...
  c = list_get(debug_category_list, category);
  assert(c);
  if (c->status == debug_status_off) return;

  /* Print spaces */
  if (c->space_count >= sizeof(spc)) c->space_count = sizeof(spc) - 1;
