...
struct cuda_stream_command_t *cuda_stream_command_create(
    CUstream stream, cuda_stream_command_func_t func,
    struct memory_args_t *m_args, struct kernel_args_t *k_args,
    struct event_args_t *e_args, struct callback_t *cb) {
  ...
  if (e_args != NULL) command->e_args.event = e_args->event;
  if (cb != NULL) {
