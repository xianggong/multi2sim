...
#define __CUDA_NOT_IMPL__ \
  warning("%s: not implemented.\n%s", __func__, cuda_err_not_impl)
...
void cuda_debug(char *fmt, ...) {
  ...
  if (!cuda_debug_init) {
    ...
    if (cuda_debug_file_name) {
      ...
      if (!cuda_debug_file)
        fatal("%s: cannot open debug output file", cuda_debug_file_name);
    }
  }

  /* Exit if not enabled */
  if (!cuda_debug_file) return;
...
void versionCheck(void) {
  ...
  if (ret) fatal("[CUDA Runtime] %s: Unexpected error", __FUNCTION__);
  ...
  if (version.major != CUDA_VERSION_MAJOR || version.minor < CUDA_VERSION_MINOR)
    fatal(
        "%s:%d: incompatible CUDA versions. Guest library v. %d.%d / "
        "Host implementation v. %d.%d.\n\t%s",
        __FILE__, __LINE__, CUDA_VERSION_MAJOR, CUDA_VERSION_MINOR,
...
CUresult cuDeviceGetAttribute(int *pi, CUdevice_attribute attrib,
  ...
  /* Get attribute */
  *pi = (((struct cuda_device_t *)list_get(device_list, dev))
             ->attributes)[attrib];
...
CUresult cuDeviceGetProperties(CUdevprop *prop, CUdevice dev) {
  ...
  prop->maxGridSize[0] = device->attributes[CU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_X];
  prop->maxGridSize[1] = device->attributes[CU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_Y];
  prop->maxGridSize[2] = device->attributes[CU_DEVICE_ATTRIBUTE_MAX_GRID_DIM_Z];
...
CUresult cuModuleLoad(CUmodule *module, const char *fname) {
  ...
  *module = cuda_module_create(fname);
  ...
  if (ret) fatal("%s: not implemented", __FUNCTION__);
...
CUresult cuMemcpyAsync(CUdeviceptr dst, CUdeviceptr src, size_t ByteCount,
  ...
  cuda_debug("CUDA driver API '%s'", __func__);
  cuda_debug("\t(driver) '%s' in: dst = 0x%08x, stream id = %d", __func__, dst,
             hStream->id);
  cuda_debug("\t(driver) '%s' in: src = 0x%08x, stream id = %d", __func__, src,
             hStream->id);
  cuda_debug("\t(driver) '%s' in: ByteCount = %d, stream id = %d", __func__,
             ByteCount, hStream->id);
  cuda_debug("\t(driver) '%s' in: hStream = [%p], strean id = %d", __func__,
             hStream, hStream->id);
  ...
  if (hStream == 0) hStream = list_get(active_device->stream_list, 0);

  command = cuda_stream_command_create(hStream, cuMemcpyAsyncImpl, args, NULL,
                                       NULL, NULL);
  ...
  cuda_debug("\t(driver) '%s' out: return = %d stream id = %d", __func__,
             CUDA_SUCCESS, hStream->id);
...
CUresult cuLaunchKernel(CUfunction f, unsigned int gridDimX,
  ...
  cuda_debug("\t(driver) '%s' in: hStream id = [%d]", __func__, hStream->id);

  cuda_debug("\t(driver) '%s' in: kernelParams = [%p]", __func__, kernelParams);
  ...
  for (i = 0; i < list_count(stream->command_list); ++i) {
    ...
    if (command->func == cuLaunchKernelImpl) break;
  }
