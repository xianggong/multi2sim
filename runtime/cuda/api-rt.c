...
#define __CUDART_NOT_IMPL__ \
  warning("%s: not implemented.\n%s", __func__, cuda_rt_err_not_impl)
...
void __cudaRegisterFunction(void **fatCubinHandle, const char *hostFun,
  ...
  cuda_debug("\t(runtime) '%s' in: thread_limit = %d", __func__, thread_limit);
  ...
  if (env)
    ...
    dev_func_bin_sec = (unsigned char *)(*(struct {
                         int m;
                         int v;
    ...
    dev_func_bin_sec_size = (((unsigned)dev_func_bin_sec[8]) |
                             ...
                             (((unsigned)dev_func_bin_sec[11]) << 24)) +
                            16u;
    ...
    for (i = 16; i < dev_func_bin_sec_size; ++i) {
      if (!strncmp((char *)dev_func_bin_sec + i, identifier,
                   strlen(identifier))) {
        elf_start = (unsigned char *)dev_func_bin_sec +
                    (i + strlen(identifier) + 8) / 8 * 8;
        break;
      }
    }
    ...
    for (i = section_header_count - 1; i >= 1; --i) {
      if ((((Elf32_Shdr *)(elf_start + section_header_offset +
                           i * sizeof(Elf32_Shdr)))
               ->sh_type) == SHT_NOBITS)
        continue;
      else {
        elf_size = ((Elf32_Shdr *)(elf_start + section_header_offset +
                                   i * sizeof(Elf32_Shdr)))
                       ->sh_offset +
                   ((Elf32_Shdr *)(elf_start + section_header_offset +
                                   i * sizeof(Elf32_Shdr)))
                       ->sh_size;
  ...
  if (abi_version < 4 || abi_version > 7)
    fatal(
        "%s:%d: The cubin has a unrecognized ABI version (0x%x).\n"
        "\tMulti2Sim CUDA library is currently compatible with Fermi\n"
        "\tor Kepler binary only.",
        __FILE__, __LINE__, abi_version);
...
cudaError_t cudaGetDeviceProperties(struct cudaDeviceProp *prop_ptr,
                                    int device) {
  unsigned free;
  ...
  cuDeviceGetAttribute(&prop_ptr->integrated, CU_DEVICE_ATTRIBUTE_INTEGRATED,
                       device);
  cuDeviceGetAttribute(&prop_ptr->canMapHostMemory,
                       CU_DEVICE_ATTRIBUTE_CAN_MAP_HOST_MEMORY, device);
  cuDeviceGetAttribute(&prop_ptr->computeMode, CU_DEVICE_ATTRIBUTE_COMPUTE_MODE,
                       device);
...
cudaError_t cudaConfigureCall(dim3 gridDim, dim3 blockDim, size_t sharedMem,
  ...
  if (stream == 0) stream = list_get(active_device->stream_list, 0);
...
cudaError_t cudaSetupArgument(const void *arg, size_t size, size_t offset) {
  ...
  if (!command->k_args.args) command->k_args.args = list_create();
...
cudaError_t cudaFuncSetCacheConfig(const void *func,
  ...
  for (i = 0; i < list_count(function_list); ++i) {
    ...
    if (f->host_func_ptr == (unsigned)func) break;
  }
...
cudaError_t cudaLaunch(const void *func) {
  ...
  /* Get arguments */
  arg_ptr_array = xcalloc(list_count(command->k_args.args), sizeof(void *));
  ...
  for (i = 0; i < list_count(function_list); i++) {
    ...
    if (function->host_func_ptr == (unsigned)func) break;
  }
  assert(i < list_count(function_list));

  /* Launch kernel */
  cuLaunchKernel(function, grid_dim[0], grid_dim[1], grid_dim[2], block_dim[0],
                 block_dim[1], block_dim[2], shared_mem_size, stream,
                 arg_ptr_array, NULL);
...
cudaError_t cudaFuncGetAttributes(struct cudaFuncAttributes *attr,
  ...
  cuFuncGetAttribute(&attr->binaryVersion, CU_FUNC_ATTRIBUTE_BINARY_VERSION, f);
...
cudaError_t cudaFree(void *devPtr) {
  ...
  if (!active_device) cuInit(0);
  cuda_debug("the size of the mem need to be deallocated is %d",
             sizeof(devPtr));
...
cudaError_t cudaHostGetDevicePointer(void **pDevice, void *pHost,
  ...
  /*
         ...
         cuda_debug("CUDA runtime API '%s'", __func__);
         cuda_debug("\t(runtime) '%s' in: device pointer address pDevice =
     [%p]",
                                         __func__, pDevice);
         ...
         *(CUdeviceptr *)pDevice = dptr;
         ...
         cuda_debug("\t(runtime) '%s' out: Device pointer dptr = 0x%d",
     __func__,
                                         dptr);
...
struct cudaChannelFormatDesc cudaCreateChannelDesc(
