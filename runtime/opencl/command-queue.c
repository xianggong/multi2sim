...
struct opencl_command_t *opencl_command_queue_dequeue(
  ...
  if (!command_queue->command_list->count) command_queue->process = 0;
...
cl_int clRetainCommandQueue(cl_command_queue command_queue) {
  ...
  return opencl_object_retain(command_queue, OPENCL_OBJECT_COMMAND_QUEUE,
...
cl_int clReleaseCommandQueue(cl_command_queue command_queue) {
  ...
  return opencl_object_release(command_queue, OPENCL_OBJECT_COMMAND_QUEUE,
                               CL_INVALID_COMMAND_QUEUE);
}
...
cl_int clGetCommandQueueInfo(cl_command_queue command_queue,
  ...
  switch (param_name) {
    case CL_QUEUE_CONTEXT:
      return opencl_set_param(&command_queue->context,
                              sizeof command_queue->context, param_value_size,
                              param_value, param_value_size_ret);

    case CL_QUEUE_DEVICE:
      return opencl_set_param(&command_queue->device,
                              sizeof command_queue->device, param_value_size,
                              param_value, param_value_size_ret);

    case CL_QUEUE_REFERENCE_COUNT:
      object = opencl_object_find(command_queue, OPENCL_OBJECT_COMMAND_QUEUE);
      return opencl_set_param(&object->ref_count, sizeof object->ref_count,
                              param_value_size, param_value,
                              param_value_size_ret);

    case CL_QUEUE_PROPERTIES:
      return opencl_set_param(
          &command_queue->properties, sizeof command_queue->properties,
...
cl_int clEnqueueReadBuffer(cl_command_queue command_queue, cl_mem buffer,
  ...
  if (buffer->size < offset + cb) return CL_INVALID_VALUE;

  /* Check events before they are needed */
  status =
      opencl_event_wait_list_check(num_events_in_wait_list, event_wait_list);
  if (status != CL_SUCCESS) return status;

  /* Create command */
  command = opencl_command_create_mem_read(
      ptr, buffer->device_ptr + offset, cb, command_queue, event,
      num_events_in_wait_list, (cl_event *)event_wait_list);
  opencl_command_queue_enqueue(command_queue, command);

  /* Flush command queue if blocking */
  if (blocking_read) clFinish(command_queue);
...
cl_int clEnqueueWriteBuffer(cl_command_queue command_queue, cl_mem buffer,
  ...
  /* Create command */
  command = opencl_command_create_mem_write(
      buffer->device_ptr + offset, (void *)ptr, cb, command_queue, event,
      num_events_in_wait_list, (cl_event *)event_wait_list);
  opencl_command_queue_enqueue(command_queue, command);

  /* If it is a blocking write, wait for command queue completion */
  if (blocking_write) clFinish(command_queue);
...
cl_int clEnqueueCopyBuffer(cl_command_queue command_queue, cl_mem src_buffer,
  ...
  command = opencl_command_create_mem_copy(
      dst_buffer->device_ptr + dst_offset, src_buffer->device_ptr + src_offset,
      cb, command_queue, event, num_events_in_wait_list,
...
cl_int clEnqueueReadImage(cl_command_queue command_queue, cl_mem image,
                          cl_bool blocking_read, const size_t *origin3,
                          const size_t *region3, size_t row_pitch,
...
cl_int clEnqueueWriteImage(cl_command_queue command_queue, cl_mem image,
                           cl_bool blocking_write, const size_t *origin3,
                           const size_t *region3, size_t input_row_pitch,
...
cl_int clEnqueueCopyImage(cl_command_queue command_queue, cl_mem src_image,
                          cl_mem dst_image, const size_t *src_origin3,
                          const size_t *dst_origin3, const size_t *region3,
                          cl_uint num_events_in_wait_list,
                          const cl_event *event_wait_list, cl_event *event) {
  __OPENCL_NOT_IMPL__
  return 0;
...
cl_int clEnqueueCopyImageToBuffer(cl_command_queue command_queue,
                                  ...
                                  const size_t *region3, size_t dst_offset,
                                  cl_uint num_events_in_wait_list,
...
cl_int clEnqueueCopyBufferToImage(cl_command_queue command_queue,
                                  ...
                                  size_t src_offset, const size_t *dst_origin3,
...
void *clEnqueueMapBuffer(cl_command_queue command_queue, cl_mem mem,
  ...
  if (mem->mapped) fatal("%s: cl_mem object already mapped", __FUNCTION__);
  ...
  /* Create command */
  command = opencl_command_create_map_buffer(mem, command_queue, event,
                                             num_events_in_wait_list,
                                             (cl_event *)event_wait_list);
  opencl_command_queue_enqueue(command_queue, command);

  /* If mapping is blocking, flush command queue */
  if (blocking_map) clFinish(command_queue);
...
void *clEnqueueMapImage(cl_command_queue command_queue, cl_mem image,
                        cl_bool blocking_map, cl_map_flags map_flags,
                        const size_t *origin3, const size_t *region3,
...
cl_int clEnqueueUnmapMemObject(cl_command_queue command_queue, cl_mem mem,
                               void *mapped_ptr,
                               cl_uint num_events_in_wait_list,
                               const cl_event *event_wait_list,
                               cl_event *event) {
  ...
  if (!mem->mapped) return CL_INVALID_VALUE;
...
cl_int clEnqueueNDRangeKernel(
  ...
  if (!IN_RANGE(work_dim, 1, 3)) return CL_INVALID_WORK_DIMENSION;
  ...
  for (i = 0; i < work_dim; i++)
    if (!global_work_size[i]) return CL_INVALID_GLOBAL_WORK_SIZE;

  /* Invalid local size components */
  if (local_work_size) {
    for (i = 0; i < work_dim; i++) {
      /* Component is 0 */
      if (!local_work_size[i]) return CL_INVALID_WORK_GROUP_SIZE;
  ...
  /* Create command */
  command = opencl_command_create_ndrange(
      device, kernel, work_dim, (unsigned int *)global_work_offset,
