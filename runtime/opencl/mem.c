...
void *opencl_mem_get_buffer(struct opencl_mem_t *mem) {
  /* Check memory object */
  if (!opencl_object_verify(mem, OPENCL_OBJECT_MEM)) return NULL;
...
cl_mem clCreateBuffer(cl_context context, cl_mem_flags flags, size_t size,
  ...
  if (context->device_list->count != 1)
    fatal(
        "%s: only supported for contexts with 1 "
        "associated device",
        __FUNCTION__);
  ...
  if (mem->use_host_ptr) mem->host_ptr = host_ptr;
  ...
  if (!opencl_device_shared_memory &&
    ...
    device->arch_device_mem_write_func(device->arch_device, mem->device_ptr,
                                       host_ptr, size);
