...
struct opencl_si_device_t *opencl_si_device_create(
  ...
  parent->arch_device_free_func =
      (opencl_arch_device_free_func_t)opencl_si_device_free;
  ...
  if (opencl_device_shared_memory) {
    ...
    parent->arch_device_mem_alloc_func =
        (opencl_arch_device_mem_alloc_func_t)opencl_si_device_mem_alloc;
    parent->arch_device_mem_free_func =
        (opencl_arch_device_mem_free_func_t)opencl_si_device_mem_free;
    parent->arch_device_mem_read_func =
        (opencl_arch_device_mem_read_func_t)opencl_si_device_mem_read;
    parent->arch_device_mem_write_func =
        (opencl_arch_device_mem_write_func_t)opencl_si_device_mem_write;
    parent->arch_device_mem_copy_func =
        (opencl_arch_device_mem_copy_func_t)opencl_si_device_mem_copy;
  }

  /* Call-back functions for kernel */
  parent->arch_kernel_create_func =
      (opencl_arch_kernel_create_func_t)opencl_si_kernel_create;
  parent->arch_kernel_free_func =
      (opencl_arch_kernel_free_func_t)opencl_si_kernel_free;
  parent->arch_kernel_set_arg_func =
      (opencl_arch_kernel_set_arg_func_t)opencl_si_kernel_set_arg;

  /* Call-back functions for ND-Range */
  parent->arch_ndrange_create_func =
      (opencl_arch_ndrange_create_func_t)opencl_si_ndrange_create;
  parent->arch_ndrange_init_func =
      (opencl_arch_ndrange_init_func_t)opencl_si_ndrange_init;
  parent->arch_ndrange_run_func =
      (opencl_arch_ndrange_run_func_t)opencl_si_ndrange_run;
  parent->arch_ndrange_run_partial_func =
      (opencl_arch_ndrange_run_partial_func_t)opencl_si_ndrange_run_partial;
  parent->arch_ndrange_finish_func =
      (opencl_arch_ndrange_finish_func_t)opencl_si_ndrange_finish;
  parent->arch_ndrange_free_func =
      (opencl_arch_ndrange_free_func_t)opencl_si_ndrange_free;
  ...
  parent->arch_program_create_func =
      (opencl_arch_program_create_func_t)opencl_si_program_create;
  parent->arch_program_free_func =
      (opencl_arch_program_free_func_t)opencl_si_program_free;
  parent->arch_program_valid_binary_func =
      (opencl_arch_program_valid_binary_func_t)opencl_si_program_valid_binary;
  ...
  if (parent->fd < 0)
    fatal(
        "Cannot communicate with the Southern Islands driver\n\n"
...
void opencl_si_device_free(struct opencl_si_device_t *device) { free(device); }
