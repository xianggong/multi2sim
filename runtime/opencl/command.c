...
static void opencl_command_run_mem_read(struct opencl_command_t *command) {
  ...
  device->arch_device_mem_read_func(
      device->arch_device, command->mem_read.host_ptr,
      command->mem_read.device_ptr, command->mem_read.size);
}
...
static void opencl_command_run_mem_write(struct opencl_command_t *command) {
  ...
  device->arch_device_mem_write_func(
      device->arch_device, command->mem_write.device_ptr,
      command->mem_write.host_ptr, command->mem_write.size);
}
...
static void opencl_command_run_mem_copy(struct opencl_command_t *command) {
  ...
  device->arch_device_mem_copy_func(
      device->arch_device, command->mem_copy.device_dest_ptr,
      command->mem_copy.device_src_ptr, command->mem_copy.size);
}
...
static void opencl_command_run_map_buffer(struct opencl_command_t *command) {
  ...
  if (!mem->mapped)
    fatal("%s: cl_mem no longer mapped - race condition?", __FUNCTION__);
  ...
  if (!opencl_device_shared_memory && (mem->map_flags & 1)) {
    device->arch_device_mem_read_func(
        device->arch_device, mem->host_ptr + mem->map_offset,
        mem->device_ptr + mem->map_offset, mem->map_size);
    opencl_debug(
        "\t[opencl_command_run_map_buffer] %d bytes "
        "copied from device [%p+%u] to host [%p+%u]",
        mem->map_size, mem->device_ptr, mem->map_offset, mem->host_ptr,
        mem->map_offset);
  }
  opencl_debug(
      "\t[opencl_command_run_map_buffer] host [%p+%u], "
      "device [%p+%u]",
      mem->host_ptr, mem->map_offset, mem->device_ptr, mem->map_offset);
}
...
static void opencl_command_run_unmap_buffer(struct opencl_command_t *command) {
  ...
  if (!opencl_device_shared_memory && (mem->map_flags & 2)) {
    device->arch_device_mem_write_func(
        device->arch_device, mem->device_ptr + mem->map_offset,
        mem->host_ptr + mem->map_offset, mem->map_size);
    opencl_debug(
        "\t[opencl_command_run_unmap_buffer] %d bytes "
        "copied from host [%p+%u] to devicem [%p+%u]",
        mem->map_size, mem->host_ptr, mem->map_offset, mem->device_ptr,
        mem->map_offset);
  }
  ...
  opencl_debug(
      "\t[opencl_command_run_unmap_buffer] host [%p+%u], "
      "device [%p+%u]",
      mem->host_ptr, mem->map_offset, mem->device_ptr, mem->map_offset);
}
...
struct opencl_command_t *opencl_command_create(
    enum opencl_command_type_t type, opencl_command_func_t func,
    struct opencl_command_queue_t *command_queue, cl_event *done_event_ptr,
    int num_wait_events, cl_event *wait_events) {
...
struct opencl_command_t *opencl_command_create_nop(
    ...
    struct opencl_event_t **done_event_ptr, int num_wait_events,
...
struct opencl_command_t *opencl_command_create_mem_read(
  ...
  command = opencl_command_create(opencl_command_mem_read,
                                  opencl_command_run_mem_read, command_queue,
                                  done_event_ptr, num_wait_events, wait_events);
...
struct opencl_command_t *opencl_command_create_mem_write(
  ...
  command = opencl_command_create(opencl_command_mem_write,
                                  opencl_command_run_mem_write, command_queue,
                                  done_event_ptr, num_wait_events, wait_events);
...
struct opencl_command_t *opencl_command_create_mem_copy(
  ...
  command = opencl_command_create(opencl_command_mem_copy,
                                  opencl_command_run_mem_copy, command_queue,
                                  done_event_ptr, num_wait_events, wait_events);
...
struct opencl_command_t *opencl_command_create_map_buffer(
    struct opencl_mem_t *mem, struct opencl_command_queue_t *command_queue,
    struct opencl_event_t **done_event_ptr, int num_wait_events,
    struct opencl_event_t **wait_events) {
  struct opencl_command_t *command;
...
struct opencl_command_t *opencl_command_create_unmap_buffer(
  ...
  /* Initialize */
  command = opencl_command_create(
      opencl_command_unmap_buffer, opencl_command_run_unmap_buffer,
      command_queue, done_event_ptr, num_wait_events, wait_events);
...
struct opencl_command_t *opencl_command_create_ndrange(
    struct opencl_device_t *device, struct opencl_kernel_t *kernel,
    int work_dim, unsigned int *global_work_offset,
  ...
  command->ndrange =
      opencl_ndrange_create(device, kernel, work_dim, global_work_offset,
...
void opencl_command_run(struct opencl_command_t *command) {
  ...
  if (command->func) command->func(command);
  if (command->done_event) {
    opencl_event_set_status(command->done_event, CL_COMPLETE);
  }
  if (command->done_event)
    opencl_debug(
        "Command execution time: %lld",
        command->done_event->time_end - command->done_event->time_start);
}
