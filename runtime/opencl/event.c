...
void opencl_event_set_status(struct opencl_event_t *event, cl_int status) {
  ...
  if (event->command_queue &&
    ...
    switch (status) {
      case CL_QUEUED:
        if (!event->time_queued) event->time_queued = cltime;
        break;

      case CL_SUBMITTED:
        if (!event->time_submit) event->time_submit = cltime;
        break;

      case CL_RUNNING:
        if (!event->time_start) event->time_start = cltime;
        break;

      case CL_COMPLETE:
        if (!event->time_end) event->time_end = cltime;
        break;
  ...
  if (status == CL_COMPLETE) pthread_cond_broadcast(&event->cond);
...
cl_int clWaitForEvents(cl_uint num_events, const cl_event *event_list) {
  ...
  if (!num_events || !event_list) return CL_INVALID_VALUE;
  ...
  for (i = 0; i < num_events; i++) opencl_event_wait(event_list[i]);
...
cl_int clGetEventInfo(cl_event event, cl_event_info param_name,
  ...
  switch (param_name) {
    case CL_EVENT_COMMAND_QUEUE:
      return opencl_set_param(&event->command_queue,
                              sizeof event->command_queue, param_value_size,
