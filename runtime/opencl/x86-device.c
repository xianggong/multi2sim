...
static int opencl_x86_device_get_num_cores(void) {
  ...
  if (!f) fatal("%s: cannot access /proc/cpuinfo", __FUNCTION__);

  /* Count entries starting with token 'processor' */
  while (fgets(s, sizeof s, f)) {
    strtok(s, "\n\t :");
    if (!strcmp(s, "processor")) num_cores++;
  }
...
void opencl_x86_device_reinit_work_item(
  ...
  struct opencl_x86_device_work_item_t *work_item = work_group->work_item[0];
  struct opencl_x86_device_fiber_t *fiber = work_group->work_fibers;
  size_t size = work_group->nd->arch_kernel->stack_param_words * sizeof(size_t);
  char *stack_top = (char *)fiber->stack_bottom + fiber->stack_size - size;

  /* Compute the local ID of the work item */
  opencl_nd_address(work_group->current_item, work_group->nd->local_work_size,
                    work_item->global_id);
...
void opencl_x86_device_init_work_item(
    int i, struct opencl_x86_work_group_t *work_group) {
  struct opencl_x86_device_work_item_t *work_item = work_group->work_item[i];
...
void opencl_x86_device_work_group_launch(
    int work_group_num, struct opencl_x86_device_exec_t *exec,
    struct opencl_x86_work_group_t *work_group) {
  struct opencl_x86_ndrange_t *ndrange = exec->ndrange;

  /* Compute the work-groups ID */
  opencl_nd_address(work_group_num, ndrange->num_groups, work_group->group_id);
...
void opencl_x86_device_work_group_init(
  ...
  for (int i = 0; i < X86_MAX_WORK_GROUP_SIZE; i++) {
    ...
    fiber->stack_bottom =
        work_group->aligned_stacks + (i * OPENCL_WORK_GROUP_STACK_SIZE);
    fiber->stack_size = OPENCL_WORK_GROUP_STACK_SIZE -
                        sizeof(struct opencl_x86_device_work_item_t);

    work_item =
        (struct opencl_x86_device_work_item_t *)((char *)fiber->stack_bottom +
                                                 fiber->stack_size);

    work_item->work_group_data = (int)work_group;
    work_item->barrier_func = (int)&opencl_x86_device_barrier_func;
 ...
 */
void opencl_x86_device_run_exec(struct opencl_x86_work_group_t *work_group,
                                struct opencl_x86_device_exec_t *exec) {
  /* Launch work-groups */
  for (;;) {
    /* Get next work-group */
    ...
    */
    ...
    if (num >= (exec->work_group_start + exec->work_group_count)) break;
...
void *opencl_x86_device_core_func(struct opencl_x86_device_t *device) {
  ...
  if (!opencl_native_mode) {
    ...
    sched_param_new.sched_priority =
        sched_get_priority_max(sched_policy_new) - 1;
    pthread_setschedparam(pthread_self(), sched_policy_new, &sched_param_new);
  }
  ...
  for (;;) {
    ...
    if (exec->ndrange->done) {
       ...
       * resources */
      opencl_x86_kernel_work_group_done(&work_group, exec->kernel);
    } else {
      ...
      if (!exec->ndrange->scheduling_pass) {
        opencl_x86_kernel_work_group_init(&work_group, exec);
      }
...
void opencl_x86_device_sync_wait(struct opencl_x86_device_sync_t *sync,
                                 int value) {
#ifndef HAVE_SYNC_BUILTINS
  pthread_mutex_lock(&sync->lock);
  while (sync->count != value) pthread_cond_wait(&sync->cond, &sync->lock);
...
struct opencl_x86_device_t *opencl_x86_device_create(
  ...
  parent->double_fp_config = CL_FP_DENORM | CL_FP_INF_NAN |
  ...
  parent->single_fp_config = CL_FP_DENORM | CL_FP_INF_NAN |
                             CL_FP_ROUND_TO_NEAREST | CL_FP_ROUND_TO_ZERO |
                             CL_FP_ROUND_TO_INF | CL_FP_FMA | CL_FP_SOFT_FLOAT;
  ...
  parent->arch_device_free_func =
      (opencl_arch_device_free_func_t)opencl_x86_device_free;
  parent->arch_device_mem_alloc_func =
      (opencl_arch_device_mem_alloc_func_t)opencl_x86_device_mem_alloc;
  parent->arch_device_mem_free_func =
      (opencl_arch_device_mem_free_func_t)opencl_x86_device_mem_free;
  parent->arch_device_mem_read_func =
      (opencl_arch_device_mem_read_func_t)opencl_x86_device_mem_read;
  parent->arch_device_mem_write_func =
      (opencl_arch_device_mem_write_func_t)opencl_x86_device_mem_write;
  parent->arch_device_mem_copy_func =
      (opencl_arch_device_mem_copy_func_t)opencl_x86_device_mem_copy;
  ...
  parent->arch_program_create_func =
      (opencl_arch_program_create_func_t)opencl_x86_program_create;
  parent->arch_program_free_func =
      (opencl_arch_program_free_func_t)opencl_x86_program_free;
  ...
  parent->arch_kernel_create_func =
      (opencl_arch_kernel_create_func_t)opencl_x86_kernel_create;
  parent->arch_kernel_free_func =
      (opencl_arch_kernel_free_func_t)opencl_x86_kernel_free;
  parent->arch_kernel_set_arg_func =
      (opencl_arch_kernel_set_arg_func_t)opencl_x86_kernel_set_arg;

  /* Call-back functions for architecture-specific ND-Range */
  parent->arch_ndrange_create_func =
      (opencl_arch_ndrange_create_func_t)opencl_x86_ndrange_create;
  parent->arch_ndrange_finish_func =
      (opencl_arch_ndrange_finish_func_t)opencl_x86_ndrange_finish;
  parent->arch_ndrange_free_func =
      (opencl_arch_ndrange_free_func_t)opencl_x86_ndrange_free;
  parent->arch_ndrange_init_func =
      (opencl_arch_ndrange_init_func_t)opencl_x86_ndrange_init;
  parent->arch_ndrange_run_func =
      (opencl_arch_ndrange_run_func_t)opencl_x86_ndrange_run;
  parent->arch_ndrange_run_partial_func =
      (opencl_arch_ndrange_run_partial_func_t)opencl_x86_ndrange_run_partial;
  ...
  for (i = 0; i < device->num_cores - 1; i++) {
    ...
    /* Create thread */
    err =
        pthread_create(device->threads + i, NULL,
                       (opencl_callback_t)opencl_x86_device_core_func, device);
    if (err) fatal("%s: could not create thread", __FUNCTION__);
...
int opencl_x86_device_preferred_workgroups(struct opencl_x86_device_t *device) {
  ...
  opencl_debug("[%s] preferred groups = %d", __FUNCTION__, device->num_cores);
