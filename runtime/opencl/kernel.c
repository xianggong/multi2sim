...
void opencl_kernel_free(struct opencl_kernel_t *kernel) {
  ...
  opencl_object_release(kernel->program, OPENCL_OBJECT_PROGRAM,
                        CL_INVALID_PROGRAM);
  free(kernel);
}
...
cl_kernel clCreateKernel(cl_program program, const char *kernel_name,
  ...
  /* Check valid program */
  if (opencl_object_retain(program, OPENCL_OBJECT_PROGRAM, CL_INVALID_VALUE) !=
      CL_SUCCESS) {
  ...
  LIST_FOR_EACH(program->entry_list, index) {
    ...
    arch_kernel =
        device->arch_kernel_create_func(kernel, arch_program, kernel_name);
...
cl_int clSetKernelArg(cl_kernel kernel, cl_uint arg_index, size_t arg_size,
  ...
  LIST_FOR_EACH(kernel->entry_list, i) {
    ...
    status = entry->device->arch_kernel_set_arg_func(
        entry->arch_kernel, arg_index, arg_size, (void *)arg_value);
...
cl_int clGetKernelWorkGroupInfo(cl_kernel kernel, cl_device_id device,
  ...
  switch (param_name) {
    case CL_KERNEL_WORK_GROUP_SIZE: {
      ...
      return opencl_set_param(&size, sizeof size, param_value_size, param_value,
    ...
    case CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE: {
      size_t mult = 1;
      return opencl_set_param(&mult, sizeof mult, param_value_size, param_value,
                              param_value_size_ret);
    }
...
struct opencl_ndrange_t *opencl_ndrange_create(struct opencl_device_t *device,
                                               struct opencl_kernel_t *kernel,
  ...
  LIST_FOR_EACH(kernel->entry_list, i) {
    ...
    if (kernel_entry->device == device) arch_kernel = kernel_entry->arch_kernel;
  }
  assert(arch_kernel);

  ndrange->arch_ndrange = device->arch_ndrange_create_func(
      ndrange, arch_kernel, work_dim, global_work_offset, global_work_size,
