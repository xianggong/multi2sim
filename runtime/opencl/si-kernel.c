...
void opencl_si_arg_debug(struct opencl_si_arg_t *arg) {
  opencl_debug("\ttype = %s",
               str_map_value(&opencl_si_arg_type_map, arg->type));
...
struct opencl_si_kernel_t *opencl_si_kernel_create(
  ...
  if (!symbol)
    fatal("%s: %s: kernel function not found in binary", __FUNCTION__,
          func_name);
  ...
  kernel->id = ioctl(opencl_si_device->fd, SIKernelCreate, args);
...
int opencl_si_kernel_set_arg(struct opencl_si_kernel_t *kernel, int arg_index,
  ...
  if (!arg) fatal("%s: invalid argument index (%d)\n", __FUNCTION__, arg_index);
  /* Perform ABI call depending on the argument type */
  switch (arg->type) {
    case opencl_si_arg_value: {
      /* ABI call */
      unsigned int args[] = {kernel->id, arg_index, (unsigned int)arg_value,
                             arg_size};

      ioctl(opencl_si_device->fd, SIKernelSetArgValue, args);
      break;
    }
    case opencl_si_arg_pointer: {
      ...
      if (arg_value) {
        ...
        if (!opencl_object_verify(mem, OPENCL_OBJECT_MEM))
          fatal("%s: argument %d is not a cl_mem object", __FUNCTION__,
                arg_index);
        if (arg_size != 4) {
          fatal(
              "%s: cl_mem argument %d expects size 4 "
              "(%d given)",
      ...
      ioctl(opencl_si_device->fd, SIKernelSetArgPointer, args);
      break;
    }
    default:
...
struct opencl_si_ndrange_t *opencl_si_ndrange_create(
  ...
  arch_ndrange = (struct opencl_si_ndrange_t *)xcalloc(
      1, sizeof(struct opencl_si_ndrange_t));
  ...
  for (i = work_dim; i < 3; i++) {
    ...
    arch_ndrange->num_groups[i] =
        arch_ndrange->global_work_size[i] / arch_ndrange->local_work_size[i];
  }
  ...
  ioctl(opencl_si_device->fd, SINDRangeSetFused, args);
  ...
  arch_ndrange->id = ioctl(opencl_si_device->fd, SINDRangeCreate, args);
  ...
  ioctl(opencl_si_device->fd, SINDRangePassMemObjs, args);
...
void opencl_si_ndrange_finish(struct opencl_si_ndrange_t *ndrange) {
  ...
  opencl_debug("[%s] finish start at %llu", __FUNCTION__, opencl_get_time());
  ...
  opencl_debug("[%s] flush done at %llu", __FUNCTION__, opencl_get_time());
}
