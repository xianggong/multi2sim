...
int opencl_context_has_device(struct opencl_context_t *context,
  ...
  LIST_FOR_EACH(context->device_list, i) {
    if (list_get(context->device_list, i) == device) return 1;
  }
...
cl_context clCreateContext(const cl_context_properties *properties,
  ...
  /* Add devices */
  for (i = 0; i < num_devices; i++) list_add(context->device_list, devices[i]);

  /* Initialize context properties if given */
  if (properties) {
    context->prop_count =
        getPropertiesCount(properties, sizeof(cl_context_properties));
    context->props =
        xmalloc(sizeof(cl_context_properties) * context->prop_count);
    copyProperties(context->props, properties, sizeof(cl_context_properties),
                   context->prop_count);
  }
...
cl_int clRetainContext(cl_context context) {
  ...
  return opencl_object_retain(context, OPENCL_OBJECT_CONTEXT,
                              CL_INVALID_CONTEXT);
}
...
cl_int clReleaseContext(cl_context context) {
  ...
  return opencl_object_release(context, OPENCL_OBJECT_CONTEXT,
                               CL_INVALID_CONTEXT);
}
...
cl_int clGetContextInfo(cl_context context, cl_context_info param_name,
  ...
  switch (param_name) {
    case CL_CONTEXT_REFERENCE_COUNT: {
      cl_int count =
          opencl_object_find(context, OPENCL_OBJECT_CONTEXT)->ref_count;
    ...
    case CL_CONTEXT_DEVICES: {
      ...
      cl_int status =
          opencl_set_param(devices, sizeof(cl_device_id) * count,
    ...
    /*
            case CL_CONTEXT_PROPERTIES:
                    if (context->props)
                            return opencl_set_param(context->props, sizeof
       (cl_context_properties)
                                    * context->prop_count, param_value_size,
       param_value, param_value_size_ret);
                    return opencl_set_param(NULL, 0, param_value_size,
       param_value, param_value_size_ret);
