...
struct opencl_x86_program_t *opencl_x86_program_create(
  ...
  /* Load ELF binary */
  elf_file =
      elf_file_create_from_buffer((void *)binary, length, "OpenCL binary");
  ...
  /* Section not found */
  if (!found) fatal("%s: no '.text' section in binary", __FUNCTION__);

  /* Extract section to temporary file */
  program->file_name = xstrdup("/tmp/tmp_XXXXXX");
  f = mkstemp(program->file_name);
  if (f == -1) fatal("%s: could not create temporary file", __FUNCTION__);
  if (write(f, section->buffer.ptr, section->buffer.size) !=
      section->buffer.size)
