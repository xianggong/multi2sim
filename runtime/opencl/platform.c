...
char *opencl_err_version =
    "\tYour OpenCL program is using a version of the Multi2Sim Runtime "
    "library\n"
    "\tthat is incompatible with this version of Multi2Sim. Please download "
    "the\n"
    "\tlatest Multi2Sim version, and recompile your application with the "
    "latest\n"
    "\tMulti2Sim OpenCL Runtime library ('libm2s-opencl.so').\n"
    "\n"
    "\tIf you are using statically compiled benchmarks from a pre-compiled\n"
    "\tbenchmark suite, please download the latest compatible package from "
    "the\n"
...
static char *opencl_err_native =
    ...
    "\tMulti2Sim's OpenCL runtime (libm2s-opencl.so). In native mode, the "
    "only\n"
    "\tdevice visible to the application is the x86 CPU. If you want to see\n"
    "\tadditional devices, please run your application on Multi2Sim. If you "
    "want\n"
    "\tto run your program using native libraries, please compile it with "
    "flag\n"
...
static void opencl_platform_add_devices(struct opencl_platform_t *platform) {
  ...
  while (token) {
    ...
    if (!strcasecmp(token, "x86")) {
      ...
      device = opencl_device_create();
      device->arch_device =
          opencl_union_device_create(device, platform->device_list);
      list_add(platform->device_list, device);
  ...
  if (!platform->device_list->count)
    fatal(
        "No device added\n\n"
        "\tThe list of OpenCL devices is empty. Please use environment "
        "variable\n"
        "\tM2S_OPENCL_DEVICES to select a list of devices that the OpenCL "
        "runtime\n"
...
cl_int clGetPlatformIDs(cl_uint num_entries, cl_platform_id *platforms,
  ...
  if (!opencl_platform) opencl_platform = opencl_platform_create();
  ...
  if ((!num_entries && platforms) || (num_entries && !platforms) ||
  ...
  /* The client wants the platform itself. Also return the number of platforms
   * inserted if they asked for it */
