...
static int opencl_x86_kernel_get_arg_type_size(
...
static int opencl_x86_kernel_get_arg_words(
    enum opencl_x86_kernel_arg_type_t arg_type) {
  int size;
  int rem;
  ...
  if (rem) rem = sizeof(size_t) - rem;
  return (size + rem) / sizeof(size_t);
}

static int opencl_x86_kernel_arg_is_vector(
    enum opencl_x86_kernel_arg_type_t arg_type) {
  switch (arg_type) {
    case OPENCL_X86_KERNEL_ARG_CHAR:
    case OPENCL_X86_KERNEL_ARG_SHORT:
...
struct opencl_x86_kernel_t *opencl_x86_kernel_create(
    struct opencl_kernel_t *parent, struct opencl_x86_program_t *program,
    char *func_name) {
  int i;
  int stride;
  int num_reg;
  ...
  kernel->func = opencl_x86_kernel_get_func_info(program->dlhandle, func_name,
                                                 &kernel->metadata);
  ...
  kernel->param_info =
      xcalloc(1, sizeof kernel->param_info[0] * kernel->num_params);
  stride = 8;
  num_reg = 0;
  stack_offset = 0;
  ...
  for (i = 0; i < kernel->num_params; i++) {
    ...
    if (opencl_x86_kernel_arg_is_vector(param_info->arg_type) &&
      ...
      if (remainder) stack_offset += align_size - remainder;
  ...
  if (!remainder)
    ...
    kernel->stack_param_words =
        stack_offset + SSE_REG_SIZE_IN_WORDS - remainder;
...
int opencl_x86_kernel_set_arg(struct opencl_x86_kernel_t *kernel, int arg_index,
  ...
  assert((!arg_value) ==
         (param_info->mem_arg_type == OPENCL_X86_KERNEL_MEM_ARG_LOCAL));
  ...
  if (!arg_value) {
    ...
    if (!addr) return CL_INVALID_MEM_OBJECT;
    memcpy(kernel->cur_stack_params + param_info->stack_offset, &addr,
           sizeof addr);
  } else if (param_info->is_stack) {
    memcpy(kernel->cur_stack_params + param_info->stack_offset, arg_value,
           arg_size);
  } else {
    memcpy(kernel->cur_register_params + param_info->reg_offset, arg_value,
           arg_size);
  }
...
struct opencl_x86_ndrange_t *opencl_x86_ndrange_create(
    struct opencl_ndrange_t *ndrange, struct opencl_x86_kernel_t *x86_kernel,
  ...
  arch_ndrange = (struct opencl_x86_ndrange_t *)xcalloc(
      1, sizeof(struct opencl_x86_ndrange_t));
  ...
  for (i = 0; i < work_dim; i++) {
    ...
    arch_ndrange->num_groups[i] =
        global_work_size[i] / arch_ndrange->local_work_size[i];
  }
  ...
  arch_ndrange->total_num_groups = arch_ndrange->num_groups[0] *
  ...
  size_t reg_size =
      sizeof x86_kernel->cur_register_params[0] * MAX_SSE_REG_PARAMS;
  if (posix_memalign((void **)&arch_ndrange->register_params, MEMORY_ALIGN,
                     reg_size)) {
  ...
  arch_ndrange->stack_params =
      xmalloc(x86_kernel->stack_param_words * sizeof(size_t));
  if (!arch_ndrange->stack_params) fatal("%s: out of memory", __FUNCTION__);
...
void opencl_x86_kernel_work_group_init(
   ...
   * of other threads */
  work_group->stack_params =
      (size_t *)xmalloc(sizeof(size_t) * e->kernel->stack_param_words);
...
void opencl_x86_kernel_work_group_done(
  ...
  if (work_group->local_reserved) free(work_group->local_reserved);
}
...
void opencl_x86_ndrange_finish(struct opencl_x86_ndrange_t *ndrange) {
  ...
  opencl_x86_kernel_work_group_done(&device->work_group, ndrange->arch_kernel);
}
...
void opencl_x86_ndrange_run(struct opencl_x86_ndrange_t *ndrange,
  ...
  if (!opencl_native_mode) {
    ...
    pthread_getschedparam(pthread_self(), &sched_policy_old, &sched_param_old);

    /* Give dispatch threads the highest priority */
    sched_policy_new = SCHED_RR;
    sched_param_new.sched_priority = sched_get_priority_max(sched_policy_new);
  ...
  if (!opencl_native_mode) {
    ...
    pthread_setschedparam(pthread_self(), sched_policy_old, &sched_param_old);
