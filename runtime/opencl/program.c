...
int opencl_program_has_device(struct opencl_program_t *program,
  ...
  LIST_FOR_EACH(program->entry_list, index) {
    entry = list_get(program->entry_list, index);
    if (entry->device == device) return 1;
  }
...
struct opencl_program_entry_t *opencl_program_add(
    struct opencl_program_t *program, struct opencl_device_t *device,
    void *arch_program) {
  struct opencl_program_entry_t *entry;
...
void opencl_program_set_source(struct opencl_program_t *program,
                               unsigned int count, const char **strings,
...
cl_program clCreateProgramWithBinary(cl_context context, cl_uint num_devices,
  ...
  for (i = 0; i < num_devices; i++) {
    ...
    if (opencl_program_has_device(program, device)) continue;

    /* Create the architecture-specific program object. */
    assert(device->arch_program_create_func);
    arch_program = device->arch_program_create_func(
        program, device->arch_device, (void *)binaries[i], lengths[i]);
    ...
    /* Success */
    if (errcode_ret) *errcode_ret = CL_SUCCESS;
    if (binary_status) binary_status[0] = CL_SUCCESS;
  }
...
cl_int clRetainProgram(cl_program program) {
  ...
  return opencl_object_retain(program, OPENCL_OBJECT_PROGRAM,
                              CL_INVALID_PROGRAM);
}
...
cl_int clReleaseProgram(cl_program program) {
  ...
  return opencl_object_release(program, OPENCL_OBJECT_PROGRAM,
                               CL_INVALID_PROGRAM);
}
...
cl_int clBuildProgram(cl_program program, cl_uint num_devices,
  ...
  if (!program->source) return CL_SUCCESS;
  ...
  if (!binary_name || !*binary_name)
    fatal("%s: runtime kernel compilation not supported.\n%s", __FUNCTION__,
          opencl_err_program_compile);

  /* Load binary */
  f = fopen(binary_name, "rb");
  if (!f)
    fatal("%s: %s: cannot open file.\n%s", __FUNCTION__, binary_name,
          opencl_err_program_not_found);
  ...
  for (i = 0; i < num_devices; i++) {
    ...
    /* Make sure the type of the binary matches */
    assert(device->arch_program_valid_binary_func);
    if (!device->arch_program_valid_binary_func(device->arch_device, binary,
                                                size))
      fatal("%s: invalid binary for device '%s'.\n%s", __FUNCTION__,
            device->name, opencl_err_program_invalid);
    ...
    assert(device->arch_program_create_func);
    arch_program = device->arch_program_create_func(
        program, device->arch_device, binary, size);
    ...
    warning("%s: %s: program binary loaded for device '%s'.\n%s", __FUNCTION__,
            binary_name, device->name, opencl_note_program_binary);
  }
...
cl_int clGetProgramBuildInfo(cl_program program, cl_device_id device,
  ...
  switch (param_name) {
    case CL_PROGRAM_BUILD_STATUS:
      return opencl_set_param(&status, sizeof status, param_value_size,
                              param_value, param_value_size_ret);

    case CL_PROGRAM_BUILD_OPTIONS:
      return opencl_set_param(&str, sizeof str, param_value_size, param_value,
