...
struct opencl_x86_device_work_item_t {
  unsigned int
      work_group_data; /* 0x60 (Not actually part of AMD runtime, padding_0) */
...
struct opencl_x86_work_group_t {
  const void*
      register_params;   /* this is accessed in assembly - don't move it */
  ...
  unsigned int*
      stack_params; /* these parameters have the correct local buffers for this
                       core */
  char* aligned_stacks;
  void* local_reserved;
