...
static void opencl_object_init(void) {
  ...
  if (opencl_object_initialized) return;
...
struct opencl_object_t *opencl_object_create(
    void *data, enum opencl_object_type_t type,
...
struct opencl_object_t *opencl_object_find(void *data,
                                           enum opencl_object_type_t type) {
  ...
  thread_list_visit(opencl_object_list, opencl_object_list_visit_match,
                    &match_data);
...
int opencl_object_ref_update(void *data, enum opencl_object_type_t type,
                             int change) {
  struct opencl_object_t *object;
  int count;
  ...
  if (!object) return CL_INVALID_VALUE;
...
int opencl_object_retain(void *data, enum opencl_object_type_t type,
                         int err_code) {
  if (opencl_object_ref_update(data, type, 1) != CL_SUCCESS) return err_code;
  return CL_SUCCESS;
}

/* Decrement the number of references of an object */
int opencl_object_release(void *data, enum opencl_object_type_t type,
                          int err_code) {
  if (opencl_object_ref_update(data, type, -1) != CL_SUCCESS) return err_code;
  return CL_SUCCESS;
}
