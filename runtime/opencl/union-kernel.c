...
/* Some of the dynamic scheduling algorithms might try to schedule a zero-size
   NDRange.
   Better to just detect this in general */
int device_ndrange_has_work(unsigned int dims,
                            const unsigned int *group_count) {
  int i;
  for (i = 0; i < dims; i++)
    if (group_count[i] == 0) return 0;
  return 1;
}
...
void *device_ndrange_dispatch(void *ptr) {
  ...
  arch_ndrange = info->device->arch_ndrange_create_func(
      ndrange->parent, info->arch_kernel, ndrange->work_dim,
  ...
  while (ndrange->next_group < ndrange->total_num_groups) {
    ...
    num_groups_to_exec =
        MIN(preferred_groups, ndrange->total_num_groups - ndrange->next_group);
    /*
    ...
    */
    ...
    opencl_debug(
        "[%s] running work groups %d to %d"
        " on device %s",
        __FUNCTION__, start_group, start_group + num_groups_to_exec - 1,
        info->device->name);
    ...
    */
    ...
    info->device->arch_ndrange_run_partial_func(arch_ndrange, start_group,
                                                num_groups_to_exec);
  ...
  */
  ...
  opencl_debug("[%s] Device %s ran %d work-groups", __FUNCTION__,
               info->device->name, work_groups_executed);
...
void opencl_union_ndrange_run(struct opencl_union_ndrange_t *ndrange,
  ...
  opencl_debug("[%s] num fused sub-devices = %d", __FUNCTION__, num_devices);
  ...
  for (i = 0; i < num_devices; i++) {
    ...
    opencl_debug("[%s] ndrange->kernel[%d] = %p", __FUNCTION__, i,
                 info[i].arch_kernel);
    info[i].lock = &lock;
    ...
    if (i != num_devices - 1) {
      opencl_debug("[%s] dispatching %s as new thread", __FUNCTION__,
                   info[i].device->name);
      pthread_create(threads + i, NULL, device_ndrange_dispatch, info + i);
    } else {
      opencl_debug("[%s] dispatching %s as main thread", __FUNCTION__,
                   info[i].device->name);
...
struct opencl_union_kernel_t *opencl_union_kernel_create(
    struct opencl_kernel_t *parent, struct opencl_union_program_t *program,
    char *func_name) {
  int i;
  ...
  LIST_FOR_EACH(devices, i) {
    ...
    list_add(kernel->arch_kernels,
             subdevice->arch_kernel_create_func(
                 NULL, list_get(program->programs, i), func_name));
  }
...
void opencl_union_kernel_free(struct opencl_union_kernel_t *kernel) {
  ...
  LIST_FOR_EACH(device_list, i) {
    ...
    device->arch_kernel_free_func(list_get(kernel->arch_kernels, i));
  }
...
int opencl_union_kernel_set_arg(struct opencl_union_kernel_t *kernel,
                                int arg_index, unsigned int arg_size,
                                void *arg_value) {
  int i;
  struct list_t *device_list;
  ...
  LIST_FOR_EACH(device_list, i) {
    ...
    if (subdevice->arch_kernel_set_arg_func(list_get(kernel->arch_kernels, i),
...
struct opencl_union_ndrange_t *opencl_union_ndrange_create(
    ...
    struct opencl_union_kernel_t *union_kernel, unsigned int work_dim,
  ...
  union_ndrange = (struct opencl_union_ndrange_t *)xcalloc(
      1, sizeof(struct opencl_union_ndrange_t));
  ...
  LIST_FOR_EACH(devices, i) {
    ...
    opencl_debug("[%s] union nd-range kernel %d: %p ", __FUNCTION__, i,
                 subkernel);
  }

  /* Work sizes */
  for (i = 0; i < work_dim; i++) {
    union_ndrange->global_work_offset[i] =
    ...
    union_ndrange->local_work_size[i] =
        local_work_size ? local_work_size[i] : 1;
    assert(!(global_work_size[i] % union_ndrange->local_work_size[i]));
    union_ndrange->num_groups[i] =
        global_work_size[i] / union_ndrange->local_work_size[i];
  }
  ...
  for (i = work_dim; i < 3; i++) {
    ...
    union_ndrange->num_groups[i] =
        union_ndrange->global_work_size[i] / union_ndrange->local_work_size[i];
  }

  /* Calculate the number of work groups in the ND-Range */
  union_ndrange->total_num_groups = union_ndrange->num_groups[0] *
