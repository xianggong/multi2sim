...
char *opencl_err_note =
    ...
    "\tThis implementation only provides a subset of the OpenCL specification. "
    "Please\n"
    "\temail 'development@multi2sim.org' for further support.\n";

char *opencl_err_param_note =
    "\tThis error message is generated by the Multi2Sim OpenCL Runtime "
    "library\n"
 ...
 * Debug
 *
 * If environment variable 'M2S_OPENCL_DEBUG' is set, the Multi2Sim OpenCL
 * Runtime
...
void opencl_debug(char *fmt, ...) {
  ...
  if (!opencl_debug_initialized) {
    ...
    if (file_name) {
      ...
      if (!opencl_debug_file) fatal("%s: Cannot open file", file_name);
    }
  }

  /* Exit if not debugging */
  if (!opencl_debug_file) return;
...
cl_int opencl_set_param(const void *src_value, size_t src_size,
  ...
  assert(src_value);
  if (dest_value && src_size > dest_size) return CL_INVALID_VALUE;
  if (size_ret) *size_ret = src_size;
  if (dest_value) memcpy(dest_value, src_value, src_size);
  return CL_SUCCESS;
}
...
cl_int opencl_set_string(const char *src_string, size_t dest_size,
  ...
  return opencl_set_param(src_string, src_size, dest_size, dest_string,
                          size_ret);
}
