...
TEST(TestISASOP2, S_ADD_U32) {
  ...
  Instruction::BytesSOP2 inst_bytes = {// ssrc0 - s0
                                       0,
  ...
  EXPECT_EQ(4294967294, work_item->ReadSReg(sdst))
      << "carry test: error with sum";
...
TEST(TestISASOP2, S_SUB_I32) {
  ...
  EXPECT_EQ(2147483648, work_item->ReadSReg(sdst))
      << "overflow test: sum error";
