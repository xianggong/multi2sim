...
enum UopInstanceType { UopInstanceTypeBranch = 0, UopInstanceTypeOther };
  ...
  switch (instance_type) {
    case UopInstanceTypeBranch:
      ...
      uop_list.push_back(misc::new_unique<Uop>(
          object_pool->getThread(), object_pool->getContext(), uinst_branch));
    ...
    case UopInstanceTypeOther:
      ...
      uop_list.push_back(misc::new_unique<Uop>(
          object_pool->getThread(), object_pool->getContext(), uinst_move));
      uop = uop_list.back().get();
      uop->eip = address;
      uop->mop_size = instruction_size;
      break;
