...
TEST(TestBranchPredictor, test_bimodal_branch_predictor_1) {
  ...
  uops.emplace_back(misc::new_unique<Uop>(object_pool->getThread(),
                                          object_pool->getContext(), uinst_1));
  ...
  uops.emplace_back(misc::new_unique<Uop>(object_pool->getThread(),
                                          object_pool->getContext(), uinst_2));
  uop = uops.back().get();
  uop->eip = branch_addr;
  ...
  BranchPredictor::Prediction pred_result[num_branch_uop] = {
      BranchPredictor::PredictionTaken, BranchPredictor::PredictionTaken,
      BranchPredictor::PredictionTaken};
  int bimod_status_trace[num_branch_uop] = {3, 2, 1};
  int bimod_index[num_branch_uop] = {0, 0, 0};
...
TEST(TestBranchPredictor, test_two_level_branch_predictor_1) {
  ...
  // [000000]: Strongly Taken; [000001]: Weakly NotTaken; [000010]: Weakly
  // NotTaken
  // [000100]: Strongly Taken; [001001]: Weakly NotTaken; [010010]: Weakly
  // NotTaken
  ...
  BranchPredictor::Prediction pred_result[num_branch_uop * N] = {
      ...
      BranchPredictor::PredictionTaken,   BranchPredictor::PredictionTaken,
      BranchPredictor::PredictionTaken,   BranchPredictor::PredictionTaken,
      BranchPredictor::PredictionTaken,   BranchPredictor::PredictionNotTaken,
      BranchPredictor::PredictionNotTaken};
  int bht_status_trace[num_branch_uop * N] = {0b000001, 0b000010, 0b000100,
                                              0b001001, 0b010010, 0b100100,
                                              0b001001, 0b010010, 0b100100};
  int pht_status_trace[num_branch_uop * N] = {3, 1, 1, 3, 1, 1, 3, 0, 0};
  int bht_index = 0;
  int pht_row[num_branch_uop * N] = {0b000000, 0b000001, 0b000010,
                                     0b000100, 0b001001, 0b010010,
                                     0b100100, 0b001001, 0b010010};
  int pht_col[num_branch_uop * N] = {0, 16, 32, 0, 16, 32, 0, 16, 32};
  for (unsigned int i = 0; i < uops.size(); i++) {
    // Look up predictor and verify prediction
    branch_predictor.Lookup(uops[i].get());
    pred = uops[i]->prediction;
    ...
    branch_predictor.Update(uops[i].get());
    bht_status =
        branch_predictor.getTwoLevelBhtStatus(uops[i]->two_level_bht_index);
    pht_status = branch_predictor.getTwoLevelPhtStatus(
        uops[i]->two_level_pht_row, uops[i]->two_level_pht_col);
...
TEST(TestBranchPredictor, test_combined_branch_predictor_1) {
  ...
  // [000100]: Strongly Taken; [001001]: Weakly NotTaken; [010010]: Weakly
  // NotTaken
  // [100100]: Strongly Taken; [001001]: Weakly NotTaken; [001001]: Weakly
  // NotTaken
  ...
  int bht_status_trace[num_branch_uop * N] = {0b000001, 0b000010, 0b000100,
                                              0b001001, 0b010010, 0b100100,
                                              0b001001, 0b001001, 0b010010};
  int pht_status_trace[num_branch_uop * N] = {3, 1, 1, 3, 1, 1, 3, 1, 1};
  ...
  // Weakly Taken(2) -> Weakly Taken(2) -> Weakly Taken(2) -> Weakly Not
  // Taken(1)
  // Prediction: Taken -> Taken -> Taken
  //
  ...
  BranchPredictor::Prediction bimodal_pred_result[num_branch_uop * N] = {
      ...
      BranchPredictor::PredictionTaken, BranchPredictor::PredictionTaken,
      BranchPredictor::PredictionTaken};
  int bimodal_status_trace[num_branch_uop * N] = {2, 2, 2, 2, 2, 2, 2, 1, 2};
  ...
  BranchPredictor::Prediction choice_pred_result[num_branch_uop * N] = {
      ...
      BranchPredictor::PredictionTaken, BranchPredictor::PredictionNotTaken,
      BranchPredictor::PredictionTaken};
  int choice_status_trace[num_branch_uop * N] = {2, 2, 2, 2, 2, 2, 2, 1, 2};
