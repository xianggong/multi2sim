...
TEST(TestAlu, read_ini_configuration_file_int) {
  ...
  for (int i = 0; i < alu_type_end - alu_type_start + 1; i++) {
    ...
    EXPECT_EQ(ref_configuration[i][1],
              Alu::getAluOperationLatency(i + alu_type_start));
    EXPECT_EQ(ref_configuration[i][2],
              Alu::getAluIssueLatency(i + alu_type_start));
  }
}
...
TEST(TestAlu, reserve_functional_unit_int_add) {
  ...
  //            |S   |S   |F   |S   |S   |F   |
  int ref_value[running_cycle] = {2, 2, 0, 2, 2, 0};
  for (int i = 0; i < running_cycle; i++) {
    // Reserve
