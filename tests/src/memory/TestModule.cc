...
TEST(TestModule, is_in_flight_address) {
  try {
    ...
    // Set up block
    module_mm->getCache()->getBlock(4, 15)->setStateTag(Cache::BlockExclusive,
                                                        0x400);
...
TEST(TestModule, can_access_0) {
  try {
    ...
    module_l1_0->getCache()->getBlock(0, 0)->setStateTag(Cache::BlockExclusive,
                                                         0x0);
    module_l1_0->getCache()->getBlock(4, 1)->setStateTag(Cache::BlockExclusive,
                                                         0x400);
    module_l1_0->getCache()->getBlock(6, 1)->setStateTag(Cache::BlockExclusive,
                                                         0x600);
    module_l1_0->getCache()->getBlock(8, 1)->setStateTag(Cache::BlockExclusive,
                                                         0x800);
...
TEST(TestModule, can_access_1) {
  try {
    ...
    module_mm->getCache()->getBlock(4, 15)->setStateTag(Cache::BlockExclusive,
                                                        0x400);
    module_mm->getCache()->getBlock(6, 15)->setStateTag(Cache::BlockExclusive,
                                                        0x600);
    module_mm->getCache()->getBlock(8, 15)->setStateTag(Cache::BlockExclusive,
                                                        0x800);
