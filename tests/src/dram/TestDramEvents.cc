...
TEST(TestSystemEvents, section_one_read) {
  ...
  try {
    ...
    // Get relevent bank
    Bank* bank =
        dram_system->getController(0)->getChannel(0)->getRank(0)->getBank(0);
...
TEST(TestSystemEvents, section_two_reads_different_row) {
  ...
  std::array<std::string, 5> expected = {"Activate", "Read", "Precharge",
                                         "Activate", "Read"};
...
TEST(TestSystemEvents, section_two_writes_different_row) {
  ...
  std::array<std::string, 5> expected = {"Activate", "Write", "Precharge",
                                         "Activate", "Write"};
...
TEST(TestSystemEvents, section_two_reads_same_row_closed_page_policy) {
  ...
  // Set up INI file
  misc::IniFile ini_file;
  std::string closed_page_config = zero_time_config + "PagePolicy = Closed\n";
  ...
  std::array<std::string, 6> expected = {"Activate", "Read", "Precharge",
...
TEST(TestSystemEvents, section_two_writes_same_row_closed_page_policy) {
  ...
  std::array<std::string, 6> expected = {"Activate", "Write", "Precharge",
...
TEST(TestSystemEvents, section_test_precharge_timing) {
  ...
  try {
    ...
    EXPECT_TRUE(bank->getFrontCommandTiming() -
                    System::frequency_domain->getCycle() ==
                16);
  } catch (misc::Error& e) {
    e.Dump();
    FAIL();
...
TEST(TestSystemEvents, section_test_negtive_address) {
  ...
  EXPECT_REGEX_MATCH(misc::fmt("Invalid Address").c_str(), message.c_str());
}
