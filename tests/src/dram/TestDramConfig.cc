...
TEST(TestSystemConfiguration, section_invalid_vaiable) {
  ...
  EXPECT_REGEX_MATCH(
      misc::fmt(".*%s:.*invalid variable.*", ini_file.getPath().c_str())
          .c_str(),
      message.c_str());
}
...
TEST(TestSystemConfiguration, section_correct_timings_DDR3_1600) {
  ...
  try {
    ...
    EXPECT_TRUE(controller->getCommandDuration(CommandPrecharge) == 11);

    // Check Activate timing
    EXPECT_TRUE(controller->getCommandDuration(CommandActivate) == 11);

    // Check Read timing
    EXPECT_TRUE(controller->getCommandDuration(CommandRead) == 15);

    // Check Write timing
    EXPECT_TRUE(controller->getCommandDuration(CommandWrite) == 15);
  } catch (misc::Error& error) {
    error.Dump();
    FAIL();
  }
