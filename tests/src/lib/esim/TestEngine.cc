...
void testHandler_0(Event* event, Frame* frame) { handler_called_0 = true; }
...
TEST(TestEngine, test_schedule_event) {
  try {
    ...
    FrequencyDomain* domain =
        engine->RegisterFrequencyDomain("Test frequency domain", 2e3);

    // Register event
    Event* event = engine->RegisterEvent("test event", testHandler_0, domain);
    ...
    for (int i = 0; i < 10; i++) engine->ProcessEvents();
...
TEST(TestEngine, test_event_frame) {
  try {
    ...
    // Register event
    Event* event = engine->RegisterEvent("test event", testHandler_1, domain);
...
TEST(TestEngine, test_frequency_domains) {
  try {
    ...
    FrequencyDomain* fast_domain =
        engine->RegisterFrequencyDomain("Fast frequency domain", 1000);

    // Set up slow frequency domain
    FrequencyDomain* slow_domain =
        engine->RegisterFrequencyDomain("Slow frequency domain", 600);

    // Set up event
    Event* fast_event =
        engine->RegisterEvent("fast event", testHandler_2_0, fast_domain);

    // Set up event
    Event* slow_event =
        engine->RegisterEvent("slow event", testHandler_2_1, slow_domain);
...
void testHandler_3_0(Event* event, Frame* frame) {
  ...
  // Set up frequency domain
  FrequencyDomain* domain =
      engine->RegisterFrequencyDomain("frequency domain", 1000);
...
// Initialize event handler
void testHandler_3_3(Event* event, Frame* frame) { handler_called_3_3 = true; }
...
TEST(TestEngine, test_event_queue) {
  try {
    ...
    // Run simulation for 10 cycles
    for (int i = 0; i < 11; i++) engine->ProcessEvents();
...
// Initialize event handler
void testHandler_4_1(Event* event, Frame* frame) { handler_called_4_1 = true; }
