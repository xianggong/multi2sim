...
bool Engine::Drain(int max_events) {
  ...
  while (1) {
    ...
    debug << misc::fmt(
        "[%.2fns] Event '%s/%s' drained\n", (double)current_time / 1000,
        frequency_domain->getName().c_str(), event->getName().c_str());
    ...
    // finish draining.
    if (num_events == max_events) return true;
  }
...
void Engine::ProcessEndEvents() {
  while (end_frames.size()) {
    ...
    debug << misc::fmt("[%.2fns] End event '%s' triggered\n",
                       (double)current_time / 1000, event->getName().c_str());
...
void Engine::ProcessEvents() {
  ...
  while (1) {
    ...
    // future.
    if (heap.top()->time > current_time) break;
    ...
    if (period > 0) Schedule(event, current_frame, period, period);
...
Event* Engine::RegisterEvent(const std::string& name, EventHandler handler,
                             FrequencyDomain* frequency_domain) {
  events.emplace_back(name, handler, frequency_domain);
  return &events.back();
}

void Engine::Schedule(Event* event, std::shared_ptr<Frame> frame, int after,
                      int period) {
  // Sanity
  assert(after >= 0);
  ...
  if (frame->in_queue)
    throw misc::Panic(
        "Cannot schedule an event in an event chain "
        "that is currently suspended in a queue");

  // Frame must not be enqueued in the heap
  if (frame->in_heap)
    throw misc::Panic(
        "Cannot schedule a second event for this "
  ...
  debug << misc::fmt("[%.2fns] Event '%s/%s' scheduled for [%.2fns]\n",
                     ...
                     event->getName().c_str(), (double)frame->time / 1000);

  // Warn when heap is overloaded
  if (!max_inflight_events_warning && (int)heap.size() >= max_inflight_events) {
    max_inflight_events_warning = true;
    misc::Warning(
        "[esim] Maximum number of %d "
        "in-flight events exceeds\n\n%s",
        max_inflight_events, engine_err_max_inflight_events);
  }
}

void Engine::Next(Event* event, int after, int period) {
  ...
  if (!frame) frame = misc::new_shared<Frame>();
...
void Engine::Execute(Event* event, std::shared_ptr<Frame> frame,
                     Event* receive_event) {
  // Null event
  if (event == nullptr || event == null_event) return;
...
void Engine::Call(Event* event, std::shared_ptr<Frame> frame,
                  Event* return_event, int after, int period) {
  // Create new frame if none passed
  if (frame == nullptr) frame = misc::new_shared<Frame>();
...
void Engine::Return(int after) {
  ...
  if (!current_frame)
    throw misc::Panic(
        "Function cannot be invoked outside of "
        "an event handler");

  // If this is the bottom of the stack, ignore
  if (current_frame->return_event == nullptr) return;

  // Schedule return event
  Schedule(current_frame->return_event, current_frame->parent_frame, after);
}
...
void Engine::ProcessAllEvents() {
  ...
  if (overflow) {
    misc::Warning(
        "[esim] Maximum number of finalization events "
        "exceeded\n\n%s",
