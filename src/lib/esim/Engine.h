...
class Engine {
 ...
 public:
  ...
  Event* getCurrentEvent() const {
    return current_frame == nullptr ? nullptr : current_frame->event;
  }
  ...
  Event* RegisterEvent(const std::string& name, EventHandler handler,
  ...
  void Schedule(Event* event, std::shared_ptr<Frame> event_frame, int after = 0,
                int period = 0);
  ...
  ///	respect to the event's frequency domain.
  ///
  void Call(Event* event, std::shared_ptr<Frame> frame = nullptr,
            Event* return_event = nullptr, int after = 0, int period = 0);
