...
static void mhandle_hash_table_insert(void *ptr, unsigned long size, char *at,
                                      int corrupt_info);
...
static void mhandle_init(void) {
  /* Already initialized */
  if (mhandle_initialized) return;
  mhandle_initialized = 1;

  /* Initialize hash table */
  mhandle_hash_table =
      calloc(MHANDLE_HASH_TABLE_SIZE, sizeof(struct mhandle_item_t));
  mhandle_hash_table_size = MHANDLE_HASH_TABLE_SIZE;
  mhandle_hash_table_count = 0;
  if (!mhandle_hash_table) mhandle_out_of_memory("mhandle_init");
}
...
static void mhandle_hash_table_grow(void) {
  ...
  mhandle_hash_table = (struct mhandle_item_t *)calloc(
      mhandle_hash_table_size, sizeof(struct mhandle_item_t));
  ...
  for (i = 0; i < old_size; i++) {
    if (old_ht[i].active && !old_ht[i].removed) {
      ...
      mhandle_hash_table_insert(old_ht[i].ptr, old_ht[i].size, old_ht[i].at,
                                old_ht[i].corrupt_info);
    }
  }
  free(old_ht);
}
...
static struct mhandle_item_t *mhandle_hash_table_get(void *ptr) {
  ...
  while (mhandle_hash_table[idx].ptr != ptr ||
         !mhandle_hash_table[idx].active || mhandle_hash_table[idx].removed) {
    if (!mhandle_hash_table[idx].active) return NULL;
...
static void mhandle_mark_corrupt(void *eff_ptr, unsigned long eff_size) {
  ...
  for (i = 0; i < MHANDLE_CORRUPT_RANGE; i++) as_char[i] = MHANDLE_MARK_START;
  for (i = 0; i < MHANDLE_CORRUPT_RANGE; i++)
    as_char[eff_size - MHANDLE_CORRUPT_RANGE + i] = MHANDLE_MARK_END;
}

static void mhandle_check_corrupt(void *eff_ptr, unsigned long eff_size,
                                  char *at) {
  unsigned long i;
  unsigned long size;
  ...
  assert(size > 0);
  for (i = 0; i < MHANDLE_CORRUPT_RANGE; i++)
    if (as_char[i] != MHANDLE_MARK_START) corrupt = 1;
  for (i = 0; i < MHANDLE_CORRUPT_RANGE; i++)
  ...
  for (i = 0; i < mhandle_hash_table_size; i++) {
    if (mhandle_hash_table[i].active && !mhandle_hash_table[i].removed) {
      if (mhandle_hash_table[i].ptr < ptr &&
          (prev == -1 ||
  ...
  if (prev >= 0)
    fprintf(stderr, "\tprev block: %s (%p, %ld bytes)\n",
            mhandle_hash_table[prev].at, mhandle_hash_table[prev].ptr,
            mhandle_hash_table[prev].size);
  if (next >= 0) {
    fprintf(stderr, "\tnext block: %s (%p, %ld bytes)\n",
            mhandle_hash_table[next].at, mhandle_hash_table[next].ptr,
...
void *mhandle_realloc(void *ptr, unsigned long size, char *at) {
  ...
  if (!ptr) return mhandle_malloc(size, at);
...
void *__xmalloc(size_t size, char *at) {
  ...
  ptr = malloc(size);
  if (!ptr) mhandle_out_of_memory(at);
...
void *__xrealloc(void *ptr, size_t size, char *at) {
  ...
  if (!new_ptr) mhandle_out_of_memory(at);
...
unsigned long __mhandle_used_memory() { return mhandle_mem_used; }
