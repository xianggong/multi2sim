...
class List {
 ...
 public:
  ...
  Iterator PushBack(Node& node) { return Insert(end(), node); }

  /// Insert an element at the beginning of the list, and return an
  /// iterator to the inserted element.
  Iterator PushFront(Node& node) { return Insert(begin(), node); }
  ...
  /// argument.
  Iterator Erase(Node& node) { return Erase(Iterator(&node)); }
};
