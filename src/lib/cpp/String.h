...
std::string fmt(const char* fmt_str, ...) __attribute__((format(printf, 1, 2)));
...
class StringMap {
  struct Item {
    ...
    Item(const char* text, int value) : text(text), value(value) {}
  };
 ...
 public:
  ...
  const char* operator[](int value) const { return MapValue(value); }
  ...
  int operator[](const std::string& s) const { return MapString(s); }
