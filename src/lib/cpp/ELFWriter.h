...
class Buffer {
  ...
  // Constructor
  Buffer(File* file, int index) : file(file), index(index) {}
 ...
 public:
  ...
  /// operators.
  void Read(char* buffer, size_t size) { stream.read(buffer, size); }
  ...
  void Write(const char* buffer, size_t size) { stream.write(buffer, size); }
...
class Symbol {
  ...
  Symbol(const std::string& name) : name(name), info({0}) {}
...
class SymbolTable {
  ...
  // Private constructor
  SymbolTable(File* file, const std::string& symtab, const std::string& strtab);
