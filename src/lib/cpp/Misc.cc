...
unsigned LogBase2(unsigned value) {
  ...
  if (value == 0) throw misc::Panic("Value cannot be 0");
...
std::string getCwd() {
  ...
  if (!getcwd(path, sizeof path))
    panic(
        "%s: cannot store the current working directory in a "
        "buffer of %d bytes",
        __FUNCTION__, (int)sizeof path);
  return path;
}
...
std::string getFullPath(const std::string& path, const std::string& cwd) {
  ...
  if (path_local[0] == '/') return path_local;

  while (StringPrefix(path_local, "./")) path_local.erase(0, 2);

  // File name is empty
  if (path_local.empty()) return path_local;

  // Default value for base directory
  std::string cwd_local = cwd.empty() ? getCwd() : cwd;

  // Add '/' suffix if not present
  if (cwd_local.back() != '/') cwd_local += '/';
...
std::string getExtension(const std::string& path) {
  ...
  if (slash_index != std::string::npos && slash_index > dot_index) return "";
...
std::string getBaseName(const std::string& path) {
  ...
  // No '.' found
  if (dot_index == std::string::npos) return path;

  // Last '.' comes before last '/'
  if (slash_index != std::string::npos && slash_index > dot_index) return path;
