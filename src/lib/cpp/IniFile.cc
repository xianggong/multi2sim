...
std::string IniFile::SectionVarToItem(const std::string& _section,
  ...
  // Check section name
  if (!section.length()) panic("%s: invalid section name", __FUNCTION__);

  // Create return std::string
  std::string item = section;
  if (var.length()) item += '|' + var;
...
bool IniFile::InsertSection(std::string section) {
  ...
  // Add section
  items[section] = "";
  if (!exists) sections.push_back(section);
...
bool IniFile::InsertVariable(std::string section, std::string var,
                             std::string value) {
  std::string item;
  bool exists;
...
IniFile::IniFile() { path = "<IniFile>"; }

IniFile::IniFile(const std::string& path) { Load(path); }
  ...
  for (auto it = sections.begin(); it != sections.end(); ++it) {
    ...
    for (auto it2 = items.begin(); it2 != items.end(); ++it2) {
      ...
      if (!strcasecmp(section.c_str(), tmp_section.c_str()) &&
          item.length() > section.length() && item[section.length()] == '|')
        os << var << " = " << it2->second << '\n';
    }
    os << '\n';
  }
...
void IniFile::Load(const std::string& path) {
  ...
  this->path = path;
  std::ifstream f(path);
  if (!f) throw Error(fmt("%s: Cannot read from file", path.c_str()));
    ...
    if (line[0] == '[' && line[line.length() - 1] == ']') {
      ...
      if (!ok)
        throw misc::Error(misc::fmt(
            ...
            path.c_str(), line_num, section.c_str(), ini_file_err_format));
    ...
    if (!ok)
      throw Error(
          fmt("%s: Line %d: "
              ...
              path.c_str(), line_num, var.c_str(), ini_file_err_format));

    // Debug
    debug << fmt("%s: Parsed section [%s], variable '%s', value '%s'\n",
                 path.c_str(), section.c_str(), var.c_str(), value.c_str());
  }
}
...
void IniFile::Save(const std::string& path) const {
  // Open file
  std::ofstream f(path);
  if (!f) throw Error(fmt("%s: Cannot write to file", path.c_str()));
...
bool IniFile::Exists(const std::string& section, const std::string& var) const {
...
void IniFile::WriteEnum(const std::string& section, const std::string& var,
  ...
  if (error)
    throw Error(
        fmt("%s: Invalid value for enumeration (%d)", __FUNCTION__, value));
...
std::string IniFile::ReadString(const std::string& section,
  ...
  if (it == items.end()) {
    debug << fmt(
        "%s: Read section [%s], variable '%s', "
        "not found, default is '%s'\n",
        path.c_str(), section.c_str(), var.c_str(), def.c_str());
    return def;
  } else {
    debug << fmt(
        "%s: Read section [%s], variable '%s', "
        "value '%s'\n",
        path.c_str(), section.c_str(), var.c_str(), it->second.c_str());
    return it->second;
  }
}
...
int IniFile::ReadInt(const std::string& section, const std::string& var,
  ...
  if (value.empty()) {
    debug << fmt(
        ...
        path.c_str(), section.c_str(), var.c_str(), def);
    return def;
  }
  ...
  debug << fmt(
      ...
      "variable '%s', "
      "value %d\n",
      path.c_str(), section.c_str(), var.c_str(), result);
  return result;
}

long long IniFile::ReadInt64(const std::string& section, const std::string& var,
                             long long def) {
  std::string value;
  StringError error;
  long long result;
...
bool IniFile::ReadBool(const std::string& section, const std::string& var,
  ...
  if (s.empty()) {
    debug << fmt(
        "%s: Read section [%s], variable '%s', "
        "not found, default is %s\n",
        path.c_str(), section.c_str(), var.c_str(), def ? "True" : "False");
    return def;
  }
  ...
  // Invalid value
  throw Error(
      fmt("%s: Section [%s], variable '%s', invalid value '%s'\n"
          "\tPossible values are {t|True|On|f|False|Off}\n",
          path.c_str(), section.c_str(), var.c_str(), s.c_str()));
  return false;
}

double IniFile::ReadDouble(const std::string& section, const std::string& var,
                           double def) {
  // Read value
  std::string s = ReadString(section, var);
  if (s.empty()) {
  ...
  debug << fmt(
      ...
      "variable '%s', "
      "value %f\n",
      path.c_str(), section.c_str(), var.c_str(), value);
  return value;
}
...
int IniFile::ReadEnum(const std::string& section, const std::string& var,
  ...
  if (s.empty()) {
    debug << fmt(
        ...
        "variable '%s', "
        "not found, default is '%s'\n",
        path.c_str(), section.c_str(), var.c_str(), map.MapValue(def));
    return def;
  }
  ...
  if (!error) {
    debug << fmt(
        ...
        "variable '%s', "
        "value '%s'\n",
        path.c_str(), section.c_str(), var.c_str(), map.MapValue(value));
    return value;
  }
...
void IniFile::Check() const {
  ...
  for (auto it = enforced_items.begin(); it != enforced_items.end(); ++it) {
    ...
    if (var == "")
      throw Error(
          fmt("%s: Section [%s] missing", path.c_str(), section.c_str()));
    else
      throw Error(fmt("%s: Section [%s], variable '%s' missing", path.c_str(),
                      section.c_str(), var.c_str()));
  }
  ...
  for (auto it = items.begin(); it != items.end(); ++it) {
    // Item is allowed
    item = it->first;
    if (allowed_items.count(item)) continue;

    // Item not allowed
    ItemToSectionVar(item, section, var);
    if (var == "")
      throw Error(
          fmt("%s: Invalid section [%s]", path.c_str(), section.c_str()));
    else
      throw Error(fmt("%s: Section [%s], invalid variable '%s'", path.c_str(),
                      section.c_str(), var.c_str()));
  }
}
...
void IniFile::Check(const std::string& ref_section) const {
  ...
  for (auto it = enforced_items.begin(); it != enforced_items.end(); ++it) {
    ...
    std::string item = *it;
    if (!StringPrefix(item, ref_section + "|")) continue;
