...
Section::Section(File* file, int index, unsigned info_offset)
  ...
  if (info_offset < 0 || info_offset + sizeof(Elf32_Shdr) > file->getSize())
...
ProgramHeader::ProgramHeader(File* file, int index, unsigned info_offset)
  ...
  if (info_offset < 0 || info_offset + sizeof(Elf32_Phdr) > file->getSize())
    throw Error(file->getPath(),
                "Invalid position for program "
                "header");
...
Symbol::Symbol(File* file, Section* section, unsigned int pos) {
  ...
  buffer = nullptr;
  if (this->section &&
      info->st_value + info->st_size <= this->section->getSize())
...
void Symbol::getStream(std::istringstream& stream, unsigned int offset,
  ...
  if (!buffer)
    throw Error(
        file->getPath(),
        misc::fmt("symbol '%s' does not have any valid content", name.c_str()));

  // Check valid offset/size
  if (size > info->st_size)
    throw Error(file->getPath(),
                misc::fmt("symbol '%s': invalid size", name.c_str()));
...
void File::ReadHeader() {
  ...
  if (size < sizeof(Elf32_Ehdr)) throw Error(path, "Invalid ELF file");
  ...
  if (info->e_ident[EI_CLASS] == ELFCLASS64)
    throw Error(path,
                misc::fmt("64-bit ELF files not supported\n\n%s", err_64bit));
}
...
void File::ReadSections() {
  ...
  for (int i = 0; i < info->e_shnum; i++)
    sections.emplace_back(misc::new_unique<Section>(
        this, i, info->e_shoff + i * info->e_shentsize));
  ...
  for (auto& section : sections)
    section->setName(string_table->getBuffer() + section->getNameOffset());
}
...
void File::ReadProgramHeaders() {
  ...
  if (!info->e_phnum) return;

  // Check program header size
  if (info->e_phentsize != sizeof(Elf32_Phdr))
    throw Error(path, misc::fmt("Program header size %d (should be %d)",
  ...
  for (int i = 0; i < info->e_phnum; i++)
    program_headers.emplace_back(misc::new_unique<ProgramHeader>(
        this, i, info->e_phoff + i * info->e_phentsize));
}
...
void File::ReadSymbols() {
  ...
  for (auto& section : sections) {
    ...
    if (section->getType() != 2 && section->getType() != 11) continue;
    ...
    for (int i = 0; i < num_symbols; i++) {
      ...
      symbols.emplace_back(
          misc::new_unique<Symbol>(this, section.get(), i * sizeof(Elf32_Sym)));

      // Discard empty symbol
      Symbol* symbol = symbols.back().get();
      if (symbol->getName().empty()) symbols.pop_back();
    }
  }
...
File::File(const std::string& path, bool read_content) : path(path) {
  // Open file
  std::ifstream f(path);
  if (!f) throw Error(path, "Cannot open file");
...
std::ostream& operator<<(std::ostream& os, const File& file) {
  ...
  // Dump
  os << "Symbol table:\n";
  os << misc::fmt("%-40s %-15s %-12s %-12s %-10s %-10s", "name", "section",
                  "value", "size", "info", "other");
  os << std::string(80, '-') << '\n';
...
Section* File::getSection(const std::string& name) const {
  // Search
  for (auto& section : sections)
    if (section->getName() == name) return section.get();
...
Symbol* File::getSymbol(const std::string& name) const {
  ...
  for (auto& symbol : symbols)
    if (symbol->getName() == name) return symbol.get();
...
Symbol* File::getSymbolByAddress(unsigned int address,
  ...
  // Empty symbol table
  if (!symbols.size()) return nullptr;

  // All symbols in the table have a higher address
  if (address < symbols[0]->getValue()) return nullptr;
  ...
  if (!symbol->getValue()) return nullptr;
  ...
  for (;;) {
    ...
    if (prev_symbol->getValue() != symbol->getValue()) break;

    // Take symbol if it has global/local/weak binding
    if (ELF32_ST_BIND(prev_symbol->getInfo()) < 3) symbol = prev_symbol;
  }
