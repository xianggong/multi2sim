...
void Section::Dump(std::ostream& os) const {
  ...
  os << "Dumping section '" << name << "' (buffers " << first << last << ")\n";
...
void Segment::Dump(std::ostream& os) const {
  ...
  int first = first_buffer->getIndex();
  int last = last_buffer->getIndex();
  os << "Dumping segment '" << name << "' (buffers " << first << last << ")\n";
...
void SymbolTable::Generate() {
  ...
  for (auto& symbol : symbols) {
    ...
    strtab_buffer->Write(symbol->name.c_str(), symbol->name.length() + 1);
  }
}
...
File::File() {
  ...
  Section* shstrtab_section =
      newSection(".shstrtab", shstrtab_buffer, shstrtab_buffer);
...
Buffer* ELFWriter::File::newBuffer() {
  buffers.push_back(std::unique_ptr<Buffer>(new Buffer(this, buffers.size())));
  return buffers.back().get();
}

Segment* File::newSegment(const std::string& name, Buffer* first,
                          Buffer* last) {
  segments.push_back(std::unique_ptr<Segment>(
      new Segment(this, name, first, last, segments.size())));
  return segments.back().get();
}

Section* File::newSection(const std::string& name, Buffer* first,
                          Buffer* last) {
  // Add to list and set index
  sections.push_back(std::unique_ptr<Section>(
      new Section(this, name, first, last, sections.size())));
...
SymbolTable* File::newSymbolTable(const std::string& symtab,
                                  const std::string& strtab) {
  symbol_tables.push_back(
      std::unique_ptr<SymbolTable>(new SymbolTable(this, symtab, strtab)));
...
void File::Generate(std::ostream& os) {
  ...
  for (auto& segment : segments) {
    ...
    segment->info.p_offset = sizeof(Elf32_Ehdr) + phtab_size + buf_offset;
  }

  // Find actual segment sizes
  for (auto& segment : segments) {
    for (int j = segment->first_buffer->index; j <= segment->last_buffer->index;
         j++) {
  ...
  for (auto& section : sections) {
    ...
    for (int j = 0; j < section->first_buffer->index; j++)
      buf_offset += buffers[j]->getSize();
    section->info.sh_offset = sizeof(Elf32_Ehdr) + phtab_size + buf_offset;
  }
  ...
  for (auto& section : sections) {
    for (int j = section->first_buffer->index; j <= section->last_buffer->index;
         j++)
  ...
  for (unsigned i = 0; i < sections.size(); i++) shtab[i] = sections[i]->info;

  // Create program header table
  Elf32_Phdr phtab[segments.size()];
  for (unsigned i = 0; i < segments.size(); i++) phtab[i] = segments[i]->info;
...
void File::Generate(const std::string& path) {
  ...
  if (!of) misc::fatal("%s: cannot write to file", path.c_str());
  Generate(of);
  of.close();
}
