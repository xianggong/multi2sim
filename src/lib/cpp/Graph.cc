...
void Graph::addEdge(std::unique_ptr<Edge>&& edge, Vertex* source_vertex,
                    Vertex* destination_vertex) {
  edges.emplace_back(std::move(edge));
  source_vertex->outgoing_vertices.emplace_back(destination_vertex);
  destination_vertex->incoming_vertices.emplace_back(source_vertex);
}

bool Graph::removeEdge(Edge* edge) {
  // Look for the edge in the list of edges
  auto it = std::find_if(edges.begin(), edges.end(),
                         [&](std::unique_ptr<Edge> const& unique_edge) {
  ...
  // If found
    ...
    auto destination_vertex_it =
        std::find(source_vertex->outgoing_vertices.begin(),
                  source_vertex->outgoing_vertices.end(), destination_vertex);
    assert(destination_vertex_it != source_vertex->outgoing_vertices.end());
    ...
    auto source_vertex_it =
        std::find(destination_vertex->incoming_vertices.begin(),
                  destination_vertex->incoming_vertices.end(), source_vertex);
    assert(source_vertex_it != destination_vertex->incoming_vertices.end());
...
Edge* Graph::findEdge(Vertex* source_vertex, Vertex* destination_vertex) {
  ...
  auto it = std::find_if(
      ...
      [&](std::unique_ptr<Edge> const& unique_edge) {
        return ((unique_edge.get()->source_vertex == source_vertex) &&
                (unique_edge.get()->destination_vertex == destination_vertex));
      });
...
void Graph::GreedyCycleRemoval() {
  ...
  for (auto& vertex : vertices) {
    ...
    vertex->key =
        vertex->getNumOutgoingVertices() - vertex->getNumIncomingVertices();
...
int Graph::CoffmanGrahamLayering(int width) {
  ...
  while (label <= vertices.size()) {
    ...
    for (auto& vertex : vertices) {
      ...
      if (is_candidate) candidates.emplace_back(vertex.get());
    }

    // Choose one of the candidates for the current label
    if (candidates.empty())
      throw Panic(
          "Graph drawing algorithm is not functioning"
          "properly.");
    ...
    for (unsigned i = 1; i < candidates.size(); i++) {
      ...
      std::sort(alternate_candidate->incoming_vertices.begin(),
                alternate_candidate->incoming_vertices.end(), Vertex::Compare);
      ...
      while (it <= minimum_indegree) {
        ...
        candidate_lexical =
            candidate
                ->incoming_vertices[candidate->incoming_vertices.size() - it];
        alternate_lexical =
            alternate_candidate->incoming_vertices
  ...
  for (auto& vertex : vertices) unordered_list.emplace_back(vertex.get());
  ...
  while (!unordered_list.empty()) {
    ...
    for (auto i = unordered_list.end() - 1; i >= unordered_list.begin(); --i) {
      Vertex* vertex = *i;
      ...
      if (vertex->outgoing_vertices.size() == 0) {
        ...
        if (!unordered_vertex) candidate = vertex;
      }
      ...
      for (Vertex* ordered_vertex : ordered_list)
        if (ordered_vertex->y_value == layer) layer_width++;
  ...
  int num_layers = 0;
  for (auto& vertex : vertices)
    if (vertex->y_value > num_layers) num_layers = vertex->y_value;
  return num_layers;
}
...
void Graph::InitializeXValues(int num_layers) {
  ...
  for (int layer = 0; layer <= num_layers; layer++) {
    ...
    for (auto& vertex : vertices)
      if (vertex->y_value == layer) vertex->neighbors = num_vertices_in_layer;
  }
}
...
void Graph::CrossReduction(int num_layers) {
  ...
  for (int it = 0; it < reduction_count; it++) {
    ...
    for (int layer = begin; layer <= end; layer += direction) {
      ...
      for (auto& vertex : vertices) {
        ...
        if (vertex->y_value == layer) {
          ...
          // source vertices
            ...
            // edges
            for (Vertex* incoming_vertex : vertex->incoming_vertices)
      ...
      std::sort(neighboring_vertices.begin(), neighboring_vertices.end(),
                Vertex::Compare);
...
bool Graph::CycleDetectionDepthFirstTraverse(int vertex_id, bool* visited,
                                             bool* stacked) {
  ...
  if (visited[vertex_id] == false) {
    ...
    for (int i = 0; i < vertex->getNumOutgoingVertices(); i++) {
      ...
      Vertex* adjacent_vertex = vertex->outgoing_vertices[i];
      ...
      auto it = std::find_if(vertices.begin(), vertices.end(),
                             [&](std::unique_ptr<Vertex> const& unique_vertex) {
                               return unique_vertex.get() == adjacent_vertex;
                             });
      ...
      int adjacent_vertex_id = std::distance(vertices.begin(), it);
      ...
      if ((visited[adjacent_vertex_id] == false) &&
          (CycleDetectionDepthFirstTraverse(adjacent_vertex_id, visited,
                                            stacked)))
        return true;
