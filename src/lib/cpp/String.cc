...
void StringTokenize(const std::string& s, std::vector<std::string>& tokens,
  ...
  for (unsigned i = 0; i <= s.length(); i++) {
    ...
    // Start a token
    if (!is_end && !StringHasChar(set, s[i]) && token_start == -1)
      token_start = i;
...
int StringMap::MapString(const std::string& s, bool& error) const {
  ...
  error = false;
  for (auto& item : items)
    if (item.text == s) return item.value;
...
int StringMap::MapStringCase(const std::string& s, bool& error) const {
  ...
  error = false;
  for (auto& item : items)
    if (!strcasecmp(item.text, s.c_str())) return item.value;
...
void StringFormatter::AddIndent() {
  ...
  if (total_indent >= width) total_indent = width - 1;
...
void StringFormatter::AddWord(const std::string& word) {
  ...
  if (!first_word_in_line &&
      current_line_width + (int)word.length() + 1 <= width) {
    stream << ' ' << word;
  ...
  while (true) {
    ...
    if (chunk_offset >= (int)word.length()) break;
  }
}
...
void StringFormatter::Format() {
  ...
  for (int i = 0; i <= (int)text.length(); i++) {
    ...
    if (first_word_in_paragraph && c == ' ') paragraph_indent++;

    // Add 8 extra indentations spaces to current paragraph
    if (first_word_in_paragraph && c == '\t') paragraph_indent += 8;
