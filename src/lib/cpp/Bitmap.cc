...
Bitmap::Bitmap(size_t size) {
  ...
  mask = num_bits_in_last_word == bits_per_block
             ? -1ul
...
Bitmap::Bitmap(const Bitmap& b) {
  ...
  mask = b.mask;
  data = misc::new_unique_array<size_t>(size_in_blocks);
  for (size_t i = 0; i < size_in_blocks; i++) data.get()[i] = b.data.get()[i];
}
...
void Bitmap::Dump(std::ostream& os) const {
  for (size_t i = 0; i < size; i++) os << (Test(i) ? '1' : '0');
}
...
Bitmap& Bitmap::Set() {
  for (size_t i = 0; i < size_in_blocks; i++) data.get()[i] = ~0ul;
...
Bitmap& Bitmap::Flip() {
  for (size_t i = 0; i < size_in_blocks; i++) data.get()[i] = ~data.get()[i];
  data.get()[size_in_blocks - 1] &= mask;
  return *this;
}
...
bool Bitmap::operator==(const Bitmap& b) const {
  ...
  for (size_t i = 0; i < size_in_blocks; i++)
    if (data.get()[i] != b.data.get()[i]) return false;
  return true;
}
...
Bitmap& Bitmap::operator<<=(size_t n) {
  ...
  for (size_t i = size - 1; i >= n; i--) Set(i, Test(i - n));
  for (size_t i = n - 1; i >= 0; i--) Set(i, false);
  return *this;
}
...
Bitmap& Bitmap::operator>>=(size_t n) {
  ...
  for (size_t i = 0; i < size - n; i++) Set(i, Test(i + n));
  for (size_t i = size - n; i < size; i++) Set(i, false);
  return *this;
}
