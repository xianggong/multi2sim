...
class Section {
 ...
 public:
  ...
  void getStream(std::istringstream& stream, unsigned offset,
                 unsigned size) const;
...
class File {
 ...
 public:
  ...
  const std::vector<std::unique_ptr<ProgramHeader>>& getProgramHeaders() const {
  ...
  Symbol* getSymbol(int index) const {
    return index >= 0 && index < (int)symbols.size() ? symbols[index].get()
                                                     : nullptr;
  }
  ...
  Symbol* getSymbolByAddress(unsigned int address, unsigned int& offset) const;
