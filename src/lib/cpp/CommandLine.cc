...
CommandLineOption::CommandLineOption(Type type, const std::string& name,
    ...
    : type(type), num_args(num_args), help(help) {
  // Option name
  ...
  *variable = StringToInt(argument, error);
  ...
  if (error)
    throw CommandLine::Error(
        misc::fmt("Invalid value for option "
                  "'%s': %s",
                  getName().c_str(), StringErrorToString(error)));
}
  ...
  if (error)
    throw CommandLine::Error(
        misc::fmt("Invalid value for option "
                  ...
                  getName().c_str(), argument.c_str(), map.toString().c_str()));
}

void CommandLineCategory::Help(std::ostream& os) {
  // Skip category if it has no options
  if (option_map.size() == 0) return;

  // Header for categories other than default
  std::string header = description.empty() ? name + " Options" : description;
  os << "\n";
  os << header << "\n";
  ...
  for (auto it : option_map) it.second->Help(os);
}
...
CommandLine::CommandLine() {
  ...
  categories.emplace_back(misc::new_unique<CommandLineCategory>("default"));
...
void CommandLine::Register(std::unique_ptr<CommandLineOption>&& option) {
  ...
  if (!StringPrefix(option->getName(), "-"))
    throw misc::Panic(
        misc::fmt("Option name must start with a "
                  "dash (%s)",
                  option->getName().c_str()));

  // Options with one single dash must have one letter only
  if (!StringPrefix(option->getName(), "--") && option->getName().length() != 2)
    throw misc::Panic(
        misc::fmt("Option with one dash must be one "
                  "single character (%s)",
                  option->getName().c_str()));
  ...
  if (option_table.find(option->getName()) != option_table.end())
    throw misc::Panic(
        misc::fmt("Option name already registered "
                  "(%s)",
                  option->getName().c_str()));
...
void CommandLine::Help(std::ostream& os) {
  ...
  for (auto it : category_map) {
    ...
    if (category->getName() != "default") category->Help();
  }
}
...
void CommandLine::setCategory(const std::string& name,
  ...
  if (category_map.find(name) == category_map.end()) {
    categories.emplace_back(misc::new_unique<CommandLineCategory>(name));
  ...
  if (!description.empty()) category->setDescription(description);
...
void CommandLine::setIncompatible(const std::string& name) {
  ...
  if (it == option_table.end())
    throw misc::Panic(misc::fmt("Option no registered (%s)", name.c_str()));
...
void CommandLine::Process(int argc, char** argv, bool options_anywhere) {
  ...
  while (arguments.size() > 0) {
    ...
    bool attached_argument =
        !StringPrefix(argument, "--") && argument.length() > 2;
    ...
    if (attached_argument && option->getNumArguments() != 1)
      throw Error(
          misc::fmt("Option '%s' expects %d "
                    "argument(s)",
                    option->getName().c_str(), option->getNumArguments()));
    ...
    if (options.find(argument) != options.end())
      throw Error(
          misc::fmt("Multiple occurrences of "
                    "option '%s'",
                    option->getName().c_str()));
  ...
  if (show_help) {
    if (this->arguments.size() || options.size())
      throw Error(
          "Options '--help' is incompatible with any "
