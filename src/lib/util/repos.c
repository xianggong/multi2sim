...
struct repos_t *repos_create(int object_size, char *name) {
  ...
  /* Check */
  if (object_size <= 0) panic("%s: invalid object size", __FUNCTION__);
...
void repos_free(struct repos_t *repos) { repos_free_dump(repos, NULL); }
...
void *repos_create_object(struct repos_t *repos) {
  ...
  if (next_obj) next_objtail->prev = NULL;
  ...
  next_objtail = next_obj + repos->object_size;
  if (next_obj) next_objtail->prev = obj;
...
void repos_free_object(struct repos_t *repos, void *obj) {
  ...
  next_obj = objtail->next;
  next_objtail = next_obj + repos->object_size;
  if (prev_obj) prev_objtail->next = next_obj;
  if (next_obj) next_objtail->prev = prev_obj;
  if (repos->alloc_head == obj) repos->alloc_head = next_obj;
