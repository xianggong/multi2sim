...
static int hash_table_get_index(struct hash_table_t *table, char *key) {
  ...
  while (*key) {
    c = *(unsigned char *)key;
    if (!table->case_sensitive) c = tolower(c);
    hash = (hash ^ c) * prime;
    key++;
  }
  hash %= table->size;
...
static struct hash_table_elem_t *hash_table_find(struct hash_table_t *table,
                                                 char *key, int *index_ptr) {
  struct hash_table_elem_t *elem;
  ...
  for (elem = table->elem_vector[index]; elem; elem = elem->next)
    if (!table->str_compare_func(key, elem->key)) return elem;
...
struct hash_table_t *hash_table_create(int size, int case_sensitive) {
  ...
  table->size =
      size < HASH_TABLE_MIN_INITIAL_SIZE ? HASH_TABLE_MIN_INITIAL_SIZE : size;
...
int hash_table_insert(struct hash_table_t *table, char *key, void *data) {
  ...
  /* Rehashing */
  if (table->count >= table->size / 2) hash_table_grow(table);
...
int hash_table_count(struct hash_table_t *table) { return table->count; }
...
void *hash_table_remove(struct hash_table_t *table, char *key) {
  ...
  for (elem = table->elem_vector[index]; elem; elem = elem->next) {
    ...
    if (!table->str_compare_func(elem->key, key)) break;
...
char *hash_table_find_next(struct hash_table_t *table, void **data_ptr) {
  ...
  if (!table->find_elem) return NULL;
