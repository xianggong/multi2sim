...
static void heapify(struct heap_t *heap, int i) {
  ...
  for (;;) {
    ...
    r = RIGHT(i);
    k = i;
    if (l < heap->count && heap_less_than(heap, l, k)) k = l;
    if (r < heap->count && heap_less_than(heap, r, k)) k = r;
    if (k == i) break;
...
int heap_error(struct heap_t *heap) { return heap->error; }
...
long long heap_next(struct heap_t *heap, void **data) {
  ...
  heap->current++;
  heap->error = 0;
  if (data) *data = heap->elem[heap->current].data;
