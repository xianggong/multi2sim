...
void bit_map_free(struct bit_map_t *bit_map) { free(bit_map); }

/* Set 'size' bits to 'value' starting at position 'where'. Argument 'size'
 * cannot
 * exceed 32 bits. */
void bit_map_set(struct bit_map_t *bit_map, unsigned int where,
  ...
  if (where_word1 >= bit_map->word_count) return;
...
unsigned int bit_map_get(struct bit_map_t *bit_map, unsigned int where,
                         unsigned int size) {
  unsigned int where_word1, where_offset1, size_align1;
  ...
  /* Special case: 32-bit aligned word */
  if (size == 32 && !where_offset1) return word1;
  ...
  /* If the whole size was covered in the first chunk, exit */
  if (size_align1 == size) return result;
...
int bit_map_count_ones(struct bit_map_t *bit_map, unsigned int where,
                       unsigned int size) {
  unsigned int i;
  int count = 0;

  for (i = 0; i < size; i++) count += bit_map_get(bit_map, where + i, 1);
  return count;
}

void bit_map_copy(struct bit_map_t *dst, unsigned int dst_where,
                  struct bit_map_t *src, unsigned int src_where,
                  unsigned int size) {
...
void bit_map_dump(struct bit_map_t *bit_map, unsigned int where,
                  unsigned int size, FILE *f) {
  unsigned int i;
  for (i = 0; i < size; i++)
