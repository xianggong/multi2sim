...
int str_map_string_err(struct str_map_t *map, char *string, int *err_ptr) {
  ...
  /* Find string in map */
  for (i = 0; i < map->count; i++)
    if (!strcmp(string, map->map[i].string)) return map->map[i].value;

  /* Error */
  if (err_ptr) *err_ptr = 1;
  return 0;
}
...
int str_map_string_case_err(struct str_map_t *map, char *s, int *err_ptr) {
  ...
  /* Find string in map */
  for (i = 0; i < map->count; i++)
    if (!strcasecmp(s, map->map[i].string)) return map->map[i].value;
...
char *str_map_value(struct str_map_t *map, int value) {
  int i;
  for (i = 0; i < map->count; i++)
    if (map->map[i].value == value) return map->map[i].string;
  return unknown;
}
...
char *str_token_list_shift(struct list_t *token_list) {
  ...
  if (!list_count(token_list)) return "";
...
int str_token_list_find(struct list_t *token_list, char *token) {
  ...
  if (!strcmp(token, list_get(token_list, index))) return index;
  return -1;
}
...
int str_token_list_find_case(struct list_t *token_list, char *token) {
  ...
  LIST_FOR_EACH(token_list, index)
  if (!strcasecmp(token, list_get(token_list, index))) return index;
  return -1;
}
...
char *str_error(int err) { return str_map_value(&str_error_map, err); }
...
int str_suffix(char *str, char *suffix) {
  ...
  if (strcmp(str + str_len - suffix_len, suffix)) return 0;
  return 1;
}
...
int str_prefix(char *str, char *prefix) {
  ...
  if (!strncmp(str, prefix, prefix_len)) return 1;
  return 0;
}

/* Copy a substring of 'src' into 'dest' */
void str_substr(char *dest, int dest_size, char *src, int src_pos,
                int src_count) {
  int src_len;
  int count;
  ...
  src_len = strlen(src);
  count = src_count;
  if (count > dest_size - 1) count = dest_size - 1;
  if (count > src_len - src_pos) count = src_len - src_pos;
...
void str_token(char *dest, int dest_size, char *src, int index, char *delim) {
  ...
  for (i = 0; i < index; i++) token = strtok(NULL, delim);
...
void str_trim(char *dest, int size, char *src) {
  ...
  /* Check valid 'size' */
  if (size < 1) panic("%s: invalid value for 'size'", __FUNCTION__);
  ...
  right_trim = 0;
  for (i = len - 1; i >= 0 && isspace(src[i]); i--) right_trim++;
  ...
  left_trim = 0;
  for (i = 0; i < len && isspace(src[i]); i++) left_trim++;
  ...
  /* Create new string */
  dest[new_len] = '\0';
  for (i = 0; i < new_len; i++) dest[i] = src[i + left_trim];
}
