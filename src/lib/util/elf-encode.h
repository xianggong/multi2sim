...
/* Writes 'size' bytes from the buffer pointed to by 'ptr' into the current
 * position
 * in the section. If the written contents exceed the size of the section, this
 * size
 * will increase automatically. The current position in the section moves
 * forward
 * 'size' bytes. */
void elf_enc_buffer_write(struct elf_enc_buffer_t* buffer, void* ptr,
                          unsigned int size);

/* Tries to read 'size' bytes from the section at the current position, and
 * stores
 ...
 * is the user's responsibility to make sure ptr has enough space for its
 * contents */
