...
int file_can_open_for_write(char *fname) {
  FILE *f;
  if (!fname[0]) return 0;
  if (!strcmp(fname, "stdout") || !strcmp(fname, "stderr")) return 1;
  f = fopen(fname, "wt");
  if (!f) return 0;
  fclose(f);
  return 1;
...
int file_read_line(FILE *f, char *line, int size) {
  ...
  if (!line || feof(f)) return -1;
  while (strlen(line) &&
         (line[strlen(line) - 1] == 13 || line[strlen(line) - 1] == 10))
...
FILE *file_create_temp(char *ret_path, int ret_path_size) {
  ...
  if (ret_path) snprintf(ret_path, ret_path_size, "%s", path);
  return f;
}

/* Return the absolute path for 'file_name'. If 'file_name' is an absolute path,
 * the
 * same string is returned. If 'file_name' does not start with a slash '/', it
 * is
 * considered relative to 'default_path'. If 'default_path' is NULL or an empty
 * string,
 * 'default_path' defaults to the current working directory.
 * The absolute path is placed in 'full_path', a buffer with 'size' bytes
 * allocated.
