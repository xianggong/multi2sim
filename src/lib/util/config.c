 ...
 *    If a key is the name of a section, it is represented as the name of the
 * section.
 *    If a key is the name of a variable, it is represented as
 * "<section>\n<var>"
 *
 */
struct config_t {
   ...
   * The values for sections are (void *) 1, while the values for variables are
   * the
...
static void get_section_var_from_item(char *item, char *section,
                                      int section_size, char *var,
                                      int var_size) {
  char *section_brk;
...
static int config_insert_section(struct config_t *config, char *section) {
  ...
  if (ret) linked_list_add(config->section_list, xstrdup(section_trim));

  return !ret;
}

/* Return non-zero if the variable already exists */
static int config_insert_var(struct config_t *config, char *section, char *var,
                             char *value) {
...
void config_load(struct config_t *config) {
  ...
  if (!f) fatal("%s: cannot open configuration file", config->file_name);
  ...
  while (!feof(f)) {
    ...
    if (!line_trim[0] || line_trim[0] == ';' || line_trim[0] == '#') continue;
    ...
    if (line_trim[0] == '[' && line_trim[length - 1] == ']') {
      ...
      if (err)
        fatal("%s: line %d: duplicated section '%s'.\n%s", config->file_name,
              line_num, section, config_err_format);
    ...
    if (!section[0])
      fatal("%s: line %d: section name expected.\n%s", config->file_name,
            line_num, config_err_format);

    /* New "<var> = <value>" entry. */
    err = get_var_value_from_item(line_trim, var, sizeof var, value,
                                  sizeof value);
    if (err)
      fatal("%s: line %d: invalid format.\n%s", config->file_name, line_num,
            config_err_format);
    ...
    if (err)
      fatal("%s: line %d: duplicated variable '%s'.\n%s", config->file_name,
            line_num, var, config_err_format);
  }
...
void config_save(struct config_t *config) {
  ...
  if (!f) fatal("%s: cannot save configuration file", config->file_name);
  ...
  LINKED_LIST_FOR_EACH(config->section_list) {
    ...
    HASH_TABLE_FOR_EACH(config->items, item, value) {
      get_section_var_from_item(item, section_buf, sizeof section_buf, var_buf,
                                sizeof var_buf);
...
void config_write_string(struct config_t *config, char *section, char *var,
                         char *value) {
  char item[MAX_LONG_STRING_SIZE];
...
void config_write_int(struct config_t *config, char *section, char *var,
                      int value) {
  char value_str[MAX_LONG_STRING_SIZE];
...
void config_write_llint(struct config_t *config, char *section, char *var,
                        long long value) {
  char value_str[MAX_LONG_STRING_SIZE];
...
void config_write_bool(struct config_t *config, char *section, char *var,
                       int value) {
  char value_str[MAX_LONG_STRING_SIZE];
...
void config_write_double(struct config_t *config, char *section, char *var,
                         double value) {
  char value_str[MAX_LONG_STRING_SIZE];
...
void config_write_ptr(struct config_t *config, char *section, char *var,
                      void *value) {
  char value_str[MAX_LONG_STRING_SIZE];
...
char *config_read_string(struct config_t *config, char *section, char *var,
                         char *def) {
  char item[MAX_LONG_STRING_SIZE];
  char *value;
...
int config_read_int(struct config_t *config, char *section, char *var,
                    int def) {
  char *result;
...
long long config_read_llint(struct config_t *config, char *section, char *var,
                            long long def) {
  char *result;
  long long value;
  int err;
...
int config_read_bool(struct config_t *config, char *section, char *var,
                     int def) {
  char *result;
  ...
  /* Invalid value */
  fatal(
      "%s: Section [ %s ]: Invalid value for '%s'\n"
      "\tPossible values are {t|True|On|f|False|Off}\n",
      config_get_file_name(config), section, var);
  return 0;
}

double config_read_double(struct config_t *config, char *section, char *var,
                          double def) {
  char *result;
  double d;
...
int config_read_enum(struct config_t *config, char *section, char *var, int def,
                     char **map, int map_count) {
  char *result;
  int i;
  ...
  /* Translate */
  for (i = 0; i < map_count; i++)
    if (!strcasecmp(map[i], result)) return i;
...
void *config_read_ptr(struct config_t *config, char *section, char *var,
                      void *def) {
  char *result;
  void *ptr;
  result = config_read_string(config, section, var, NULL);
...
/* Insert a section (and variable) into the hash table of allowed sections
 * (variables).
...
static void allowed_items_insert(struct config_t *config, char *section,
                                 char *var, void *property) {
  char item[MAX_LONG_STRING_SIZE];
...
void config_check(struct config_t *config) {
  ...
  for (item = hash_table_find_first(config->allowed_items, &property); item;
       item = hash_table_find_next(config->allowed_items, &property)) {
    /* If this is an allowed (not mandatory) item, continue */
    if (property == ITEM_ALLOWED) continue;
  ...
  for (item = hash_table_find_first(config->items, NULL); item;
       item = hash_table_find_next(config->items, NULL)) {
    /* Check if it is allowed */
    get_section_var_from_item(item, section, sizeof section, var, sizeof var);
    if (item_is_allowed(config, section, var)) continue;

    /* It is not, error */
    if (!var[0])
      fatal("%s: invalid section [ %s ]", config->file_name, section);
    else
      fatal("%s: section [ %s ]: invalid variable '%s'", config->file_name,
            section, var);
  }
}
...
void config_section_check(struct config_t *config, char *section_ref) {
  ...
  for (item = hash_table_find_first(config->allowed_items, &property); item;
    ...
    get_section_var_from_item(item, section, sizeof section, var, sizeof var);
    if (strcasecmp(section, section_ref)) continue;
