...
static void list_grow(struct list_t *list) {
  ...
  for (i = list->head, index = 0; index < list->count;
...
static void sort(struct list_t *list, int lo, int hi,
  ...
  ptr = ELEM(hi);
  do {
    while (comp(ELEM(i), ptr) < 0) i++;
    while (comp(ELEM(j), ptr) > 0) j--;
    if (i <= j) {
      tmp = ELEM(i);
      ELEM(i) = ELEM(j);
      ELEM(j) = tmp;
  ...
  if (lo < j) sort(list, lo, j, comp);
  if (i < hi) sort(list, i, hi, comp);
}
...
int list_count(struct list_t *list) { return list->count; }

void list_add(struct list_t *list, void *elem) {
  /* Grow list if necessary */
  if (list->count == list->size) list_grow(list);
...
void list_insert(struct list_t *list, int index, void *elem) {
  ...
  if (index > list->count / 2) {
    ...
    for (i = 0, pos = list->tail; i < shiftcount; i++, pos = INLIST(pos - 1))
      list->elem[pos] = list->elem[INLIST(pos - 1)];
    list->tail = (list->tail + 1) % list->size;
  } else {
    for (i = 0, pos = list->head; i < index; i++, pos = (pos + 1) % list->size)
...
int list_index_of(struct list_t *list, void *elem) {
  ...
  /* Search element */
  list->error_code = LIST_ERR_OK;
  for (i = 0, pos = list->head; i < list->count;
       i++, pos = (pos + 1) % list->size) {
    if (list->elem[pos] == elem) return i;
  }
...
void *list_remove_at(struct list_t *list, int index) {
  ...
  if (index > list->count / 2) {
    ...
    for (i = 0, pos = (list->head + index) % list->size; i < shiftcount;
         i++, pos = (pos + 1) % list->size)
      list->elem[pos] = list->elem[(pos + 1) % list->size];
    list->elem[pos] = NULL;
    list->tail = INLIST(list->tail - 1);
  } else {
    for (i = 0, pos = (list->head + index) % list->size; i < index;
         i++, pos = INLIST(pos - 1))
      list->elem[pos] = list->elem[INLIST(pos - 1)];
...
void *list_remove(struct list_t *list, void *elem) {
  ...
  if (list->error_code) return NULL;
...
void list_sort(struct list_t *list, int (*comp)(const void *, const void *)) {
  if (list->count) sort(list, 0, list->count - 1, comp);
}

void list_push(struct list_t *list, void *elem) { list_add(list, elem); }
...
void list_enqueue(struct list_t *list, void *elem) { list_add(list, elem); }
