...
void m2s_timer_start(struct m2s_timer_t *timer) {
  ...
  if (timer->state == m2s_timer_state_running) return;
...
void m2s_timer_stop(struct m2s_timer_t *timer) {
  ...
  if (timer->state == m2s_timer_state_stopped) return;
...
long long m2s_timer_get_value(struct m2s_timer_t *timer) {
  ...
  /* Timer is stopped */
  if (timer->state == m2s_timer_state_stopped) return timer->total_value;
...
char *m2s_timer_get_name(struct m2s_timer_t *timer) { return timer->name; }
