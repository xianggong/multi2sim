...
static void sort(struct linked_list_elem_t **array, int lo, int hi,
  ...
  ptr = array[(lo + hi) / 2];
  do {
    while (comp(array[i]->data, ptr->data) < 0) i++;
    while (comp(array[j]->data, ptr->data) > 0) j--;
    if (i <= j) {
      tmp = array[i];
      array[i] = array[j];
      array[j] = tmp;
  ...
  if (lo < j) sort(array, lo, j, comp);
  if (i < hi) sort(array, i, hi, comp);
}
...
int linked_list_sorted(struct linked_list_t *list,
  ...
  while (elem) {
    i++;
    if (comp(prev->data, elem->data) > 0) return 0;
    prev = elem;
    elem = elem->next;
  }
  return 1;
...
static void linked_list_iter_check_version(struct linked_list_iter_t *iter) {
  /* Check */
  if (iter->version == iter->list->version) return;

  /* Version mismatch */
  panic(
      "%s: version mismatch for linked list iterator.\n"
...
void linked_list_iter_next(struct linked_list_iter_t *iter) {
  linked_list_iter_check_version(iter);
  if (iter->elem) iter->elem = iter->elem->next;
}
...
int linked_list_iter_find(struct linked_list_iter_t *iter, void *data) {
  ...
  for (iter->elem = list->head; iter->elem; iter->elem = iter->elem->next)
    if (iter->elem->data == data) return 1;
