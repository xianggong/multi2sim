...
static struct bin_config_elem_t *bin_config_elem_create(
    struct bin_config_t *bin_config, void *data, int size, int dup_data) {
...
static void bin_config_elem_free(struct bin_config_elem_t *elem) {
  ...
  /* Free element */
  if (elem->dup_data && elem->data) free(elem->data);
  free(elem);
}

static struct bin_config_elem_t *bin_config_add_dup_data(
    struct bin_config_t *bin_config, struct bin_config_elem_t *parent_elem,
    char *var, void *data, int size, int dup_data) {
  struct bin_config_elem_t *elem;
  ...
  if (parent_elem) {
    ...
    if (!bin_config->elem_list) bin_config->elem_list = hash_table_create(1, 0);
...
static void bin_config_dump_elem_list(struct hash_table_t *elem_list, int level,
                                      FILE *f) {
  struct bin_config_elem_t *elem;
  char *var;
  char *comma;
...
struct bin_config_elem_t *bin_config_add(struct bin_config_t *bin_config,
  ...
  /* Add variable */
  return bin_config_add_dup_data(bin_config, parent_elem, var, data_copy, size,
                                 1);
}

struct bin_config_elem_t *bin_config_add_no_dup(
    struct bin_config_t *bin_config, struct bin_config_elem_t *parent_elem,
    char *var, void *data, int size) {
  return bin_config_add_dup_data(bin_config, parent_elem, var, data, size, 0);
}
...
int bin_config_remove(struct bin_config_t *bin_config,
  ...
  /* Get list of child elements */
  child_elem_list =
      parent_elem ? parent_elem->child_elem_list : bin_config->elem_list;
  if (!child_elem_list) {
...
struct bin_config_elem_t *bin_config_get(struct bin_config_t *bin_config,
  ...
  if (data_ptr) *data_ptr = elem->data;
  if (size_ptr) *size_ptr = elem->size;
...
static void bin_config_elem_list_save(struct hash_table_t *elem_list,
                                      gzFile f) {
  struct bin_config_elem_t *elem;
  int num_elem;
  ...
  for (var = hash_table_find_first(elem_list, (void **)&elem); elem;
    ...
    if (elem->data) gzwrite(f, elem->data, elem->size);
...
static struct hash_table_t *bin_config_load_elem_list(
    struct bin_config_t *bin_config, gzFile f) {
...
struct bin_config_elem_t *bin_config_find_first(
    struct bin_config_t *bin_config, struct bin_config_elem_t *parent_elem,
...
struct bin_config_elem_t *bin_config_find_next(
    struct bin_config_t *bin_config, struct bin_config_elem_t *parent_elem,
    char **var_ptr, void **data_ptr, int *size_ptr) {
  struct hash_table_t *elem_list;
  struct bin_config_elem_t *elem;
