...
#define LINKED_LIST_FOR_EACH(list)                        \
  for (linked_list_head(list); !linked_list_is_end(list); \
 ...
 * 	current index is N. The error code is set to one of the following
 * values:
 *
 * 	LINKED_LIST_ERR_OK
 * 		No error.
 ...
 * 	List object.
 * @param index
 * 	Value between [0..N] specifying the list position to go to. If 'index'
 * is
 * 	N, the current position is set to a virtual NULL element at the end of
 * the
 * 	list.
 *
 * @return
 ...
 *
 * 	LINKED_LIST_ERR_OK
 * 		No error. The value was found, and the current element is set to
 * its
 * 		first occurrence.
 * 	LINKED_LIST_ERR_NOT_FOUND
 * 		Element not found.
...
/** Return number of elements in the list. The value returned by this function
 * can
 ...
 *
 * @return
 * 	Boolean value representing whether the current element is past the end
 * of
...
#define LINKED_LIST_ITER_FOR_EACH(iter)                                 \
  for (linked_list_iter_head((iter)); !linked_list_iter_is_end((iter)); \
