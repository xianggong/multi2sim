...
struct elf_enc_section_t *elf_enc_section_create(
    char *name, struct elf_enc_buffer_t *first_buffer,
...
void elf_enc_section_dump(struct elf_enc_section_t *section,
  ...
  if (first_index < 0 || last_index < 0)
    fatal("%s: buffers in section not added to ELF file", __FUNCTION__);
...
struct elf_enc_segment_t *elf_enc_segment_create(
    char *name, struct elf_enc_buffer_t *first_buffer,
    struct elf_enc_buffer_t *last_buffer) {
...
void elf_enc_segment_dump(struct elf_enc_segment_t *segment,
  ...
  if (first_index < 0 || last_index < 0)
    fatal("%s: buffers in segment not added to ELF file", __FUNCTION__);
...
struct elf_enc_symbol_table_t *elf_enc_symbol_table_create(char *symtab_name,
  ...
  table->symbol_table_section = elf_enc_section_create(
      symtab_name, table->symbol_table_buffer, table->symbol_table_buffer);
  table->string_table_section = elf_enc_section_create(
      strtab_name, table->string_table_buffer, table->string_table_buffer);
...
void elf_enc_symbol_table_add(struct elf_enc_symbol_table_t *table,
  ...
  elf_enc_buffer_write(table->symbol_table_buffer, &symbol->symbol,
                       sizeof(Elf32_Sym));
  elf_enc_buffer_write(table->string_table_buffer, symbol->name,
                       strlen(symbol->name) + 1);
}
...
struct elf_enc_file_t *elf_enc_file_create(void) {
  ...
  elf_enc_buffer_write(shstrtab, null_section->name,
                       strlen(null_section->name) + 1);
  elf_enc_buffer_write(shstrtab, shstrtab_section->name,
                       strlen(shstrtab_section->name) + 1);
...
void elf_enc_file_generate(struct elf_enc_file_t *file,
  ...
  /* Set ELF Header Properties for sections. Use elf.h structs for shentsize and
   * ehsize */
  ...
  /* Set ELF Header Properties for segments - If there are no segments, even the
   * phentsize should be zero */
  ...
  /* Calculate Program Header and Section Header table size - Actual header
   * tables are created at the end */
  ...
  */
  ...
  for (i = 0; i < list_count(file->section_list); i++)
  {
          ...
          section->header.sh_name = shstrtab->size;
          elf_enc_buffer_write(shstrtab, section->name, strlen(section->name) +
  1);
  }
  */
  ...
  for (i = 0; i < list_count(file->segment_list); i++) {
    ...
    for (j = segment->first_buffer->index; j <= segment->last_buffer->index;
         j++) {
      buffer = list_get(file->buffer_list, j);
  ...
  for (i = 0; i < list_count(file->section_list); i++) {
    ...
    for (j = section->first_buffer->index; j <= section->last_buffer->index;
         j++) {
      buffer = list_get(file->buffer_list, j);
