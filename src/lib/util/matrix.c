...
struct matrix_t *matrix_create(int num_rows, int num_cols) {
  ...
  if (num_rows < 0 || num_cols < 0) fatal("%s: invalid sizes", __FUNCTION__);
...
void matrix_set(struct matrix_t *matrix, int row, int col, void *value) {
  /* Check range */
  if (row < 0 || row >= matrix->num_rows) return;
  if (col < 0 || col >= matrix->num_cols) return;
...
void *matrix_get(struct matrix_t *matrix, int row, int col) {
  /* Check range */
  if (row < 0 || row >= matrix->num_rows) return NULL;
  if (col < 0 || col >= matrix->num_cols) return NULL;
...
void matrix_clear(struct matrix_t *matrix) {
  if (matrix->elem)
    memset(matrix->elem, 0,
           matrix->num_rows * matrix->num_cols * sizeof(void *));
}

int matrix_get_num_rows(struct matrix_t *matrix) { return matrix->num_rows; }

int matrix_get_num_cols(struct matrix_t *matrix) { return matrix->num_cols; }
