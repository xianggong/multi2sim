...
Link::Link(Network* network, const std::string& name,
           const std::string& descriptive_name, Node* src_node, Node* dst_node,
           int bandwidth, int source_buffer_size, int destination_buffer_size,
  ...
  for (int i = 0; i < num_virtual_channels; i++) {
    ...
    Buffer* source_buffer = src_node->addOutputBuffer(source_buffer_size, this);
    addSourceBuffer(source_buffer);

    // Create and add destination buffer
    Buffer* destination_buffer =
        dst_node->addInputBuffer(destination_buffer_size, this);
...
void Link::TransferPacket(Packet* packet) {
  ...
  if (std::find(source_buffers.begin(), source_buffers.end(), source_buffer) ==
      source_buffers.end())
    throw misc::Panic(
        "Packet is not ready to be send over the "
        "link");
  ...
  System::trace << misc::fmt(
      "net.link_transfer net=\"%s\" link=\"%s\" "
      "transB=%lld last_size=%d busy=%lld\n",
      network->getName().c_str(), getName().c_str(), transferred_bytes,
      packet->getSize(), busy);
...
Buffer* Link::VirtualChannelArbitration() {
  ...
  if (scheduled_when == cycle) return scheduled_buffer;
  ...
  for (unsigned i = 0; i < source_buffers.size(); i++) {
    ...
    int next_index =
        (last_scheduled_buffer_index + i + 1) % num_virtual_channels;
