...
void System::EventTypeSendHandler(esim::Event* event, esim::Frame* frame) {
  // Useful objects
  ...
  if (!output_buffer)
    throw misc::Panic(
        misc::fmt("%s: no route from "
                  "%s to %s.",
                  network->getName().c_str(), source_node->getName().c_str(),
                  destination_node->getName().c_str()));

  // Check if buffer fits the message
  if (message->getSize() > output_buffer->getSize())
    throw misc::Panic(
        misc::fmt("%s: message does not "
                  "fit in buffer.\n",
                  __FUNCTION__));

  // Check if the buffer is too full to fit the new message
  if (output_buffer->getCount() + packet->getSize() > output_buffer->getSize())
    throw misc::Panic(misc::fmt("%s: output buffer full.", __FUNCTION__));
...
void System::EventTypeInputBufferHandler(esim::Event* event,
  ...
  if (buffer->getBufferHead() != packet) {
    ...
    debug << misc::fmt(
        ...
        network->getName().c_str(), message->getId(), packet->getId(),
        node->getName().c_str(), buffer->getName().c_str());
...
void System::EventTypeReceiveHandler(esim::Event* event, esim::Frame* frame) {
  // Cast event frame type
  Frame* network_frame = misc::cast<Frame*>(frame);
  ...
  if (!node) throw misc::Panic("The message is not arriving at an end node");
