...
Buffer::Buffer(const std::string& name, int size, int index, Node* node,
...
void Buffer::InsertPacket(Packet* packet) {
  ...
  System::debug << misc::fmt(
      "net: %s - M-%lld:%d - "
      "insert_buf: %s:%s\n",
      message->getNetwork()->getName().c_str(), message->getId(),
      packet->getId(), node->getName().c_str(), name.c_str());
}
...
void Buffer::RemovePacket(Packet* packet) {
  ...
  if (std::find(packets.begin(), packets.end(), packet) == packets.end())
    throw misc::Panic(
        "Trying to remove a packet that is not in"
        " current buffer");
  ...
  if (!event_queue.isEmpty()) event_queue.WakeupAll();
}
...
void Buffer::ExtractPacket() {
  ...
  if (packets.size() == 0) throw misc::Panic("No packets to pop");
...
void Buffer::Dump(std::ostream& os) {
  ...
  long long cycle = system->getCycle();
  os << misc::fmt(
      "%s.PacketOccupancy = %.2f\n", name.c_str(),
      cycle ? (double)accumulated_occupancy_in_packets / cycle : 0.0);
  os << misc::fmt("%s.ByteOccupancy = %.2f\n", name.c_str(),
                  cycle ? (double)accumulated_occupancy_in_bytes / cycle : 0.0);
  os << misc::fmt(
      "%s.Utilization = %.2f\n", name.c_str(),
      cycle ? (double)accumulated_occupancy_in_bytes / cycle / size : 0.0);
}
