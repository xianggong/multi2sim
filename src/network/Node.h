...
class Node {
 ...
 public:
  /// Constructor
  Node(Network* network, int index, int input_buffer_size,
  ...
  Buffer* getOutputBuffer(int index) { return output_buffers[index].get(); }

  /// Return the number of input buffers
  int getNumInputBuffers() const { return input_buffers.size(); }

  /// Return input buffer by index
  Buffer* getInputBuffer(int index) { return input_buffers[index].get(); }
  ...
  void incReceivedBytes(long long bytes) { received_bytes += bytes; }
