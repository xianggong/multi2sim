...
void Switch::Dump(std::ostream& os) const {
  ...
  os << misc::fmt("SendRate = %0.4f\n",
                  cycle ? (double)sent_bytes / cycle : 0.0);
  os << misc::fmt("ReceivedBytes = %lld\n", received_bytes);
  os << misc::fmt("ReceivedPackets = %lld\n", received_packets);
  os << misc::fmt("ReceiveRate = %0.4f\n",
                  cycle ? (double)received_bytes / cycle : 0.0);

  // Dumping input buffers' information
  for (auto& buffer : input_buffers) buffer->Dump(os);

  // Dumping output buffers' information
  for (auto& buffer : output_buffers) buffer->Dump(os);
...
void Switch::Forward(Packet* packet) {
  ...
  if (input_buffer->read_busy >= cycle) {
    ...
    System::debug << misc::fmt(
        ...
        network->getName().c_str(), message->getId(), packet->getId(),
        node->getName().c_str(), input_buffer->getName().c_str());

    // Coming back to this event when buffer is not busy
    esim_engine->Next(current_event, input_buffer->read_busy - cycle + 1);
    return;
  }
  ...
  RoutingTable::Entry* entry = routing_table->Lookup(node, destination_node);
  if (!entry)
    throw misc::Panic(
        misc::fmt("%s: no route from %s "
                  "to %s.",
                  network->getName().c_str(), node->getName().c_str(),
  ...
  if (output_buffer->write_busy >= cycle) {
    ...
    esim_engine->Next(current_event, output_buffer->write_busy - cycle + 1);
    return;
  }
  ...
  System::trace << misc::fmt(
      ...
      network->getName().c_str(), input_buffer->getNode()->getName().c_str(),
      input_buffer->getName().c_str(), message->getId(), packet->getId(),
      input_buffer->getOccupancyInBytes());

  System::trace << misc::fmt(
      "net.packet_insert net=\"%s\" "
      "node=\"%s\" buffer=\"%s\" "
      "name=\"P-%lld:%d\" occpncy=%d\n",
      network->getName().c_str(), output_buffer->getNode()->getName().c_str(),
      output_buffer->getName().c_str(), message->getId(), packet->getId(),
...
Buffer* Switch::Schedule(Buffer* output_buffer) {
  ...
  if (!is_output_buffer)
    throw misc::Panic(
        misc::fmt("Buffer %s is not and "
                  "output buffer of switch %s.",
                  output_buffer->getName().c_str(), this->getName().c_str()));
  ...
  int previous_input_buffer_index =
      output_buffer->getScheduledBuffer()
          ? output_buffer->getScheduledBuffer()->getIndex()
          : 0;

  // Checks if the output buffer is in write cycle
  if (output_buffer->write_busy >= cycle)
    throw misc::Panic(
        misc::fmt("Cannot schedule a busy output "
                  "buffer %s",
                  output_buffer->getName().c_str()));
  ...
  for (unsigned int i = 0; i < input_buffers.size(); i++) {
    ...
    int index = (previous_input_buffer_index + i + 1) % input_buffers.size();

    // Get the input buffer
    Buffer* input_buffer = input_buffers[index].get();

    // Skip empty buffer
    if (input_buffer->getCount() == 0) continue;
    ...
    RoutingTable::Entry* entry = routingTable->Lookup(this, destination_node);
    if (!entry)
      throw misc::Panic(misc::fmt(
          "No route found from "
          "node %s to node %s",
          this->getName().c_str(), destination_node->getName().c_str()));
    Buffer* next_buffer = entry->getBuffer();
    if (next_buffer != output_buffer) continue;
