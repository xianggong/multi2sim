...
Graph::Graph(Network* network) : misc::Graph(), network(network) {}
...
void Graph::Populate() {
  ...
  for (int i = 0; i < network->getNumNodes(); i++) {
    ...
    addVertex(
        misc::new_unique<Vertex>(node, node->getName(), Vertex::KindNode));
  ...
  for (int i = 0; i < network->getNumConnections(); i++) {
    ...
    if (dynamic_cast<Link*>(network->getConnection(i))) {
      ...
      if (!bidirectional) {
        addEdge(misc::new_unique<Edge>(link, source_vertex, destination_vertex),
                source_vertex, destination_vertex);
      }
    } else
      throw misc::Panic(
          "Network contains unknown link to "
          "graph generation");
  }
}
...
void Graph::AddDummyVertices() {
  ...
  for (unsigned i = 0; i < edges.size(); i++) {
    ...
    Vertex* destination = misc::cast<Vertex*>(edge->getDestinationVertex());
    if (source->y_value - destination->y_value > 1) {
      // Creating a dummy vertices and adding it to the vertex list
      vertices.emplace_back(
          misc::new_unique<Vertex>(nullptr, "dummy", Vertex::KindDummy));
      ...
      if (it == source->outgoing_vertices.end())
        throw misc::Panic(
            "Layered graph drawing encountered a problem "
      ...
      // if required.
      edges.emplace_back(
          misc::new_unique<Edge>(edge->downstream_link, dummy, destination));
...
void Graph::Scale() {
  ...
  for (int i = 0; i < getNumVertices(); i++) {
    ...
    vertex->x_value =
        ((2 * vertex->x_value) + getMaxVerticesInLayers() - vertex->neighbors) *
        optimal_distance;
  }
}
...
void Graph::DumpGraph(std::ostream& os) const {
  ...
  for (int i = 0; i < getNumEdges(); i++) {
    ...
    Vertex* source_vertex = misc::cast<Vertex*>(edge->getSourceVertex());
    Vertex* destination_vertex =
        misc::cast<Vertex*>(edge->getDestinationVertex());
    ...
    if (edge->upstream_link) {
      ...
      assert(link);
      utilization = (int)((cycle
                               ? (double)link->getTransferredBytes() /
                                     (cycle * link->getBandwidth())
                               : 0) *
                          10);
      os << misc::fmt("link = %f %d %f %d %d %d\n",
                      ...
                      destination_vertex->y_value, utilization, edge_type);
    }
    ...
    os << misc::fmt("link = %f %d %f %d %d %d\n",
                    ...
                    (double)source_vertex->x_value / getNumVertices(),
                    source_vertex->y_value, utilization, edge_type);
  }
}
}
