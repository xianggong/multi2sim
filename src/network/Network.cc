...
Network::Network(const std::string& name) : name(name), routing_table(this) {}

void Network::ParseConfiguration(misc::IniFile* config,
                                 const std::string& section) {
  // Non-empty values
  default_output_buffer_size =
      config->ReadInt(section, "DefaultOutputBufferSize", 0);
  default_input_buffer_size =
      config->ReadInt(section, "DefaultInputBufferSize", 0);
  default_bandwidth = config->ReadInt(section, "DefaultBandwidth", 0);
  ...
  if (ideal && fix_latency) {
    throw Error(misc::fmt(
        "%s: An ideal network is a network"
        "with a fix latency of 1. Both "
        "variables cannot be set at the same "
        "time for the network %s\n%s",
        config->getPath().c_str(), name.c_str(), System::err_config_note));
  ...
  if (fix_latency < 0) {
    throw Error(
        misc::fmt("%s: Network %s cannot have a "
                  "negative fix latency",
                  config->getPath().c_str(), name.c_str()));
  }

  // Print a warning in case constant network is used
  if (fix_latency > 0) {
    misc::Warning(
        "Network %s: Simulator is using "
  ...
  if (!ParseConfigurationForRoutes(config)) routing_table.FloydWarshall();

  // If the network with current routing contains a cycle, warn
  if (routing_table.hasCycle())
    misc::Warning(
        "Network %s: Cycle found in the "
        "routing table.\n%s",
        name.c_str(), err_cycle_detected);
}
...
void Network::ParseConfigurationForNodes(misc::IniFile* config) {
  for (int i = 0; i < config->getNumSections(); i++) {
    ...
    if (strcasecmp(tokens[0].c_str(), "Network")) continue;
    if (strcasecmp(tokens[1].c_str(), name.c_str())) continue;
    if (strcasecmp(tokens[2].c_str(), "Node")) continue;
    ...
    if (getNodeByName(node_name) != nullptr)
      throw Error(
          misc::fmt("%s: Node '%s' already "
                    "exists in the network\n",
                    config->getPath().c_str(), node_name.c_str()));

    // Get properties
    std::string type = config->ReadString(section, "Type");
    int input_buffer_size =
        config->ReadInt(section, "InputBufferSize", default_input_buffer_size);
    ...
    if ((input_buffer_size < 1) || (output_buffer_size < 1) || (bandwidth < 1))
      throw Error(
          misc::fmt("%s: Invalid argument for Node "
                    "'%s'\n%s",
                    config->getPath().c_str(), node_name.c_str(),
    ...
    if (!strcasecmp(type.c_str(), "EndNode")) {
      ...
      if (packet_size != 0)
        required_buffer_size =
            (((System::getMessageSize() - 1) / packet_size + 1) * packet_size);
      else
        required_buffer_size = System::getMessageSize();

      if (input_buffer_size < required_buffer_size ||
          output_buffer_size < required_buffer_size)
        throw Error(
            misc::fmt("%s: Buffer size on the "
      ...
      addEndNode(input_buffer_size, output_buffer_size, node_name, NULL);
    } else if (!strcasecmp(type.c_str(), "Switch")) {
      addSwitch(input_buffer_size, output_buffer_size, bandwidth, node_name);
    } else {
      throw Error(misc::fmt(
          "%s: Node type '%s' is not "
          "supported.\n%s",
          config->getPath().c_str(), type.c_str(), System::err_config_note));
    }
  }
}
...
void Network::ParseConfigurationForBuses(misc::IniFile* ini_file) {
  for (int i = 0; i < ini_file->getNumSections(); i++) {
    ...
    if (strcasecmp(tokens[0].c_str(), "Network")) continue;
    if (strcasecmp(tokens[1].c_str(), name.c_str())) continue;
    if (strcasecmp(tokens[2].c_str(), "Bus")) continue;
    ...
    if (bandwidth < 1)
      throw Error(
          misc::fmt("%s: Bus '%s', bandwidth cannot "
                    "be zero/negative.\n",
                    ini_file->getPath().c_str(), name.c_str()));

    // Get the number of lanes
    int lanes = ini_file->ReadInt(section, "Lanes", 1);
    if (lanes < 1)
      throw Error(
          misc::fmt("%s: Bus '%s', number of lanes "
...
Bus* Network::addBus(const std::string name, int bandwidth, int lanes) {
  // Create bus
  connections.emplace_back(misc::new_unique<Bus>(this, name, bandwidth, lanes));
...
void Network::ParseConfigurationForBusPorts(misc::IniFile* ini_file) {
  for (int i = 0; i < ini_file->getNumSections(); i++) {
    ...
    if (strcasecmp(tokens[0].c_str(), "Network")) continue;
    if (strcasecmp(tokens[1].c_str(), name.c_str())) continue;
    if (strcasecmp(tokens[2].c_str(), "BusPort")) continue;

    // Get the bus name
    std::string bus_name = ini_file->ReadString(section, "Bus");
    if (bus_name == "")
      throw Error(
          misc::fmt("%s: Bus name should be "
    ...
    if (!bus)
      throw Error(
          misc::fmt("%s: Bus '%s' does not exist "
                    "in the network.",
                    ini_file->getPath().c_str(), bus_name.c_str()));

    // Get the node's name
    std::string node_name = ini_file->ReadString(section, "Node");
    if (node_name == "")
      throw Error(
          misc::fmt("%s: Section [%s]: Node is "
    ...
    if (!node)
      throw Error(misc::fmt(
          ...
          ini_file->getPath().c_str(), section.c_str(), node_name.c_str()));
    ...
    if (!strcasecmp(type.c_str(), "Sender")) {
      buffer_size =
          ini_file->ReadInt(section, "BufferSize", default_output_buffer_size);
      if (buffer_size < 1)
        throw Error(
            misc::fmt("%s: Section [%s]: Buffer"
    ...
    // The node only receives through the port
      ...
      buffer_size =
          ini_file->ReadInt(section, "BufferSize", default_input_buffer_size);
      if (buffer_size < 1)
        throw Error(
            misc::fmt("%s: Section [%s]: Buffer"
                      " size cannot be less than 1",
    ...
    // The node can both send and receive through the port
      ...
      if (buffer_size == 0) {
        buffer = node->addOutputBuffer(default_output_buffer_size, connection);
        bus->addSourceBuffer(buffer);
        buffer = node->addInputBuffer(default_input_buffer_size, connection);
        bus->addDestinationBuffer(buffer);
      ...
      throw misc::Error(misc::fmt(
          "%s: Section [%s]: Type "
          "'%s' is not recognized",
          ini_file->getPath().c_str(), section.c_str(), type.c_str()));
    }
  }
}
...
void Network::ParseConfigurationForLinks(misc::IniFile* ini_file) {
  for (int i = 0; i < ini_file->getNumSections(); i++) {
    ...
    if (strcasecmp(tokens[0].c_str(), "Network")) continue;
    if (strcasecmp(tokens[1].c_str(), name.c_str())) continue;
    if (strcasecmp(tokens[2].c_str(), "Link")) continue;
    ...
    if (!strcasecmp(type.c_str(), "Bidirectional") ||
      ...
      if (src_name.empty())
        throw Error(
            misc::fmt("%s: Source node is not "
                      "provided for link '%s'.\n",
                      ini_file->getPath().c_str(), link_name.c_str()));
      ...
      if (!source)
        throw Error(misc::fmt(
            "%s: Source node '%s' is "
            "invalid for link '%s'.\n",
            ini_file->getPath().c_str(), src_name.c_str(), link_name.c_str()));
      ...
      if (dst_name.empty())
        throw Error(
            misc::fmt("%s: Destination node is "
      ...
      if (!destination)
        throw Error(misc::fmt(
            ...
            "'%s'.\n",
            ini_file->getPath().c_str(), dst_name.c_str(), link_name.c_str()));

      // Make sure link has different source and destination
      if (source == destination)
        throw Error(
            misc::fmt("%s: Link '%s', source "
      ...
      // Bandwidth
      int bandwidth =
          ini_file->ReadInt(section, "Bandwidth", default_bandwidth);
      if (bandwidth < 1)
        throw Error(
            misc::fmt("%s: Link '%s', "
                      "bandwidth cannot be "
      ...
      int num_virtual_channel = ini_file->ReadInt(section, "VC", 1);
      if (num_virtual_channel < 1)
        throw Error(
            misc::fmt("%s: Link '%s', virtual "
                      "channel cannot be "
                      "zero/negative.\n",
                      ini_file->getPath().c_str(), link_name.c_str()));
      ...
      // Add link
      if (!strcasecmp(type.c_str(), "Bidirectional"))
        addBidirectionalLink(link_name, source, destination, bandwidth,
                             input_buffer_size, output_buffer_size,
                             num_virtual_channel);
      else if (!strcasecmp(type.c_str(), "Unidirectional"))
        addLink(link_name, source, destination, bandwidth, input_buffer_size,
                output_buffer_size, num_virtual_channel);
    } else {
      throw Error(misc::fmt(
          "%s: Link type '%s' is not "
          "supported.\n%s",
          ini_file->getPath().c_str(), type.c_str(), System::err_config_note));
    }
  }
}
...
bool Network::ParseConfigurationForRoutes(misc::IniFile* ini_file) {
  ...
  for (int i = 0; i < ini_file->getNumSections(); i++) {
    ...
    if (strcasecmp(tokens[0].c_str(), "Network")) continue;
    if (strcasecmp(tokens[1].c_str(), name.c_str())) continue;
    if (strcasecmp(tokens[2].c_str(), "Routes")) continue;
    ...
    for (auto& source : nodes) {
      for (auto& destination : nodes) {
        if (dynamic_cast<EndNode*>(destination.get())) {
          ...
          // in the section
          std::string destination_string = ini_file->ReadString(section, route);
          ...
          if (destination_string != "") {
            ...
            misc::StringTokenize(destination_string, string_tokens, ":");
            if (string_tokens.size() > 2)
              throw Error(
                  misc::fmt("Network %s: route "
                            "%s.to.%s: wrong format "
                            "for next node\n",
                            name.c_str(), source->getName().c_str(),
            ...
            if (!next)
              throw Error(misc::fmt(
                  ...
                  "invalid node name '%s'\n",
                  name.c_str(), source->getName().c_str(),
                  destination->getName().c_str(), string_tokens[0].c_str()));
            ...
            routing_table.UpdateRoute(source.get(), destination.get(), next,
                                      virtual_channel);
          }
        }
      }
...
void Network::addBidirectionalLink(const std::string name, Node* source_node,
                                   Node* dest_node, int bandwidth,
                                   int source_buffer_size, int dest_buffer_size,
                                   int num_virtual_channels) {
  addLink(name, source_node, dest_node, bandwidth, source_buffer_size,
          dest_buffer_size, num_virtual_channels);
  addLink(name, dest_node, source_node, bandwidth, source_buffer_size,
          dest_buffer_size, num_virtual_channels);
}
...
Node* Network::getNodeByUserData(void* user_data) const {
  ...
  // Search node
  for (auto& node : nodes)
    if (node->getUserData() == user_data) return node.get();
...
void Network::DumpReport(const std::string& path) {
  ...
  if (!f)
    throw Error(misc::fmt("%s: cannot open file for write", path.c_str()));
...
void Network::DumpReport(std::ostream& os) const {
  ...
  os << misc::fmt("AverageMessageSize = %0.2f\n",
                  transfers ? (double)accumulated_bytes / transfers : 0.0);
  os << misc::fmt("TransferredBytes = %lld\n", accumulated_bytes);
  os << misc::fmt("AverageLatency = %.4f\n",
                  transfers ? (double)accumulated_latency / transfers : 0.0);
  ...
  // Print links
  for (auto& link : connections) link->Dump(os);

  // Print node information
  for (auto& node : nodes) node->Dump(os);
...
Message* Network::newMessage(EndNode* source_node, EndNode* destination_node,
  ...
  message_table.emplace(
      message_id_counter,
      misc::new_unique<Message>(message_id_counter, this, source_node,
...
bool Network::CanSend(EndNode* source_node, EndNode* destination_node, int size,
  ...
  RoutingTable::Entry* entry =
      routing_table.Lookup(source_node, destination_node);
  ...
  if (output_buffer->write_busy >= cycle) {
    if (retry_event)
      esim_engine->Next(retry_event, output_buffer->write_busy - cycle + 1);
    return false;
  }
  ...
  if (required_size > output_buffer->getSize())
    throw Error(
        "Buffer too small for the "
        "message size.");

  // Check if the buffer has enough space for the current message
  if (output_buffer->getCount() + required_size > output_buffer->getSize()) {
    if (retry_event) output_buffer->Wait(retry_event);
    return false;
  }
...
Message* Network::Send(EndNode* source_node, EndNode* destination_node,
                       int size, esim::Event* receive_event) {
  // Get esim engine
  esim::Engine* esim_engine = esim::Engine::getInstance();
  ...
  net::System::trace << misc::fmt(
      ...
      name.c_str(), message->getId(), message->getSize(),
  ...
  net::System::trace << misc::fmt(
      "net.msg net=\"%s\" name=\"M-%lld\" "
      "state=\"%s:packetize\"\n",
      name.c_str(), message->getId(), source_node->getName().c_str());
  ...
  for (int i = 0; i < message->getNumPackets(); i++) {
    ...
    net::System::trace << misc::fmt(
        "net.new_packet net=\"%s\" "
        "name=\"P-%lld:%d\" size=%d state=\"%s:packetizer\"\n",
        name.c_str(), message->getId(), packet->getId(), packet->getSize(),
        source_node->getName().c_str());

    // Update the trace with the new packet association
    net::System::trace << misc::fmt(
        "net.packet_msg net=\"%s\" "
        "name=\"P-%lld:%d\" message=\"M-%lld\"\n",
        name.c_str(), message->getId(), packet->getId(), message->getId());
...
Message* Network::TrySend(EndNode* source_node, EndNode* destination_node,
                          int size, esim::Event* receive_event,
                          esim::Event* retry_event) {
  // Get output buffer
  RoutingTable::Entry* entry =
...
void Network::Receive(EndNode* node, Message* message) {
  ...
  for (int i = 0; i < message->getNumPackets(); i++) {
    ...
    if (packet_node != node)
      throw Error(
          misc::fmt("Packet %d of the message %lld has "
                    "not arrived.",
  ...
  for (int i = 0; i < message->getNumPackets(); i++) {
    ...
    if (!hasConstantLatency()) {
      ...
      // from the buffer
      System::trace << misc::fmt(
          "net.packet_extract "
          "net=\"%s\" node=\"%s\" buffer=\"%s\" "
          "name=\"P-%lld:%d\" occpncy=%d\n",
          name.c_str(), buffer->getNode()->getName().c_str(),
          buffer->getName().c_str(), message->getId(), packet->getId(),
...
EndNode* Network::addEndNode(int input_buffer_size, int output_buffer_size,
                             const std::string& name, void* user_data) {
  // Create end node
  nodes.emplace_back(
      misc::new_unique<EndNode>(this, nodes.size(), input_buffer_size,
                                output_buffer_size, name, user_data));
  EndNode* node = misc::cast<EndNode*>(nodes.back().get());
  num_end_nodes++;
  return node;
}

Switch* Network::addSwitch(int input_buffer_size, int output_buffer_size,
                           int bandwidth, const std::string& name) {
  // Create switch
  nodes.emplace_back(
      misc::new_unique<Switch>(this, nodes.size(), input_buffer_size,
                               output_buffer_size, bandwidth, name, nullptr));
  Switch* node = misc::cast<Switch*>(nodes.back().get());
  return node;
}

Link* Network::addLink(const std::string name, Node* source_node,
                       Node* dest_node, int bandwidth, int source_buffer_size,
                       int dest_buffer_size, int num_virtual_channels) {
  // Check if the nodes are not both end-nodes
  if ((dynamic_cast<EndNode*>(source_node)) &&
      (dynamic_cast<EndNode*>(dest_node)))
    throw Error(
        misc::fmt("Network '%s': Link "
                  "'%s' cannot connect two end-nodes.",
                  this->name.c_str(), name.c_str()));

  // Creating a unique name for unidirectional link
  std::string descriptive_name =
      "link_" + source_node->getName() + "_" + dest_node->getName();

  // Create link
  connections.emplace_back(misc::new_unique<Link>(
      this, name, descriptive_name, source_node, dest_node, bandwidth,
      source_buffer_size, dest_buffer_size, num_virtual_channels));
...
void Network::TraceHeader() {
  ...
  net_system->trace.Header(
      misc::fmt("net.create "
                "name=\"%s\" "
                "num_nodes=\"%d\" "
                "packet_size=\"%d\"\n",
                name.c_str(), (int)nodes.size(), packet_size));

  // Dump information about nodes in trace file
  for (auto& node : nodes) {
    if (dynamic_cast<EndNode*>(node.get()))
      net_system->trace.Header(
          misc::fmt("net.node net_name=\"%s\" "
                    "node_index=\"%d\" node_name=\"%s\" node_type=1\n",
                    name.c_str(), node->getIndex(), node->getName().c_str()));
    ...
    if (link)
      net_system->trace.Header(
          misc::fmt("net.link net_name=\"%s\" "
                    ...
                    link->getNumVirtualChannels(), link->getBandwidth()));
    else
      throw misc::Panic(
          "Buses are not supported in the current "
  ...
  for (auto& node : nodes) {
    ...
    for (int i = 0; i < node->getNumInputBuffers(); i++) {
      ...
      net_system->trace.Header(misc::fmt(
          "net.input_buffer "
          "net_name=\"%s\" node_name=\"%s\" buffer_name=\"%s\" "
          "buffer_size=%d buffer_type=1 connection=\"%s\"\n",
          name.c_str(), node->getName().c_str(), buffer->getName().c_str(),
          buffer->getSize(), buffer->getConnection()->getName().c_str()));
    }
