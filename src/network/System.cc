...
System::System() {
  ...
  event_send = esim_engine->RegisterEvent("send", EventTypeSendHandler,
                                          frequency_domain);
  event_output_buffer = esim_engine->RegisterEvent(
      "output_buffer", EventTypeOutputBufferHandler, frequency_domain);
  event_input_buffer = esim_engine->RegisterEvent(
      "input_buffer", EventTypeInputBufferHandler, frequency_domain);
  event_receive = esim_engine->RegisterEvent("receive", EventTypeReceiveHandler,
                                             frequency_domain);
}
...
void System::RegisterOptions() {
  ...
  command_line->RegisterInt32(
      "--net-msg-size <number> (default = 1 Byte)", message_size,
  ...
  command_line->RegisterDouble(
      "--net-injection-rate <number> (default 0.001)", injection_rate,
  ...
  command_line->RegisterString(
      "--net-sim <network name>", sim_net_name,
  ...
  command_line->RegisterString(
      "--net-dump-routes <file>", route_file,
...
void System::ProcessOptions() {
  ...
  // Debugger
  if (!debug_file.empty()) debug.setPath(debug_file);

  // Stand-Alone activation
  if (!sim_net_name.empty()) stand_alone = true;

  // Stand-Alone requires config file
  if (stand_alone && config_file.empty())
    throw Error(
        misc::fmt("Option --net-sim requires "
                  " --net-config option "));
}
...
void System::ReadConfiguration() {
  ...
  if (!config_file.empty()) {
    ...
    trace.On();
    if ((trace) && (stand_alone)) TraceHeader();
  }
}
...
void System::TraceHeader() {
  ...
  for (auto& network : networks) network->TraceHeader();
}
...
void System::UniformTrafficSimulation(Network* network) {
  ...
  while (1) {
    ...
    for (int i = 0; i < network->getNumNodes(); i++) {
      // Get end node
      EndNode* node = dynamic_cast<EndNode*>(network->getNode(i));
      if (!node) continue;
      ...
      while (1) {
        ...
        destination_node = dynamic_cast<EndNode*>(network->getNode(index));
        if (destination_node && destination_node != node) break;
      }

      // Inject
      while (inject_time[i] < cycle) {
        // Schedule next injection
        inject_time[i] += RandomExponential(injection_rate);

        // Send the packet
        if (network->CanSend(node, destination_node, System::message_size))
          network->Send(node, destination_node, System::message_size);
      }
    }
...
void System::StandAlone() {
  ...
  if (!network)
    throw Error(
        misc::fmt("%s: The network does not exist for "
...
void System::DumpReport() {
  ...
  if (!report_file.empty()) {
    for (auto& network : networks) network->DumpReport(report_file);
  }
}
...
void System::StaticGraph() {
  ...
  if (!graph_file.empty()) {
    for (auto& network : networks) network->StaticGraph(graph_file);
  }
}
...
void System::DumpRoutes() {
  ...
  if (!route_file.empty()) {
    for (auto& network : networks) {
      std::string net_path = network->getName() + "_" + route_file;
