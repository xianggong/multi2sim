...
RoutingTable::RoutingTable(Network* network) : network(network) {
...
void RoutingTable::Initialize() {
  ...
  if (!entries.empty()) throw misc::Panic("Routing table already initialized.");
  ...
  for (int i = 0; i < dimension; i++) {
    for (int j = 0; j < dimension; j++) {
      entries.emplace_back(
          misc::new_unique<Entry>(i == j ? 0 : dimension, nullptr, nullptr));
    }
  }
  ...
  for (int i = 0; i < dimension; i++) {
    ...
    for (int j = 0; j < node->getNumOutputBuffers(); j++) {
      ...
      for (int k = 0; k < connection->getNumDestinationBuffers(); k++) {
        Buffer* dst_buffer = connection->getDestinationBuffer(k);
  ...
  for (int k = 0; k < dimension; k++) {
    for (int i = 0; i < dimension; i++) {
      for (int j = 0; j < dimension; j++) {
        ...
        int temp_cost = entry_i_k->cost + entry_k_j->cost;
  ...
  for (int i = 0; i < dimension; i++) {
    for (int j = 0; j < dimension; j++) {
      ...
      for (int k = 0; k < node_i->getNumOutputBuffers(); k++) {
        ...
        for (int m = 0; m < connection->getNumDestinationBuffers(); m++) {
          Buffer* destination_buffer = connection->getDestinationBuffer(m);
          Node* receive_node = destination_buffer->getNode();
          if (next_node != receive_node) continue;
...
bool RoutingTable::hasCycle() {
  ...
  for (int node_id = 0; node_id < dimension; node_id++) {
    ...
    for (int buffer_id = 0; buffer_id < node->getNumOutputBuffers();
      ...
      Buffer* output_buffer = node->getOutputBuffer(buffer_id);

      // Find the buffer in the routing table
      for (int destination_node_id = 0; destination_node_id < dimension;
           destination_node_id++) {
        // Get the destination node
        Node* destination_node = network->getNode(destination_node_id);
        ...
        if (output_buffer == entry->getBuffer()) {
          ...
          // a vertex for it
          graph->addVertex(
              misc::new_unique<misc::Vertex>(output_buffer->getName().c_str()));

          // Get the vertex which is the last
          // vertex in the vertices list
          misc::Vertex* vertex = graph->getVertex(graph->getNumVertices() - 1);

          // Map the vertex with the buffer for
          // future easy reference
          buffer_to_vertex.insert(std::make_pair(output_buffer, vertex));
  ...
  for (int source_id = 0; source_id < dimension; source_id++) {
    ...
    for (int destination_id = 0; destination_id < dimension; destination_id++) {
      // Get the destination node
      Node* destination_node = network->getNode(destination_id);
      ...
      if (source_vertex_buffer) {
        ...
        Entry* next_entry = Lookup(next_node, destination_node);

        // Get the associated output buffer for the
        // next entry if any
        Buffer* destination_vertex_buffer = next_entry->getBuffer();
        if (destination_vertex_buffer) {
          // Find the source vertex based on the
          // source output buffer in the unordered
          // map
          auto source_vertex_it = buffer_to_vertex.find(source_vertex_buffer);
          assert(source_vertex_it != buffer_to_vertex.end());

          // Find the destination vertex based on
          // the destination output buffer
          auto destination_vertex_it =
              buffer_to_vertex.find(destination_vertex_buffer);
          assert(destination_vertex_it != buffer_to_vertex.end());
          ...
          if (!graph->findEdge(source_vertex_it->second,
            ...
            // vertex
            graph->addEdge(
                misc::new_unique<misc::Edge>(source_vertex_it->second,
                                             destination_vertex_it->second),
                source_vertex_it->second, destination_vertex_it->second);
          }
        }
      }
    }
...
void RoutingTable::Dump(std::ostream& os) const {
  ...
  for (int i = 0; i < dimension; i++) {
    ...
    for (int j = 0; j < dimension; j++) {
      ...
      if (next) entry_text_size += next->getName().length();
      entry_text_size += 2;

      // Another separator (+2) followed by the name of
      // the buffer
      Buffer* buffer = entry->getBuffer();
      if (buffer) entry_text_size += buffer->getName().length();
      entry_text_size += 2;
  ...
  for (int i = 0; i <= dimension; i++) {
    ...
    if (i == 0) {
      ...
      os << "| " << std::left << std::setw(element_size - 3)
         << std::setfill(' ') << node->getName().c_str() << "|";
    }
  }
  ...
  for (int i = 0; i < dimension; i++) {
    ...
    for (int j = 0; j < dimension; j++) {
      ...
      if (entry->getNextNode()) {
        ...
        // the string
        if (entry->getBuffer())
          element = element + entry->getBuffer()->getName();
...
void RoutingTable::UpdateRoute(Node* source, Node* destination, Node* next,
                               int virtual_channel) {
  // Check if source and destination are not the same
  if (source == destination)
    throw Error(
        misc::fmt("Network %s: route %s.to.%s: "
                  "source and destination cannot be the same\n",
                  network->getName().c_str(), source->getName().c_str(),
  ...
  for (int i = 0; i < source->getNumOutputBuffers(); i++) {
    ...
    if (dynamic_cast<Link*>(connection)) {
      ...
      for (int j = 0; j < connection->getNumDestinationBuffers(); j++) {
        Buffer* destination_buffer = connection->getDestinationBuffer(j);
    ...
    for (int j = 0; j < dimension; j++) {
      ...
      if (next) {
        ...
        for (;;) {
          ...
          Node* path_entry = Lookup(next, destination)->getNextNode();
