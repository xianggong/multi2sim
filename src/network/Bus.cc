...
Bus::Bus(Network* network, const std::string& name, int bandwidth,
         int num_lanes)
...
void Lane::Dump(std::ostream& os) const {
  ...
  os << misc::fmt("BusLane.%d.BytesPerCycle = %0.4f\n", index,
                  cycle ? (double)transferred_bytes / cycle : 0.0);
  os << misc::fmt(
      "BusLane.%d.Utilization = %0.4f\n", index,
      cycle ? (double)transferred_bytes / (cycle * bandwidth) : 0.0);
}
...
void Bus::Dump(std::ostream& os = std::cout) const {
  ...
  for (auto buffer : source_buffers)
    os << misc::fmt("%s:%s ", buffer->getNode()->getName().c_str(),
  ...
  // Dump statistics
  for (unsigned i = 0; i < lanes.size(); i++) lanes[i]->Dump(os);
...
void Bus::TransferPacket(Packet* packet) {
  ...
  if (std::find(source_buffers.begin(), source_buffers.end(), source_buffer) ==
      source_buffers.end())
    throw Error(
        misc::fmt("Packet %lld:%d is not in a source buffer."
                  "of the bus",
  ...
  if (!entry)
    throw misc::Panic(misc::fmt(
        "%s: no route from %s to %s.", network->getName().c_str(),
        node->getName().c_str(), destination_node->getName().c_str()));
  ...
  if (destination_buffer == nullptr)
    throw misc::Panic(
        misc::fmt("%s: Next node is not connected "
                  "to the bus '%s'.",
  ...
  if (packet_size > destination_buffer->getSize())
    throw misc::Panic(
        misc::fmt("%s: Packet does not in node %s of "
                  "buffer %s",
                  network->getName().c_str(),
                  destination_buffer->getNode()->getName().c_str(),
                  destination_buffer->getName().c_str()));
  if (destination_buffer->getCount() + packet_size >
    ...
    System::debug << misc::fmt(
        ...
        network->getName().c_str(), message->getId(), packet->getId(),
        name.c_str(), destination_buffer->getNode()->getName().c_str(),
        destination_buffer->getName().c_str());
  ...
  System::trace << misc::fmt(
      "net.packet_extract net=\"%s\" node=\"%s\" "
      "buffer=\"%s\" name=\"P-%lld:%d\" occpncy=%d\n",
      network->getName().c_str(), source_buffer->getNode()->getName().c_str(),
      source_buffer->getName().c_str(), message->getId(), packet->getId(),
      source_buffer->getOccupancyInBytes());
  System::trace << misc::fmt(
      "net.packet_insert net=\"%s\" node=\"%s\" "
      "buffer=\"%s\" name=\"P-%lld:%d\" occpncy=%d\n",
      network->getName().c_str(),
      destination_buffer->getNode()->getName().c_str(),
      destination_buffer->getName().c_str(), message->getId(), packet->getId(),
...
Buffer* Bus::LaneArbitration(Lane* lane) {
  ...
  if (lane->scheduled_when >= cycle) return lane->scheduled_buffer;
  ...
  for (int i = 0; i < getNumSourceBuffers(); i++) {
    ...
    int input_buffer_index =
        (last_input_node_index + i + 1) % input_buffer_count;
    ...
    if (packet->getBusy() >= cycle) continue;

    // The buffer must be ready to be read
    if (buffer->read_busy >= cycle) continue;
...
Lane* Bus::Arbitration(Buffer* current_buffer) {
  ...
  for (auto& lane : lanes) {
    ...
    if (LaneArbitration(lane.get()) == current_buffer) return lane.get();
  }
