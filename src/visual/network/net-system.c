...
static char *err_vi_net_system_trace_version =
    "\tThe network system trace file has been created with an incompatible "
    "version\n"
...
static void vi_net_system_read_checkpoint(struct vi_net_system_t *net_system,
                                          FILE *f);
static void vi_net_system_write_checkpoint(struct vi_net_system_t *net_system,
                                           FILE *f);

static void vi_net_system_new_message(struct vi_net_system_t *net_system,
                                      struct vi_trace_line_t *trace_line);
static void vi_net_system_msg_set_access(struct vi_net_system_t *net_system,
                                         struct vi_trace_line_t *trace_line);
static void vi_net_system_message(struct vi_net_system_t *net_system,
                                  struct vi_trace_line_t *trace_line);
static void vi_net_system_end_message(struct vi_net_system_t *net_system,
                                      struct vi_trace_line_t *trace_line);
static void vi_net_system_link_transfer(struct vi_net_system_t *net_system,
                                        struct vi_trace_line_t *trace_line);
static void vi_net_system_new_packet(struct vi_net_system_t *net_system,
                                     struct vi_trace_line_t *trace_line);
static void vi_net_system_packet_set_message(
    struct vi_net_system_t *net_system, struct vi_trace_line_t *trace_line);
static void vi_net_system_end_packet(struct vi_net_system_t *net_system,
                                     struct vi_trace_line_t *trace_line);
static void vi_net_system_packet_extract(struct vi_net_system_t *net_system,
                                         struct vi_trace_line_t *trace_line);
static void vi_net_system_packet_insert(struct vi_net_system_t *net_system,
                                        struct vi_trace_line_t *trace_line);
static void vi_net_system_packet(struct vi_net_system_t *net_system,
                                 struct vi_trace_line_t *trace_line);
...
void vi_net_system_init(void) {
  ...
  vi_state_new_command(
      "net.msg", (vi_state_process_trace_line_func_t)vi_net_system_message,
      vi_net_system);
  ...
  VI_STATE_FOR_EACH_HEADER(trace_line) {
    ...
    if (!strcmp(command, "net.init")) {
      ...
      if (version_major != VI_NET_SYSTEM_TRACE_VERSION_MAJOR ||
        ...
        fatal(
            "incompatible network system trace version.\n"
...
static void vi_net_system_read_checkpoint(struct vi_net_system_t *net_system,
  ...
  for (i = 0; i < hash_table_count(vi_net_system->net_table); i++) {
    ...
    if (!net) panic("%s: %s: invalid module name", __FUNCTION__, net_name);
...
static void vi_net_system_new_message(struct vi_net_system_t *net_system,
  ...
  net = hash_table_get(vi_net_system->net_table, net_name);
  if (!net) panic("%s: invalid network name '%s'", __FUNCTION__, net_name);
...
static void vi_net_system_msg_set_access(struct vi_net_system_t *net_system,
  ...
  if (!message) panic("%s: invalid message name '%s'", __FUNCTION__, net_name);
...
static void vi_net_system_packet_set_message(
  ...
  message = hash_table_get(net->message_table, message_name);
  if (!message)
    panic("%s: invalid message: wrong association of packet and message '%s'",
          __FUNCTION__, message_name);

  packet_name = vi_trace_line_get_symbol(trace_line, "name");

  /* Create new access */
  packet = hash_table_get(net->packet_table, packet_name);
  if (!packet) panic("%s: invalid message name '%s'", __FUNCTION__, net_name);
...
static void vi_net_system_message(struct vi_net_system_t *net_system,
  ...
  if (!message) panic("%s: %s: access not found", __FUNCTION__, name);
...
static void vi_net_system_packet(struct vi_net_system_t *net_system,
  ...
  if (!packet) panic("%s: %s: access not found", __FUNCTION__, name);

  stage = str_map_string(&vi_packet_stage_map,
...
static void vi_net_system_end_message(struct vi_net_system_t *net_system,
  ...
  if (!message) panic("%s: message not found", __FUNCTION__);
...
static void vi_net_system_end_packet(struct vi_net_system_t *net_system,
  ...
  if (!packet) panic("%s: packet not found", __FUNCTION__);
...
static void vi_net_system_link_transfer(struct vi_net_system_t *net_system,
  ...
  if (!link) panic("%s: Link not found", __FUNCTION__);
...
static void vi_net_system_packet_insert(struct vi_net_system_t *net_system,
  ...
  if (!net) panic("%s: invalid node name '%s'", __FUNCTION__, node_name);

  packet_name = vi_trace_line_get_symbol(trace_line, "name");
  packet = hash_table_get(net->packet_table, packet_name);
  if (!packet) panic("%s: invalid packet name '%s'", __FUNCTION__, packet_name);
