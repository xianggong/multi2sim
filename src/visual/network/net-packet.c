...
struct str_map_t vi_packet_stage_map = {vi_packet_stage_count - 1,
...
void vi_net_packet_free(struct vi_net_packet_t *packet) {
  if (packet->message_name) str_free(packet->message_name);
  if (packet->net_name) str_free(packet->net_name);
  if (packet->name) {
    str_free(packet->name);
  }
  if (packet->state) str_free(packet->state);
  free(packet);
}

struct vi_net_packet_t *vi_net_packet_create(char *net_name, char *name,
                                             int size) {
...
void vi_net_packet_write_checkpoint(struct vi_net_packet_t *packet, FILE *f) {
  ...
  count = fwrite(&packet->state_update_cycle, 1,
                 sizeof packet->state_update_cycle, f);
...
void vi_net_packet_read_checkpoint(struct vi_net_packet_t *packet, FILE *f) {
  ...
  count = fread(&packet->state_update_cycle, 1,
                sizeof packet->state_update_cycle, f);
  if (count != sizeof packet->state_update_cycle)
    panic("%s: cannot read checkpoint", __FUNCTION__);
