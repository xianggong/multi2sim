...
struct vi_net_node_t *vi_net_node_assign(struct vi_trace_line_t *trace_line) {
  ...
  } else if (node->type == vi_net_node_switch ||
             node->type == vi_net_node_end) {
    node->input_buffer_list = hash_table_create(0, FALSE);
    node->output_buffer_list = hash_table_create(0, FALSE);
  } else if (node->type == vi_net_node_dummy ||
             node->type == vi_net_node_invalid) {
...
void vi_net_node_free(struct vi_net_node_t *node) {
  ...
  if (node->src_buffer_list) hash_table_free(node->src_buffer_list);

  if (node->dst_buffer_list) hash_table_free(node->dst_buffer_list);
...
void vi_net_bus_free(struct vi_net_bus_t *bus) { free(bus); }

void vi_node_insert_packet(struct vi_net_node_t *node, char *buffer_name,
  ...
  if (!buffer)
    panic("%s: Invalid buffer %s for node %s", __FUNCTION__, node->name,
          buffer_name);
...
void vi_node_extract_packet(struct vi_net_node_t *node, char *buffer_name,
                            int buffer_occupancy,
                            struct vi_net_packet_t *packet) {
  struct vi_net_buffer_t *buffer;
