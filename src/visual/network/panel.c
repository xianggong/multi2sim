...
static gboolean vi_net_board_toggle_button_toggled(
    GtkWidget *widget, struct vi_net_board_t *board);
static void vi_net_board_destroy(GtkWidget *widget,
                                 struct vi_net_board_t *board);
static struct vi_net_window_t *vi_net_window_create(
    struct vi_net_t *net, GtkWidget *parent_toggle_button);
static void vi_net_window_free(struct vi_net_window_t *net_window);
static void vi_net_window_destroy(GtkWidget *widget,
                                  struct vi_net_window_t *net_window);
static gboolean vi_net_window_delete(GtkWidget *widget, GdkEvent *event,
                                     struct vi_net_window_t *net_window);
...
struct vi_net_board_t *vi_net_board_create(struct vi_net_t *net) {
  ...
  /* Access list */
  struct vi_list_t *message_list = vi_list_create(
      "Message list", 10, 10,
      (vi_list_get_elem_name_func_t)vi_net_message_get_name_short,
      (vi_list_get_elem_desc_func_t)vi_net_message_get_desc);
  gtk_box_pack_start(GTK_BOX(vbox), vi_list_get_widget(message_list), TRUE,
                     TRUE, 0);
  gtk_widget_override_background_color(vi_list_get_widget(message_list),
                                       GTK_STATE_FLAG_NORMAL, &frame_color);
  board->message_list = message_list;

  /* Main widget */
  board->widget = event_box;
  g_signal_connect(G_OBJECT(board->widget), "destroy",
                   G_CALLBACK(vi_net_board_destroy), board);
...
static struct vi_net_window_t *vi_net_window_create(
  ...
  gtk_container_add(GTK_CONTAINER(window),
                    vi_net_widget_get_widget(net_widget));
  net_window->net_widget = net_widget;

  /* Associated widget */
  net_window->widget = window;
  gtk_widget_show_all(net_window->widget);
  g_signal_connect(G_OBJECT(net_window->widget), "destroy",
                   G_CALLBACK(vi_net_window_destroy), net_window);
  g_signal_connect(G_OBJECT(net_window->widget), "delete_event",
                   G_CALLBACK(vi_net_window_delete), net_window);
...
static gboolean vi_net_window_delete(GtkWidget *widget, GdkEvent *event,
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(net_window->parent_toggle_button), FALSE);
  return TRUE;
}
...
void vi_net_board_refresh(struct vi_net_board_t *board) {
  ...
  if (board->net_window) vi_net_window_refresh(board->net_window);
}
