...
static void vi_node_widget_destroy(GtkWidget *widget,
...
static void vi_node_widget_size_allocate(GtkWidget *widget,
                                         GdkRectangle *allocation,
                                         struct vi_node_widget_t *node_widget) {
  vi_node_widget_refresh(node_widget);
}

static void vi_node_widget_scroll_bar_value_changed(
    GtkRange *range, struct vi_node_widget_t *node_widget) {
  vi_node_widget_refresh(node_widget);
}
...
void vi_node_widget_free(struct vi_node_widget_t *node_widget) {
  ...
  if (node_widget->content_matrix) matrix_free(node_widget->content_matrix);

  /* buffer list */
  if (node_widget->buffer_list) list_free(node_widget->buffer_list);

  /* buffer queue list */
  if (node_widget->buffer_queue_list) list_free(node_widget->buffer_queue_list);

  if (node_widget->buffer_title_list) list_free(node_widget->buffer_title_list);

  /* Free widget */
  free(node_widget);
}

static void vi_node_widget_refresh_scrollbars(
    struct vi_node_widget_t *node_widget) {
  ...
  num_buffers = hash_table_count(node->input_buffer_list) +
                hash_table_count(node->output_buffer_list);
  ...
  if (table_width > node_widget->content_layout_width) {
    ...
    gtk_range_set_increments(
        GTK_RANGE(node_widget->hscrollbar), VI_NODE_CONTENT_CELL_WIDTH / 3,
        node_widget->content_layout_width - VI_NODE_CONTENT_CELL_WIDTH / 3);
  ...
  if (table_height > node_widget->content_layout_height) {
    ...
    gtk_range_set_increments(
        GTK_RANGE(node_widget->vscrollbar), VI_NODE_CONTENT_CELL_HEIGHT,
        node_widget->content_layout_height - VI_NODE_CONTENT_CELL_HEIGHT);
...
static void vi_node_widget_refresh_content_layout(
    struct vi_node_widget_t *node_widget) {
  ...
  num_rows =
      MIN(num_buffers,
          node_widget->content_layout_height / VI_NODE_CONTENT_CELL_HEIGHT + 2);
  num_cols =
      MIN(num_packets,
          node_widget->content_layout_width / VI_NODE_CONTENT_CELL_WIDTH + 2);
  ...
  MATRIX_FOR_EACH(node_widget->content_matrix, row, col) {
    ...
    PangoAttribute *size_attr =
        pango_attr_size_new_absolute(VI_NODE_CONTENT_FONT_SIZE << 10);
...
static void vi_node_widget_refresh_buffer_title_layout(
    struct vi_node_widget_t *node_widget) {
  ...
  int num_cols = MIN(
      6,
      node_widget->buffer_table_layout_width / VI_NODE_CONTENT_CELL_WIDTH + 2);
  ...
  if (node_widget->buffer_title) gtk_widget_destroy(node_widget->buffer_title);
  buffer_title = gtk_table_new(1, num_cols, FALSE);
  gtk_layout_put(GTK_LAYOUT(buffer_table_title_layout), buffer_title, 0,
                 node_widget->top_offset);
  ...
  for (col = 0; col < num_cols; col++) {
    ...
    gtk_frame_set_shadow_type(GTK_FRAME(frame), GTK_SHADOW_IN);
    gtk_container_add(GTK_CONTAINER(frame), event_box);
    gtk_table_attach_defaults(GTK_TABLE(buffer_title), frame, col, col + 1, 0,
                              1);
    gtk_widget_set_size_request(frame, width, VI_NODE_CONTENT_CELL_HEIGHT);
    /* Color */
    GdkColor color;
...
static void vi_node_widget_refresh_buffer_table_layout(
    struct vi_node_widget_t *node_widget) {
  ...
  num_buffers = hash_table_count(node->input_buffer_list) +
                hash_table_count(node->output_buffer_list);
  num_rows = MIN(num_buffers, node_widget->buffer_table_layout_height /
                                      VI_NODE_CONTENT_CELL_HEIGHT +
                                  2);

  /* Clear instruction label list */
  if (!node_widget->buffer_list) node_widget->buffer_list = list_create();
  list_clear(node_widget->buffer_list);

  /* Create new table */
  buffer_table_layout = node_widget->buffer_table_layout;
  if (node_widget->buffer_table) gtk_widget_destroy(node_widget->buffer_table);
  buffer_table = gtk_table_new(num_rows, 6, FALSE);
  gtk_layout_put(GTK_LAYOUT(buffer_table_layout), buffer_table, 0,
                 node_widget->top_offset);
  ...
  for (row = 0; row < num_rows; row++) {
    for (int col = 0; col < 6; col++) {
      ...
      gtk_frame_set_shadow_type(GTK_FRAME(frame), GTK_SHADOW_IN);
      gtk_container_add(GTK_CONTAINER(frame), event_box);
      gtk_table_attach_defaults(GTK_TABLE(buffer_table), frame, col, col + 1,
                                row, row + 1);
      gtk_widget_set_size_request(frame, width, VI_NODE_CONTENT_CELL_HEIGHT);
      /* Color */
      GdkColor color;
...
static void vi_node_widget_refresh_buffer_queue_layout(
    struct vi_node_widget_t *node_widget) {
  ...
  if (node_widget->buffer_queue) gtk_widget_destroy(node_widget->buffer_queue);
  buffer_queue = gtk_table_new(1, num_cols, TRUE);
  gtk_layout_put(GTK_LAYOUT(buffer_queue_layout), buffer_queue,
  ...
  for (col = 0; col < num_cols; col++) {
    ...
    GtkWidget *event_box = gtk_event_box_new();
    gtk_container_add(GTK_CONTAINER(event_box), label);
    gtk_table_attach_defaults(GTK_TABLE(buffer_queue), event_box, col, col + 1,
                              0, 1);
...
void vi_node_widget_refresh_content(struct vi_node_widget_t *node_widget) {
  ...
  LIST_FOR_EACH(node_widget->buffer_title_list, col) {
    ...
    snprintf(str, sizeof str, "%s",
             str_map_value(&vi_net_buffer_attrib_map, col));
    gtk_label_set_text(GTK_LABEL(label), str);
  }
  ...
  HASH_TABLE_FOR_EACH(node->input_buffer_list, buffer_name, buffer) {
    ...
    label = gtk_bin_get_child(GTK_BIN(event_box));
    snprintf(str, sizeof str, "%s",
             str_map_value(&vi_net_buffer_direction_map, buffer->direction));
    ...
    float utilization = (float)buffer->link->transferred_bytes /
                        (vi_cycle_bar_get_cycle() * buffer->link->bandwidth);
    ...
    MATRIX_FOR_EACH_COLUMN(node_widget->content_matrix, col) {
      ...
      char *color_str;
      color_str = packet
                      ? str_map_value(&vi_packet_stage_color_map, packet->stage)
                      : "#EEEEEE";
  ...
  HASH_TABLE_FOR_EACH(node->output_buffer_list, buffer_name, buffer) {
    ...
    /* Buffer is in range */
    buffer_row =
        buffer->id + hash_table_count(node->input_buffer_list) - top_buffer;
    if (!IN_RANGE(buffer_row, 0, list_count(node_widget->buffer_list) - 1))
      continue;
    ...
    /* Connection Utilization */
    float utilization = 0;
    utilization = (float)buffer->link->transferred_bytes /
                  (vi_cycle_bar_get_cycle() * buffer->link->bandwidth);
    if (utilization > 1) utilization = 1;
    if (utilization < 0) utilization = 0;
    event_box = list_get(node_widget->buffer_list, buffer_row * 6 + 2);
  ...
  /*
          HASH_TABLE_FOR_EACH(node->input_buffer_list, buffer_name, buffer)
          {
                  ...
                  fprintf(stderr,"buffer %s:%s content:", node->name,
     buffer->name);
...
struct vi_node_widget_t *vi_node_widget_create(struct vi_net_node_t *node) {
  ...
  gtk_widget_set_size_request(buffer_queue_layout,
                              VI_NODE_BUFFER_QUEUE_LAYOUT_WIDTH,
  ...
  gtk_widget_set_size_request(buffer_table_layout,
                              VI_NODE_BUFFER_TABLE_LAYOUT_WIDTH,
  ...
  gtk_widget_set_size_request(buffer_table_title_layout,
                              VI_NODE_BUFFER_TABLE_LAYOUT_WIDTH,
                              VI_NODE_CONTENT_CELL_HEIGHT);
  ...
  gtk_box_pack_start(GTK_BOX(left_vbox), buffer_table_title_layout, FALSE,
                     FALSE, 0);
  ...
  g_signal_connect(G_OBJECT(hscrollbar), "value-changed",
                   G_CALLBACK(vi_node_widget_scroll_bar_value_changed),
                   node_widget);
...
void vi_node_widget_refresh(struct vi_node_widget_t *node_widget) {
  ...
  content_layout_width =
      gtk_widget_get_allocated_width(node_widget->content_layout);
  content_layout_height =
      gtk_widget_get_allocated_height(node_widget->content_layout);

  buffer_table_layout_width =
      gtk_widget_get_allocated_width(node_widget->buffer_table_layout);
  buffer_table_layout_height =
      gtk_widget_get_allocated_height(node_widget->buffer_table_layout);
  ...
  content_layout_width_changed =
      content_layout_width != node_widget->content_layout_width;
  content_layout_height_changed =
      content_layout_height != node_widget->content_layout_height;

  buffer_table_layout_width_changed =
      buffer_table_layout_width != node_widget->buffer_table_layout_width;
  buffer_table_layout_height_changed =
      buffer_table_layout_height != node_widget->buffer_table_layout_height;
  ...
  if (left_offset_changed || top_offset_changed)
    gtk_layout_move(GTK_LAYOUT(node_widget->content_layout),
                    node_widget->content_table, node_widget->left_offset,
                    node_widget->top_offset);
  ...
  if (top_offset_changed)
    gtk_layout_move(GTK_LAYOUT(node_widget->buffer_table_layout),
                    node_widget->buffer_table, 0, node_widget->top_offset);
  ...
  if (left_offset_changed)
    gtk_layout_move(GTK_LAYOUT(node_widget->buffer_queue_layout),
                    node_widget->buffer_queue, node_widget->left_offset, 0);
