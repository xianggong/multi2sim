...
void vi_net_graph_populate(struct vi_net_t *net,
                           struct vi_net_graph_t *net_graph) {
  int i;
  ...
  for (i = 0; i < list_count(net->node_list); i++) {
    ...
    if (node->type == vi_net_node_switch) {
      ...
      vertex->kind = graph_vertex_node;
    } else if (node->type == vi_net_node_bus ||
               node->type == vi_net_node_photonic) {
  ...
  HASH_TABLE_FOR_EACH(net->link_table, link_name, link) {
    ...
    if (check == 0) {
      ...
      if (edge_data) vi_net_edge_data_free(edge_data);
  ...
  for (i = 0; i < list_count(graph->vertex_list); i++) {
    ...
    if (vertex_data->kind == vi_net_vertex_bus) {
      ...
      HASH_TABLE_FOR_EACH(bus_node->src_buffer_list, src_buffer_name,
                          src_buffer) {
      ...
      HASH_TABLE_FOR_EACH(bus_node->dst_buffer_list, dst_buffer_name,
                          dst_buffer) {
        int check = 0;
...
void vi_net_graph_free(struct vi_net_graph_t *net_graph) {
  ...
  for (i = 0; i < list_count(graph->vertex_list); i++) {
    ...
    vertex = list_get(graph->vertex_list, i);
    vertex_data = (struct vi_net_vertex_data_t *)vertex->data;
    if (vertex_data) vi_net_vertex_data_free(vertex_data);
  }
...
void vi_net_graph_draw_scale(struct vi_net_graph_t *net_graph) {
  ...
  for (int i = 0; i < list_count(graph->vertex_list); i++) {
    ...
    int opt_distance = graph->max_vertex_in_layer * scale /
                       (2 * graph->max_vertex_in_layer - 1);
    vertex->x_coor = (2 * (vertex->x_coor) +
                      (graph->max_vertex_in_layer - vertex->neighbours)) *
                     opt_distance;
  }
