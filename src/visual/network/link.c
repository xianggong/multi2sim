...
void vi_link_color_utilization(struct vi_net_link_t *link) {
  ...
  utilization =
      (double)link->transferred_bytes / (current_cycle * link->bandwidth);
...
void vi_link_color_per_cycle(struct vi_net_link_t *link) {
  ...
  if (link->busy_cycle >= cycle)
    remain = link->last_packet_size -
             (cycle + lat - 1 - link->busy_cycle) * link->bandwidth;
...
void vi_net_sub_link_free(struct vi_net_sub_link_t *subLink) { free(subLink); }
...
void vi_link_read_checkpoint(struct vi_net_link_t *link, FILE *f) {
  ...
  if (count != 8) fatal("%s: error reading from checkpoint", __FUNCTION__);
...
void vi_link_write_checkpoint(struct vi_net_link_t *link, FILE *f) {
  ...
  if (count != 8) fatal("%s: cannot write to checkpoint file", __FUNCTION__);
}
