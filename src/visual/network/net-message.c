...
void vi_net_message_free(struct vi_net_message_t *message) {
  if (message->access_name) str_free(message->access_name);
...
struct vi_net_message_t *vi_net_message_create(char *net_name, char *name,
                                               int size) {
...
void vi_net_message_read_checkpoint(struct vi_net_message_t *message, FILE *f) {
  ...
  count = fread(&message->state_update_cycle, 1,
                sizeof message->state_update_cycle, f);
  if (count != sizeof message->state_update_cycle)
    panic("%s: cannot read checkpoint", __FUNCTION__);
}

void vi_net_message_write_checkpoint(struct vi_net_message_t *message,
                                     FILE *f) {
  int count;
  ...
  count =
      fwrite(&message->creation_cycle, 1, sizeof message->creation_cycle, f);
  if (count != sizeof message->creation_cycle)
    panic("%s: cannot write checkpoint", __FUNCTION__);

  /* Write state update cycle */
  count = fwrite(&message->state_update_cycle, 1,
                 sizeof message->state_update_cycle, f);
  if (count != sizeof message->state_update_cycle)
    panic("%s: cannot write checkpoint", __FUNCTION__);
}

void vi_net_message_get_name_short(struct vi_net_message_t *message, char *buf,
                                   int size) {
  /* Name */
  str_printf(&buf, &size, "%s", message->name);
}

void vi_net_message_get_desc(struct vi_net_message_t *message, char *buf,
                             int size) {
  ...
  for (trace_line = vi_state_trace_line_first(cycle); trace_line;
    ...
    /* Access starts */
    if ((!strcmp(command, "net.new_msg") &&
         !strcmp(message_name, message->name) &&
         !strcmp(net_name, message->net_name)) ||
        (!strcmp(command, "net.msg") && !strcmp(message_name, message->name) &&
         !strcmp(net_name, message->net_name))) {
