...
struct str_map_t vi_net_buffer_attrib_map = {6,
                                             {{"Direction", 0},
                                              {"Connection Name", 1},
                                              {"Link Util", 2},
                                              {"Buffer Name", 3},
...
struct vi_net_buffer_t *vi_net_buffer_create(
  ...
  /*	if (net->packet_size != 0)
                  buffer->packet_capacity = (buffer_size -1 ) / net->packet_size
     + 1;
          else
  ...
  */
  ...
  if (buffer->connection_type == vi_buffer_link) {
    ...
    assert(bus_node && ((bus_node->type == vi_net_node_bus) ||
                        (bus_node->type == vi_net_node_photonic)));
