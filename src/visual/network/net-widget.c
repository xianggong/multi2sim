...
static void vi_net_widget_destroy(GtkWidget *widget,
...
static gboolean vi_node_board_toggle_button_toggled(
    GtkWidget *widget, struct vi_node_board_t *board);
static void vi_node_board_destroy(GtkWidget *widget,
...
static void vi_link_board_destroy(GtkWidget *widget,
                                  struct vi_link_board_t *subLink);
static void vi_link_board_refresh(struct vi_link_board_t *board);

static struct vi_node_window_t *vi_node_window_create(
    struct vi_net_node_t *node, GtkWidget *parent_toggle_button);
static void vi_node_window_free(struct vi_node_window_t *node_window);
static void vi_node_window_destroy(GtkWidget *widget,
                                   struct vi_node_window_t *node_window);
static void vi_node_window_refresh(struct vi_node_window_t *window);
static gboolean vi_node_window_delete(GtkWidget *widget, GdkEvent *event,
                                      struct vi_node_window_t *node_window);
static GtkWidget *vi_node_window_get_widget(
    struct vi_node_window_t *node_window);

static gboolean vi_link_board_draw(GtkWidget *widget, GdkEventConfigure *event,
...
struct vi_net_widget_t *vi_net_widget_create(struct vi_net_t *net) {
  ...
  gtk_widget_set_size_request(scrolled_window, VI_NODE_BOARD_WIDTH * 3 / 2,
                              VI_NODE_BOARD_HEIGHT * 3 / 2);
  ...
  HASH_TABLE_FOR_EACH(net->link_table, link_name, link) {
    for (int i = 0; i < list_count(link->sublink_list); i++) {
      ...
      if (subLink->src_x < subLink->dst_x) {
        ...
        drwlink->src_y =
            -ALPHA + ((VI_NODE_BOARD_HEIGHT / 2 + VI_NODE_BOARD_PADDING) +
                      (2 * subLink->src_y) *
        ...
        if (subLink->src_y > subLink->dst_y) {
          drwlink->src_x =
              -ALPHA + ((VI_NODE_BOARD_WIDTH / 2 + VI_NODE_BOARD_PADDING / 2) +
                        (2 * subLink->src_x) *
                            (VI_NODE_BOARD_PADDING + VI_NODE_BOARD_WIDTH));
  ...
  LIST_FOR_EACH(net->node_list, node_index) {
    ...
    /* Get board Position */
    x = VI_NODE_BOARD_PADDING / 2 +
        (2 * node->X) * (VI_NODE_BOARD_PADDING + VI_NODE_BOARD_WIDTH);
    y = VI_NODE_BOARD_PADDING / 2 +
        (2 * node->Y) * (VI_NODE_BOARD_PADDING + VI_NODE_BOARD_HEIGHT);
    ...
    /* Size of layout */
    layout_width =
        MAX(layout_width, x + VI_NODE_BOARD_WIDTH + VI_NODE_BOARD_PADDING);
    layout_height =
        MAX(layout_height, y + VI_NODE_BOARD_HEIGHT + VI_NODE_BOARD_PADDING);
  }
  ...
  LIST_FOR_EACH(net->dummy_node_list, node_index) {
    ...
    x = +VI_NODE_BOARD_PADDING / 2 + VI_NODE_BOARD_WIDTH / 4 +
        (2 * node->X) * (VI_NODE_BOARD_PADDING + VI_NODE_BOARD_WIDTH);
    y = +VI_NODE_BOARD_PADDING / 2 + VI_NODE_BOARD_HEIGHT / 4 +
        (2 * node->Y) * (VI_NODE_BOARD_PADDING + VI_NODE_BOARD_HEIGHT);
    board = vi_node_board_create(node);
    list_add(panel->node_board_list, board);
    gtk_layout_put(GTK_LAYOUT(layout), board->widget, x, y);
...
static struct vi_node_board_t *vi_node_board_create(
    struct vi_net_node_t *node) {
  struct vi_node_board_t *board;
  ...
  gtk_widget_override_background_color(event_box, GTK_STATE_FLAG_NORMAL,
                                       &frame_color);
  ...
  g_signal_connect(G_OBJECT(board->widget), "destroy",
                   G_CALLBACK(vi_node_board_destroy), board);
...
static struct vi_node_window_t *vi_node_window_create(
  ...
  gtk_container_add(GTK_CONTAINER(window),
                    vi_node_widget_get_widget(node_widget));
  node_window->node_widget = node_widget;

  /* Associated widget */
  node_window->widget = window;
  gtk_widget_show_all(node_window->widget);
  g_signal_connect(G_OBJECT(node_window->widget), "destroy",
                   G_CALLBACK(vi_node_window_destroy), node_window);
  g_signal_connect(G_OBJECT(node_window->widget), "delete_event",
                   G_CALLBACK(vi_node_window_delete), node_window);
...
static gboolean vi_node_window_delete(GtkWidget *widget, GdkEvent *event,
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(node_window->parent_toggle_button), FALSE);
  return TRUE;
}
...
static gboolean vi_link_board_draw(GtkWidget *widget, GdkEventConfigure *event,
  ...
  /* Link */
  cairo_set_source_rgb(cr, link->color.red, link->color.green,
                       link->color.blue);
  ...
  float Alpha = M_PI / 2;
  float Cprime = sqrt(pow(board->dst_x - board->src_x, 2) +
                      pow(board->dst_y - board->src_y, 2));
  float C = 15;
  ...
  for (int i = 0; i < iter; i++) {
    ...
    float tri_center_x =
        board->src_x + ((i + 1) * ((board->dst_x - board->src_x) / iter));
    float tri_center_y =
        board->src_y + ((i + 1) * ((board->dst_y - board->src_y) / iter));
...
static void vi_link_board_free(struct vi_link_board_t *board) { free(board); }
static void vi_link_board_destroy(GtkWidget *widget,
                                  struct vi_link_board_t *board) {
  vi_link_board_free(board);
}
static struct vi_link_board_t *vi_link_board_create(
    struct vi_net_link_t *link) {
  struct vi_link_board_t *board;
...
static void vi_link_board_project(struct vi_link_board_t *subLink) {
  ...
  gtk_widget_set_size_request(drawing_area,
                              MAX(subLink->src_x, subLink->dst_x) + 10,
                              MAX(subLink->src_y, subLink->dst_y) + 10);
  gtk_widget_override_background_color(drawing_area, GTK_STATE_NORMAL, &color);
  g_signal_connect(G_OBJECT(drawing_area), "draw",
                   G_CALLBACK(vi_link_board_draw), subLink);

  /* Main widget */
  subLink->widget = drawing_area;
  g_signal_connect(G_OBJECT(subLink->widget), "destroy",
                   G_CALLBACK(vi_link_board_destroy), subLink);
}
...
static void vi_node_window_refresh(struct vi_node_window_t *window) {
  ...
  if (window->node_widget) vi_node_widget_refresh_content(window->node_widget);
}
static void vi_link_board_refresh(struct vi_link_board_t *board) {
