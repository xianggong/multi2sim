...
static void vi_mem_panel_destroy(GtkWidget *widget,
  ...
  if (panel->net_board_list) list_free(panel->net_board_list);
...
struct vi_mem_panel_t *vi_mem_panel_create(void) {
    ...
    LIST_FOR_EACH(vi_net_system->level_list, level_id) {
      ...
      LIST_FOR_EACH(net_list, net_id) {
        ...
        x = VI_MOD_BOARD_PADDING +
            net_id * (VI_MOD_BOARD_PADDING + VI_MOD_BOARD_WIDTH);
        y = VI_MOD_BOARD_PADDING +
            (2 * level_id + 1) * (VI_MOD_BOARD_PADDING + VI_MOD_BOARD_HEIGHT);
  ...
  LIST_FOR_EACH(vi_mem_system->mod_level_list, level_id) {
    ...
    LIST_FOR_EACH(mod_list, mod_id) {
      ...
      y = VI_MOD_BOARD_PADDING +
          (1 + tab) * level_id * (VI_MOD_BOARD_PADDING + VI_MOD_BOARD_HEIGHT);
