...
Disassembler::Disassembler() : comm::Disassembler("MIPS") {
  ...
  dec_table[OPCODE_SPECIAL2].next_table = dec_table_special2;
  dec_table[OPCODE_SPECIAL2].next_table_low = 0;
  dec_table[OPCODE_SPECIAL2].next_table_high = 5;

  dec_table[OPCODE_SPECIAL3].next_table = dec_table_special3;
...
void Disassembler::DisassembleBinary(const std::string& path) {
  ...
  for (int i = 0; i < file.getNumSections(); i++) {
    ...
    if (!(section->getFlags() & SHF_EXECINSTR)) continue;
    ...
    for (pos = 0; pos < section->getSize(); pos += 4) {
      ...
      if (symbol && symbol->getValue() == section->getAddr() + pos)
        std::cout << misc::fmt("\n\n%08x <%s>:", section->getAddr() + pos,
                               symbol->getName().c_str());

      /* Decode and dump */
      inst.Decode(section->getAddr() + pos, section->getBuffer() + pos);
        ...
        if (print_symbol) {
          if (print_symbol->getValue() == inst.getTarget())
            ...
            std::cout << misc::fmt(" <%s+0x%x>",
                                   ...
                                   inst.getTarget() - print_symbol->getValue());
        }
      }
    }
  }
