...
void Instruction::Decode(unsigned addr, const char* buf) {
  ...
  loop_iteration = 0;
  table_arg =
      misc::getBits32(bytes.word, current_table_high, current_table_low);

  // Find next tables if the instruction belongs to another table
  while (1) {
    if (current_table[table_arg].next_table && loop_iteration < 4) {
      ...
      throw Disassembler::Error(
          misc::fmt("Invalid instruction at "
                    "0x%x",
                    addr));
    } else {
      break;
...
void Instruction::Dump(std::ostream& os) {
  ...
  if (!fmt_str || !*fmt_str) throw misc::Panic("Instruction not implemented");
  ...
  while (*fmt_str) {
    ...
    else if (comm::Disassembler::isToken(fmt_str, "addiu", token_len)) {
      if (rs == 0) {
        os << "li\t";
        DumpRt(os);
        os << ',';
    ...
    if (comm::Disassembler::isToken(fmt_str, "sa", token_len))
      ...
      DumpRs(os);
    else if (comm::Disassembler::isToken(fmt_str, "target", token_len))
      DumpTarget(os);
    else if (comm::Disassembler::isToken(fmt_str, "offset", token_len))
      DumpOffset(os);
    else if (comm::Disassembler::isToken(fmt_str, "offsetbr", token_len))
      DumpOffsetbr(os);
    else if (comm::Disassembler::isToken(fmt_str, "Imm", token_len))
      DumpImm(os);
    else if (comm::Disassembler::isToken(fmt_str, "Immhex", token_len))
      DumpImmhex(os);
