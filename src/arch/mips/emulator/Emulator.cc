...
void Emulator::RegisterOptions() {
  ...
  command_line->RegisterString(
      "--mips-debug-loader <file>", loader_debug_file,
  ...
  command_line->RegisterString(
      "--mips-debug-syscall <file>", syscall_debug_file,
...
void Emulator::AddContextToList(ContextListType type, Context* context) {
  ...
  if (context->context_list_present[type]) return;
...
void Emulator::ProcessEvents() {
  ...
  for (auto iter = suspended_context_list.begin();
       iter != suspended_context_list.end(); iter = iter_next) {
