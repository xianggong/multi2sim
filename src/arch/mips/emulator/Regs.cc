...
unsigned int Regs::getGPR(int n) { return gpr[n]; }

// Read from HI register
unsigned int Regs::getHI() { return hi; }

// Write HI register with value
void Regs::setHI(unsigned int value) { hi = value; }

// Write LO register with value
void Regs::setLO(unsigned int value) { lo = value; }
...
int Regs::show_all_registers(int n, char *output_buffer)
{
        ...
        if(n==16)
        {
                ...
                {
                        m+=sprintf(output_buffer+m,"%4s%-2d %#-10x", "r",i,
read_r(i));
        ...
        else
        {
                ...
                for(int i=0; i<GPR_COUNT; i++)
                {
                        m+=sprintf(output_buffer+m, "%4s%-2d %-8s",
"r",i,(read_r(i), buffer, n));
                        if((i+1)%4==0)m+=sprintf(output_buffer+m, "%c",'\n');

                }

                m+=sprintf(output_buffer+m, "%6s %-8s", "PC",(read_PC(), buffer,
n));
                m+=sprintf(output_buffer+m, "%6s %-8s",  "HI",(read_HI(),
buffer, n));
