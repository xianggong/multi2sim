...
void Context::MipsIsaBranch(unsigned int dest) { n_next_ip = dest; }
...
void Context::ExecuteInst_J() {
  ...
  // Perform Operation
  unsigned int dest =
      ((misc::getBits32(next_ip, 32, 28)) << 28) | (target << 2);
  MipsIsaBranch(dest);
}
...
void Context::ExecuteInst_ADDI() {
  ...
  // Perform Operation
  int temp = ((rs_reg & 0x80000000) | rs_reg) + misc::SignExtend32(offset, 16);
...
void Context::ExecuteInst_LBU() {
  ...
  unsigned int virtual_addr =
      regs.getGPR(base) + misc::SignExtend32((signed)imm, 16);
  unsigned char temp;
...
void Context::ExecuteInst_JR() {
  ...
  // Debug
  emulator->isa_debug << misc::fmt("jump to reg[%d]=0x%x", rs,
                                   regs.getCoprocessor0GPR(rs));
}
...
void Context::ExecuteInst_MOVZ() {
  ...
  if (regs.getGPR(rt) == 0) regs.setGPR(rd, regs.getGPR(rs));
}
...
void Context::ExecuteInst_SYSCALL() { ExecuteSyscall(); }
...
void Context::ExecuteInst_MULT() {
  ...
  unsigned long long int temp =
      (long long int)regs.getGPR(rs) * (long long int)regs.getGPR(rt);
...
void Context::ExecuteInst_EXT() {
  ...
  if (rd + sa > 31) misc::Panic("inst EXT, unexpected behavior");
...
void Context::ExecuteInst_RDHWR() {
  ...
  if (rd == 29) regs.setGPR(rt, regs.getCoprocessor0GPR(rd));
...
void Context::ExecuteInst_CFC1() {
  ...
  switch (fs) {
    ...
    default:
      misc::Panic(
          misc::fmt("%s: CFC1 instruction, unknown value for fs", __FILE__));
  }
  regs.setGPR(rt, temp);
}
