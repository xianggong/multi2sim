...
void Context::UpdateState(unsigned state) {
  ...
  if (diff & ~ContextSpecMode) emulator->setScheduleSignal();

  // Update state
  this->state = state;
  if (this->state & ContextFinished)
    this->state =
        ContextFinished | (state & ContextAlloc) | (state & ContextMapped);
  if (this->state & ContextZombie)
    this->state =
        ContextZombie | (state & ContextAlloc) | (state & ContextMapped);
  if (!(this->state & ContextSuspended) && !(this->state & ContextFinished) &&
      !(this->state & ContextZombie) && !(this->state & ContextLocked))
...
void Context::Execute() {
  ...
  char* buffer_ptr =
      memory->getBuffer(regs.getPC(), 4, mem::Memory::AccessExec);
  if (!buffer_ptr) {
    ...
    memory->setSafe(false);
    buffer_ptr = buffer;
    memory->Access(regs.getPC(), 4, buffer_ptr, mem::Memory::AccessExec);
  }
  ...
  if (emulator->isa_debug) {
    ELFReader::Symbol* symbol =
        (loader->binary)->getSymbolByAddress(regs.getPC());
    std::string symbol_string = symbol->getName();
    if ((regs.getPC() - previous_ip) != 4)
      emulator->isa_debug << misc::fmt("\nIN %s\n", symbol_string.c_str());

    emulator->isa_debug << misc::fmt(
        "%d %8lld %x: ", pid, emulator->getNumInstructions(), regs.getPC());
...
void Context::FinishGroup(int exit_code) {
  ...
  if (getState(ContextFinished) || getState(ContextZombie)) return;
