...
int Context::ExecuteSyscall_syscall() {
  throw misc::Panic(misc::fmt("Unimplemented syscall (code %d)",
                              regs.getGPR(2) - __NR_Linux));
}
...
bool Context::SyscallWriteCanWakeup() {
  if (emulator->syscall_debug) misc::fmt(" syscall can wake up");
  return false;
}
...
int Context::ExecuteSyscall_write() {
  ...
  unsigned count = regs.getGPR(6);
  emulator->syscall_debug << misc::fmt(
      "  guest_fd=%d, buf_ptr=0x%x, count=%d\n", guest_fd, buf_ptr, count);
...
int Context::ExecuteSyscall_open() {
  ...
  emulator->syscall_debug << misc::fmt(
      "  file_name='%s' flags=0x%x, mode=0x%x\n", file_name.c_str(), flags,
      mode);
...
static const misc::StringMap mmap_flags_map = {
    ...
    {"MAP_STACK", 0x40000},      // give out an address that is best suited for
...
int Context::ExecuteSyscall_mmap() {
  ...
  emulator->syscall_debug << misc::fmt(
      "  addr=0x%x, len=%d, prot=0x%x, flags=0x%x, "
...
int Context::ExecuteSyscall_writev() {
  ...
  if (emulator->syscall_debug)
    emulator->syscall_debug
        << misc::fmt("  guest_fd=%d, iovec_ptr = 0x%x, vlen=0x%x\n", guest_fd,
                     iovec_ptr, vlen);
  ...
  if (emulator->syscall_debug)
    emulator->syscall_debug
        << misc::fmt("  total_len for writev: %d", total_len);
...
int Context::ExecuteSyscall_fstat64() {
  ...
  emulator->syscall_debug << misc::fmt(
      ...
      sim_statbuf.ino, sim_statbuf.mode, sim_statbuf.nlink);
  emulator->syscall_debug << misc::fmt("  uid=%d, gid=%d, rdev=%lld\n",
                                       sim_statbuf.uid, sim_statbuf.gid,
                                       sim_statbuf.rdev);
  emulator->syscall_debug << misc::fmt("  size=%lld, blksize=%d, blocks=%lld\n",
                                       sim_statbuf.size, sim_statbuf.blksize,
                                       sim_statbuf.blocks);
...
int Context::ExecuteSyscall_fcntl64() {
  ...
  if (desc->getHostIndex() < 0)
    misc::fatal("%s: not supported for this type of file", __FUNCTION__);
  emulator->syscall_debug << misc::fmt("    host_fd=%d\n",
                                       desc->getHostIndex());
  ...
  switch (cmd) {
    ...
    default:
      ...
      misc::fatal("%s: command %s not implemented.", __FUNCTION__,
                  fcntl_cmd_map.MapValue(cmd));
  }
