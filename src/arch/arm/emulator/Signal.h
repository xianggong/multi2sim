...
class SignalSet {
 ...
 public:
  ...
  bool isMember(int sig) const {
    return misc::inRange(sig, 1, 64) ? bitmap.Test(sig - 1) : false;
  }
  ...
  /// Operator "=="
  bool operator==(const SignalSet& set) const { return bitmap == set.bitmap; }
  ...
  /// Same as Dump()
  friend std::ostream& operator<<(std::ostream& os, const SignalSet& set) {
    set.Dump(os);
    return os;
  }
