...
void Context::LoadProgramHeaders() {
  ...
  emulator->loader_debug << misc::fmt(
      "  virtual address for program header "
      "table: 0x%x\n",
      phdt_base);

  // Allocate memory for program headers
  memory->Map(phdt_base, phdt_size,
              mem::Memory::AccessInit | mem::Memory::AccessRead);
  ...
  for (auto& program_header : binary->getProgramHeaders()) {
    ...
    memory->Init(address, phdr_size, (char*)program_header->getRawInfo());

    // Debug
    emulator->loader_debug
        << misc::fmt("  header loaded at 0x%x\n", address)
        << misc::fmt(
               "    type=%s, offset=0x%x, vaddr=0x%x, paddr=0x%x\n",
               program_header_type_map.MapValue(program_header->getType()),
               program_header->getOffset(), program_header->getVaddr(),
               program_header->getPaddr())
        << misc::fmt("    filesz=%d, memsz=%d, flags=%d, align=%d\n",
                     program_header->getFilesz(), program_header->getMemsz(),
                     program_header->getFlags(), program_header->getAlign());
...
void Context::LoadELFSections(ELFReader::File* binary) {
  ...
  for (auto& section : binary->getSections()) {
    ...
    Emulator::loader_debug << misc::fmt(
        "  section '%s': offset=0x%x, "
        "addr=0x%x, size=%u, flags=%s\n",
        section->getName().c_str(), section->getOffset(), section->getAddr(),
        section->getSize(), flags_str.c_str());

    // Process section
    if (section->getFlags() & SHF_ALLOC) {
      // Permissions
      if (section->getFlags() & SHF_WRITE) perm |= mem::Memory::AccessWrite;
      if (section->getFlags() & SHF_EXECINSTR) {
        perm |= mem::Memory::AccessExec;

        // Add region to call stack
        if (call_stack != nullptr) {
          call_stack->Map(binary->getPath(), section->getOffset(),
                          section->getAddr(), section->getSize(), false);
        }
      }
      ...
      if (section->getType() == 8) {
        ...
        memory->Init(section->getAddr(), section->getSize(), zero_buffer);
        delete zero_buffer;
      } else {
...
void Context::LoadStack() {
  ...
  emulator->loader_debug << misc::fmt(
      "mapping region for stack from 0x%x to 0x%x\n", loader->stack_top,
      loader->stack_base - 1);
  ...
  memory->Write(loader->at_random_addr_holder, 4,
                (char*)&loader->at_random_addr);

  // Check that we didn't overflow the stack
  if (sp > LoaderStackBase) throw misc::Panic("Stack overflow in loader");
}
...
void Context::LoadBinary() {
  ...
  if (!loader->stdin_file_name.empty()) {
    ...
    if (f < 0)
      misc::fatal("%s: cannot open stdin", loader->stdin_file_name.c_str());

    // Replace file descriptor 0
    file_table->freeFileDescriptor(0);
    file_table->newFileDescriptor(comm::FileDescriptor::TypeStandard, 0, f,
  ...
  if (!loader->stdout_file_name.empty()) {
    ...
    if (f < 0)
      throw Emulator::Error(
          misc::fmt("[%s] Cannot open standard "
                    "output",
    ...
    file_table->newFileDescriptor(comm::FileDescriptor::TypeStandard, 1, f,
                                  loader->stdout_file_name, O_WRONLY);
    file_table->newFileDescriptor(comm::FileDescriptor::TypeStandard, 2, f,
                                  loader->stdout_file_name, O_WRONLY);
  }
  ...
  memory->setHeapBreak(
      misc::RoundUp(memory->getHeapBreak(), mem::Memory::PageSize));
  ...
  ELFReader::Symbol* symbol =
      loader->binary->getSymbolByAddress(loader->prog_entry);
  ContextMode mode;
