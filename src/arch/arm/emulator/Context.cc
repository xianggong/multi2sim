...
void Context::UpdateState(unsigned state) {
  ...
  if (diff & ~ContextStateSpecMode) emulator->setScheduleSignal();

  // Update state
  this->state = state;
  if (this->state & ContextStateFinished)
    this->state = ContextStateFinished | (state & ContextStateAlloc);
  if (this->state & ContextStateZombie)
    this->state = ContextStateZombie | (state & ContextStateAlloc);
  ...
  emulator->UpdateContextInList(ContextListRunning, this,
                                this->state & ContextStateRunning);
  emulator->UpdateContextInList(ContextListZombie, this,
                                this->state & ContextStateZombie);
  emulator->UpdateContextInList(ContextListFinished, this,
                                this->state & ContextStateFinished);
  emulator->UpdateContextInList(ContextListSuspended, this,
                                this->state & ContextStateSuspended);
  emulator->UpdateContextInList(ContextListAlloc, this,
                                this->state & ContextListAlloc);
}
...
Context::Context() {
  ...
  for (int i = 0; i < ContextListCount; i++) context_list_present[i] = false;
...
void Context::Load(const std::vector<std::string>& args,
  ...
  if (!args.size() || args[0].empty())
    misc::panic(
        "%s: function invoked with no program name, or with an "
        "empty program.",
        __FUNCTION__);
  ...
  if (loader.get() || memory.get())
    misc::panic(
        "%s: program '%s' has already been loaded in a "
...
void Context::Execute() {
  ...
  if (regs.getCPSR().thumb != 0)
    buffer_ptr =
        memory->getBuffer((regs.getPC() - 2), 2, mem::Memory::AccessExec);
  else
    buffer_ptr =
        memory->getBuffer((regs.getPC() - 4), 4, mem::Memory::AccessExec);
    ...
    if (IsThumb32(buffer_ptr)) {
      ...
      if (inst.getThumb32Opcode() == Instruction::Thumb32OpcodeInvalid)
        throw misc::Panic(misc::fmt(
            "0x%x: not supported arm instruction (%02x %02x %02x %02x...)",
            (regs.getPC() - 4), buffer_ptr[0], buffer_ptr[1], buffer_ptr[2],
            buffer_ptr[3]));
    } else {
...
void Context::ExecuteInst() {
  ...
  if (regs.getCPSR().thumb != 0) {
    ...
    if (getInstType() == ContextInstTypeThumb16) {
      ...
      throw misc::Panic(
          misc::fmt("%d : ARM Wrong Instruction Type \n", getInstType()));
    }
  } else {
    // ARM mode
    current_ip = regs.getPC() - 4;
  ...
  if (emulator->isa_debug) {
    emulator->isa_debug << misc::fmt(
        "%d %8lld %x: ", pid, emulator->getNumInstructions(), current_ip);
    if (regs.getCPSR().thumb != 0) {
  ...
  switch (getInstType()) {
    ...
    case ContextInstTypeThumb16:
      ...
      if (inst.getThumb16Opcode()) {
        ExecuteInstThumb16Fn fn =
            execute_inst_thumb16_fn[inst.getThumb16Opcode()];
        (this->*fn)();
      }
      break;
    ...
    case ContextInstTypeThumb32:
      ...
      if (inst.getThumb32Opcode()) {
        ExecuteInstThumb32Fn fn =
            execute_inst_thumb32_fn[inst.getThumb32Opcode()];
        (this->*fn)();
      }
      break;
...
void Context::FinishGroup(int exit_code) {
  ...
  if (getState(ContextStateFinished) || getState(ContextStateZombie)) return;

  // Finish all contexts in the group
  for (auto& context : emulator->getContexts()) {
    if (context->group_parent != this && context.get() != this) continue;

    if (context.get() == this)
      context->setState(context->parent ? ContextStateZombie
                                        : ContextStateFinished);
    else
