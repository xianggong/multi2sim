...
Context::ExecuteInstThumb32Fn
    Context::execute_inst_thumb32_fn[Instruction::Thumb32OpcodeCount] = {
...
void Context::ExecuteInstThumb32_LDRD_immd() {
  ...
  if (inst.getThumb32Bytes()->ld_st_double.rn < 15) {
    ...
    if (wback) IsaRegStore(inst.getThumb32Bytes()->ld_st_double.rn, offset);
...
void Context::ExecuteInstThumb32_AND_reg9() {
  ...
  if (cat == Instruction::Thumb32CategoryDprShftreg) {
    ...
    shift = (inst.getThumb32Bytes()->data_proc_shftreg.imm3 << 2) |
            (inst.getThumb32Bytes()->data_proc_shftreg.imm2);
  }
...
void Context::ExecuteInstThumb32_RSB_imm() {
  ...
  IsaThumbRevSubtract(inst.getThumb32Bytes()->data_proc_immd.rd,
                      inst.getThumb32Bytes()->data_proc_immd.rn, immd, 0,
                      flags);
}
...
void Context::ExecuteInstThumb32_B_2() { IsaThumb32Branch(); }

void Context::ExecuteInstThumb32_B_21() { IsaThumb32Branch(); }
...
void Context::ExecuteInstThumb32_B_3() { IsaThumb32Branch(); }
...
void Context::ExecuteInstThumb32_BL_1() { IsaThumb32BranchLink(); }

void Context::ExecuteInstThumb32_BLX_0() {
  throw misc::Panic("Unimplemented instruction");
}

void Context::ExecuteInstThumb32_BLX_1() { IsaThumb32BranchLink(); }
...
void Context::ExecuteInstThumb32_STRHT_immd6() {
  ...
  emulator->isa_debug << misc::fmt(
      " immd12 = %x; offset = %x;  addr  = %d; (0x%x)\n", immd12, offset, addr,
      addr);
  if (wback) IsaRegStore(inst.getThumb32Bytes()->ldstr_imm.rn, offset);
...
void Context::ExecuteInstThumb32_STR_immdd() {
  ...
  IsaRegLoad(inst.getThumb32Bytes()->ldstr_imm.rd, rd_val);

  emulator->isa_debug << misc::fmt(" r%d (0x%x) => [0x%x]\n",
                                   inst.getThumb32Bytes()->ldstr_imm.rd, rd_val,
                                   addr);
...
void Context::ExecuteInstThumb32_LDRB_immA7140() {
  ...
  addr = offset;
  emulator->isa_debug << misc::fmt(
      " immd12 = 0x%x; offset = 0x%x;  addr  = %d; (0x%x)\n", immd12, offset,
      addr, addr);
