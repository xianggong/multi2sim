...
void Context::ExecuteInst_ADD_reg() {
  ...
  if (IsaCheckCond()) {
    operand2 = IsaGetOp2(inst.getBytes()->dpr.op2, ContextOp2CatecoryReg);
    IsaAdd(inst.getBytes()->dpr.dst_reg, inst.getBytes()->dpr.op1_reg, operand2,
           0);
  }
}
...
void Context::ExecuteInst_STRH_ptip1() {
  ...
  if (IsaCheckCond()) {
    ...
    value = value & (0x0000ffff);
    if (!(IsaInvalidAddrStr(addr, value))) memory->Write(addr, 2, (char*)buf);
  }
}
...
void Context::ExecuteInst_STRD_ofim() {
  ...
  if (IsaCheckCond()) {
    ...
    if (!(IsaInvalidAddrStr(addr, value))) memory->Write(addr, 4, (char*)buf);
...
void Context::ExecuteInst_SWI_SVC() { IsaSyscall(); }
