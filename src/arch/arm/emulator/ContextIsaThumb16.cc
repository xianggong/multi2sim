...
Context::ExecuteInstThumb16Fn
    Context::execute_inst_thumb16_fn[Instruction::Thumb16OpcodeCount] = {
...
void Context::ExecuteInstThumb16_STR_immd5() {
  ...
  IsaRegLoad(inst.getThumb16Bytes()->ldstr_immd_ins.reg_rd, rd_val);
  emulator->isa_debug << misc::fmt(
      " r%d (0x%x) => [0x%x]\n", inst.getThumb16Bytes()->ldstr_immd_ins.reg_rd,
      rd_val, addr);
...
void Context::ExecuteInstThumb16_STR_imm6() {
  ...
  IsaRegLoad(inst.getThumb16Bytes()->sp_immd_ins.reg_rd, rn_val);
  ...
  emulator->isa_debug << misc::fmt(" r%d (0x%x) => [0x%x]\n",
                                   inst.getThumb16Bytes()->sp_immd_ins.reg_rd,
                                   rn_val, addr);
  memory->Write(addr, 4, (char*)buf);
}
...
void Context::ExecuteInstThumb16_LDR_imm6() {
  ...
  emulator->isa_debug << misc::fmt(" r%d (0x%x) <= [0x%x]\n",
                                   ...
                                   rn_val, addr);
  memory->Read(addr, 4, (char*)buf);
  IsaRegStore(inst.getThumb16Bytes()->sp_immd_ins.reg_rd, rn_val);
}
...
void Context::ExecuteInstThumb16_SVC() { IsaSyscall(); }
...
void Context::ExecuteInstThumb16_CBZ_8() {
  ...
  if ((inst_addr + 2) % 4)
    immd5 = (inst_addr + 4) +
            ((inst.getThumb16Bytes()->cbnz_ins.i_ext << 6) | (immd5 << 1));
  else
    immd5 = (inst_addr + 4) +
            ((inst.getThumb16Bytes()->cbnz_ins.i_ext << 6) | (immd5 << 1));
  ...
  */
  emulator->isa_debug << misc::fmt(
      "  Branch addr = 0x%x, Before Branch pc <= 0x%x\n", immd5, regs.getPC());
...
void Context::ExecuteInstThumb16_IT_118() { IsaThumbIteq(); }
