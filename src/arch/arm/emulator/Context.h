...
enum ContextThumbIteq { ContextThumbIteqDisabled = 0, ContextThumbIteqEnabled };

enum ContextMode { ContextModeArm = 1, ContextModeThumb };
...
enum ContextOp2Catecory { ContextOp2CatecoryImmd = 0, ContextOp2CatecoryReg };
...
#undef DEFINST
  ...
  // Prototype of a member function of class Context devoted to the
  // execution of ISA thumb16 instructions. The emulator has a table indexed by
  // an
  // instruction identifier that points to all instruction emulation
  // functions.
...
#undef DEFINST
  ...
  // Table of functions
  static ExecuteInstThumb16Fn
      execute_inst_thumb16_fn[Instruction::Thumb16OpcodeCount];

  // Prototype of a member function of class Context devoted to the
  // execution of ISA thumb 32 instructions. The emulator has a table indexed by
  // an
  // instruction identifier that points to all instruction emulation
  // functions.
...
#undef DEFINST
  ...
  // Substract
  void IsaSubtract(unsigned int rd, unsigned int rn, int op2, unsigned int op3);
  ...
  // Add
  void IsaAdd(unsigned int rd, unsigned int rn, int op2, unsigned int op3);
  ...
  // Thumb add
  void IsaThumbAdd(unsigned int rd, unsigned int rn, int op2, unsigned int op3,
...
#undef DEFSYSCALL
 ...
 public:
  ...
  //    instruction next to the system call, and register '//FIXME: 2(?)' is set
  //    to
  //    \c -EINTR.
  ...
  void clearState(ContextState state) { UpdateState(this->state & ~state); }
