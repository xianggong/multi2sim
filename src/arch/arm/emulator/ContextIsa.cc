...
int Context::IsaOp2Carry(unsigned int op2, ContextOp2Catecory cat) {
  ...
  if (cat == ContextOp2CatecoryImmd) {
    ...
    emulator->isa_debug << misc::fmt(
        "  carry bit = %d, rm_val = 0x%x, rotate = %d\n", carry_ret, rm_val,
        rot_val);
  }
...
unsigned int Context::IsaGetAddrAmode2() {
  ...
  if (inst.getBytes()->sdtr.imm == 1) {
    ...
    if (inst.getBytes()->sdtr.idx_typ) {
      ...
      if (inst.getBytes()->sdtr.up_dn) {
        ...
        switch ((shift >> 1) & 0x00000003) {
          ...
          case Instruction::ShiftOperatorRor:
            ...
            ret_addr = rn_val + IsaRotr(rm_val, shift_val);
            break;
      ...
      if (inst.getBytes()->sdtr.wb) IsaRegStore(rn, ret_addr);
    } else {
      // Post Indexed
      if (inst.getBytes()->sdtr.up_dn) {
        // Increment
        switch ((shift >> 1) & 0x00000003) {
          ...
          case Instruction::ShiftOperatorRor:
            ...
            ret_addr = rn_val;
            rn_val = ret_addr + IsaRotr(rm_val, shift_val);
            break;
...
unsigned int Context::IsaGetAddrAmode3Imm() {
  ...
  if (inst.getBytes()->hfwrd_imm.idx_typ) {
    ...
    if (inst.getBytes()->hfwrd_imm.wb) {
      throw misc::Panic(
          misc::fmt("%s: Arm instruction not according to v7 ISA specification,"
...
bool Context::IsaCheckCond() {
  ...
  switch (cond) {
    ...
    case (Instruction::ConditionCodesGT):
      ...
      ret_val =
          (((regs.getCPSR().n) & (regs.getCPSR().v) & !(regs.getCPSR().z)) |
           (!(regs.getCPSR().n) & !(regs.getCPSR().v) & !(regs.getCPSR().z)))
              ? true
              : false;
    ...
    case (Instruction::ConditionCodesLE):
      ...
      ret_val =
          (((regs.getCPSR().z) | (!(regs.getCPSR().n) && (regs.getCPSR().v)) |
            ((regs.getCPSR().n) && !(regs.getCPSR().v))))
              ? true
              : false;
...
void Context::IsaCpsrPrint() {
  emulator->isa_debug << misc::fmt(
      ...
      "  q = %d\n"
      "  mode = 0x%x\n",
      regs.getCPSR().n, regs.getCPSR().z, regs.getCPSR().C, regs.getCPSR().v,
      regs.getCPSR().q, regs.getCPSR().mode);
}

unsigned int Context::IsaRetCpsrVal() {
  unsigned int cpsr_val =
      ((regs.getCPSR().n << 31) | (regs.getCPSR().z << 30) |
...
bool Context::IsaInvalidAddrLdr(unsigned int addr, unsigned int* value) {
  ...
  if (addr == 0x5bd4dc) {
    // Fault subroutine return address
    if (fault_addr == addr) *(value) = fault_value;
    return true;
  } else if (addr == 0x5bd4c8) {
    // Fault subroutine return address
    *(value) = 0x8fd6c;
...
void Context::IsaThumb32BranchLink() {
  ...
  if (cat == Instruction::Thumb32CategoryBranch) {
    addr = (inst.getThumb32Bytes()->branch_link.sign << 24) |
           ((!(inst.getThumb32Bytes()->branch.j1 ^
               inst.getThumb32Bytes()->branch_link.sign))
            << 23) |
           ((!(inst.getThumb32Bytes()->branch.j2 ^
               inst.getThumb32Bytes()->branch_link.sign))
            << 22) |
    ...
    emulator->isa_debug << misc::fmt(
        "  Inst_32 addr = 0x%x, Branch offset = 0x%x\n", inst.getAddress(),
        addr);
  ...
  emulator->isa_debug << misc::fmt(
      "  Branch addr_32 = 0x%x, Current pc <= 0x%x\n", addr, regs.getPC());
  IsaRegStore(14, regs.getPC() - 1);
  regs.setPC(addr + 2);
  emulator->isa_debug << misc::fmt(
      "  Branch addr_32 = 0x%x, Written pc <= 0x%x\n", addr, regs.getPC());
}
