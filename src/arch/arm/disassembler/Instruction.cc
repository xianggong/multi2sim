    ...
    if (this->dword.sdtr.up_dn) {
      switch ((shift >> 1) & 0x00000003) {
        case (ShiftOperatorLsl):
          os << misc::fmt("[r%d, r%d, lsl #%d]", rn, rm,
                          ((shift >> 3) & 0x0000001f));
          break;

        case (ShiftOperatorLsr):
          os << misc::fmt("[r%d, r%d, lsr #%d]", rn, rm,
                          ((shift >> 3) & 0x0000001f));
          break;

        case (ShiftOperatorAsr):
          os << misc::fmt("[r%d, r%d, asr #%d]", rn, rm,
                          ((shift >> 3) & 0x0000001f));
          break;

        case (ShiftOperatorRor):
          os << misc::fmt("[r%d, r%d, ror #%d]", rn, rm,
                          ((shift >> 3) & 0x0000001f));
          break;
      }
    } else {
  ...
  if (cat == Instruction::CategoryHfwrdReg) {
    ...
    if (this->dword.hfwrd_imm.idx_typ == 0 && this->dword.hfwrd_imm.wb == 1) {
      if (rm) {
        ...
        throw misc::Panic(
            misc::fmt("%d: amode 3 disasm fmt not recognized", cat));
      }
    }
    // Pre index or offset
    else {
...
void Instruction::DumpIdx(std::ostream& os) {
  ...
  if (idx == 1 && wb == 1) {
    if (this->dword.sdtr.off) os << misc::fmt("!");
  } else
    os << misc::fmt(" ");
}
...
void Instruction::DumpImmd16(std::ostream& os) {
  ...
  if (cat == Instruction::CategoryDprReg)
    ...
    immd16 = ((this->dword.dpr.op1_reg << 12) | this->dword.dpr.op2);
  else if (cat == Instruction::CategoryDprSat)
    throw misc::Panic(misc::fmt("%d: movt_movw fmt not recognized", cat));
  else if (cat == Instruction::CategoryPsr)
    throw misc::Panic(misc::fmt("%d: movt_movw fmt not recognized", cat));
...
void Instruction::DumpVfp1stm(std::ostream& os) {
  ...
  if (cat == Instruction::CategoryVfp) vfp1 = this->dword.vfp_mv.immd8;
  // TODO: destinations for BDTR CDTR CDO
  else
...
void Instruction::DumpVfpRegs(std::ostream& os) {
  ...
  if (cat == Instruction::CategoryVfp) {
    ...
    reg_start =
        ((this->dword.vfp_mv.d << 4) | (this->dword.vfp_mv.vd)) & (0x0000001f);
  }
  // TODO: destinations for BDTR CDTR CDO
  else
    throw misc::Panic(misc::fmt("%d: vfp regs fmt not recognized", cat));

  os << misc::fmt("{d%d-d%d}", reg_start, (reg_start + immd8 / 2 - 1));
}
...
void Instruction::DumpFreg(std::ostream& os) {
  ...
  if (cat == Instruction::CategoryCprDtr) freg = this->dword.cpr_dtr.cpr_sr_dst;
  // TODO: destinations for BDTR CDTR CDO
  else
...
void Instruction::DumpRt(std::ostream& os) {
  ...
  if (cat == Instruction::CategoryVfp) rt = this->dword.vfp_strreg_tr.vfp_rt;
  // TODO: destinations for BDTR CDTR CDO
  else
...
void Instruction::DumpHex(std::ostream& os, unsigned int* inst_ptr,
...
void Instruction::Thumb16DumpRD(std::ostream& os) {
  ...
  if (cat == Instruction::Thumb16CategoryMovshiftReg)
    ...
    rd = ((this->dword_16.high_oprs_ins.h1 << 3) |
          this->dword_16.high_oprs_ins.reg_rd);
...
void Instruction::Thumb16DumpHex(std::ostream& os, unsigned int* inst_ptr,
                                 unsigned int inst_addr) {
...
void Instruction::Thumb16Dump(std::ostream& os) {
  ...
  while (*fmt_str) {
    if (*fmt_str != '%') {
      if (*fmt_str != ' ') os << misc::fmt("%c", *fmt_str);
      ++fmt_str;
      continue;
    }
...
void Instruction::Thumb32DumpSHFTREG(std::ostream& os) {
  ...
  if (cat == Instruction::Thumb32CategoryDprShftreg) {
    ...
    shift = (this->dword_32.data_proc_shftreg.imm3 << 2) |
            (this->dword_32.data_proc_shftreg.imm2);
  }
...
void Instruction::Thumb32DumpIMMD8(std::ostream& os) {
  ...
  if (immd8) {
    if (this->dword_32.ld_st_double.add_sub) {
      if (this->dword_32.ld_st_double.index == 1 &&
          this->dword_32.ld_st_double.wback == 0)
        os << misc::fmt("#%d", (immd8));
      else if (this->dword_32.ld_st_double.index == 1 &&
               this->dword_32.ld_st_double.wback == 1)
        os << misc::fmt("#%d!", (immd8));
      else if (this->dword_32.ld_st_double.index == 0 &&
               this->dword_32.ld_st_double.wback == 0)
        os << misc::fmt("#%d", (immd8));
    } else {
      if (this->dword_32.ld_st_double.index == 1 &&
...
void Instruction::Thumb32DumpADDR(std::ostream& os) {
  ...
  if (cat == Instruction::Thumb32CategoryBranch) {
    addr = (this->dword_32.branch_link.sign << 24) |
           ((!(this->dword_32.branch.j1 ^ this->dword_32.branch_link.sign))
            << 23) |
           ((!(this->dword_32.branch.j2 ^ this->dword_32.branch_link.sign))
            << 22) |
...
void Instruction::Thumb32DumpHex(std::ostream& os, unsigned int* inst_ptr,
                                 unsigned int inst_addr) {
  int thumb_32;
  thumb_32 = *inst_ptr;
...
void Instruction::Thumb32InstTableDecode() {
  ...
  thumb32_table_arg = misc::getBits32(*(unsigned int*)this->dword_32.bytes,
  ...
  while (1) {
    if (current_table[thumb32_table_arg].next_table && loop_iteration < 8) {
      ...
      thumb32_table_arg =
          misc::getBits32(*(unsigned int*)this->dword_32.bytes,
                          current_table_high, current_table_low);
      loop_iteration++;
    } else if (loop_iteration > 8) {
      throw misc::Panic(misc::fmt(
          "Can not find the correct table containing the instruction\n"));
    } else
      break;
  }
...
void Instruction::Thumb16InstTableDecode() {
  ...
  thumb16_table_arg = misc::getBits16(*(unsigned short*)this->dword_16.bytes,
                                      current_table_high, current_table_low);
  ...
  while (1) {
    if (current_table[thumb16_table_arg].next_table && loop_iteration < 6) {
      ...
      thumb16_table_arg =
          misc::getBits16(*(unsigned short*)this->dword_16.bytes,
                          current_table_high, current_table_low);
      loop_iteration++;
...
void Instruction::Thumb32Decode(const char* buf, unsigned int ip) {
  ...
  for (byte_index = 0; byte_index < 4; ++byte_index)
    this->dword_32.bytes[byte_index] =
