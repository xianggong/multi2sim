...
#define DEFINST(_name, _fmt_str, _category, _arg1, _arg2)                  \
  SetupInstInfo(#_name, _fmt_str, Instruction::Category##_category, _arg1, \
                _arg2, Instruction::Opcode##_name);
...
#undef DEFINST
  ...
  dec_table_thumb32_asm_ldst_dual[(0x07)].next_table =
      dec_table_thumb32_asm_ldst1_dual;
  dec_table_thumb32_asm_ldst_dual[(0x07)].next_table_high = 19;
  dec_table_thumb32_asm_ldst_dual[(0x07)].next_table_low = 16;

  dec_table_thumb32_asm_ldst_dual[(0x08)].next_table =
      dec_table_thumb32_asm_ldst2_dual;
  dec_table_thumb32_asm_ldst_dual[(0x08)].next_table_high = 19;
  dec_table_thumb32_asm_ldst_dual[(0x08)].next_table_low = 16;

  dec_table_thumb32_asm_ldst_dual[(0x0d)].next_table =
      dec_table_thumb32_asm_ldst3_dual;
  ...
  dec_table_thumb32_dproc_bin_imm[0].next_table =
      dec_table_thumb32_dproc_bin_imm1;
  dec_table_thumb32_dproc_bin_imm[0].next_table_high = 19;
  dec_table_thumb32_dproc_bin_imm[0].next_table_low = 16;

  dec_table_thumb32_dproc_bin_imm[(0x0a)].next_table =
      dec_table_thumb32_dproc_bin_imm2;
  dec_table_thumb32_dproc_bin_imm[(0x0a)].next_table_high = 19;
  dec_table_thumb32_dproc_bin_imm[(0x0a)].next_table_low = 16;

  dec_table_thumb32_dproc_bin_imm[(0x16)].next_table =
      dec_table_thumb32_dproc_bin_imm3;
...
#define DEFINST(_name, _fmt_str, _cat, _op1, _op2, _op3, _op4, _op5, _op6, \
                _op7, _op8)                                                \
  Thumb32SetupTable(#_name, _fmt_str, Instruction::Thumb32Category##_cat,  \
                    _op1, _op2, _op3, _op4, _op5, _op6, _op7, _op8,        \
                    Instruction::Thumb32Opcode##_name);
...
#undef DEFINST
  ...
  dec_table_thumb16_spcl_data_brex[0].next_table =
      dec_table_thumb16_spcl_data_brex_lv1;
  ...
  dec_table_thumb16_asm_lv7[1].next_table =
      dec_table_thumb16_asm_lv9;  // entries [0 to 0xe] of lv9 are inst B //
  dec_table_thumb16_asm_lv7[1].next_table_high = 11;
  dec_table_thumb16_asm_lv7[1].next_table_low = 8;

#define DEFINST(_name, _fmt_str, _cat, _op1, _op2, _op3, _op4, _op5, _op6) \
  Thumb16SetupTable(#_name, _fmt_str, Instruction::Thumb16Category##_cat,  \
                    _op1, _op2, _op3, _op4, _op5, _op6,                    \
                    Instruction::Thumb16Opcode##_name);
...
void Disassembler::SetupInstInfo(const char* name, const char* fmt_str,
                                 Instruction::Category category, int arg1,
                                 int arg2, Instruction::Opcode inst_name) {
...
void Disassembler::Thumb32SetupTable(const char* name, const char* fmt_str,
                                     Instruction::Thumb32Category cat32,
...
void Disassembler::Thumb16SetupTable(const char* name, const char* fmt_str,
                                     Instruction::Thumb16Category cat16,
                                     int op1, int op2, int op3, int op4,
                                     int op5, int op6,
                                     Instruction::Thumb16Opcode inst_name) {
...
unsigned int Disassembler::ElfDumpWordSymbol(const ELFReader::File& file,
                                             std::ostream& os,
                                             unsigned int inst_addr,
...
unsigned int Disassembler::ElfDumpThumbWordSymbol(const ELFReader::File& file,
                                                  std::ostream& os,
                                                  unsigned int inst_addr,
                                                  unsigned int* inst_ptr) {
  ELFReader::Symbol* symbol;
...
void Disassembler::ElfSymbolListCreate(
    const ELFReader::File& file, std::vector<ELFReader::Symbol*>& symbol_list) {
  for (int i = 0; i < file.getNumSymbols(); i++) {
...
Disassembler::DisassemblyMode Disassembler::DissassembleMode(
    const std::vector<ELFReader::Symbol*>& symbol_list, unsigned int addr) {
  ...
  if (!symbol) return DisassemblyModeArm;
...
int Disassembler::TestThumb32(const char* inst_ptr) {
  ...
  for (byte_index = 0; byte_index < 4; ++byte_index)
    inst.getBytes()->bytes[byte_index] = *(inst_ptr + byte_index);
...
void Disassembler::DisassembleBinary(const std::string& path) {
  ...
  for (i = 0; i < file.getNumSections(); ++i) {
    ...
    if (!(section->getName().compare(0, 5, ".text"))) break;
  }
  ...
  for (inst_pos = 0; inst_pos < section->getSize();) {
    disasm_mode =
        DissassembleMode(symbol_list, (section->getAddr() + inst_index));

    if (disasm_mode == DisassemblyModeArm) {
      ElfSymbolFunc(file, std::cout, (section->getAddr() + inst_index),
                    disasm_mode);

      inst.DumpHex(std::cout, (unsigned int*)(section->getBuffer() + inst_pos),
                   (section->getAddr() + inst_index));

      if (!ElfDumpWordSymbol(
              file, std::cout, (section->getAddr() + inst_index),
              (unsigned int*)(section->getBuffer() + inst_pos))) {
        inst.Decode(section->getAddr() + inst_index,
                    section->getBuffer() + inst_pos);
        inst.Dump(std::cout);
      }
      ...
      if (TestThumb32(section->getBuffer() + inst_pos)) {
        ...
        inst.Thumb32DumpHex(std::cout,
                            (unsigned int*)(section->getBuffer() + inst_pos),
                            (section->getAddr() + inst_index));

        if (!ElfDumpThumbWordSymbol(
                file, std::cout, (section->getAddr() + inst_index),
                (unsigned int*)(section->getBuffer() + inst_pos))) {
        ...
        inst.Thumb16DumpHex(std::cout,
                            (unsigned int*)(section->getBuffer() + inst_pos),
                            (section->getAddr() + inst_index));
        if (!ElfDumpThumbWordSymbol(
                file, std::cout, (section->getAddr() + inst_index),
                (unsigned int*)(section->getBuffer() + inst_pos))) {
          inst.Thumb16Decode(section->getBuffer() + inst_pos,
                             section->getAddr() + inst_index);
