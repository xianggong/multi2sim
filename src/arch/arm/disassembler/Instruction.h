...
class Instruction {
 public:
  ...
  /// PSR register
  enum PsrRegisters { PsrRegistersCPSR = 0, PsrRegistersSPSR };
  ...
  enum Opcode {
    OpcodeInvalid = 0,
#define DEFINST(_name, _fmt_str, _category, _arg1, _arg2) Opcode##_name,
...
#undef DEFINST
 ...
 public:
  ...
  void DumpHex(std::ostream& os, unsigned int* inst_ptr,
  ...
  void Thumb16DumpHex(std::ostream& os, unsigned int* inst_ptr,
                      unsigned int inst_addr);
  ...
  void Thumb32DumpHex(std::ostream& os, unsigned int* inst_ptr,
                      unsigned int inst_addr);
  void Thumb32Dump(std::ostream& os);

  /// Populate the 'info' field of an instruction by decoding the instruction
  /// in 'buf'. The first 4 bytes of 'buf' are copied to field 'bytes'. The
  /// value
  ...
  Thumb16Opcode getThumb16Opcode() {
    return info_16 ? info_16->opcode : Thumb16OpcodeInvalid;
  }
  Thumb32Opcode getThumb32Opcode() {
    return info_32 ? info_32->opcode : Thumb32OpcodeInvalid;
  }
