...
ELFReader::File* CallStack::getELFFile(const std::string& path) {
  ...
  if (it != elf_file_map.end()) return it->second;
...
void CallStack::DumpCurrentLevel(std::ostream& os) {
  ...
  debug << misc::fmt("[%s] %s%s\n", path.c_str(), levels.c_str(), name.c_str());
}
...
void CallStack::Map(const std::string& binary_path, unsigned offset,
                    unsigned address, unsigned size, bool dynamic) {
  // Add map
  maps.emplace_back(binary_path, offset, address, size, dynamic);

  // Debug
  debug << misc::fmt("[%s] %s@0x%x mapped to %x-%x\n", path.c_str(),
                     binary_path.c_str(), offset, address, address + size - 1);
}
...
void CallStack::Call(unsigned ip, unsigned sp) {
  ...
  if (stack.size() > max_size) stack.pop_front();

  // Increase call level
  level++;

  // Debug
  if (debug) DumpCurrentLevel(debug);
}
...
void CallStack::BackTrace(unsigned address, std::ostream& os) {
  ...
  for (int i = 0; i < (int)stack.size(); i++) {
    unsigned ip = i == (int)stack.size() - 1 ? address : stack[i].getIp();
    std::string name = getSymbolName(ip);
    os << misc::fmt("\t%d. %s\n", i + 1, name.c_str());
  }
}
