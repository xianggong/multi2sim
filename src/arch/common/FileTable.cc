...
const misc::StringMap FileDescriptor::TypeTypeMap = {
    {"Regular", TypeRegular}, {"Standard", TypeStandard},
    {"Pipe", TypePipe},       {"Virtual", TypeVirtual},
    {"Device", TypeDevice},   {"Socket", TypeSocket}};
...
FileTable::FileTable() {
  // Add stdin
  FileDescriptor* desc =
      new FileDescriptor(FileDescriptor::TypeStandard, 0, 0, 0, "");
  descriptors.emplace_back(desc);

  // Add stdout
  desc = new FileDescriptor(FileDescriptor::TypeStandard, 1, 1, 0, "");
  descriptors.emplace_back(desc);

  // Add stderr
  desc = new FileDescriptor(FileDescriptor::TypeStandard, 2, 2, 0, "");
  descriptors.emplace_back(desc);
}
...
void FileTable::Dump(std::ostream& os) const {
  ...
  os << "\t" << descriptors.size() << " entries, " << occupied
     << " occupied\n\n";
}

int FileTable::getHostIndex(int guest_index) const {
  // Invalid index
  if (!misc::inRange(guest_index, 0, (int)descriptors.size())) return -1;
...
int FileTable::getGuestIndex(int host_index) const {
  ...
  for (auto& desc : descriptors) {
    if (desc.get() && desc->getHostIndex() == host_index) return guest_index;
    guest_index++;
  }
...
FileDescriptor* FileTable::newFileDescriptor(FileDescriptor::Type type,
                                             int guest_index, int host_index,
                                             const std::string& path,
                                             int flags) {
  // Look for a free entry
  for (int i = 0; i < (int)descriptors.size() && guest_index < 0; i++)
    if (!descriptors[i].get()) guest_index = i;
  ...
  FileDescriptor* desc =
      new FileDescriptor(type, guest_index, host_index, flags, path);
...
void FileTable::freeFileDescriptor(int index) {
  // Out of range
  if (!misc::inRange(index, 0, (int)descriptors.size() - 1)) return;
  ...
  if (desc->getType() == FileDescriptor::TypeVirtual) {
    if (unlink(desc->getPath().c_str()))
      misc::Warning(
          "%s: temporary host virtual file could not "
          "be deleted",
          desc->getPath().c_str());
  }
