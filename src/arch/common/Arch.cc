...
Arch* ArchPool::getByName(const std::string& name) {
  ...
  for (auto& arch : arch_list)
    if (!strcasecmp(arch->getName().c_str(), name.c_str())) return arch.get();
...
void ArchPool::Run(int& num_active_emulators,
  ...
  for (auto& arch : arch_list) {
    switch (arch->getSimKind()) {
      case Arch::SimFunctional: {
        ...
        if (active) num_active_emulators++;
...
const misc::StringMap Arch::SimKindMap = {{"functional", SimFunctional},
