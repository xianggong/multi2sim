...
void RuntimePool::Register(const std::string& name,
  ...
  runtimes.emplace_back(
      misc::new_unique<Runtime>(name, library_name, redirect_library_name));
}
...
bool RuntimePool::isLibraryMatch(const std::string& _file_name,
  ...
  if (!misc::StringPrefix(file_name, library_name)) return false;
  file_name.erase(0, library_name.length());

  /* Then, '.so' */
  if (!misc::StringPrefix(file_name, ".so")) return false;
  file_name.erase(0, 3);
  ...
  while (index < file_name.length()) {
    // Dot
    if (file_name[index] != '.') return false;
    index++;

    // One digit
    if (!isdigit(file_name[index])) return false;
    index++;

    /* More digits */
    while (index < file_name.length() && isdigit(file_name[index])) index++;
  }
...
std::string RuntimePool::getBuildTreeRoot() {
  ...
  if (index == std::string::npos || path.substr(index) != "/bin") return "";
  path.erase(index);
...
bool RuntimePool::Redirect(const std::string& library_name,
  ...
  if (library_name.empty() || library_name[0] != '/') return false;
  ...
  for (auto& runtime : runtimes) {
    ...
    bool matches_library = isLibraryMatch(base_name, runtime->getLibraryName());
    bool matches_redirect_library =
        isLibraryMatch(base_name, runtime->getRedirectLibraryName());

    if (!matches_library && !matches_redirect_library) continue;
    ...
    if (!access(path.c_str(), R_OK)) {
      ...
      misc::Warning(
          ...
          library_name.c_str(), redirect_library_name.c_str(),
          runtime->getName().c_str(), runtime_err_redirect_current);
    ...
    if (in_build_tree) {
      std::string path =
          misc::fmt("%s/lib/.libs/%s.so", build_tree_root.c_str(),
                    runtime->getRedirectLibraryName().c_str());
      if (!access(path.c_str(), R_OK)) {
        redirect_library_name = path;
        runtime->setOpenAttempt(false);
        misc::Warning(
            ...
            "runtime.\n%s",
            library_name.c_str(), redirect_library_name.c_str(),
            runtime->getName().c_str(), runtime_err_redirect_root);
