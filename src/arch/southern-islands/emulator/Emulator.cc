...
void Emulator::DumpSummary(std::ostream& os) const {
  ...
  os << "ScalarMemInstructions = " << num_scalar_memory_instructions
     << std::endl;
  os << "VectorALUInstructions = " << num_vector_alu_instructions << std::endl;
  os << "VectorMemInstructions = " << num_vector_memory_instructions
     << std::endl;
}
...
bool Emulator::Run() {
  ...
  if (!getNumNDRanges()) return false;
  ...
  for (auto it = getNDRangesBegin(), e = getNDRangesEnd(); it != e; ++it) {
    ...
    if (ndrange->isRunningWorkGroupsEmpty()) continue;
    ...
    while (!work_group->getFinished()) {
      ...
      for (auto wf_i = work_group->getWavefrontsBegin(),
        ...
        if (wavefront->getFinished() || wavefront->at_barrier) continue;
...
void Emulator::RegisterOptions() {
  ...
  command_line->RegisterString(
      "--si-debug-isa <file>", isa_debug_file,
  ...
  command_line->RegisterInt64(
      "--si-max-inst <inst>", max_instructions,
  ...
  command_line->RegisterString(
      "--si-debug-scheduler <file>", scheduler_debug_file,
...
NDRange* Emulator::getNDRangeById(unsigned id) {
  ...
  for (auto it = getNDRangesBegin(), e = getNDRangesEnd(); it != e; ++it) {
    ...
    // Check ID
    if ((unsigned)ndrange->getId() == id) return ndrange;
  }
