...
class Emulator : public comm::Emulator {
 ...
 public:
  ...
  static void createBufferDesc(unsigned base_addr, unsigned size, int num_elems,
  ...
  friend std::ostream& operator<<(std::ostream& os, const Emulator& emulator) {
    emulator.Dump(os);
    return os;
  }
  ...
  void incNDRangesRunning() {
    ...
    ndranges_running++;
    if (ndranges_running > 0) StartTimer();
  }
  ...
  void decNDRangesRunning() {
    ...
    ndranges_running--;
    assert(ndranges_running >= 0);
    if (ndranges_running == 0) StopTimer();
  }
