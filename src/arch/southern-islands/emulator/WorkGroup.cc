...
WorkGroup::WorkGroup(NDRange* ndrange, unsigned id) {
  ...
  unsigned work_items_per_group = ndrange->getLocalSize(0) *
                                  ndrange->getLocalSize(1) *
                                  ndrange->getLocalSize(2);
  assert(work_items_per_group > 0);

  // Number of wavefronts in work-group
  unsigned wavefronts_per_group =
      (work_items_per_group + (WorkGroup::WavefrontSize - 1)) /
  ...
  for (unsigned i = 0; i < wavefronts_per_group; ++i) {
    // Create wavefront
    wavefronts.emplace_back(
        misc::new_unique<Wavefront>(this, id * wavefronts_per_group + i));

    // Create work-items
    for (unsigned j = 0; j < WorkGroup::WavefrontSize; ++j) {
      unsigned work_item_id = i * wavefronts_per_group + j;
      work_items.emplace_back(
          misc::new_unique<WorkItem>(wavefronts[i].get(), work_item_id));
  ...
  id_3d[1] = (id / ndrange->getGroupCount(0)) % ndrange->getGroupCount(1);
  id_3d[2] = id / (ndrange->getGroupCount(0) * ndrange->getGroupCount(1));

  // Number of work-items in work-group
  this->work_item_count = ndrange->getLocalSize(2) * ndrange->getLocalSize(1) *
                          ndrange->getLocalSize(0);
    ...
    for (lidy = 0; lidy < ndrange->getLocalSize(1); lidy++) {
      for (lidx = 0; lidx < ndrange->getLocalSize(0); lidx++) {
        ...
        if (!wavefront->getWorkItemCount()) {
          assert((this->work_items.begin() + lid) < this->work_items.end());
          wavefront->setWorkItemsBegin(this->work_items.begin() + lid);
        }
        // Need a past-the-end iterator so add tid + 1
        assert(this->work_items.begin() + lid + 1 <= this->work_items.end());
        wavefront->setWorkItemsEnd(this->work_items.begin() + lid + 1);
        ...
        if (work_item->getIdInWavefront() < 32) {
          unsigned exec_mask =
              wavefront->getSregUint(Instruction::RegisterExec);
          ...
          unsigned exec_mask =
              wavefront->getSregUint(Instruction::RegisterExec + 1);
  ...
  for (auto wf_i = getWavefrontsBegin(), wf_e = getWavefrontsEnd();
    ...
    for (unsigned i = 0; i < user_element_count; i++) {
      ...
      if (user_element->dataClass == BinaryUserDataConstBuffer) {
        ...
        wavefront->setSRegWithConstantBuffer(user_element->startUserReg,
                                             user_element->userRegCount,
                                             user_element->apiSlot);
      } else if (user_element->dataClass == BinaryUserDataUAV) {
        // Store UAV pointer in sregs
        wavefront->setSRegWithUAV(user_element->startUserReg,
                                  user_element->userRegCount,
                                  user_element->apiSlot);
      } else if (user_element->dataClass == BinaryUserDataConstBufferTable) {
        // Store CB table in sregs
        wavefront->setSRegWithConstantBufferTable(user_element->startUserReg,
                                                  user_element->userRegCount);
      } else if (user_element->dataClass == BinaryUserDataUAVTable) {
        // Store UAV table in sregs
        wavefront->setSRegWithUAVTable(user_element->startUserReg,
                                       user_element->userRegCount);
      }
      ...
      // FIXME: SUB_PTR_FETCH_SHADER doesn't match binary
        ...
        assert(0);
      } else if (user_element->dataClass == BinaryUserDataPtrResourceTable) {
        // Store resource table in sregs
        assert(0);
      } else if (user_element->dataClass == BinaryUserDataInternalGlobalTable) {
        throw misc::Panic(
            "PTR_INTERNAL_GLOBAL_TABLE "
            "not supported");
      } else {
        throw misc::Panic(
            misc::fmt("Unimplemented "
