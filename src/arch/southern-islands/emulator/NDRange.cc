...
void NDRange::SetupSize(unsigned* global_size, unsigned* local_size,
  ...
  // Global work sizes
  for (int i = 0; i < work_dim; i++) global_size3[i] = global_size[i];

  this->global_size = global_size3[0] * global_size3[1] * global_size3[2];
  ...
  for (int i = 0; i < work_dim; i++) {
    ...
    if (local_size3[i] < 1)
      throw Emulator::Error(
          "Local work size must be greater "
          "than 0");
  }
  this->local_size = local_size3[0] * local_size3[1] * local_size3[2];

  // Check valid global/local sizes
  if (global_size3[0] < 1 || global_size3[1] < 1 || global_size3[2] < 1)
    throw Emulator::Error("Invalid global size");
  if (local_size3[0] < 1 || local_size3[1] < 1 || local_size3[2] < 1)
    throw Emulator::Error("Invalid local size");

  // Check divisibility of global by local sizes
  if ((global_size3[0] % local_size3[0]) ||
      (global_size3[1] % local_size3[1]) || (global_size3[2] % local_size3[2]))
    throw Emulator::Error(
        "The global work size must be a multiple "
        "of the local size");

  // Calculate number of groups
  for (int i = 0; i < 3; i++) {
    group_count3[i] = global_size3[i] / local_size3[i];
  }
  group_count = group_count3[0] * group_count3[1] * group_count3[2];
}

void NDRange::SetupInstructionMemory(const char* buf, unsigned size,
                                     unsigned pc) {
...
void NDRange::InitializeFromKernel(Kernel* kernel) {
  ...
  if (!text_size) throw Emulator::Error("Cannot load kernel code");
...
void NDRange::ConstantBufferWrite(int const_buffer_num, unsigned offset,
  ...
  // Calculate address
  unsigned addr =
      this->const_buf_table + const_buffer_num * ConstBufTableEntrySize;

  // Read in buffer description
  emulator->getGlobalMemory()->Read(
      addr, (unsigned)sizeof(WorkItem::BufferDescriptor),
...
void NDRange::InsertBufferIntoUAVTable(
    WorkItem::BufferDescriptor* buffer_descriptor, unsigned uav) {
...
void NDRange::InsertBufferIntoConstantBufferTable(
    WorkItem::BufferDescriptor* buffer_descriptor, unsigned const_buffer_num) {
  ...
  const_buf_table_entries[const_buffer_num].kind = TableEntryKindEmuBufferDesc;
  uav_table_entries[const_buffer_num].size = sizeof(*buffer_descriptor);
}

void NDRange::ImageIntoUAVTable(WorkItem::ImageDescriptor* image_descriptor,
                                unsigned uav) {
  assert(uav < MaxNumUAVs);
