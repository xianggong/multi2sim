...
class NDRange {
 ...
 public:
  ...
  friend std::ostream& operator<<(std::ostream& os, const NDRange& ndrange) {
    ndrange.Dump(os);
    return os;
  }
  ...
  // Get num_sgpr_used
  unsigned getNumSgprUsed() const { return num_sgpr_used; }
  ...
  unsigned getInstructionBufferSize() const { return instruction_buffer_size; }
  ...
  bool isWaitingWorkGroupsEmpty() const { return waiting_work_groups.empty(); }
  ...
  // Return an iterator to the past-to-end iterator in the running_work_group
  // list.
  ...
  void SetupInstructionMemory(const char* buf, unsigned size, unsigned pc);
  ...
  void ConstantBufferWrite(int const_buf_num, unsigned offset, void* pvalue,
                           unsigned size);
  ...
  /// \param size Size of data
  void ConstantBufferRead(int const_buf_num, unsigned offset, void* pvalue,
                          unsigned size);
