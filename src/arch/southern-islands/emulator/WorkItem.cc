...
void WorkItem::ISAUnimplemented(Instruction* inst) {
  throw misc::Panic(
      misc::fmt("%s: Unimplemented Southern Islands "
                "instruction\n"
                "\n\t"
...
unsigned WorkItem::ReadSReg(int sreg) { return wavefront->getSregUint(sreg); }

void WorkItem::WriteSReg(int sreg, unsigned value) {
...
void WorkItem::WriteVReg(int vreg, unsigned value) {
  assert(vreg >= 0);
  assert(vreg < 256);
...
void WorkItem::WriteBitmaskSReg(int sreg, unsigned value) {
  unsigned mask = 1;
  unsigned bitfield;
...
int WorkItem::ReadBitmaskSReg(int sreg) {
  ...
  if (id_in_wavefront < 32) {
    mask <<= id_in_wavefront;
    return (ReadSReg(sreg) & mask) >> id_in_wavefront;
  } else {
    mask <<= (id_in_wavefront - 32);
    return (ReadSReg(sreg + 1) & mask) >> (id_in_wavefront - 32);
  }
}

// Initialize a buffer resource descriptor
void WorkItem::ReadBufferResource(int sreg,
...
void WorkItem::ReadMemPtr(int sreg, WorkItem::MemoryPointer& mem_ptr) {
