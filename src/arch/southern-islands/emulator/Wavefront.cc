...
void Wavefront::setSregUint(int sreg, unsigned int value) {
  ...
  if (sreg == Instruction::RegisterVcc ||
      sreg == Instruction::RegisterVcc + 1) {
  ...
  if (sreg == Instruction::RegisterExec ||
      sreg == Instruction::RegisterExec + 1) {
...
void Wavefront::Execute() {
  ...
  switch (format) {
    ...
    case Instruction::FormatSOPP: {
      ...
      // Stats
      if (bytes->sopp.op > 1 && bytes->sopp.op < 10) {
    ...
    case Instruction::FormatVOP2: {
      ...
      for (auto it = work_items_begin, e = work_items_end; it != e; ++it) {
        work_item = (*it).get();
    ...
    case Instruction::FormatDS: {
      ...
      // Record access type
      if ((op >= 13 && op < 16) || (op >= 30 && op < 32) ||
          (op >= 77 && op < 80)) {
        lds_write = 1;
      } else if ((op >= 54 && op < 61) || (op >= 118 && op < 120)) {
        lds_read = 1;
      } else {
    ...
    case Instruction::FormatMUBUF: {
      ...
      // Record access type
      if ((op >= 0 && op < 4) || (op >= 8 && op < 15)) {
        vector_memory_read = true;
      } else if ((op >= 4 && op < 8) || (op >= 24 && op < 30)) {
        vector_memory_write = true;
      } else if (op == 50) {
        vector_memory_atomic = true;
      } else {
        throw misc::Panic(
            misc::fmt("Unsupported MUBUF "
                      "opcode (%d)",
                      op));
      }
    ...
    default:
      ...
      throw misc::Panic(misc::fmt("Unsupported instruction type (%d)", format));
  }
...
bool Wavefront::isWorkItemActive(int id_in_wavefront) {
  ...
  if (id_in_wavefront < 32) {
    ...
    return (sreg[Instruction::RegisterExec].as_uint & mask) >> id_in_wavefront;
  } else {
...
void Wavefront::setSRegWithConstantBuffer(int first_reg, int num_regs, int cb) {
  ...
  buf_desc_addr =
      ndrange->getConstBufferTableAddr() + cb * NDRange::ConstBufTableEntrySize;
...
void Wavefront::setSRegWithUAV(int first_reg, int num_regs, int uav) {
  ...
  buf_desc_addr = ndrange->getUAVTableAddr() + uav * NDRange::UAVTableEntrySize;
