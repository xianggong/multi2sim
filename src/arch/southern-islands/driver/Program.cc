...
#define __UNIMPLEMENTED__ \
  misc::fatal("%s: unimplemented function", __FUNCTION__);
...
void Program::InitializeConstantBuffers() {
  ...
  for (int i = 2; i < 25; i++) {
    ...
    Driver::debug << misc::fmt(
        "\tconstant buffer '%s' found with size %d and offset 0x%x\n",
        symbol->getName().c_str(), (unsigned)symbol->getSize(),
        (unsigned)symbol->getValue());

    // Create the constant buffer object and add it to the vector
    ConstantBuffer* constant_buffer = addConstantBuffer(i, symbol->getSize());

    // Set device pointer
    constant_buffer->device_address = emulator->getVideoMemoryTop();

    // Copy the symbol data into the constant buffer object
    symbol_stream.read(constant_buffer->getData(), symbol->getSize());

    // Map new pages
    video_memory->Map(emulator->getVideoMemoryTop(), symbol->getSize(),
                      mem::Memory::AccessRead | mem::Memory::AccessWrite);

    // Copy constant buffer into device memory
    video_memory->Write(emulator->getVideoMemoryTop(), symbol->getSize(),
...
ConstantBuffer* Program::getConstantBufferByIndex(int index) const {
  ...
  if (index >= 0 && index < 2)
    throw misc::Error(
        misc::fmt("Progam object does not hold"
