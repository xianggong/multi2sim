...
void Kernel::Expect(std::vector<std::string>& token_list,
  ...
  if (token != head_token)
    throw Driver::Error(misc::fmt(
        "Token '%s' expected, "
        "'%s' found.\n%s",
        head_token.c_str(), token_list[0].c_str(), OpenCLErrSIKernelMetadata));
}
...
void Kernel::ExpectInt(std::vector<std::string>& token_list) {
  ...
  if (err)
    throw Driver::Error(
        misc::fmt("Integer number expected, "
                  "'%s' found.\n%s",
                  token.c_str(), OpenCLErrSIKernelMetadata));
}
...
void Kernel::ExpectCount(std::vector<std::string>& token_list, unsigned count) {
  ...
  if (token_list.size() != count)
    throw Driver::Error(
        misc::fmt("%d tokens expected for '%s', "
                  "%d found.\n%s",
                  count, head_token.c_str(), (unsigned)token_list.size(),
...
void Kernel::LoadMetaDataV3() {
  ...
  for (;;) {
    ...
    if (token_list.front() == "value") {
      ...
      int data_type_int =
          Argument::data_type_map.MapString(*token_list.begin(), err);
      Argument::DataType data_type =
          static_cast<Argument::DataType>(data_type_int);
      if (err)
        throw Driver::Error(
            misc::fmt("Invalid data "
                      "type: %s.\n%s",
                      data_type_string, OpenCLErrSIKernelMetadata));
      ...
      std::unique_ptr<SI::Argument> arg(new SI::ValueArgument(
          name, data_type, num_elems, constant_buffer_num, constant_offset));

      // Debug
      Driver::debug << misc::fmt(
          "\targument '%s' - "
          "value stored in constant buffer %d at "
          "offset %d\n",
          name.c_str(), constant_buffer_num, constant_offset);
    ...
    if (token_list.front() == "pointer") {
      ...
      int arg_scope_int =
          Argument::scope_map.MapString(*token_list.begin(), err);
      Argument::Scope arg_scope = static_cast<Argument::Scope>(arg_scope_int);
      if (err)
        throw Driver::Error(
            misc::fmt("Invalid scope: "
                      "%s\n%s",
                      arg_scope_string, OpenCLErrSIKernelMetadata));
      ...
      int access_type_int =
          Argument::access_type_map.MapString(*token_list.begin(), err);
      Argument::AccessType access_type =
          static_cast<Argument::AccessType>(access_type_int);
      if (err)
        throw Driver::Error(
            misc::fmt("Invalid access "
                      "type: %s\n%s",
                      arg_access_type_string, OpenCLErrSIKernelMetadata));
      ...
      // Create argument object
      std::unique_ptr<SI::Argument> arg(new SI::PointerArgument(
          name, data_type, num_elems, constant_buffer_num, constant_offset,
          arg_scope, buffer_num, alignment, access_type));
    ...
    if (token_list.front() == "image") {
      ...
      const char* dimension_string =
          Argument::dimension_map.MapValue(dimension, err);
      if (err)
        throw Driver::Error(
            misc::fmt("Invalid image "
                      "dimensions: %s\n%s",
                      dimension_string, OpenCLErrSIKernelMetadata));
      ...
      std::unique_ptr<SI::Argument> arg(
          new SI::ImageArgument(name, dimension, access_type, uav,
                                constant_buffer_num, constant_offset));

      // Debug
      Driver::debug << misc::fmt(
          "\targument '%s' - Image stored in "
    ...
    if (token_list.front() == "sampler") {
      ...
      // Create argument object
      std::unique_ptr<SI::Argument> arg(
          new SI::SamplerArgument(name, id, location, value));
    ...
    throw Driver::Error(
        misc::fmt("Kernel %s: Unknown metadata "
                  "entry: %s",
...
void Kernel::LoadMetaData() {
  ...
  switch (version) {
    ...
    default:
      ...
      throw Driver::Error(
          misc::fmt("Unsupported metadata version "
                    "%d\n%s",
                    version, OpenCLErrSIKernelMetadata));
  }
}

void Kernel::CreateBufferDescriptor(
    unsigned base_addr, unsigned size, int num_elems,
    Argument::DataType data_type,
  ...
  switch (data_type) {
    ...
    default:
      ...
      throw Driver::Error(
          misc::fmt("Invalid data type for buffer "
                    "(%d)",
                    data_type));
  }
...
Kernel::Kernel(int id, const std::string& name, Program* program)
    : id(id), name(name), program(program) {
  ...
  if (!metadata_symbol || !header_symbol || !kernel_symbol)
    throw Driver::Error(
        misc::fmt("Invalid kernel function\n"
...
void Kernel::CreateNDRangeTables(NDRange* ndrange /* MMU *gpu_mmu */) {
  ...
  // Debug print out
  Driver::debug << misc::fmt(
      "\t%u bytes of device memory allocated at "
      "0x%x for SI internal tables\n",
      size_of_tables, emulator->getVideoMemoryTop());
...
void Kernel::SetupNDRangeArgs(NDRange* ndrange /* MMU *gpu_mmu */) {
    ...
    if (!(arg->set))
      throw Driver::Error(
          misc::fmt("Kernel '%s': "
                    ...
                    getName().c_str(), arg->name.c_str()));

    // Debug
    Driver::debug << misc::fmt("\targ[%d] = %s ", index, arg->name.c_str());

    // Process argument depending on its type
    switch (arg->getType()) {
      case Argument::TypeValue: {
        ValueArgument* arg_value = dynamic_cast<ValueArgument*>(arg.get());

        // Value copied directly into device constant memory
        assert(arg_value->size);
        ndrange->ConstantBufferWrite(arg_value->getConstantBufferNum(),
      ...
      case Argument::TypePointer: {
        ...
        switch (arg_ptr->getScope()) {
          ...
          case Argument::ScopeHwLocal:
            ...
            // space for it.
            ndrange->ConstantBufferWrite(arg_ptr->getConstantBufferNum(),
                                         arg_ptr->getConstantOffset(),
                                         ndrange->getLocalMemTopPtr(), 4);

            Driver::debug << misc::fmt("%u bytes at 0x%x", arg_ptr->size,
          ...
          case Argument::ScopeUAV: {
            ...
            CreateBufferDescriptor(arg_ptr->getDevicePtr(), arg_ptr->size,
                                   arg_ptr->getNumElems(),
                                   arg_ptr->getDataType(), &buffer_descriptor);

            // Add to UAV table
            ndrange->InsertBufferIntoUAVTable(&buffer_descriptor,
                                              arg_ptr->getBufferNum());

            // Write 0 to CB1
            ndrange->ConstantBufferWrite(arg_ptr->getConstantBufferNum(),
                                         arg_ptr->getConstantOffset(), &zero,
                                         4);
          ...
          case Argument::ScopeHwConstant: {
            ...
            ndrange->InsertBufferIntoConstantBufferTable(
                &buffer_descriptor, arg_ptr->getBufferNum());
      ...
      default:
        ...
        throw Driver::Error(
            misc::fmt("Invalid argument type "
                      "(%d)",
                      arg->getType()));
    }
  ...
  for (unsigned i = 2; i < NDRange::MaxNumConstBufs; i++) {
    ...
    ConstantBuffer* constant_buffer = program->getConstantBufferByIndex(i);
    ...
    WorkItem::BufferDescriptor buffer_descriptor;
    CreateBufferDescriptor(constant_buffer->device_address,
                           constant_buffer->getSize(), 4,
                           Argument::DataTypeFloat, &buffer_descriptor);
    ...
    // Add to Constant Buffer table
    ndrange->InsertBufferIntoConstantBufferTable(&buffer_descriptor, i);
  }
}
...
void Kernel::DebugNDRangeState(NDRange* ndrange) {
  ...
  Emulator::isa_debug << misc::fmt(
      "\t------------------------------------------------\n");
  Emulator::isa_debug << misc::fmt(
      "\t| Const Buffer table | [%10u:%10u] |\n",
      ndrange->getConstBufferTableAddr(),
      ndrange->getConstBufferTableAddr() + NDRange::ConstBufTableSize - 1);
  Emulator::isa_debug << misc::fmt(
      "\t| Resource table     | [%10u:%10u] |\n",
      ndrange->getResourceTableAddr(),
      ndrange->getResourceTableAddr() + NDRange::ResourceTableSize - 1);
  Emulator::isa_debug << misc::fmt(
  ...
  Emulator::isa_debug << misc::fmt(
      "Scalar register initialization prior to execution:\n");
  Emulator::isa_debug << misc::fmt(
      "\t-------------------------------------------\n");
  Emulator::isa_debug << misc::fmt(
    ...
    if (user_elements[i].dataClass == BinaryUserDataConstBuffer) {
      ...
      if (user_elements[i].userRegCount > 1) {
        Emulator::isa_debug << misc::fmt(
            ...
            user_elements[i].startUserReg + user_elements[i].userRegCount - 1,
            user_elements[i].apiSlot);
      } else {
        Emulator::isa_debug << misc::fmt(
            "\t| SREG[%2d]    |  CB%1d "
            "Descriptor         |\n",
            user_elements[i].startUserReg, user_elements[i].apiSlot);
      }
    }
      ...
      Emulator::isa_debug << misc::fmt(
          ...
          user_elements[i].startUserReg + user_elements[i].userRegCount - 1,
          user_elements[i].apiSlot);
    } else if (user_elements[i].dataClass == BinaryUserDataConstBufferTable) {
  ...
  for (int i = 0; i < NDRange::MaxNumConstBufs; i++) {
    ...
    if (!ndrange->getConstBuffer(i)->valid) continue;

    // Read a buffer description of the constant buffer
    video_memory->Read(ndrange->getConstBufferTableAddr() +
  ...
  for (int i = 0; i < NDRange::MaxNumUAVs; i++) {
    ...
    if (!ndrange->getUAV(i)->valid) continue;

    // Read a buffer description of the UAV
    video_memory->Read(
        ndrange->getUAVTableAddr() + i * NDRange::UAVTableEntrySize,
        sizeof(buffer_desc), (char*)&buffer_desc);
...
void Kernel::SetupNDRangeConstantBuffers(NDRange* ndrange) {
  ...
  // Constant buffer 0
  CreateBufferDescriptor(ndrange->getConstBufferAddr(0), NDRange::ConstBuf0Size,
                         1, Argument::DataTypeInt32, &buffer_descriptor);

  ndrange->InsertBufferIntoConstantBufferTable(&buffer_descriptor, 0);

  // Constant buffer 1
  CreateBufferDescriptor(ndrange->getConstBufferAddr(1), NDRange::ConstBuf1Size,
                         1, Argument::DataTypeInt32, &buffer_descriptor);
  ...
  ndrange->ConstantBufferWrite(0, 0, ndrange->getGlobalSizePtr(0), 4);
  ndrange->ConstantBufferWrite(0, 4, ndrange->getGlobalSizePtr(1), 4);
  ndrange->ConstantBufferWrite(0, 8, ndrange->getGlobalSizePtr(2), 4);
  ...
  ndrange->ConstantBufferWrite(0, 16, ndrange->getLocalSizePtr(0), 4);
  ndrange->ConstantBufferWrite(0, 20, ndrange->getLocalSizePtr(1), 4);
  ndrange->ConstantBufferWrite(0, 24, ndrange->getLocalSizePtr(2), 4);
  ...
  ndrange->ConstantBufferWrite(0, 32, ndrange->getGroupCountPtr(0), 4);
  ndrange->ConstantBufferWrite(0, 36, ndrange->getGroupCountPtr(1), 4);
  ndrange->ConstantBufferWrite(0, 40, ndrange->getGroupCountPtr(2), 4);
