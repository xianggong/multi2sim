...
Timing::Timing() : comm::Timing("SouthernIslands") {
  ...
  trace.Header(misc::fmt(
      "si.init version=\"%d.%d\" "
      "num_compute_units=%d\n",
      trace_version_major, trace_version_minor, gpu->num_compute_units));
...
void Timing::WriteMemoryConfiguration(misc::IniFile* ini_file) {
  ...
  for (int i = 0; i < (Gpu::num_compute_units + 3) / 4; i++) {
    ...
    ini_file->WriteString(section, "Geometry", "si-geo-scalar-l1");
    ini_file->WriteString(section, "LowNetwork", "si-net-l1-l2");
  ...
  for (int i = 0; i < Gpu::num_compute_units; i++) {
    ...
    ini_file->WriteString(section, "Type", "Cache");
    ini_file->WriteString(section, "Geometry", "si-geo-vector-l1");
    ini_file->WriteString(section, "LowNetwork", "si-net-l1-l2");
    ini_file->WriteString(section, "LowModules",
                          "si-l2-0 si-l2-1 si-l2-2 si-l2-3 si-l2-4 si-l2-5");
  }
  ...
  ini_file->WriteString(section, "AddressRange", "ADDR DIV 64 MOD 6 EQ 0");
  ...
  ini_file->WriteString(section, "AddressRange", "ADDR DIV 64 MOD 6 EQ 1");
  ...
  ini_file->WriteString(section, "AddressRange", "ADDR DIV 64 MOD 6 EQ 2");
  ...
  ini_file->WriteString(section, "AddressRange", "ADDR DIV 64 MOD 6 EQ 3");
  ...
  ini_file->WriteString(section, "AddressRange", "ADDR DIV 64 MOD 6 EQ 4");
  ...
  ini_file->WriteString(section, "AddressRange", "ADDR DIV 64 MOD 6 EQ 5");
...
void Timing::ParseMemoryConfigurationEntry(misc::IniFile* ini_file,
  ...
  if (!(unified_present ^ separate_present))
    throw Error(
        misc::fmt("%s: section [%s]: invalid combination "
  ...
  if (compute_unit_id < 0)
    throw Error(
        misc::fmt("%s: section [%s]: invalid or missing "
  ...
  if (compute_unit_id >= Gpu::num_compute_units) {
    misc::Warning(
        ...
        ini_file->getPath().c_str(), section.c_str(), compute_unit_id);
    return;
  }
  ...
  if (vector_cache_name.empty() || scalar_cache_name.empty())
    throw misc::Error(
        misc::fmt("%s: [%s]: invalid name for vector "
                  "or scalar cache",
                  ini_file->getPath().c_str(), section.c_str()));
...
void Timing::CheckMemoryConfiguration(misc::IniFile* ini_file) {
  ...
  for (auto it = gpu->getComputeUnitsBegin(), e = gpu->getComputeUnitsEnd();
       it != e; ++it) {
    ComputeUnit* compute_unit = it->get();
    if (!compute_unit->vector_cache)
      throw Error(
          misc::fmt("%s: Southern Islands compute "
                    ...
                    ini_file->getPath().c_str(), compute_unit->getIndex()));
  }
}
...
void Timing::RegisterOptions() {
  ...
  command_line->RegisterInt64(
      "--si-max-cycles <cycles>", Gpu::max_cycles,
  ...
  // Option --si-debug
  command_line->RegisterString(
      "--si-debug <file>", pipeline_debug_file,
...
void Timing::ProcessOptions() {
  ...
  if (!config_file.empty()) ini_file.Load(config_file);
...
void Timing::ParseConfiguration(misc::IniFile* ini_file) {
  ...
  Gpu::num_compute_units =
      ini_file->ReadInt(section, "NumComputeUnits", Gpu::num_compute_units);

  // Section [ComputeUnit]
  section = "ComputeUnit";
  ComputeUnit::num_wavefront_pools = ini_file->ReadInt(
      section, "NumWavefrontPools", ComputeUnit::num_wavefront_pools);
  ComputeUnit::max_work_groups_per_wavefront_pool =
      ini_file->ReadInt(section, "MaxWorkGroupsPerWavefrontPool",
                        ComputeUnit::max_work_groups_per_wavefront_pool);
  ComputeUnit::max_wavefronts_per_wavefront_pool =
      ini_file->ReadInt(section, "MaxWavefrontsPerWavefrontPool",
                        ComputeUnit::max_wavefronts_per_wavefront_pool);
  ComputeUnit::num_vector_registers = ini_file->ReadInt(
      section, "NumVectorRegisters", ComputeUnit::num_vector_registers);
  ComputeUnit::num_scalar_registers = ini_file->ReadInt(
      section, "NumScalarRegisters", ComputeUnit::num_scalar_registers);

  // Section [FrontEnd]
  section = "FrontEnd";
  ComputeUnit::fetch_latency =
      ini_file->ReadInt(section, "FetchLatency", ComputeUnit::fetch_latency);
  ComputeUnit::fetch_width =
      ini_file->ReadInt(section, "FetchWidth", ComputeUnit::fetch_width);
  ComputeUnit::fetch_buffer_size = ini_file->ReadInt(
      section, "FetchBufferSize", ComputeUnit::fetch_buffer_size);
  ComputeUnit::issue_latency =
      ini_file->ReadInt(section, "IssueLatency", ComputeUnit::issue_latency);
  ComputeUnit::issue_width =
      ini_file->ReadInt(section, "IssueWidth", ComputeUnit::issue_width);
  ComputeUnit::max_instructions_issued_per_type =
      ini_file->ReadInt(section, "MaxInstructionsIssuedPerType",
                        ComputeUnit::max_instructions_issued_per_type);

  // Section [SimdUnit]
  section = "SimdUnit";
  SimdUnit::num_simd_lanes =
      ini_file->ReadInt(section, "NumSIMDLanes", SimdUnit::num_simd_lanes);
  ...
  SimdUnit::read_exec_write_latency = ini_file->ReadInt(
      section, "ReadExecWriteLatency", SimdUnit::read_exec_write_latency);
  SimdUnit::read_exec_write_buffer_size =
      ini_file->ReadInt(section, "ReadExecWriteBufferSize",
                        SimdUnit::read_exec_write_buffer_size);

  // Section [ScalarUnit]
  section = "ScalarUnit";
  ScalarUnit::width = ini_file->ReadInt(section, "Width", ScalarUnit::width);
  ScalarUnit::issue_buffer_size = ini_file->ReadInt(
      section, "IssueBufferSize", ScalarUnit::issue_buffer_size);
  ScalarUnit::decode_latency =
      ini_file->ReadInt(section, "DecodeLatency", ScalarUnit::decode_latency);
  // TODO ScalarUnit::decode_width
  ScalarUnit::read_latency =
      ini_file->ReadInt(section, "ReadLatency", ScalarUnit::read_latency);
  ScalarUnit::read_buffer_size = ini_file->ReadInt(
      section, "ReadBufferSize", ScalarUnit::read_buffer_size);
  // TODO ScalarUnit::alu_latency
  ScalarUnit::exec_buffer_size = ini_file->ReadInt(
      section, "ExecBufferSize", ScalarUnit::exec_buffer_size);
  ScalarUnit::write_latency =
      ini_file->ReadInt(section, "WriteLatency", ScalarUnit::write_latency);
  ScalarUnit::write_buffer_size = ini_file->ReadInt(
      section, "write_buffer_size", ScalarUnit::write_buffer_size);

  // Section [BranchUnit]
  section = "BranchUnit";
  BranchUnit::width = ini_file->ReadInt(section, "Width", BranchUnit::width);
  BranchUnit::issue_buffer_size = ini_file->ReadInt(
      section, "IssueBufferSize", BranchUnit::issue_buffer_size);
  BranchUnit::decode_latency =
      ini_file->ReadInt(section, "DecodeLatency", BranchUnit::decode_latency);
  // TODO BranchUnit::decode_width
  BranchUnit::read_latency =
      ini_file->ReadInt(section, "ReadLatency", BranchUnit::read_latency);
  BranchUnit::read_buffer_size = ini_file->ReadInt(
      section, "ReadBufferSize", BranchUnit::read_buffer_size);
  BranchUnit::exec_latency =
      ini_file->ReadInt(section, "ExecLatency", BranchUnit::exec_latency);
  BranchUnit::exec_buffer_size = ini_file->ReadInt(
      section, "ExecBufferSize", BranchUnit::exec_buffer_size);
  BranchUnit::write_latency =
      ini_file->ReadInt(section, "WriteLatency", BranchUnit::write_latency);
  BranchUnit::write_buffer_size = ini_file->ReadInt(
      section, "WriteBufferSize", BranchUnit::write_buffer_size);

  // Section [LDSUnit]
  section = "LDSUnit";
  LdsUnit::width = ini_file->ReadInt(section, "Width", LdsUnit::width);
  LdsUnit::issue_buffer_size =
      ini_file->ReadInt(section, "IssueBufferSize", LdsUnit::issue_buffer_size);
  LdsUnit::decode_latency =
      ini_file->ReadInt(section, "DecodeLatency", LdsUnit::decode_latency);
  // TODO LdsUnit::decode_width
  LdsUnit::read_latency =
      ini_file->ReadInt(section, "ReadLatency", LdsUnit::read_latency);
  LdsUnit::read_buffer_size =
      ini_file->ReadInt(section, "ReadBufferSize", LdsUnit::read_buffer_size);
  LdsUnit::max_in_flight_mem_accesses = ini_file->ReadInt(
      section, "MaxInflightMem", LdsUnit::max_in_flight_mem_accesses);
  LdsUnit::write_latency =
      ini_file->ReadInt(section, "WriteLatency", LdsUnit::write_latency);
  ...
  VectorMemoryUnit::issue_buffer_size = ini_file->ReadInt(
      section, "IssueBufferSize", VectorMemoryUnit::issue_buffer_size);
  VectorMemoryUnit::decode_latency = ini_file->ReadInt(
      section, "DecodeLatency", VectorMemoryUnit::decode_latency);
  // TODO VectorMemoryUnit::decode_width
  VectorMemoryUnit::read_latency =
      ini_file->ReadInt(section, "ReadLatency", VectorMemoryUnit::read_latency);
  VectorMemoryUnit::read_buffer_size = ini_file->ReadInt(
      section, "ReadBufferSize", VectorMemoryUnit::read_buffer_size);
  VectorMemoryUnit::max_inflight_mem_accesses = ini_file->ReadInt(
      section, "MaxInflightMem", VectorMemoryUnit::max_inflight_mem_accesses);
  VectorMemoryUnit::write_latency = ini_file->ReadInt(
      section, "WriteLatency", VectorMemoryUnit::write_latency);
...
void Timing::DumpConfiguration(std::ofstream& os) const {
  ...
  os << misc::fmt("NumVectorRegisters = %d\n",
                  ComputeUnit::num_vector_registers);
  os << misc::fmt("NumScalarRegisters = %d\n",
                  ComputeUnit::num_scalar_registers);
...
void Timing::DumpReport() const {
  ...
  for (auto it = gpu->getComputeUnitsBegin(), e = gpu->getComputeUnitsEnd();
    ...
    long long coalesced_reads =
        compute_unit->getLdsModule()->num_coalesced_reads;
    long long coalesced_writes =
        compute_unit->getLdsModule()->num_coalesced_writes;
    instructions_per_cycle =
        getCycle() ? (double)(compute_unit->num_total_instructions / getCycle())
                   : 0.0;
...
bool Timing::Run() {
  ...
  if (!emulator->getNumNDRanges()) return false;
  ...
  for (auto it = emulator->getNDRangesBegin(); it != emulator->getNDRangesEnd();
       ++it) {
    ...
    if (ndrange->address_space == nullptr) {
      ...
      ndrange->address_space = gpu->getMmu()->newSpace("Southern Islands");
    ...
    if (!ndrange->isWaitingWorkGroupsEmpty()) {
      ...
      unsigned num_waiting_work_groups = ndrange->getNumWaitingWorkgroups();

      // Map work groups to compute units
      for (unsigned i = 0; i < num_waiting_work_groups; i++) {
        // Get an available compute unit
        ComputeUnit* available_compute_unit = gpu->getAvailableComputeUnit();
  ...
  if (esim_engine->hasFinished()) return true;
