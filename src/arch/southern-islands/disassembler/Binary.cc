...
#define SI_BIN_FILE_NOT_SUPPORTED(__var) \
  throw misc::Panic(misc::fmt(           \
      "Value 0x%x not supported for parameter '" #__var "'", (__var)))
#define SI_BIN_FILE_NOT_SUPPORTED_NEQ(__var, __val) \
  if ((__var) != (__val))                           \
  throw misc::Panic(                                \
      misc::fmt("Parameter '" #__var "' was expected to be 0x%x", (__val)))
...
misc::StringMap binary_machine_map = {
    {"R600", 0},    {"RV610", 1},      {"RV630", 2},     {"RV670", 3},
    {"R700", 4},    {"RV770", 5},      {"RV710", 6},     {"RV730", 7},
    {"Cypress", 8}, {"Juniper", 9},    {"Redwood", 10},  {"Cedar", 11},
    {"Sumo", 12},   {"Supersumo", 13}, {"Wrestler", 14}, {"Cayman", 15},
    {"Kauai", 16},  {"Barts", 17},     {"Turks", 18},    {"Caicos", 19}};

misc::StringMap binary_note_map = {{"ELF_NOTE_ATI_PROGINFO", 1},
...
void Binary::ReadNote(BinaryDictEntry* dict_entry, std::istringstream& ss,
  ...
  if (!ss) throw Disassembler::Error(name + ": Cannot decode note header");

  // Read note description (payload)
  const char* desc = buffer + ss.tellg();
  ss.seekg(header->descsz, std::ios_base::cur);
  if (!ss) throw Disassembler::Error(name + ": Cannot decode note description");

  // Debug
  const char* note_str = binary_note_map.MapValue(header->type);
  debug << "  note: type = " << header->type << " (" << note_str
        << "), descsz = " << header->descsz << '\n';
  ...
  switch (header->type) {
    case 1:  // ELF_NOTE_ATI_PROGINFO
    {
      ...
      for (i = 0; i < prog_info_count; i++) {
        prog_info_entry =
            (BinaryNoteProgInfoEntry*)(desc +
                                       i * sizeof(BinaryNoteProgInfoEntry));
        ...
        switch (prog_info_entry->address) {
          ...
          case 0x80001000:  // AMU_ABI_USER_ELEMENT_COUNT
            ...
            for (unsigned int j = 0; j < 4 * dict_entry->num_user_elements;
                 j++) {
              prog_info_entry =
                  (BinaryNoteProgInfoEntry*)(desc +
                                             i * sizeof(
                                                     BinaryNoteProgInfoEntry));
              ...
              switch (j % 4) {
                case 0:
                  dict_entry->user_elements[j / 4].dataClass =
                      prog_info_entry->value;
                  break;
                case 1:
                  dict_entry->user_elements[j / 4].apiSlot =
                      prog_info_entry->value;
                  break;
                case 2:
                  dict_entry->user_elements[j / 4].startUserReg =
                      prog_info_entry->value;
                  break;
                case 3:
                  dict_entry->user_elements[j / 4].userRegCount =
                      prog_info_entry->value;
                  break;
              }
    ...
    case 3:  // ELF_NOTE_ATI_OUTPUTS
    {
      ...
      if (header->descsz)
        misc::Warning(
            "pt_note '%s' with descsz != 0 ignored "
            "(desc value = 0x%x)",
            note_str, *(unsigned int*)desc);
    ...
    case 7:  // ELF_NOTE_ATI_BOOL32CONSTS
    {
      ...
      data_segment_desc_count =
          header->descsz / sizeof(BinaryNoteDataSegmentDesc);
      debug << "\tnote with data for constant buffers ("
            << data_segment_desc_count << " entries)\n";

      // Decode entries
      for (j = 0; j < data_segment_desc_count; j++) {
        data_segment_desc =
            (BinaryNoteDataSegmentDesc*)(desc +
                                         j * sizeof(BinaryNoteDataSegmentDesc));
        if (header->type == 5)
        ...
        debug << "\tdata_segment_desc[" << j << "]: offset = 0x" << std::hex
              << data_segment_desc->offset << std::dec
    ...
    case 10:  // ELF_NOTE_ATI_CONSTANT_BUFFERS
    {
      ...
      constant_buffer_count =
          header->descsz / sizeof(BinaryNoteConstantBufferMask);
      ...
      for (i = 0; i < constant_buffer_count; i++) {
        constant_buffer_mask =
            (BinaryNoteConstantBufferMask*)(desc +
                                            i * sizeof(
                                                    BinaryNoteConstantBufferMask));
    ...
    case 16:  // ELF_NOTE_ATI_UAV
    {
      ...
      for (i = 0; i < uav_entry_count; i++) {
        uav_entry =
            (BinaryNoteUAVEntry*)(desc + i * sizeof(BinaryNoteUAVEntry));
        debug << "\tuav_entry[" << i << "].uav = " << uav_entry->id << " ["
...
void Binary::ReadNotes(BinaryDictEntry* dict_entry) {
  ...
  while (ss.tellg() < ph->getSize()) ReadNote(dict_entry, ss, buffer);
}
...
void Binary::ReadDictionary() {
  ...
  for (int i = 0; i < getNumProgramHeaders(); i++) {
    ...
    if (ph->getType() == PT_LOPROC + 2) break;
    ph = NULL;
  }
  if (!ph) throw Disassembler::Error(name + ": No encoding dictionary");
  debug << "Encoding dictionary found in program header " << ph->getIndex()
        << "\n";
  ...
  for (int i = 0; i < num_dict_entries; i++) {
    ...
    if (dict_entry->header->d_machine == 9) {
      ...
      throw Disassembler::Error(
          misc::fmt("Unknown machine number "
                    "(%d)",
  ...
  for (unsigned i = 0; i < dict.size(); i++) {
    ...
    debug << "[" << i << "] "
          ...
          << "type = " << header->d_type << ", "
          << "offset = " << std::hex << header->d_offset << std::dec << ", "
          << "size = " << header->d_size << ", "
          << "flags = " << std::hex << header->d_flags << std::dec << '\n';
  }
}
...
void Binary::ReadSegments() {
  ...
  for (unsigned i = 0; i < dict.size(); i++) {
    ...
    debug << "  Dict. entry " << i << ": PT_NOTE segment: "
          << "offset = 0x" << std::hex
          << dict_entry->pt_note_segment->getOffset() << std::dec << ", "
          << "size = " << dict_entry->pt_note_segment->getSize() << '\n';
    debug << "  Dict. entry " << i << ": PT_LOAD segment: "
          << "offset = 0x" << std::hex
          << dict_entry->pt_load_segment->getOffset() << std::dec << ", "
          << "size = " << dict_entry->pt_load_segment->getSize() << '\n';
  }
}
  ...
  for (auto& dict_entry : dict) {
    ...
    for (int i = 0; i < getNumSections(); i++) {
      ...
      if (section->getOffset() < load_segment->getOffset() ||
          section->getOffset() >=
              load_segment->getOffset() + load_segment->getSize())
        continue;
    ...
    if (!dict_entry->text_section || !dict_entry->data_section ||
        !dict_entry->symtab_section || !dict_entry->strtab_section)
      throw Disassembler::Error(
          "One of these sections not found: "
...
Binary::~Binary() {
  ...
  for (auto& dict_entry : dict) delete dict_entry;
}
