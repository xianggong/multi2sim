...
const misc::StringMap Instruction::format_map = {
    {"<invalid>", FormatInvalid}, {"sop2", FormatSOP2},
    {"sopk", FormatSOPK},         {"sop1", FormatSOP1},
    {"sopc", FormatSOPC},         {"sopp", FormatSOPP},
    {"smrd", FormatSMRD},         {"vop2", FormatVOP2},
    {"vop1", FormatVOP1},         {"vopc", FormatVOPC},
    {"vop3a", FormatVOP3a},       {"vop3b", FormatVOP3b},
    {"vintrp", FormatVINTRP},     {"ds", FormatDS},
    {"mubuf", FormatMUBUF},       {"mtbuf", FormatMTBUF},
    {"mimg", FormatMIMG},         {"exp", FormatEXP}};

const std::map<enum Instruction::Format, std::string>
    Instruction::format_map_reverse = {
        {FormatInvalid, "<invalid>"}, {FormatSOP2, "sop2"},
        {FormatSOPK, "sopk"},         {FormatSOP1, "sop1"},
        {FormatSOPC, "sopc"},         {FormatSOPP, "sopp"},
        {FormatSMRD, "smrd"},         {FormatVOP2, "vop2"},
        {FormatVOP1, "vop1"},         {FormatVOPC, "vopc"},
        {FormatVOP3a, "vop3a"},       {FormatVOP3b, "vop3b"},
        {FormatVINTRP, "vintrp"},     {FormatDS, "ds"},
        {FormatMUBUF, "mubuf"},       {FormatMTBUF, "mtbuf"},
        {FormatMIMG, "mimg"},         {FormatEXP, "exp"}};

const misc::StringMap Instruction::sdst_map = {
    {"reserved", 0}, {"reserved", 1},  {"vcc_lo", 2},   {"vcc_hi", 3},
    {"tba_lo", 4},   {"tba_hi", 5},    {"tma_lo", 6},   {"tma_hi", 7},
    {"ttmp0", 8},    {"ttmp1", 9},     {"ttmp2", 10},   {"ttmp3", 11},
    {"ttmp4", 12},   {"ttmp5", 13},    {"ttmp6", 14},   {"ttmp7", 15},
    {"ttmp8", 16},   {"ttmp9", 17},    {"ttmp10", 18},  {"ttmp11", 19},
    {"m0", 20},      {"reserved", 21}, {"exec_lo", 22}, {"exec_hi", 23}};
...
const misc::StringMap Instruction::ssrc_map = {
    ...
    {"reserved", 10}, {"vccz", 11},
    {"execz", 12},    {"scc", 13},
    {"reserved", 14}, {"literal constant", 15}};
...
const misc::StringMap Instruction::op16_map = {
    {"f", 0},    {"lt", 1},   {"eq", 2},   {"le", 3},  {"gt", 4},   {"lg", 5},
    {"ge", 6},   {"o", 7},    {"u", 8},    {"nge", 9}, {"nlg", 10}, {"ngt", 11},
    {"nle", 12}, {"neq", 13}, {"nlt", 14}, {"tru", 15}};

const misc::StringMap Instruction::op8_map = {{"f", 0},  {"lt", 1}, {"eq", 2},
                                              {"le", 3}, {"gt", 4}, {"lg", 5},
                                              {"ge", 6}, {"tru", 7}};

const misc::StringMap Instruction::special_reg_map = {{"vcc", SpecialRegVcc},
                                                      {"scc", SpecialRegScc},
                                                      {"exec", SpecialRegExec},
                                                      {"tma", SpecialRegTma},
                                                      {"m0", SpecialRegM0}};
...
void Instruction::DumpOperandExp(std::ostream& os, int operand) {
  ...
  if (operand <= 7) {
    ...
    throw Disassembler::Error(misc::fmt("Operand code [%d] unused.", operand));
  } else if (operand <= 15) {
    /* EXP_POS */
...
void Instruction::DumpSeriesVdata(std::ostream& os, unsigned int vdata,
                                  int op) {
  int vdata_end;
...
void Instruction::DumpVop3Src(std::ostream& os, unsigned int src,
                              int neg) const {
  std::stringstream ss;
  ...
  if (!(misc::inRange(bytes.vop3a.op, 293, 298)) &&
    ...
    if ((bytes.vop3a.neg & neg) && (bytes.vop3a.abs & neg))
      os << "-abs(" << ss.str() << ")";
    else if ((bytes.vop3a.neg & neg) && !(bytes.vop3a.abs & neg))
      os << '-' << ss.str();
    else if (!(bytes.vop3a.neg & neg) && (bytes.vop3a.abs & neg))
      os << "abs(" << ss.str() << ')';
    else if (!(bytes.vop3a.neg & neg) && !(bytes.vop3a.abs & neg))
...
void Instruction::DumpVop364Src(std::ostream& os, unsigned int src,
                                int neg) const {
  std::stringstream ss;
...
void Instruction::DumpMaddr(std::ostream& os) const {
  /* soffset */
  assert(bytes.mtbuf.soffset <= 103 || bytes.mtbuf.soffset == 124 ||
         (bytes.mtbuf.soffset >= 128 && bytes.mtbuf.soffset <= 208));
  DumpScalar(os, bytes.mtbuf.soffset);

  /* offen */
  if (bytes.mtbuf.offen) os << " offen";

  /* index */
  if (bytes.mtbuf.idxen) os << " idxen";

  /* offset */
  if (bytes.mtbuf.offset) os << " offset:" << bytes.mtbuf.offset;
}
...
void Instruction::DumpDug(std::ostream& os) const {
  ...
  /* UNORM */
  if (bytes.mimg.unorm) os << " unorm";

  /* GLC */
  if (bytes.mimg.glc) os << " glc";
}
...
void Instruction::Dump(std::ostream& os) const {
  ...
  while (*fmt_str) {
    ...
    if (comm::Disassembler::isToken(fmt_str, "WAIT_CNT", token_len)) {
      ...
      os << misc::fmt("label_%04X", (address + (se_simm * 4) + 4) / 4);
    ...
    } else if (comm::Disassembler::isToken(fmt_str, "VOP3_64_SVDST",
                                           token_len)) {
    ...
    } else if (comm::Disassembler::isToken(fmt_str, "VOP3_64_VDST",
                                           token_len)) {
      DumpVectorSeries(os, bytes.vop3a.vdst, bytes.vop3a.vdst + 1);
    } else if (comm::Disassembler::isToken(fmt_str, "VOP3_64_SDST",
                                           token_len)) {
    ...
    } else if (comm::Disassembler::isToken(fmt_str, "VOP3_64_SRC0",
                                           token_len)) {
    ...
    } else if (comm::Disassembler::isToken(fmt_str, "VOP3_64_SRC1",
                                           token_len)) {
    ...
    } else if (comm::Disassembler::isToken(fmt_str, "VOP3_64_SRC2",
                                           token_len)) {
    ...
    } else if (comm::Disassembler::isToken(fmt_str, "SERIES_SBASE",
                                           token_len)) {
      /* The sbase field is missing the LSB,
       * so multiply by 2 */
      ...
      if (bytes.ds.offset0) os << "offset0:" << bytes.ds.offset0 << ' ';
    } else if (comm::Disassembler::isToken(fmt_str, "DS_SERIES_VDST",
                                           token_len)) {
      DumpVectorSeries(os, bytes.ds.vdst, bytes.ds.vdst + 1);
    } else if (comm::Disassembler::isToken(fmt_str, "OFFSET1", token_len)) {
      if (bytes.ds.offset1) os << "offset1:" << bytes.ds.offset1 << ' ';
    ...
    } else if (comm::Disassembler::isToken(fmt_str, "MU_SERIES_VDATA_DST",
                                           token_len) ||
               comm::Disassembler::isToken(fmt_str, "MU_SERIES_VDATA_SRC",
                                           token_len)) {
      DumpSeriesVdata(os, bytes.mubuf.vdata, bytes.mubuf.op);
    } else if (comm::Disassembler::isToken(fmt_str, "MU_GLC", token_len)) {
      if (bytes.mubuf.glc) os << "glc";
    } else if (comm::Disassembler::isToken(fmt_str, "VADDR", token_len)) {
      if (bytes.mtbuf.offen && bytes.mtbuf.idxen)
        DumpVectorSeries(os, bytes.mtbuf.vaddr, bytes.mtbuf.vaddr + 1);
      else
        DumpVector(os, bytes.mtbuf.vaddr);
    } else if (comm::Disassembler::isToken(fmt_str, "MU_MADDR", token_len)) {
      DumpMaddr(os);
    } else if (comm::Disassembler::isToken(fmt_str, "MT_SERIES_VDATA_DST",
                                           token_len) ||
               comm::Disassembler::isToken(fmt_str, "MT_SERIES_VDATA_SRC",
                                           token_len)) {
      DumpSeriesVdata(os, bytes.mtbuf.vdata, bytes.mtbuf.op);
    } else if (comm::Disassembler::isToken(fmt_str, "SERIES_SRSRC",
                                           token_len)) {
      ...
      DumpMaddr(os);
      os << " format:[" << buf_data_format_map.MapValue(bytes.mtbuf.dfmt) << ','
         << buf_num_format_map.MapValue(bytes.mtbuf.nfmt) << ']';
    } else if (comm::Disassembler::isToken(fmt_str, "MIMG_SERIES_VDATA_SRC",
                                           token_len) ||
               comm::Disassembler::isToken(fmt_str, "MIMG_SERIES_VDATA_DST",
                                           token_len)) {
      DumpVectorSeries(os, bytes.mimg.vdata, bytes.mimg.vdata + 3);
    } else if (comm::Disassembler::isToken(fmt_str, "MIMG_VADDR", token_len)) {
      DumpVectorSeries(os, bytes.mimg.vaddr, bytes.mimg.vaddr + 3);
    } else if (comm::Disassembler::isToken(fmt_str, "MIMG_SERIES_SRSRC",
                                           token_len)) {
      assert((bytes.mimg.srsrc << 2) % 4 == 0);
      DumpScalarSeries(os, bytes.mimg.srsrc << 2, (bytes.mimg.srsrc << 2) + 7);
    } else if (comm::Disassembler::isToken(fmt_str, "MIMG_DUG_SERIES_SRSRC",
                                           token_len)) {
      assert((bytes.mimg.srsrc << 2) % 4 == 0);
      DumpScalarSeries(os, bytes.mimg.srsrc << 2, (bytes.mimg.srsrc << 2) + 7);
      DumpDug(os);
    } else if (comm::Disassembler::isToken(fmt_str, "MIMG_SERIES_SSAMP",
                                           token_len)) {
      assert((bytes.mimg.ssamp << 2) % 4 == 0);
      DumpScalarSeries(os, bytes.mimg.ssamp << 2, (bytes.mimg.ssamp << 2) + 3);
      ...
      throw misc::Panic(misc::fmt("%s: token not recognized.", fmt_str));
    }
...
void Instruction::DumpAddress(std::ostream& os) const {
  // Spaces
  if (os.tellp() < 59) os << std::string(59 - os.tellp(), ' ');

  // Hex dump
  os << misc::fmt("// %08X: %08X", address, bytes.word[0]);
  if (size == 8) os << misc::fmt(" %08X", bytes.word[1]);
  os << '\n';
}
...
void Instruction::Decode(const char* buf, unsigned int address) {
  ...
  if (bytes.sopp.enc == 0x17F) {
    if (!disassembler->getDecTableSopp(bytes.sopp.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: SOPP:%d  "
                    "// %08X: %08X\n",
    ...
    if (!disassembler->getDecTableSopc(bytes.sopc.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: SOPC:%d  "
                    "// %08X: %08X\n",
    ...
    if (!disassembler->getDecTableSop1(bytes.sop1.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: SOP1:%d  "
                    "// %08X: %08X\n",
    ...
    if (!disassembler->getDecTableSopk(bytes.sopk.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: SOPK:%d  "
                    "// %08X: %08X\n",
    ...
    if (!disassembler->getDecTableSop2(bytes.sop2.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: SOP2:%d  "
                    "// %08X: %08X\n",
    ...
    if (!disassembler->getDecTableSmrd(bytes.smrd.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: SMRD:%d  "
                    "// %08X: %08X\n",
                    bytes.smrd.op, address, *(unsigned int*)buf));
    }
    ...
    if (!disassembler->getDecTableVop3(bytes.vop3a.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: VOP3:%d  "
                    "// %08X: %08X %08X\n",
                    bytes.vop3a.op, address, *(unsigned int*)buf,
                    *(unsigned int*)(buf + 4)));
    }
    ...
    if (!disassembler->getDecTableVopc(bytes.vopc.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: VOPC:%d  "
                    "// %08X: %08X\n",
    ...
    if (!disassembler->getDecTableVop1(bytes.vop1.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: VOP1:%d  "
                    "// %08X: %08X\n",
    ...
    if (!disassembler->getDecTableVop2(bytes.vop2.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: VOP2:%d  "
                    "// %08X: %08X\n",
    ...
    if (!disassembler->getDecTableVintrp(bytes.vintrp.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: VINTRP:%d  "
                    "// %08X: %08X\n",
    ...
    if (!disassembler->getDecTableDs(bytes.ds.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: DS:%d  "
                    "// %08X: %08X %08X\n",
                    bytes.ds.op, address, *(unsigned int*)buf,
                    *(unsigned int*)(buf + 4)));
    }
    ...
    if (!disassembler->getDecTableMtbuf(bytes.mtbuf.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: MTBUF:%d  "
                    "// %08X: %08X %08X\n",
                    bytes.mtbuf.op, address, *(unsigned int*)buf,
                    *(unsigned int*)(buf + 4)));
    }
    ...
    if (!disassembler->getDecTableMubuf(bytes.mubuf.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: MUBUF:%d  "
                    "// %08X: %08X %08X\n",
                    bytes.mubuf.op, address, *(unsigned int*)buf,
                    *(unsigned int*)(buf + 4)));
    }
    ...
    if (!disassembler->getDecTableMimg(bytes.mimg.op)) {
      throw misc::Panic(
          misc::fmt("Unimplemented Instruction: MIMG:%d  "
                    "// %08X: %08X %08X\n",
                    bytes.mimg.op, address, *(unsigned int*)buf,
                    *(unsigned int*)(buf + 4)));
    }
    ...
    throw misc::Panic(
        misc::fmt("Unimplemented format. Instruction is:  "
                  "// %08X: %08X\n",
