...
const misc::StringMap Argument::access_type_map = {{"RO", AccessTypeReadOnly},
                                                   {"WO", AccessTypeWriteOnly},
                                                   {"RW", AccessTypeReadWrite}};

// String map of the argument's scope
const misc::StringMap Argument::scope_map = {
    {"g", ScopeGlobal},   {"p", ScopeEmuPrivate},  {"l", ScopeEmuLocal},
    {"uav", ScopeUAV},    {"c", ScopeEmuConstant}, {"r", ScopeEmuGDS},
    {"hl", ScopeHwLocal}, {"hp", ScopeHwPrivate},  {"hc", ScopeHwConstant},
    {"hr", ScopeHwGDS}};

// String map of the argument's data type
const misc::StringMap Argument::data_type_map = {
    {"i1", DataTypeInt1},       {"i8", DataTypeInt8},
    {"i16", DataTypeInt16},     {"i32", DataTypeInt32},
    {"i64", DataTypeInt64},     {"u1", DataTypeUInt1},
    {"u8", DataTypeUInt8},      {"u16", DataTypeUInt16},
    {"u32", DataTypeUInt32},    {"u64", DataTypeUInt64},
    {"float", DataTypeFloat},   {"double", DataTypeDouble},
    {"struct", DataTypeStruct}, {"union", DataTypeUnion},
    {"event", DataTypeEvent},   {"opaque", DataTypeOpaque}};

// String map of the argument's dimension (2D or 3D)
const misc::StringMap Argument::dimension_map = {{"2D", 2}, {"3D", 3}};
...
const misc::StringMap Argument::reflection_map = {
    {"char", DataTypeInt8},     {"short", DataTypeInt16},
    {"int", DataTypeInt32},     {"long", DataTypeInt64},
    {"uchar", DataTypeUInt8},   {"ushort", DataTypeUInt16},
    {"uint", DataTypeUInt32},   {"ulong", DataTypeUInt64},
    {"float", DataTypeFloat},   {"double", DataTypeDouble},
    {"struct", DataTypeStruct}, {"union", DataTypeUnion},
    {"event", DataTypeEvent},   {"opaque", DataTypeOpaque}};
...
int Argument::getDataSize(DataType data_type) {
  switch (data_type) {
    ...
    default:
      ...
      throw misc::Panic(misc::fmt("Invalid data type (%d)", data_type));
  }
}

void PointerArgument::Dump(std::ostream& os) {
  os << data_type_map.MapValue(data_type);
  if (num_elems > 1) os << '[' << num_elems << ']';
  os << "* " << name;
}
...
void PointerArgument::WriteInfo(ELFWriter::Buffer* buffer, unsigned int index,
  ...
  line = ";pointer:" + name + ":" + data_type_str + ":1:" +
  ...
  if (((constant_offset - offset) < 16) && index)
    throw Disassembler::Error(
        misc::fmt("16 byte alignment not maintained "
                  "in argument: %s - Expected offset of %d or "
                  "higher",
                  name.c_str(), offset + 16));

  offset = constant_offset;

  /* Mark which uav's are being used */
  if (scope == ScopeUAV && !(uav[buffer_num])) {
    uav[buffer_num] = 1;
  }
...
void PointerArgument::WriteReflection(ELFWriter::Buffer* buffer,
  ...
  if (num_elems == 1) {
    line = ";reflection:" + std::to_string(index) + ":" + reflection + "*\n";
  } else if (num_elems > 1) {
    line = ";reflection:" + std::to_string(index) + ":" + reflection +
           std::to_string(num_elems) + "*\n";
  } else {
    throw Disassembler::Error("Invalid number of elements in argument " + name);
  }
...
void ValueArgument::WriteInfo(ELFWriter::Buffer* buffer, unsigned int index,
  ...
  line = ";value:" + name + ":" + data_type_str + ":" +
         std::to_string(num_elems) + ":" + std::to_string(constant_buffer_num) +
...
void ValueArgument::WriteReflection(ELFWriter::Buffer* buffer,
                                    unsigned int index) {
  std::string line;
  std::string reflection;
...
void ImageArgument::WriteReflection(ELFWriter::Buffer* buffer,
                                    unsigned int index) {
  throw misc::Panic("Not implemented");
}
...
void SamplerArgument::WriteReflection(ELFWriter::Buffer* buffer,
                                      unsigned int index) {
  throw misc::Panic("Not implemented");
}
