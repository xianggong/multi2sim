...
class AQLQueue {
  ...
  uint64_t IndexToAddress(unsigned long long index) {
    uint64_t address =
        fields->base_address + (index % fields->size) * sizeof(AQLPacket);
    return address;
  }
 ...
 public:
  ...
  /// Set queue type
  void setQueueType(uint32_t queue_type) { fields->queue_type = queue_type; }

  /// Get queue type
  uint32_t getQueueType() const { return fields->queue_type; }
  ...
  /// Get queue feature
  uint32_t getQueueFeature() { return fields->queue_features; }

  /// Get base address
  uint64_t getBaseAddress() const { return fields->base_address; }
  ...
  /// Get bell signal
  uint64_t getBellSignal() const { return fields->doorbell_signal; }
  ...
  /// Get service queue
  uint64_t getServiceQueue() const { return fields->service_queue; }
