...
WorkGroup::WorkGroup(Grid* grid, unsigned group_segment_size,
                     unsigned int group_id_x, unsigned int group_id_y,
...
void WorkGroup::Dump(std::ostream& os = std::cout) const {
  os << misc::fmt("  ***** %dD Work group (%d, %d, %d) ****\n",
                  getGrid()->getDimension(), getGroupIdX(), getGroupIdY(),
                  getGroupIdZ());
  os << misc::fmt("  ***** Group size (%d x %d x %d) *****\n",
                  getGrid()->getGroupSizeX(), getGrid()->getGroupSizeY(),
...
void WorkGroup::addWorkItem(std::unique_ptr<WorkItem> work_item) {
  ...
  unsigned int wavefront_id =
      workitem_flattened_id / getGrid()->getComponent()->getWavesize();

  // Check if wavefront is created
  auto it = wavefronts.find(wavefront_id);
  Wavefront* wavefront;
  if (it == wavefronts.end()) {
    wavefronts.insert(std::make_pair(
        wavefront_id, misc::new_unique<Wavefront>(wavefront_id, this)));
  ...
  if (local_id_x > current_group_size_x) current_group_size_x = local_id_x;
  if (local_id_y > current_group_size_y) current_group_size_y = local_id_y;
  if (local_id_z > current_group_size_z) current_group_size_z = local_id_z;
...
unsigned int WorkGroup::getGroupFlattenedId() {
  return group_id_x + group_id_y * grid->getGroupSizeX() +
         group_id_z * grid->getGroupSizeX() * grid->getGroupSizeZ();
}
