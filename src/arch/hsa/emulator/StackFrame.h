...
class StackFrame {
 ...
 public:
  ...
  SegmentManager* getArgumentSegment() const { return argument_segment.get(); }
  ...
  void addFunctionArguments(std::unique_ptr<Variable> argument) {
    function_arguments.emplace(argument->getName(), std::move(argument));
  }

  /// Add an variable to the the variable list
  void addVariable(std::unique_ptr<Variable> variable) {
    variables.emplace(variable->getName(), std::move(variable));
  }
  ...
  Variable* getArgument(const std::string& name) {
    ...
    if (it == argument_scope.end()) return nullptr;
    return it->second.get();
  }
  ...
  Variable* getFunctionArgument(const std::string& name) {
    ...
    if (it == function_arguments.end()) return nullptr;
    return it->second.get();
  }
