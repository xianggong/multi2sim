...
StackFrame::StackFrame(Function* function, WorkItem* work_item,
  ...
  register_storage = misc::new_unique_array<char>(function->getRegisterSize());
...
void StackFrame::StartArgumentScope(unsigned size) {
  ...
  argument_segment = misc::new_unique<SegmentManager>(memory, size);
};
...
void StackFrame::Dump(std::ostream& os = std::cout) const {
  ...
  os << "  ***** Registers *****\n";
  for (auto it = function->getRegisterBegin(); it != function->getRegisterEnd();
       it++) {
    os << "    ";
...
void StackFrame::DumpRegister(const std::string& name,
  ...
  switch (name[1]) {
...

    {
      unsigned int buf = 0;
      getRegisterValue(name, &buf);
      os << misc::fmt("%u, %d, %f, 0x%08x", *(unsigned int*)&buf, *(int*)&buf,
                      *(float*)&buf, *(unsigned int*)&buf);
...

    {
      ...
      getRegisterValue(name, &buf);
      os << misc::fmt("%llu, %lld, %f, 0x%016llx", *(unsigned long long*)&buf,
                      *(long long*)&buf, *(double*)&buf,
...

    {
      ...
      for (int i = 0; i < 16; i++) {
        ...
        os << misc::fmt("0x%x", *(this->register_storage.get() + offset + i));
      }
      break;
    }
    ...
    default:
      ...
      throw misc::Panic(misc::fmt("Unknown register name %s.", name.c_str()));
      break;
  }
  os << "\n";
}
