...
std::unique_ptr<Component> Component::getDefaultCPUComponent(
...
std::unique_ptr<Component> Component::getDefaultGPUComponent(
    unsigned long long handler) {
  // The component object allocated here with be cast into a unique_ptr
  // by the function who call this function. Therefore, no need to delete
  // it in this function
...
bool Component::Execute() {
  ...
  if (has_active_grid) return true;

  // 2. Otherwise, read from queue list and grab a task to start.
  for (auto it = queues.begin(); it != queues.end(); it++) {
    if (!(*it)->isEmpty()) {
      AQLDispatchPacket* packet = (*it)->ReadPacket();
      LaunchGrid(packet);
      return true;
    }
  }
...
void Component::LaunchGrid(AQLDispatchPacket* packet) {
  ...
  if (Emulator::aql_debug) grid->Dump(Emulator::aql_debug);
...
void Component::Dump(std::ostream& os = std::cout) const {
  ...
  if (agent_info.device_type == HSA_DEVICE_TYPE_GPU) deviceType = "GPU";
