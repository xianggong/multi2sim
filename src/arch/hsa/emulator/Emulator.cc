...
void Emulator::InstallComponents(const std::string& config_file = "") {
  if (config_file != "") {
    throw misc::Panic(
        "User defined component config file is "
        "not supported yet");
    return;
  }
...
bool Emulator::Run() {
  ...
  if (esim->hasFinished()) return false;
