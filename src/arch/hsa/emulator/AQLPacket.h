...
class AQLPacket {
 protected:
  ...
  Type getByOffset(unsigned short offset) const {
    if (!misc::inRange(offset, 0, 63))
      throw misc::Panic(
          misc::fmt("Trying to access invalid"
                    "index %d of an AQL packet",
                    offset));
 ...
 public:
  ...
  void setHeader(unsigned short header) {
    setByOffset<unsigned int>(2, header);
  }
};
...
class AQLDispatchPacket : public AQLPacket {
 public:
  ...
  /// Set the grid size y
  void setGridSizeY(unsigned int grid_size_y) {
    setByOffset<unsigned int>(16, grid_size_y);
  }
  ...
  /// Set the grid size z
  void setGridSizeZ(unsigned int grid_size_z) {
    setByOffset<unsigned int>(20, grid_size_z);
  }

  /// Return primary segment size in bytes per work-item
  unsigned int getPrivateSegmentSizeBytes() const {
    return getByOffset<unsigned int>(24);
  }

  /// Set primary segment size in bytes per work-item
  void setPrivateSegmentSizeBytes(unsigned int size) {
    setByOffset<unsigned int>(24, size);
  }

  /// Return group memory size in bytes per work-group
  unsigned int getGroupSegmentSizeBytes() const {
    return getByOffset<unsigned int>(28);
  }

  /// Set group memory size in bytes per work-group
  void setGroupSegmentSizeBytes(unsigned int size) {
    setByOffset<unsigned int>(28, size);
  }

  /// Return the address to kernel object
  unsigned long long getKernalObjectAddress() const {
    return getByOffset<unsigned long long>(32);
  }

  /// Set the address to kernel object
  void setKernalObjectAddress(unsigned long long address) {
    setByOffset<unsigned long long>(32, address);
  }

  /// Return the address to kernel arguments
  unsigned long long getKernargAddress() const {
    return getByOffset<unsigned long long>(40);
  }

  /// Set the address to kernel argument
  void setKernargAddress(unsigned long long address) {
    setByOffset<unsigned long long>(40, address);
  }
