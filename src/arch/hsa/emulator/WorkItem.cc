...
void WorkItem::Initialize(WorkGroup* work_group, unsigned private_segment_size,
  ...
  stack.push_back(misc::new_unique<StackFrame>(root_function, this, nullptr));
...
unsigned int WorkItem::getFlattenedId() const {
  return getLocalIdX() +
         ...
         getLocalIdZ() * work_group->getGrid()->getGroupSizeX() *
...

    {
      ...
      while (dir->getKind() != BRIG_KIND_DIRECTIVE_ARG_BLOCK_END) {
        if (dir->getKind() == BRIG_KIND_DIRECTIVE_VARIABLE) {
          size += AsmService::TypeToSize(dir->getType());
        }
        dir = dir->Next();
      }
...
void WorkItem::DeclareVariableGlobal(const std::string& name, BrigType type,
  ...
  // Create the variable
  auto variable = misc::new_unique<Variable>(name, type, dim, address,
                                             BRIG_SEGMENT_GLOBAL, false);
...
void WorkItem::DeclareVariableArgument(const std::string& name, BrigType type,
  ...
  if (!manager) throw misc::Panic("Argument scope not started.");
...
void WorkItem::DeclareVariable() {
  ...
  Emulator::isa_debug << misc::fmt("Create variable: %s %s(%d)[%lld]\n",
                                   ...
                                   name.c_str(), AsmService::TypeToSize(type),
                                   dim);
}
...
std::unique_ptr<HsaInstructionWorker> WorkItem::getInstructionWorker(
  ...
  switch (opcode) {
    ...
    case BRIG_OPCODE_ATOMICNORET:
      ...
      return misc::new_unique<AtomicNoRetInstructionWorker>(
          this, stack_top, Emulator::getInstance()->getMemory());
    ...
    case BRIG_OPCODE_BARRIER:
      ...
      return misc::new_unique<BarrierInstructionWorker>(this, stack_top);
    ...
    case BRIG_OPCODE_CURRENTWORKGROUPSIZE:
      ...
      return misc::new_unique<CurrentWorkGroupSizeInstructionWorker>(this,
                                                                     stack_top);
    ...
    case BRIG_OPCODE_GRIDSIZE:
      ...
      return misc::new_unique<GridSizeInstructionWorker>(this, stack_top);
    ...
    case BRIG_OPCODE_WORKITEMABSID:
      ...
      return misc::new_unique<WorkItemAbsIdInstructionWorker>(this, stack_top);

    case BRIG_OPCODE_WORKITEMID:

      return misc::new_unique<WorkItemIdInstructionWorker>(this, stack_top);

    case BRIG_OPCODE_WORKGROUPID:

      return misc::new_unique<WorkGroupIdInstructionWorker>(this, stack_top);
...
bool WorkItem::Execute() {
  ...
  if (status != WorkItemStatusActive) return true;
  ...
  if (inst && inst->isInstruction()) {
    ...
    if (getAbsoluteFlattenedId() == 0) {
      if (Emulator::isa_debug) stack_top->Dump(Emulator::isa_debug);
      Emulator::isa_debug << "\n";
    }
...
Grid* WorkItem::getGrid() const { return work_group->getGrid(); }
