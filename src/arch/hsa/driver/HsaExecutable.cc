...
HsaExecutable::HsaExecutable() : modules() {}
...
Function* HsaExecutable::getFunction(const std::string& name) const {
  ...
  if (it == function_table.end()) {
    throw Error(misc::fmt("Undefined reference to %s", name.c_str()));
  }
  return it->second.get();
}

void HsaExecutable::preprocessRegisters(
    BrigFile* binary, std::unique_ptr<BrigCodeEntry> first_entry,
    std::unique_ptr<BrigCodeEntry> next_module_entry, Function* function) {
  ...
  while (true) {
    ...
    if (entry.get() == nullptr)
      break;
             ...
             next_module_entry->getOffset() == entry->getOffset())
      break;
    ...
    for (unsigned int j = 0; j < entry->getOperandCount(); j++) {
      ...
      if (operand->getKind() != BRIG_KIND_OPERAND_REGISTER) continue;
  ...
  function->setLastEntry(
      std::move(binary->getCodeEntryByOffset(last_entry_offset)));
...
std::unique_ptr<BrigCodeEntry> HsaExecutable::loadArguments(
  ...
  for (int i = 0; i < num_arg; i++) {
    ...
    auto argument =
        misc::new_unique<Variable>(name, type, dim, 0, BRIG_SEGMENT_NONE, true);
    argument->setIndex(i);
...
void HsaExecutable::parseFunction(BrigFile* file,
  ...
  next_entry = loadArguments(file, num_out_arg, std::move(next_entry), false,
                             function.get());
  next_entry = loadArguments(file, num_in_arg, std::move(next_entry), true,
                             function.get());
  ...
  if (Emulator::loader_debug) function->Dump(Emulator::loader_debug);
...
HsaExecutableSymbol* HsaExecutable::getSymbol(const char* symbol_name) {
  ...
  HsaExecutableSymbol* symbol =
      new HsaExecutableSymbol(this, function->getFunctionDirective());
  return symbol;
}
