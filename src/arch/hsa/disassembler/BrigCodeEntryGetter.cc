...
bool BrigCodeEntry::isInstruction() const {
  if (getKind() >= BRIG_KIND_INST_BEGIN && getKind() < BRIG_KIND_INST_END)
    return true;
  return false;
}
...
unsigned int BrigCodeEntry::getHsailMajor() const {
  if (getKind() != BRIG_KIND_DIRECTIVE_MODULE) KindError("GetHsailMajor");
...
unsigned int BrigCodeEntry::getHsailMinor() const {
  if (getKind() != BRIG_KIND_DIRECTIVE_MODULE) KindError("GetHsailMinor");
  BrigDirectiveModule* module = (BrigDirectiveModule*)base;
  return module->hsailMinor;
}

BrigProfile BrigCodeEntry::getProfile() const {
  if (getKind() != BRIG_KIND_DIRECTIVE_MODULE) KindError("GetProfile");
  BrigDirectiveModule* module = (BrigDirectiveModule*)base;
  return (BrigProfile)module->profile;
}

BrigMachineModel BrigCodeEntry::getMachineModel() const {
  if (getKind() != BRIG_KIND_DIRECTIVE_MODULE) KindError("GetMachineModel");
  BrigDirectiveModule* module = (BrigDirectiveModule*)base;
...
bool BrigCodeEntry::isCallInst() const {
  if (!isInstruction()) throw misc::Panic("IsCallInst not valid for kind");
  if (getOpcode() == BRIG_OPCODE_CALL || getOpcode() == BRIG_OPCODE_SCALL ||
...
std::string BrigCodeEntry::getName() const {
  switch (getKind()) {
    ...
    {
      struct BrigDirectiveFbarrier* dir = (struct BrigDirectiveFbarrier*)base;
      return getBinary()->getStringByOffset(dir->name);
    }
    ...
    default:
      ...
      throw misc::Panic(
          misc::fmt("Name field are not valid "
                    "for entry 0x%x\n",
                    getKind()));
  }
  return "";
...
bool BrigCodeEntry::isDefinition() const {
  switch (getKind()) {
    case BRIG_KIND_DIRECTIVE_VARIABLE: {
      struct BrigDirectiveVariable* dir = (struct BrigDirectiveVariable*)base;
...
bool BrigCodeEntry::isConst() const {
  switch (getKind()) {
    ...
    case BRIG_KIND_INST_MEM: {
      struct BrigInstMem* dir = (struct BrigInstMem*)base;
      unsigned char modifier = (unsigned char)dir->modifier.allBits;
...
std::unique_ptr<BrigDataEntry> BrigCodeEntry::getOperands() const {
  if (getKind() >= BRIG_KIND_INST_BEGIN && getKind() < BRIG_KIND_INST_END) {
    ...
    struct BrigDirectivePragma* dir = (struct BrigDirectivePragma*)base;
...
std::unique_ptr<BrigOperandEntry> BrigCodeEntry::getOperand(
  ...
  if (isInstruction() || getKind() == BRIG_KIND_DIRECTIVE_PRAGMA) {
    ...
    unsigned int offset =
        *(const unsigned int*)(operands->getBytes() + index * 4);
...
BrigType BrigCodeEntry::getOperandType(unsigned char index) const {
  ...
  if (!isInstruction())
    throw misc::Panic(
        "GetOperandType is only valid for"
        " instruction");
...

    {
      if ((getSourceType() == BRIG_TYPE_B1 ||
        ...
        return BRIG_ROUND_FLOAT_DEFAULT;
                 ...
                 (getType() == BRIG_TYPE_F32 || getType() == BRIG_TYPE_F16)) {
        return BRIG_ROUND_FLOAT_DEFAULT;
      } else if (AsmService::isFloat(getSourceType()) &&
                 AsmService::isFloat(getType())) {
        return BRIG_ROUND_NONE;
...
BrigImageGeometry BrigCodeEntry::getGeometry() const {
  switch (getKind()) {
    ...
    case BRIG_KIND_INST_QUERY_IMAGE: {
      struct BrigInstQueryImage* inst = (struct BrigInstQueryImage*)base;
      return (BrigImageGeometry)inst->geometry;
    }
    default:
...
BrigSamplerQuery BrigCodeEntry::getSamplerQuery() const {
  switch (getKind()) {
    case BRIG_KIND_INST_QUERY_SAMPLER: {
      struct BrigInstQuerySampler* inst = (struct BrigInstQuerySampler*)base;
