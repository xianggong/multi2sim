...
std::map<unsigned, BrigOperandEntry::DumpEntryFn>
    BrigOperandEntry::dump_entry_fn = {
        ...
        {BRIG_KIND_OPERAND_CONSTANT_BYTES,
         &BrigOperandEntry::DumpOperandConstantBytes},
        {BRIG_KIND_OPERAND_CONSTANT_IMAGE,
         &BrigOperandEntry::DumpOperandConstantImage},
        {BRIG_KIND_OPERAND_CONSTANT_OPERAND_LIST,
         &BrigOperandEntry::DumpOperandConstantOperandList},
        {BRIG_KIND_OPERAND_CONSTANT_SAMPLER,
         &BrigOperandEntry::DumpOperandConstantSampler},
        {BRIG_KIND_OPERAND_OPERAND_LIST,
         &BrigOperandEntry::DumpOperandOperandList},
...
BrigOperandEntry::BrigOperandEntry(const char* buf) : BrigEntry(buf) {}
...
void BrigOperandEntry::DumpOperandConstantBytes(
    BrigType type = BRIG_TYPE_NONE, std::ostream& os = std::cout) const {
  auto immed = misc::new_unique<BrigImmed>(getBytes(), type);
  immed->Dump(os);
}

void BrigOperandEntry::DumpOperandConstantImage(
    BrigType type = BRIG_TYPE_NONE, std::ostream& os = std::cout) const {
  os << misc::fmt("Operand: %s, not supported\n", "CONSTANT_IMAGE");
}

void BrigOperandEntry::DumpOperandConstantOperandList(
    BrigType type = BRIG_TYPE_NONE, std::ostream& os = std::cout) const {
  os << misc::fmt("Operand: %s, not supported\n", "CONSTANT_OPERAND_LIST");
}

void BrigOperandEntry::DumpOperandConstantSampler(
    BrigType type = BRIG_TYPE_NONE, std::ostream& os = std::cout) const {
  os << misc::fmt("Operand: %s, not supported\n", "CONSTANT_SAMPLER");
}

void BrigOperandEntry::DumpOperandOperandList(
    BrigType type = BRIG_TYPE_NONE, std::ostream& os = std::cout) const {
  os << "(";
...
void BrigOperandEntry::DumpListOfOperand(BrigDataEntry* operands, BrigType type,
                                         std::ostream& os = std::cout) const {
...
BrigRegisterKind BrigOperandEntry::getRegKind() const {
  switch (getKind()) {
    case BRIG_KIND_OPERAND_REGISTER: {
      struct BrigOperandRegister* operand = (struct BrigOperandRegister*)base;
...
std::string BrigOperandEntry::getRegisterName() const {
  switch (getKind()) {
    case BRIG_KIND_OPERAND_REGISTER: {
      return AsmService::RegisterToString(getRegKind(), getRegNumber());
    }
    default:
...
std::string BrigOperandEntry::getString() const {
  switch (getKind()) {
    case BRIG_KIND_OPERAND_STRING: {
      struct BrigOperandString* operand = (struct BrigOperandString*)base;
...
std::unique_ptr<BrigCodeEntry> BrigOperandEntry::getRef() const {
  switch (getKind()) {
    case BRIG_KIND_OPERAND_CODE_REF: {
      struct BrigOperandCodeRef* operand = (struct BrigOperandCodeRef*)base;
...
std::unique_ptr<BrigDataEntry> BrigOperandEntry::getElements() const {
  switch (getKind()) {
    case BRIG_KIND_OPERAND_OPERAND_LIST: {
      ...
      auto data_entry = getBinary()->getDataEntryByOffset(operand->elements);
      return data_entry;
    }
    case BRIG_KIND_OPERAND_CODE_LIST: {
      struct BrigOperandCodeList* operand = (struct BrigOperandCodeList*)base;
      auto data_entry = getBinary()->getDataEntryByOffset(operand->elements);
      return data_entry;
    }
    default:
...
std::unique_ptr<BrigCodeEntry> BrigOperandEntry::getElement(
    unsigned int index) const {
  if (index >= getElementCount()) throw misc::Panic("GetElement out of range");
  auto data_entry = getElements();
  unsigned int offset = *(unsigned int*)(data_entry->getBytes() + index * 4);
  return getBinary()->getCodeEntryByOffset(offset);
}

std::unique_ptr<BrigOperandEntry> BrigOperandEntry::getOperandElement(
    unsigned int index) const {
  if (index >= getElementCount()) throw misc::Panic("GetElement out of range");
  auto data_entry = getElements();
  unsigned int offset = *(unsigned int*)(data_entry->getBytes() + index * 4);
