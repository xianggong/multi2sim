...
class BrigSection {
 ...
 public:
  ...
  std::unique_ptr<T> getFirstEntry() const {
    BrigSectionHeader* header = (BrigSectionHeader*)getBuffer();
  ...
  std::unique_ptr<T> getEntryByOffset(unsigned int offset) const {
    BrigSectionHeader* header = (BrigSectionHeader*)getBuffer();
    if (offset < header->headerByteCount) return std::unique_ptr<T>(nullptr);

    if (offset >= header->byteCount) return std::unique_ptr<T>(nullptr);
