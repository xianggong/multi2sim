...
void BrigFile::LoadFileByPath(const std::string& path) {
  // Open file
  std::ifstream f(path);
  if (!f) throw misc::Error(misc::fmt("Cannot open file %s.", path.c_str()));
  ...
  if (!isBrigFile(buffer.get())) {
    throw misc::Error(misc::fmt("%s is not a valid BRIG file", path.c_str()));
  }
...
void BrigFile::PrepareSections() {
  for (unsigned int i = 0; i < getNumSections(); i++) {
    ...
    unsigned long long* section_indices =
        (unsigned long long*)(buffer.get() + header->sectionIndex);

    // From the section index, get section offset
    unsigned long long section_offset = section_indices[i];

    // Create section
    auto section = misc::new_unique<BrigSection>(buffer.get() + section_offset);
...
std::unique_ptr<BrigOperandEntry> BrigFile::getOperandByOffset(
  ...
  std::unique_ptr<BrigOperandEntry> entry =
      operand_section->getEntryByOffset<BrigOperandEntry>(offset);
  return entry;
}
