...
const misc::StringMap Instruction::reg_map = {
    ...
    {"ah", RegAh},   {"ch", RegCh},   {"dh", RegDh},   {"bh", RegBh},
    {"es", RegEs},   {"cs", RegCs},   {"ss", RegSs},   {"ds", RegDs},
    {"fs", RegFs},   {"gs", RegGs}};

const Instruction::ModRMTableEntry Instruction::modrm_table[32] = {
    {RegEax, 0, 0},  {RegEcx, 0, 0},  {RegEdx, 0, 0},  {RegEbx, 0, 0},
    {RegNone, 0, 1}, {RegNone, 4, 0}, {RegEsi, 0, 0},  {RegEdi, 0, 0},

    {RegEax, 1, 0},  {RegEcx, 1, 0},  {RegEdx, 1, 0},  {RegEbx, 1, 0},
    {RegNone, 1, 1}, {RegEbp, 1, 0},  {RegEsi, 1, 0},  {RegEdi, 1, 0},

    {RegEax, 4, 0},  {RegEcx, 4, 0},  {RegEdx, 4, 0},  {RegEbx, 4, 0},
    {RegNone, 4, 1}, {RegEbp, 4, 0},  {RegEsi, 4, 0},  {RegEdi, 4, 0},

    {RegNone, 0, 0}, {RegNone, 0, 0}, {RegNone, 0, 0}, {RegNone, 0, 0},
    {RegNone, 0, 0}, {RegNone, 0, 0}, {RegNone, 0, 0}, {RegNone, 0, 0}};
...
void Instruction::DumpAddr(std::ostream& os) const {
  ...
  if (!ea_base && !ea_index) {
    if (segment_str.empty()) segment_str = "ds:";
    os << segment_str;
  ...
  if (disp < 0) os << misc::fmt("-0x%x", -disp);
  os << ']';
}
    ...
    while (*fmt && !isalnum(*fmt)) {
      if (*fmt == '_') {
        if (name_printed) {
          ...
          name_printed = true;
          for (int i = 0; i < 6 - name_length; i++) os << ' ';
          os << ' ';
        }
      } else {
        os << *fmt;
...
void Instruction::Decode(const char* buffer, unsigned eip) {
    ...
    switch ((unsigned char)*buffer) {
      ...
      default:
        ...
        throw Disassembler::Error(
            misc::fmt("Invalid prefix (0x%x)", (unsigned char)*buffer));
    }
  ...
  if (modrm_size) {
    ...
    if (sib_size) {
      ...
      ea_index = sib_index == 0x04 ? RegNone : (Reg)(RegEax + sib_index);
  ...
  size =
      prefix_size + opcode_size + modrm_size + sib_size + disp_size + imm_size;
}
