...
#define __UNIMPLEMENTED__ \
  IsaError("%s: unimplemented instruction", __FUNCTION__);
...
void Context::StoreXmmM128(const XmmValue& value) {
  ...
  if (inst.getModRmMod() == 3) {
    // Store into XMM
    regs.getXMM(inst.getModRmRm()).setAsUInt64(0, contentLo);
    regs.getXMM(inst.getModRmRm()).setAsUInt64(1, contentHi);
    return;
  }
  MemoryWrite(getEffectiveAddress(), 8, (void*)&contentLo);
  MemoryWrite(getEffectiveAddress() + 8, 8, (void*)&contentHi);
}
