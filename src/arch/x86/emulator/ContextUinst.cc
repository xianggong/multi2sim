...
int Context::getMemoryDepSize(Uinst* uinst, int index, Uinst::Dep& std_dep) {
  ...
  switch (dep) {
    ...
    case Uinst::DepRm32:
      ...
      if (inst.getModRmMod() == 3) return 0;
...
void Context::EmitUinstEffectiveAddress(Uinst* uinst, int index) {
  ...
  if (!getMemoryDepSize(uinst, index, mem_std_dep)) return;
...
void Context::ParseUinstDep(Uinst* uinst, int index) {
  ...
  assert(misc::inRange(index, 0, Uinst::MaxDeps - 1));
  Uinst::Dep dep = uinst->getDep(index);
  if (Uinst::isValidDependency(dep)) return;
...
void Context::newMemoryUinst(Uinst::Opcode opcode, unsigned address, int size,
                             int idep0, int idep1, int idep2, int odep0,
                             int odep1, int odep2, int odep3) {
  // Discard if we're in function simulation mode
  if (!uinst_active) return;
  ...
  for (int i = 0; i < Uinst::MaxIDeps; i++) ParseUinstIDep(uinst.get(), i);

  // Add micro-instruction to list
  uinsts.emplace_back(uinst);

  // Parse output dependences
  for (int i = 0; i < Uinst::MaxODeps; i++) ParseUinstODep(uinst.get(), i);
}
}
