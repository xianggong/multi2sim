...
#define __UNIMPLEMENTED__                                \
  throw misc::Panic(                                     \
      misc::fmt("%s: Unimplemented system call."         \
                "\n%s",                                  \
                __FUNCTION__ + 15, syscall_error_note)); \
  return 0;
...
const Context::ExecuteSyscallFn
    Context::execute_syscall_fn[SyscallCodeCount + 1] = {
...
void Context::ExecuteSyscall() {
  ...
  // Debug
  emulator->call_debug << misc::fmt(
      "[%s] System call '%s' "
      "(code %d, inst %lld)\n",
      getName().c_str(), Context::syscall_name[code], code,
  ...
  if (ret < 0 && ret >= -SIM_ERRNO_MAX)
    emulator->syscall_debug
        << misc::fmt(", errno = %s)", syscall_error_map.MapValue(-ret));
  emulator->syscall_debug << '\n';
}
...
int Context::ExecuteSyscall_restart_syscall() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_fork() { __UNIMPLEMENTED__ }
...
bool Context::SyscallReadCanWakeup() {
  ...
  if (host_thread_suspend_active) return false;

  // Context received a signal
  SignalSet pending_unblocked =
      signal_mask_table.getPending() & ~signal_mask_table.getBlocked();
  ...
  if (!desc) throw misc::Panic("Invalid file descriptor");
  ...
  if (err < 0) throw misc::Panic("Unexpected error in host call to 'poll'");
  ...
  if (host_fds.revents) {
    ...
    if (count < 0) throw misc::Panic("Unexpected error in host 'read'");
  ...
  if (pthread_create(&host_thread_suspend, nullptr, &Context::HostThreadSuspend,
                     this))
...
int Context::ExecuteSyscall_read() {
  ...
  if (err < 0) throw misc::Panic("Error executing 'poll'");
  ...
  if (fds.revents || (desc->getFlags() & O_NONBLOCK)) {
    ...
    if (err > 0) {
      ...
      emulator->syscall_debug << misc::StringBinaryBuffer(buf.get(), count, 40);
    }
...
bool Context::SyscallWriteCanWakeup() {
  ...
  int err = poll(&host_fds, 1, 0);
  if (err < 0) throw misc::Panic("Unexpected error in host 'poll'");
  ...
  if (host_fds.revents) {
    ...
    if (count < 0) throw misc::Panic("Unexpected error in host 'write'");
...
int Context::ExecuteSyscall_write() {
  ...
  emulator->syscall_debug << misc::fmt(
      "  blocking write - process suspended\n");
...
static misc::StringMap open_flags_map = {
    {"O_RDONLY", 00000000},    {"O_WRONLY", 00000001},
    {"O_RDWR", 00000002},      {"O_CREAT", 00000100},
    {"O_EXCL", 00000200},      {"O_NOCTTY", 00000400},
    {"O_TRUNC", 00001000},     {"O_APPEND", 00002000},
    {"O_NONBLOCK", 00004000},  {"O_SYNC", 00010000},
    {"FASYNC", 00020000},      {"O_DIRECT", 00040000},
    {"O_LARGEFILE", 00100000}, {"O_DIRECTORY", 00200000},
    {"O_NOFOLLOW", 00400000},  {"O_NOATIME", 01000000}};
...
comm::FileDescriptor *Context::SyscallOpenVirtualFile(const std::string &path,
  ...
  if (path == "/proc/self/maps") temp_path = OpenProcSelfMaps();
  ...
  comm::FileDescriptor *desc = file_table->newFileDescriptor(
      comm::FileDescriptor::TypeVirtual, host_fd, temp_path, flags);
  emulator->syscall_debug << misc::fmt(
      "    host file '%s' opened: "
      "guest_fd=%d, host_fd=%d\n",
      temp_path.c_str(), desc->getGuestIndex(), desc->getHostIndex());
  return desc;
}
...
comm::FileDescriptor *Context::SyscallOpenVirtualDevice(const std::string &path,
  ...
  if (!driver)
    throw Error(
        misc::fmt("%s: Cannot find device in %s", __FUNCTION__, path.c_str()));

  // Create new file descriptor
  comm::FileDescriptor *desc =
      file_table->newFileDescriptor(comm::FileDescriptor::TypeDevice,
  ...
  // Debug
  emulator->syscall_debug << misc::fmt(
      "    host device '%s' opened: "
      "guest_fd=%d, host_fd=%d\n",
      path.c_str(), desc->getGuestIndex(), desc->getHostIndex());
...
int Context::ExecuteSyscall_open() {
  ...
  if (driver_pool->isPathRegistered(full_path)) {
    ...
    comm::FileDescriptor *desc =
        SyscallOpenVirtualDevice(full_path, flags, mode);
  ...
  if (misc::StringPrefix(full_path, "/proc/")) {
    // Attempt to open virtual file
    comm::FileDescriptor *desc = SyscallOpenVirtualFile(full_path, flags, mode);
    if (desc) return desc->getGuestIndex();
  ...
  comm::FileDescriptor *desc = file_table->newFileDescriptor(
      comm::FileDescriptor::TypeRegular, host_fd, full_path, flags);
...
static const misc::StringMap waitpid_options_map = {
    {"WNOHANG", 0x00000001}, {"WUNTRACED", 0x00000002},
    {"WEXITED", 0x00000004}, {"WCONTINUED", 0x00000008},
    {"WNOWAIT", 0x01000000}, {"WNOTHREAD", 0x20000000},
    {"WALL", 0x40000000},    {"WCLONE", (int)0x80000000}};
...
bool Context::SyscallWaitpidCanWakeup() {
  ...
  if (child) {
    ...
    if (pstatus) memory->Write(pstatus, 4, (char *)&child->exit_code);
...
int Context::ExecuteSyscall_waitpid() {
  ...
  if (!child && !(options & 0x1)) {
    ...
    Suspend(&Context::SyscallWaitpidCanWakeup, &Context::SyscallWaitpidWakeup,
            StateWaitpid);
    return 0;
  }
  ...
  if (child) {
    if (status_ptr) memory->Write(status_ptr, 4, (char *)&child->exit_code);
    child->setState(StateFinished);
    return child->getId();
  }
...
int Context::ExecuteSyscall_creat() { __UNIMPLEMENTED__ }

//
// System call 'link'
//

int Context::ExecuteSyscall_link() { __UNIMPLEMENTED__ }
...
// A note
const char *execve_note =
    "A system call 'execve' is trying to run a command "
...
int Context::ExecuteSyscall_execve() {
  ...
  for (;;) {
    ...
    // Debug
    emulator->syscall_debug << misc::fmt(
        "    argv[%d] = '%s'\n", (int)arguments.size() - 1, argument.c_str());
  }
  ...
  // natively.
  if (full_path == "/bin/sh" && arguments.size() == 3 && arguments[0] == "sh" &&
      arguments[1] == "-c") {
    // Print note
    misc::Warning(
        "execve(): Child context executed natively.\n"
        "\t%s",
        execve_note);
...
int Context::ExecuteSyscall_time() {
  ...
  // Host call
  int t = time(NULL);
  if (time_ptr) memory->Write(time_ptr, 4, (char *)&t);
...
int Context::ExecuteSyscall_mknod() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_lchown16() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_17'
//

int Context::ExecuteSyscall_ni_syscall_17() { __UNIMPLEMENTED__ }

//
// System call 'stat'
//

int Context::ExecuteSyscall_stat() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_lseek() {
  ...
  Emulator::syscall_debug << misc::fmt(
      ...
      "offset=0x%x, "
      "origin=%d\n",
      guest_fd, offset, origin);

  // File descriptor
  comm::FileDescriptor *file_descriptor =
      file_table->getFileDescriptor(guest_fd);
  if (!file_descriptor) return -EBADF;
...
int Context::ExecuteSyscall_getpid() { return getId(); }

//
// System call 'mount'
//

int Context::ExecuteSyscall_mount() { __UNIMPLEMENTED__ }

//
// System call 'oldumount'
//

int Context::ExecuteSyscall_oldumount() { __UNIMPLEMENTED__ }

//
// System call 'setuid16'
//

int Context::ExecuteSyscall_setuid16() { __UNIMPLEMENTED__ }

//
// System call 'getuid16'
//

int Context::ExecuteSyscall_getuid16() { __UNIMPLEMENTED__ }

//
// System call 'stime'
//

int Context::ExecuteSyscall_stime() { __UNIMPLEMENTED__ }

//
// System call 'ptrace'
//

int Context::ExecuteSyscall_ptrace() { __UNIMPLEMENTED__ }

//
// System call 'alarm'
//

int Context::ExecuteSyscall_alarm() { __UNIMPLEMENTED__ }

//
// System call 'fstat'
//

int Context::ExecuteSyscall_fstat() { __UNIMPLEMENTED__ }

//
// System call 'pause'
//

int Context::ExecuteSyscall_pause() { __UNIMPLEMENTED__ }

//
// System call 'utime'
//

int Context::ExecuteSyscall_utime() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_31'
//

int Context::ExecuteSyscall_ni_syscall_31() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_32'
//

int Context::ExecuteSyscall_ni_syscall_32() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_access() {
  ...
  if (full_path == "/dev/multi2sim") return 0;
...
int Context::ExecuteSyscall_nice() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_35'
//

int Context::ExecuteSyscall_ni_syscall_35() { __UNIMPLEMENTED__ }

//
// System call 'sync'
//

int Context::ExecuteSyscall_sync() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_kill() {
  ...
  if (!context) throw Error(misc::fmt("%s: invalid pid %d", __FUNCTION__, pid));
...
int Context::ExecuteSyscall_rmdir() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_dup() {
  ...
  comm::FileDescriptor *dup_desc = file_table->newFileDescriptor(
      comm::FileDescriptor::TypeRegular, dup_host_fd, desc->getPath(),
      desc->getFlags());
...
int Context::ExecuteSyscall_ni_syscall_44() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_brk() {
  ...
  // Align
  unsigned new_heap_break_aligned =
      misc::RoundUp(new_heap_break, mem::Memory::PageSize);
  unsigned old_heap_break_aligned =
      misc::RoundUp(old_heap_break, mem::Memory::PageSize);
  ...
  if (new_heap_break < old_heap_break) {
    ...
    if (size) memory->Unmap(new_heap_break_aligned, size);
    memory->setHeapBreak(new_heap_break);
...
int Context::ExecuteSyscall_setgid16() { __UNIMPLEMENTED__ }

//
// System call 'getgid16'
//

int Context::ExecuteSyscall_getgid16() { __UNIMPLEMENTED__ }

//
// System call 'signal'
//

int Context::ExecuteSyscall_signal() { __UNIMPLEMENTED__ }

//
// System call 'geteuid16'
//

int Context::ExecuteSyscall_geteuid16() { __UNIMPLEMENTED__ }

//
// System call 'getegid16'
//

int Context::ExecuteSyscall_getegid16() { __UNIMPLEMENTED__ }

//
// System call 'acct'
//

int Context::ExecuteSyscall_acct() { __UNIMPLEMENTED__ }

//
// System call 'umount'
//

int Context::ExecuteSyscall_umount() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_53'
//

int Context::ExecuteSyscall_ni_syscall_53() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_fcntl() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_56'
//

int Context::ExecuteSyscall_ni_syscall_56() { __UNIMPLEMENTED__ }

//
// System call 'setpgid'
//

int Context::ExecuteSyscall_setpgid() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_58'
//

int Context::ExecuteSyscall_ni_syscall_58() { __UNIMPLEMENTED__ }

//
// System call 'olduname'
//

int Context::ExecuteSyscall_olduname() { __UNIMPLEMENTED__ }

//
// System call 'umask'
//

int Context::ExecuteSyscall_umask() { __UNIMPLEMENTED__ }

//
// System call 'chroot'
//

int Context::ExecuteSyscall_chroot() { __UNIMPLEMENTED__ }

//
// System call 'ustat'
//

int Context::ExecuteSyscall_ustat() { __UNIMPLEMENTED__ }

//
// System call 'dup2'
//

int Context::ExecuteSyscall_dup2() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_getpgrp() { __UNIMPLEMENTED__ }

//
// System call 'setsid'
//

int Context::ExecuteSyscall_setsid() { __UNIMPLEMENTED__ }

//
// System call 'sigaction'
//

int Context::ExecuteSyscall_sigaction() { __UNIMPLEMENTED__ }

//
// System call 'sgetmask'
//

int Context::ExecuteSyscall_sgetmask() { __UNIMPLEMENTED__ }

//
// System call 'ssetmask'
//

int Context::ExecuteSyscall_ssetmask() { __UNIMPLEMENTED__ }

//
// System call 'setreuid16'
//

int Context::ExecuteSyscall_setreuid16() { __UNIMPLEMENTED__ }

//
// System call 'setregid16'
//

int Context::ExecuteSyscall_setregid16() { __UNIMPLEMENTED__ }

//
// System call 'sigsuspend'
//

int Context::ExecuteSyscall_sigsuspend() { __UNIMPLEMENTED__ }

//
// System call 'sigpending'
//

int Context::ExecuteSyscall_sigpending() { __UNIMPLEMENTED__ }

//
// System call 'sethostname'
//

int Context::ExecuteSyscall_sethostname() { __UNIMPLEMENTED__ }
...
static const misc::StringMap rlimit_res_map = {
    {"RLIMIT_CPU", 0},       {"RLIMIT_FSIZE", 1},  {"RLIMIT_DATA", 2},
...
int Context::ExecuteSyscall_old_getrlimit() { __UNIMPLEMENTED__ }

//
// System call 'getrusage'
//

int Context::ExecuteSyscall_getrusage() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_settimeofday() { __UNIMPLEMENTED__ }

//
// System call 'getgroups16'
//

int Context::ExecuteSyscall_getgroups16() { __UNIMPLEMENTED__ }

//
// System call 'setgroups16'
//

int Context::ExecuteSyscall_setgroups16() { __UNIMPLEMENTED__ }

//
// System call 'oldselect'
//

int Context::ExecuteSyscall_oldselect() { __UNIMPLEMENTED__ }

//
// System call 'symlink'
//

int Context::ExecuteSyscall_symlink() { __UNIMPLEMENTED__ }

//
// System call 'lstat'
//

int Context::ExecuteSyscall_lstat() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_uselib() { __UNIMPLEMENTED__ }

//
// System call 'swapon'
//

int Context::ExecuteSyscall_swapon() { __UNIMPLEMENTED__ }

//
// System call 'reboot'
//

int Context::ExecuteSyscall_reboot() { __UNIMPLEMENTED__ }

//
// System call 'readdir'
//

int Context::ExecuteSyscall_readdir() { __UNIMPLEMENTED__ }
...
static const misc::StringMap mmap_prot_map = {{"PROT_READ", 0x1},
                                              {"PROT_WRITE", 0x2},
                                              {"PROT_EXEC", 0x4},
                                              {"PROT_SEM", 0x8},
...
static const misc::StringMap mmap_flags_map = {
    ...
    {"MAP_GROWSDOWN", 0x00100},  {"MAP_DENYWRITE", 0x00800},
    {"MAP_EXECUTABLE", 0x01000}, {"MAP_LOCKED", 0x02000},
    {"MAP_NORESERVE", 0x04000},  {"MAP_POPULATE", 0x08000},
    {"MAP_NONBLOCK", 0x10000}};

int Context::SyscallMmapAux(unsigned addr, unsigned len, int prot, int flags,
  ...
  if (guest_fd > 0 && host_fd < 0)
    throw Error(misc::fmt("mmap: Invalid guest descriptor (%d)", guest_fd));
  ...
  if (offset & ~mem::Memory::PageMask)
    throw Error(misc::fmt("mmap: Unaligned offset (0x%x)", offset));
  if (addr & ~mem::Memory::PageMask)
    throw Error(misc::fmt("mmap: Unaligned address (0x%x)", addr));
  ...
  if (flags & MAP_FIXED) {
    ...
    // allocated range.
    if (!addr) throw Error("mmap: No start specified for fixed mapping");
    ...
    if (addr == (unsigned)-1) throw Error("mmap: Out of guest memory");
  }
  ...
  if (host_fd >= 0) {
    ...
    for (int size = len_aligned; size > 0; size -= mem::Memory::PageSize) {
      ...
      if (count)
        memory->Access(curr_addr, mem::Memory::PageSize, buf,
    ...
    if (call_stack != nullptr && !desc->getPath().empty())
      call_stack->Map(desc->getPath(), offset, addr, len, true);
...
int Context::ExecuteSyscall_truncate() { __UNIMPLEMENTED__ }

//
// System call 'ftruncate'
//

int Context::ExecuteSyscall_ftruncate() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_getpriority() { __UNIMPLEMENTED__ }

//
// System call 'setpriority'
//

int Context::ExecuteSyscall_setpriority() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_98'
//

int Context::ExecuteSyscall_ni_syscall_98() { __UNIMPLEMENTED__ }

//
// System call 'statfs'
//

int Context::ExecuteSyscall_statfs() { __UNIMPLEMENTED__ }

//
// System call 'fstatfs'
//

int Context::ExecuteSyscall_fstatfs() { __UNIMPLEMENTED__ }

//
// System call 'ioperm'
//

int Context::ExecuteSyscall_ioperm() { __UNIMPLEMENTED__ }

//
// System call 'socketcall'
//

int Context::ExecuteSyscall_socketcall() { __UNIMPLEMENTED__ }

//
// System call 'syslog'
//

int Context::ExecuteSyscall_syslog() { __UNIMPLEMENTED__ }

//
// System call 'setitimer'
//

int Context::ExecuteSyscall_setitimer() { __UNIMPLEMENTED__ }

//
// System call 'getitimer'
//

int Context::ExecuteSyscall_getitimer() { __UNIMPLEMENTED__ }

//
// System call 'newstat'
//

int Context::ExecuteSyscall_newstat() { __UNIMPLEMENTED__ }

//
// System call 'newlstat'
//

int Context::ExecuteSyscall_newlstat() { __UNIMPLEMENTED__ }

//
// System call 'newfstat'
//

int Context::ExecuteSyscall_newfstat() { __UNIMPLEMENTED__ }

//
// System call 'uname'
//

int Context::ExecuteSyscall_uname() { __UNIMPLEMENTED__ }

//
// System call 'iopl'
//

int Context::ExecuteSyscall_iopl() { __UNIMPLEMENTED__ }

//
// System call 'vhangup'
//

int Context::ExecuteSyscall_vhangup() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_112'
//

int Context::ExecuteSyscall_ni_syscall_112() { __UNIMPLEMENTED__ }

//
// System call 'vm86old'
//

int Context::ExecuteSyscall_vm86old() { __UNIMPLEMENTED__ }

//
// System call 'wait4'
//

int Context::ExecuteSyscall_wait4() { __UNIMPLEMENTED__ }

//
// System call 'swapoff'
//

int Context::ExecuteSyscall_swapoff() { __UNIMPLEMENTED__ }

//
// System call 'sysinfo'
//

int Context::ExecuteSyscall_sysinfo() { __UNIMPLEMENTED__ }

//
// System call 'ipc'
//

int Context::ExecuteSyscall_ipc() { __UNIMPLEMENTED__ }

//
// System call 'fsync'
//

int Context::ExecuteSyscall_fsync() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_clone() {
  ...
  emulator->syscall_debug << misc::fmt("  exit_signal=%d (%s)\n", exit_signal,
                                       signal_map.MapValue(exit_signal));

  // New stack pointer defaults to current
  if (!new_esp) new_esp = regs.getEsp();
  ...
  if (flags & SIM_CLONE_SETTLS) {
    ...
    emulator->syscall_debug
        << misc::fmt("  entry_number=0x%x, base_addr=0x%x, limit=0x%x\n",
                     uinfo.entry_number, uinfo.base_addr, uinfo.limit);
    emulator->syscall_debug
        << misc::fmt("  seg_32bit=0x%x, contents=0x%x, read_exec_only=0x%x\n",
                     uinfo.seg_32bit, uinfo.contents, uinfo.read_exec_only);
    emulator->syscall_debug << misc::fmt(
        "  limit_in_pages=0x%x, seg_not_present=0x%x, useable=0x%x\n",
        uinfo.limit_in_pages, uinfo.seg_not_present, uinfo.useable);
    if (!uinfo.seg_32bit) throw misc::Panic("Only 32-bit segments supported");
...
int Context::ExecuteSyscall_setdomainname() { __UNIMPLEMENTED__ }
...
static simUtsName sim_utsname = {"Linux", "Multi2Sim",
                                 "3.1.9-1.fc16.i686"
...
int Context::ExecuteSyscall_modify_ldt() { __UNIMPLEMENTED__ }

//
// System call 'adjtimex'
//

int Context::ExecuteSyscall_adjtimex() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_sigprocmask() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_127'
//

int Context::ExecuteSyscall_ni_syscall_127() { __UNIMPLEMENTED__ }

//
// System call 'init_module'
//

int Context::ExecuteSyscall_init_module() { __UNIMPLEMENTED__ }

//
// System call 'delete_module'
//

int Context::ExecuteSyscall_delete_module() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_130'
//

int Context::ExecuteSyscall_ni_syscall_130() { __UNIMPLEMENTED__ }

//
// System call 'quotactl'
//

int Context::ExecuteSyscall_quotactl() { __UNIMPLEMENTED__ }

//
// System call 'getpgid'
//

int Context::ExecuteSyscall_getpgid() { __UNIMPLEMENTED__ }

//
// System call 'fchdir'
//

int Context::ExecuteSyscall_fchdir() { __UNIMPLEMENTED__ }

//
// System call 'bdflush'
//

int Context::ExecuteSyscall_bdflush() { __UNIMPLEMENTED__ }

//
// System call 'sysfs'
//

int Context::ExecuteSyscall_sysfs() { __UNIMPLEMENTED__ }

//
// System call 'personality'
//

int Context::ExecuteSyscall_personality() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_137'
//

int Context::ExecuteSyscall_ni_syscall_137() { __UNIMPLEMENTED__ }

//
// System call 'setfsuid16'
//

int Context::ExecuteSyscall_setfsuid16() { __UNIMPLEMENTED__ }

//
// System call 'setfsgid16'
//

int Context::ExecuteSyscall_setfsgid16() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_llseek() {
  ...
  if (result_ptr) memory->Write(result_ptr, 8, (char *)&offset);
...
int Context::ExecuteSyscall_getdents() { __UNIMPLEMENTED__ }

//
// System call 'select'
//

int Context::ExecuteSyscall_select() { __UNIMPLEMENTED__ }

//
// System call 'flock'
//

int Context::ExecuteSyscall_flock() { __UNIMPLEMENTED__ }
...
static const misc::StringMap msync_flags_map = {
    {"MS_ASYNC", 1}, {"MS_INAVLIAGE", 2}, {"MS_SYNC", 4}};
...
int Context::ExecuteSyscall_readv() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_getsid() { __UNIMPLEMENTED__ }

//
// System call 'fdatasync'
//

int Context::ExecuteSyscall_fdatasync() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_sysctl() {
  ...
  if (args.pnewval || args.newlen)
    throw misc::Panic(
        "Not supported for pnewval or newlen "
        "other than 0");
...
int Context::ExecuteSyscall_mlock() { __UNIMPLEMENTED__ }

//
// System call 'munlock'
//

int Context::ExecuteSyscall_munlock() { __UNIMPLEMENTED__ }

//
// System call 'mlockall'
//

int Context::ExecuteSyscall_mlockall() { __UNIMPLEMENTED__ }

//
// System call 'munlockall'
//

int Context::ExecuteSyscall_munlockall() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_sched_setparam() {
  ...
  if (pid != getId())
    throw misc::Panic(
        "Not support for pid different than "
        "current context");
  ...
  if (sched_priority < min_priority || sched_priority > max_priority)
    throw Error(
        misc::fmt("sched_setparam: invalid scheduling "
...
int Context::ExecuteSyscall_sched_getparam() {
  ...
  if (pid != getId())
    throw misc::Panic(
        "Not supported for pid other than "
        "current context");
...
int Context::ExecuteSyscall_sched_setscheduler() {
  ...
  switch (policy) {
    case SCHED_OTHER:
      ...
      if (priority != 0)
        throw Error(
            misc::fmt("%s: Invalid scheduler priority "
                      "(%d)\n%s",
                      __FUNCTION__, priority, syscall_error_note));
      break;
    ...
    case SCHED_FIFO:
      ...
      if (priority < 1 || priority > 99)
        throw Error(
            misc::fmt("%s: Invalid FIFO scheduler "
                      "priority (%d)\n%s",
                      __FUNCTION__, priority, syscall_error_note));
      break;
    ...
    default:
      ...
      throw misc::Panic(
          misc::fmt("Unsupported scheduling policy\n%s", syscall_error_note));
  }
...
int Context::ExecuteSyscall_sched_yield() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_sched_rr_get_interval() { __UNIMPLEMENTED__ }
...
bool Context::SyscallNanosleepCanWakeup() {
  ...
  if (syscall_nanosleep_wakeup_time <= now) {
    if (rmtp) memory->Write(rmtp, 8, (char *)&zero);
    regs.setEax(0);
    emulator->syscall_debug << misc::fmt(
        "[%s] Syscall 'nanosleep' - "
        "continue\n",
...
int Context::ExecuteSyscall_nanosleep() {
  ...
  Suspend(&Context::SyscallNanosleepCanWakeup, &Context::SyscallNanosleepWakeup,
          StateNanosleep);
  emulator->ProcessEventsSchedule();
...
int Context::ExecuteSyscall_mremap() {
  ...
  if (!(flags & 0x1)) throw Error("mremap: flag MAP_MAYMOVE must be present");
  if (!old_len || !new_len) throw Error("old_len or new_len cannot be zero");
  ...
  if (new_len > old_len &&
      memory->MapSpace(addr + old_len, new_len - old_len) == addr + old_len) {
  ...
  if (new_addr == (unsigned)-1) throw Error("mremap: Out of guest memory");
...
int Context::ExecuteSyscall_setresuid16() { __UNIMPLEMENTED__ }

//
// System call 'getresuid16'
//

int Context::ExecuteSyscall_getresuid16() { __UNIMPLEMENTED__ }

//
// System call 'vm86'
//

int Context::ExecuteSyscall_vm86() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_167'
//

int Context::ExecuteSyscall_ni_syscall_167() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_poll() {
  ...
  // Supported value
  if (nfds != 1) throw misc::Panic("Not suported for nfds != 1");
  ...
  emulator->syscall_debug << misc::fmt(
      "  guest_fd=%d, events=%s\n", guest_fd,
      poll_event_map.MapFlags(guest_fds.events).c_str());
  ...
  if (host_fd < 0) throw misc::Panic("Unsupported file descriptor");
  ...
  // occur.
  emulator->syscall_debug << misc::fmt(
      "  process going to sleep waiting for "
      "events on file\n");
...
int Context::ExecuteSyscall_nfsservctl() { __UNIMPLEMENTED__ }

//
// System call 'setresgid16'
//

int Context::ExecuteSyscall_setresgid16() { __UNIMPLEMENTED__ }

//
// System call 'getresgid16'
//

int Context::ExecuteSyscall_getresgid16() { __UNIMPLEMENTED__ }

//
// System call 'prctl'
//

int Context::ExecuteSyscall_prctl() { __UNIMPLEMENTED__ }

//
// System call 'rt_sigreturn'
//

int Context::ExecuteSyscall_rt_sigreturn() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_rt_sigaction() {
  ...
  // Invalid signal
  if (sig < 1 || sig > 64)
    throw Error(misc::fmt("rt_sigaction: Invalid signal (%d)", sig));
...
int Context::ExecuteSyscall_rt_sigprocmask() {
  ...
  if (set_ptr) {
    ...
    switch (how) {
      // SIG_BLOCK
      case 0:
        signal_mask_table.getBlocked().getBitmap() |= set.getBitmap();
        break;
  ...
  // Return old set
  if (old_set_ptr) memory->Write(old_set_ptr, 8, (char *)&old_set);
...
int Context::ExecuteSyscall_rt_sigpending() { __UNIMPLEMENTED__ }

//
// System call 'rt_sigtimedwait'
//

int Context::ExecuteSyscall_rt_sigtimedwait() { __UNIMPLEMENTED__ }

//
// System call 'rt_sigqueueinfo'
//

int Context::ExecuteSyscall_rt_sigqueueinfo() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_rt_sigsuspend() {
  ...
  Suspend(&Context::SyscallSigsuspendCanWakeup,
          &Context::SyscallSigsuspendWakeup, StateSigsuspend);
...
void Context::SyscallPread64Wakeup() {}

bool Context::SyscallPread64CanWakeup() {
  // If the host thread is still running for this context, do nothing.
  if (host_thread_suspend_active) return false;
  ...
  if (host_fds.revents) {
    ...
    int count = regs.getEdx();
    auto buf = misc::new_unique_array<char>(count);
    off_t offset = regs.getEsi();

    count = pread64(desc->getHostIndex(), buf.get(), count, offset);
    if (count < 0) throw misc::Panic("Unexpected error in host 'read'");
...
int Context::ExecuteSyscall_pread64() {
  ...
  unsigned buf_ptr = regs.getEcx();
  unsigned count = regs.getEdx();
  unsigned offset64 = regs.getEsi();
  emulator->syscall_debug << misc::fmt(
      "  guest_fd=%d, buf_ptr=0x%x, count=0x%x, offset64=0x%x\n", guest_fd,
      buf_ptr, count, offset64);
  ...
  if (fds.revents || (desc->getFlags() & O_NONBLOCK)) {
    // Host system call
    err = pread64(host_fd, buf.get(), count, offset64);
    if (err == -1) return -errno;
  ...
  emulator->syscall_debug << misc::fmt("  blocking read - process suspended\n");
  syscall_read_fd = guest_fd;
  Suspend(&Context::SyscallPread64CanWakeup, &Context::SyscallPread64Wakeup,
          StateRead);
  emulator->ProcessEventsSchedule();
...
int Context::ExecuteSyscall_pwrite64() { __UNIMPLEMENTED__ }

//
// System call 'chown16'
//

int Context::ExecuteSyscall_chown16() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_getcwd() {
  ...
  // Does not fit
  if (size <= loader->cwd.length()) return -ERANGE;
...
int Context::ExecuteSyscall_capget() { __UNIMPLEMENTED__ }

//
// System call 'capset'
//

int Context::ExecuteSyscall_capset() { __UNIMPLEMENTED__ }

//
// System call 'sigaltstack'
//

int Context::ExecuteSyscall_sigaltstack() { __UNIMPLEMENTED__ }

//
// System call 'sendfile'
//

int Context::ExecuteSyscall_sendfile() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_188'
//

int Context::ExecuteSyscall_ni_syscall_188() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_189'
//

int Context::ExecuteSyscall_ni_syscall_189() { __UNIMPLEMENTED__ }

//
// System call 'vfork'
//

int Context::ExecuteSyscall_vfork() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_getrlimit() {
  ...
  switch (res) {
    ...
    default:
      ...
      throw misc::Panic(
          misc::fmt("Unsupported resource: %s", rlimit_res_map.MapValue(res)));
  }
...
int Context::ExecuteSyscall_truncate64() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_lchown() { __UNIMPLEMENTED__ }

//
// System call 'getuid'
//

int Context::ExecuteSyscall_getuid() { return getuid(); }

//
// System call 'getgid'
//

int Context::ExecuteSyscall_getgid() { return getgid(); }

//
// System call 'geteuid'
//

int Context::ExecuteSyscall_geteuid() { return geteuid(); }

//
// System call 'getegid'
//

int Context::ExecuteSyscall_getegid() { return getegid(); }

//
// System call 'setreuid'
//

int Context::ExecuteSyscall_setreuid() { __UNIMPLEMENTED__ }

//
// System call 'setregid'
//

int Context::ExecuteSyscall_setregid() { __UNIMPLEMENTED__ }

//
// System call 'getgroups'
//

int Context::ExecuteSyscall_getgroups() { __UNIMPLEMENTED__ }

//
// System call 'setgroups'
//

int Context::ExecuteSyscall_setgroups() { __UNIMPLEMENTED__ }

//
// System call 'fchown'
//

int Context::ExecuteSyscall_fchown() { __UNIMPLEMENTED__ }

//
// System call 'setresuid'
//

int Context::ExecuteSyscall_setresuid() { __UNIMPLEMENTED__ }

//
// System call 'getresuid'
//

int Context::ExecuteSyscall_getresuid() { __UNIMPLEMENTED__ }

//
// System call 'setresgid'
//

int Context::ExecuteSyscall_setresgid() { __UNIMPLEMENTED__ }

//
// System call 'getresgid'
//

int Context::ExecuteSyscall_getresgid() { __UNIMPLEMENTED__ }

//
// System call 'chown'
//

int Context::ExecuteSyscall_chown() { __UNIMPLEMENTED__ }

//
// System call 'setuid'
//

int Context::ExecuteSyscall_setuid() { __UNIMPLEMENTED__ }

//
// System call 'setgid'
//

int Context::ExecuteSyscall_setgid() { __UNIMPLEMENTED__ }

//
// System call 'setfsuid'
//

int Context::ExecuteSyscall_setfsuid() { __UNIMPLEMENTED__ }

//
// System call 'setfsgid'
//

int Context::ExecuteSyscall_setfsgid() { __UNIMPLEMENTED__ }

//
// System call 'pivot_root'
//

int Context::ExecuteSyscall_pivot_root() { __UNIMPLEMENTED__ }

//
// System call 'mincore'
//

int Context::ExecuteSyscall_mincore() { __UNIMPLEMENTED__ }
...
static void sys_dirent64_host_to_guest(struct sim_dirent64 *guest,
                                       struct dirent64 *host) {
  guest->d_ino = host->d_ino;
  guest->d_off = host->d_off;
  guest->d_reclen = host->d_reclen;
  guest->d_type = host->d_type;
  strcpy(guest->d_name, host->d_name);

  Emulator *emulator = Emulator::getInstance();
  emulator->syscall_debug << misc::fmt("  dirent64 structure:\n");
  emulator->syscall_debug << misc::fmt(
      "  d_ino=%ld, d_off=%ld, d_reclen=%d, d_type=%c\n", guest->d_ino,
      guest->d_off, guest->d_reclen, guest->d_type);
  emulator->syscall_debug << misc::fmt("  d_name=%s\n", guest->d_name);
}

int Context::ExecuteSyscall_getdents64() {
  // Arguments
  int fd = regs.getEbx();
  unsigned int dirp = regs.getEcx();
  int count = regs.getEdx();
  emulator->syscall_debug << misc::fmt("  fd=%d, dirp=0x%x, count=%d\n", fd,
                                       dirp, count);
  ...
  // Host call
  struct dirent64 dir;
  int err = syscall(SYS_getdents64, host_fd, &dir, count);
  if (err == -1) return -errno;

  // Return
  struct sim_dirent64 sim_dir;
  sys_dirent64_host_to_guest(&sim_dir, &dir);
  memory->Write(dirp, sizeof(sim_dir), (char *)&sim_dir);
  return 0;
}
...
static const misc::StringMap fcntl_cmd_map = {
    ...
    {"F_SETFL", 4},    {"F_GETLK", 5},    {"F_SETLK", 6},    {"F_SETLKW", 7},
    {"F_SETOWN", 8},   {"F_GETOWN", 9},   {"F_SETSIG", 10},  {"F_GETSIG", 11},
    {"F_GETLK64", 12}, {"F_SETLK64", 13}, {"F_SETLKW64", 14}};
...
int Context::ExecuteSyscall_fcntl64() {
  ...
  switch (cmd) {
    ...
    default:
      ...
      throw misc::Panic(
          misc::fmt("Unsupported command: %s\n", fcntl_cmd_map.MapValue(cmd)));
  }
...
int Context::ExecuteSyscall_ni_syscall_222() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_223'
//

int Context::ExecuteSyscall_ni_syscall_223() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_readahead() { __UNIMPLEMENTED__ }

//
// System call 'setxattr'
//

int Context::ExecuteSyscall_setxattr() { __UNIMPLEMENTED__ }

//
// System call 'lsetxattr'
//

int Context::ExecuteSyscall_lsetxattr() { __UNIMPLEMENTED__ }

//
// System call 'fsetxattr'
//

int Context::ExecuteSyscall_fsetxattr() { __UNIMPLEMENTED__ }

//
// System call 'getxattr'
//

int Context::ExecuteSyscall_getxattr() { __UNIMPLEMENTED__ }

//
// System call 'lgetxattr'
//

int Context::ExecuteSyscall_lgetxattr() { __UNIMPLEMENTED__ }

//
// System call 'fgetxattr'
//

int Context::ExecuteSyscall_fgetxattr() { __UNIMPLEMENTED__ }

//
// System call 'listxattr'
//

int Context::ExecuteSyscall_listxattr() { __UNIMPLEMENTED__ }

//
// System call 'llistxattr'
//

int Context::ExecuteSyscall_llistxattr() { __UNIMPLEMENTED__ }

//
// System call 'flistxattr'
//

int Context::ExecuteSyscall_flistxattr() { __UNIMPLEMENTED__ }

//
// System call 'removexattr'
//

int Context::ExecuteSyscall_removexattr() { __UNIMPLEMENTED__ }

//
// System call 'lremovexattr'
//

int Context::ExecuteSyscall_lremovexattr() { __UNIMPLEMENTED__ }

//
// System call 'fremovexattr'
//

int Context::ExecuteSyscall_fremovexattr() { __UNIMPLEMENTED__ }

//
// System call 'tkill'
//

int Context::ExecuteSyscall_tkill() { __UNIMPLEMENTED__ }

//
// System call 'sendfile64'
//

int Context::ExecuteSyscall_sendfile64() { __UNIMPLEMENTED__ }

//
// System call 'futex'
//

static const misc::StringMap futex_cmd_map = {{"FUTEX_WAIT", 0},
                                              {"FUTEX_WAKE", 1},
                                              {"FUTEX_FD", 2},
                                              {"FUTEX_REQUEUE", 3},
                                              {"FUTEX_CMP_REQUEUE", 4},
...
int Context::ExecuteSyscall_futex() {
  ...
  switch (cmd) {
    ...
    case 9:  // FUTEX_WAIT_BITSET
    {
      ...
      if (futex != (unsigned)val1) return -EAGAIN;
    ...
    case 4:  // FUTEX_CMP_REQUEUE
    {
      ...
      if (futex != (unsigned)val3) return -EAGAIN;
      ...
      emulator->syscall_debug
          << misc::fmt("  futex at 0x%x: %d processes woken up\n", addr1, ret);
      ...
      for (auto it = emulator->getSuspendedContextsBegin(),
           ...
           it != e; ++it) {
        Context *context = *it;
        if (context->getState(StateFutex) && context->wakeup_futex == addr1) {
    ...
    case 5:  // FUTEX_WAKE_OP
    {
      ...
      if (cond) ret += FutexWake(addr2, val2, 0xffffffff);
    ...
    default:
      ...
      throw misc::Panic(misc::fmt("Unimplemented for cmd=%d (%s)", cmd,
                                  futex_cmd_map.MapValue(cmd)));
  }
...
int Context::ExecuteSyscall_sched_setaffinity() {
  ...
  if (emulator->syscall_debug) {
    ...
    for (int i = 0; i < size; i++)
      for (int j = 0; j < 8; j++)
        if (mask[i] & (1 << j)) emulator->syscall_debug << ' ' << (i * 8 + j);
    emulator->syscall_debug << " }\n";
  }
  ...
  if (!target_context) return -ESRCH;
...
int Context::ExecuteSyscall_set_thread_area() {
  ...
  emulator->syscall_debug << misc::fmt(
      "  entry_number=0x%x, base_addr=0x%x, limit=0x%x\n", uinfo.entry_number,
      uinfo.base_addr, uinfo.limit);
  emulator->syscall_debug << misc::fmt(
      "  seg_32bit=0x%x, contents=0x%x, read_exec_only=0x%x\n", uinfo.seg_32bit,
      uinfo.contents, uinfo.read_exec_only);
  emulator->syscall_debug << misc::fmt(
      "  limit_in_pages=0x%x, seg_not_present=0x%x, useable=0x%x\n",
      uinfo.limit_in_pages, uinfo.seg_not_present, uinfo.useable);
  if (!uinfo.seg_32bit)
...
int Context::ExecuteSyscall_get_thread_area() { __UNIMPLEMENTED__ }

//
// System call 'io_setup'
//

int Context::ExecuteSyscall_io_setup() { __UNIMPLEMENTED__ }

//
// System call 'io_destroy'
//

int Context::ExecuteSyscall_io_destroy() { __UNIMPLEMENTED__ }

//
// System call 'io_getevents'
//

int Context::ExecuteSyscall_io_getevents() { __UNIMPLEMENTED__ }

//
// System call 'io_submit'
//

int Context::ExecuteSyscall_io_submit() { __UNIMPLEMENTED__ }

//
// System call 'io_cancel'
//

int Context::ExecuteSyscall_io_cancel() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_ni_syscall_251() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_lookup_dcookie() { __UNIMPLEMENTED__ }

//
// System call 'epoll_create'
//

int Context::ExecuteSyscall_epoll_create() { __UNIMPLEMENTED__ }

//
// System call 'epoll_ctl'
//

int Context::ExecuteSyscall_epoll_ctl() { __UNIMPLEMENTED__ }

//
// System call 'epoll_wait'
//

int Context::ExecuteSyscall_epoll_wait() { __UNIMPLEMENTED__ }

//
// System call 'remap_file_pages'
//

int Context::ExecuteSyscall_remap_file_pages() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_timer_create() { __UNIMPLEMENTED__ }

//
// System call 'timer_settime'
//

int Context::ExecuteSyscall_timer_settime() { __UNIMPLEMENTED__ }

//
// System call 'timer_gettime'
//

int Context::ExecuteSyscall_timer_gettime() { __UNIMPLEMENTED__ }

//
// System call 'timer_getoverrun'
//

int Context::ExecuteSyscall_timer_getoverrun() { __UNIMPLEMENTED__ }

//
// System call 'timer_delete'
//

int Context::ExecuteSyscall_timer_delete() { __UNIMPLEMENTED__ }

//
// System call 'clock_settime'
//

int Context::ExecuteSyscall_clock_settime() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_clock_gettime() {
  ...
  switch (clk_id) {
    ...
    case 6:  // CLOCK_MONOTONIC_COARSE
      ...
      misc::Warning("%s: not implemented for 'clk_id' = %d", __FUNCTION__,
                    clk_id);
    ...
    default:
      ...
      throw Error(misc::fmt("%s: invalid value for 'clk_id' (%d)", __FUNCTION__,
                            clk_id));
  }
...
int Context::ExecuteSyscall_clock_nanosleep() { __UNIMPLEMENTED__ }

//
// System call 'statfs64'
//

int Context::ExecuteSyscall_statfs64() { __UNIMPLEMENTED__ }

//
// System call 'fstatfs64'
//

int Context::ExecuteSyscall_fstatfs64() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_tgkill() {
  ...
  emulator->syscall_debug << misc::fmt("  tgid=%d, pid=%d, sig=%d (%s)\n", tgid,
                                       pid, sig, signal_map.MapValue(sig));

  // Implementation restrictions.
  if (tgid == -1) throw misc::Panic("Not supported for tgid = -1");

  // Find context referred by pid.
  Context *context = emulator->getContext(pid);
  if (!context) throw Error(misc::fmt("Invalid pid (%d)", pid));
...
int Context::ExecuteSyscall_utimes() { __UNIMPLEMENTED__ }

//
// System call 'fadvise64_64'
//

int Context::ExecuteSyscall_fadvise64_64() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_273'
//

int Context::ExecuteSyscall_ni_syscall_273() { __UNIMPLEMENTED__ }

//
// System call 'mbind'
//

int Context::ExecuteSyscall_mbind() { __UNIMPLEMENTED__ }

//
// System call 'get_mempolicy'
//

int Context::ExecuteSyscall_get_mempolicy() { __UNIMPLEMENTED__ }

//
// System call 'set_mempolicy'
//

int Context::ExecuteSyscall_set_mempolicy() { __UNIMPLEMENTED__ }

//
// System call 'mq_open'
//

int Context::ExecuteSyscall_mq_open() { __UNIMPLEMENTED__ }

//
// System call 'mq_unlink'
//

int Context::ExecuteSyscall_mq_unlink() { __UNIMPLEMENTED__ }

//
// System call 'mq_timedsend'
//

int Context::ExecuteSyscall_mq_timedsend() { __UNIMPLEMENTED__ }

//
// System call 'mq_timedreceive'
//

int Context::ExecuteSyscall_mq_timedreceive() { __UNIMPLEMENTED__ }

//
// System call 'mq_notify'
//

int Context::ExecuteSyscall_mq_notify() { __UNIMPLEMENTED__ }

//
// System call 'mq_getsetattr'
//

int Context::ExecuteSyscall_mq_getsetattr() { __UNIMPLEMENTED__ }

//
// System call 'kexec_load'
//

int Context::ExecuteSyscall_kexec_load() { __UNIMPLEMENTED__ }

//
// System call 'waitid'
//

int Context::ExecuteSyscall_waitid() { __UNIMPLEMENTED__ }

//
// System call 'ni_syscall_285'
//

int Context::ExecuteSyscall_ni_syscall_285() { __UNIMPLEMENTED__ }

//
// System call 'add_key'
//

int Context::ExecuteSyscall_add_key() { __UNIMPLEMENTED__ }

//
// System call 'request_key'
//

int Context::ExecuteSyscall_request_key() { __UNIMPLEMENTED__ }

//
// System call 'keyctl'
//

int Context::ExecuteSyscall_keyctl() { __UNIMPLEMENTED__ }

//
// System call 'ioprio_set'
//

int Context::ExecuteSyscall_ioprio_set() { __UNIMPLEMENTED__ }

//
// System call 'ioprio_get'
//

int Context::ExecuteSyscall_ioprio_get() { __UNIMPLEMENTED__ }

//
// System call 'inotify_init'
//

int Context::ExecuteSyscall_inotify_init() { __UNIMPLEMENTED__ }

//
// System call 'inotify_add_watch'
//

int Context::ExecuteSyscall_inotify_add_watch() { __UNIMPLEMENTED__ }

//
// System call 'inotify_rm_watch'
//

int Context::ExecuteSyscall_inotify_rm_watch() { __UNIMPLEMENTED__ }

//
// System call 'migrate_pages'
//

int Context::ExecuteSyscall_migrate_pages() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_openat() {
  ...
  emulator->syscall_debug << misc::fmt(
      ...
      "flags = 0x%x %s, "
      "mode = 0x%x\n",
      dirfd, path_ptr, flags, open_flags_map.MapFlags(flags).c_str(), mode);
  ...
  emulator->syscall_debug << misc::fmt(
      ...
      "    guest_fd=%d\n"
      "    host_fd=%d\n",
      descriptor->getGuestIndex(), descriptor->getHostIndex());
...
int Context::ExecuteSyscall_mkdirat() { __UNIMPLEMENTED__ }

//
// System call 'mknodat'
//

int Context::ExecuteSyscall_mknodat() { __UNIMPLEMENTED__ }

//
// System call 'fchownat'
//

int Context::ExecuteSyscall_fchownat() { __UNIMPLEMENTED__ }

//
// System call 'futimesat'
//

int Context::ExecuteSyscall_futimesat() { __UNIMPLEMENTED__ }

//
// System call 'fstatat64'
//

int Context::ExecuteSyscall_fstatat64() { __UNIMPLEMENTED__ }

//
// System call 'unlinkat'
//

int Context::ExecuteSyscall_unlinkat() { __UNIMPLEMENTED__ }

//
// System call 'renameat'
//

int Context::ExecuteSyscall_renameat() { __UNIMPLEMENTED__ }

//
// System call 'linkat'
//

int Context::ExecuteSyscall_linkat() { __UNIMPLEMENTED__ }

//
// System call 'symlinkat'
//

int Context::ExecuteSyscall_symlinkat() { __UNIMPLEMENTED__ }

//
// System call 'readlinkat'
//

int Context::ExecuteSyscall_readlinkat() { __UNIMPLEMENTED__ }

//
// System call 'fchmodat'
//

int Context::ExecuteSyscall_fchmodat() { __UNIMPLEMENTED__ }

//
// System call 'faccessat'
//

int Context::ExecuteSyscall_faccessat() { __UNIMPLEMENTED__ }

//
// System call 'pselect6'
//

int Context::ExecuteSyscall_pselect6() { __UNIMPLEMENTED__ }

//
// System call 'ppoll'
//

int Context::ExecuteSyscall_ppoll() { __UNIMPLEMENTED__ }

//
// System call 'unshare'
//

int Context::ExecuteSyscall_unshare() { __UNIMPLEMENTED__ }
...
int Context::ExecuteSyscall_set_robust_list() {
  ...
  // Support
  if (len != 12) throw misc::Panic("Unsupported for len != 12");
...
int Context::ExecuteSyscall_get_robust_list() { __UNIMPLEMENTED__ }

//
// System call 'splice'
//

int Context::ExecuteSyscall_splice() { __UNIMPLEMENTED__ }

//
// System call 'sync_file_range'
//

int Context::ExecuteSyscall_sync_file_range() { __UNIMPLEMENTED__ }

//
// System call 'tee'
//

int Context::ExecuteSyscall_tee() { __UNIMPLEMENTED__ }

//
// System call 'vmsplice'
//

int Context::ExecuteSyscall_vmsplice() { __UNIMPLEMENTED__ }

//
// System call 'move_pages'
//

int Context::ExecuteSyscall_move_pages() { __UNIMPLEMENTED__ }

//
// System call 'getcpu'
//

int Context::ExecuteSyscall_getcpu() { __UNIMPLEMENTED__ }

//
// System call 'epoll_pwait'
//

int Context::ExecuteSyscall_epoll_pwait() { __UNIMPLEMENTED__ }

//
// System call 'utimensat'
//

int Context::ExecuteSyscall_utimensat() { __UNIMPLEMENTED__ }

//
// System call 'signalfd'
//

int Context::ExecuteSyscall_signalfd() { __UNIMPLEMENTED__ }

//
// System call 'timerfd'
//

int Context::ExecuteSyscall_timerfd() { __UNIMPLEMENTED__ }

//
// System call 'eventfd'
//

int Context::ExecuteSyscall_eventfd() { __UNIMPLEMENTED__ }

//
// System call 'fallocate'
//

int Context::ExecuteSyscall_fallocate() { __UNIMPLEMENTED__ }
