...
unsigned char Context::LoadRm8() {
  ...
  emulator->isa_debug << misc::fmt("  [0x%x]=0x%x", last_effective_address,
                                   value);
  return value;
}
...
unsigned long long Context::LoadM64() {
  ...
  emulator->isa_debug << misc::fmt("  [0x%x]=0x%llx", last_effective_address,
                                   value);
  return value;
}
...
unsigned Context::getLinearAddress(unsigned offset) {
  // No segment override
  if (!inst.getSegment()) return offset;
  ...
  if (!glibc_segment_base) {
    throw misc::Panic(
        misc::fmt("GLibc segment not set "
...
unsigned Context::getEffectiveAddress() {
  ...
  if (inst.getModRmMod() == 3) throw Error("Invalid value for 'modrm_mod'");
...
void Context::LoadFpu(int index, unsigned char* value) {
  if (!misc::inRange(index, 0, 7)) throw Error("Invalid value for 'index'");

  int eff_index = (regs.getFpuTop() + index) % 8;
  if (!regs.isFpuValid(eff_index)) throw Error("Invalid FPU stack entry");
...
void Context::StoreFpu(int index, unsigned char* value) {
  ...
  if (!regs.isFpuValid(index)) throw Error("Invalid FPU stack entry");
...
void Context::PopFpu(unsigned char* value) {
  ...
  // Copy value
  if (value) memcpy(value, regs.getFpuValue(regs.getFpuTop()), 10);

  // Debug
  if (emulator->isa_debug)
    emulator->isa_debug << misc::fmt(
        "  st(0)=%g (popped)",
        Extended::ExtendedToDouble(regs.getFpuValue(regs.getFpuTop())));
...
void Context::StoreFpuCode(unsigned short status) {
  regs.setFpuCode(0);
  if (misc::getBit32(status, 14)) regs.setFpuCode(regs.getFpuCode() | 8);
...
unsigned short Context::LoadFpuStatus() {
  ...
  if (misc::getBit32(regs.getFpuCode(), 3)) status |= 0x4000;
