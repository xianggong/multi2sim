...
void Emulator::RegisterOptions() {
  ...
  command_line->RegisterString(
      "--x86-debug-call <file>", call_debug_file,
  ...
  command_line->RegisterString(
      "--x86-debug-loader <file>", loader_debug_file,
  ...
  command_line->RegisterString(
      "--x86-debug-syscall <file>", syscall_debug_file,
  ...
  command_line->RegisterInt64(
      "--x86-max-inst <number> (default = 0)", max_instructions,
...
void Emulator::InsertInRunningContexts(Context* context) {
  ...
  context->running_contexts_iterator =
      running_contexts.insert(running_contexts.end(), context);
}
...
void Emulator::InsertInSuspendedContexts(Context* context) {
  ...
  context->suspended_contexts_iterator =
      suspended_contexts.insert(suspended_contexts.end(), context);
}
...
void Emulator::InsertInFinishedContexts(Context* context) {
  ...
  context->finished_contexts_iterator =
      finished_contexts.insert(finished_contexts.end(), context);
}
...
void Emulator::InsertInZombieContexts(Context* context) {
  ...
  context->zombie_contexts_iterator =
      zombie_contexts.insert(zombie_contexts.end(), context);
}
...
Context* Emulator::newContext() {
  ...
  auto it = contexts.insert(contexts.end(), misc::new_unique<Context>());
...
Context* Emulator::getContext(int pid) {
  ...
  for (auto& context : contexts)
    if (context->getId() == pid) return context.get();
...
void Emulator::LoadProgram(const std::vector<std::string>& args,
  ...
  context->Load(args, env, cwd, stdin_file_name, stdout_file_name);
}
...
void Emulator::ProcessEvents() {
  ...
  for (auto iterator = suspended_contexts.begin();
       iterator != suspended_contexts.end(); iterator = iterator_next) {
    ...
    // (e.g. 'read').
    if (context->getState(Context::StateCallback) && context->CanWakeup()) {
      context->Wakeup();
      continue;
    }
  }
  ...
  //
  for (Context* context : running_contexts) context->CheckSignalHandler();
...
bool Emulator::Run() {
  ...
  if (esim->hasFinished()) return true;
  ...
  for (auto& context : contexts) {
    // Skip if not running
    if (!context->getState(Context::StateRunning)) continue;
  ...
  while (finished_contexts.size()) FreeContext(finished_contexts.front());
