...
Regs::Regs() {
  ...
  for (int i = 0; i < 8; i++) fpu_stack[i].valid = false;
...
Extended Regs::ReadFpu(int index) const {
  // Invalid index
  if (!misc::inRange(index, 0, 7)) return 0.0;

  // Calculate effective index
  index = (fpu_top + index) % 8;
  if (!fpu_stack[index].valid) return 0.0;
...
void Regs::WriteFpu(int index, const Extended& value) {
  // Invalid index
  if (!misc::inRange(index, 0, 7)) return;

  // Calculate effective index
  index = (fpu_top + index) % 8;
  if (!fpu_stack[index].valid) return;
...
void Regs::Dump(std::ostream& os) const {
  ...
  os << misc::fmt("  eip=%08x\n", eip);
  os << misc::fmt(
      "  flags=%04x (cf=%d  pf=%d  af=%d  zf=%d  sf=%d  df=%d  of=%d)\n",
      eflags, (eflags & (1 << Instruction::FlagCF)) > 0,
  ...
  // XMM registers
  for (int i = 0; i < 8; i++) os << "  xmm" << i << " = " << xmm[i] << '\n';
}
