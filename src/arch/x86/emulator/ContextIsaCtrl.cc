...
#define op_cmov_r16_rm16(cc, idep1, idep2)                                   \
  void Context::ExecuteInst_cmov##cc##_r16_rm16() {                          \
    if (cc_##cc) StoreR16(LoadRm16());                                       \
    newUinst(Uinst::OpcodeMove, idep1, idep2, Uinst::DepRm16, Uinst::DepR16, \
             0, 0, 0);                                                       \
  }

#define op_cmov_r32_rm32(cc, idep1, idep2)                                   \
  void Context::ExecuteInst_cmov##cc##_r32_rm32() {                          \
    if (cc_##cc) StoreR32(LoadRm32());                                       \
    newUinst(Uinst::OpcodeMove, idep1, idep2, Uinst::DepRm32, Uinst::DepR32, \
             0, 0, 0);                                                       \
  }

#define op_cc_all(ccop)                                                        \
  op_##ccop(a, Uinst::DepCf, Uinst::DepZps) op_##ccop(ae, Uinst::DepCf, 0)     \
      op_##ccop(b, Uinst::DepCf, 0) op_##ccop(be, Uinst::DepCf, Uinst::DepZps) \
          op_##ccop(e, Uinst::DepZps, 0)                                       \
              op_##ccop(g, Uinst::DepZps, Uinst::DepOf)                        \
                  op_##ccop(ge, Uinst::DepZps, Uinst::DepOf) op_##ccop(        \
                      l, Uinst::DepZps, Uinst::DepOf)                          \
                      op_##ccop(le, Uinst::DepZps, Uinst::DepOf) op_##ccop(    \
                          ne, Uinst::DepZps, 0) op_##ccop(no, Uinst::DepOf, 0) \
...
op_cc_all(setcc) op_cc_all(jcc_rel8) op_cc_all(jcc_rel32)
  ...
  if (!regs.Read(Instruction::RegEcx)) regs.setEip(target_eip);
  newUinst(Uinst::OpcodeBranch, Uinst::DepEcx, 0, 0, 0, 0, 0, 0);
}

void Context::ExecuteInst_jcxz_rel8() {
  target_eip = regs.getEip() + inst.getImmByte();
  if (!regs.Read(Instruction::RegCx)) regs.setEip(target_eip);
  newUinst(Uinst::OpcodeBranch, Uinst::DepEcx, 0, 0, 0, 0, 0, 0);
}
