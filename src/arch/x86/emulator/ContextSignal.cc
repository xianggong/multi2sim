...
void Context::RunSignalHandler(int sig) {
  ...
  // on it.
  signal_mask_table.setRetCodePtr(
      memory->MapSpace(mem::Memory::PageSize, mem::Memory::PageSize));
  ...
  memory->Write(regs.getEsp(), sizeof signal_frame, (char*)&signal_frame);
  ...
  if (!handler) misc::fatal("%s: invalid signal handler", __FUNCTION__);
  regs.setEip(handler);
}
...
void Context::CheckSignalHandler() {
  ...
  if (getState(StateHandler)) return;
