...
#define __X86_CONTEXT_SAVE_FLAGS__ \
  asm volatile(                    \
      "pushf\n\t"                  \
      "pop %0\n\t"                 \
      : "=m"(Context::host_flags));

#define __X86_CONTEXT_RESTORE_FLAGS__ \
  asm volatile(                       \
      "push %0\n\t"                   \
      "popf\n\t"                      \
      : "=m"(Context::host_flags));
...
#undef DEFINST
  ...
  unsigned char LoadR8() {
    return regs.Read(inst.getModRmReg() + Instruction::RegAl);
  }
  unsigned short LoadR16() {
    return regs.Read(inst.getModRmReg() + Instruction::RegAx);
  }
  unsigned int LoadR32() {
    return regs.Read(inst.getModRmReg() + Instruction::RegEax);
  }
  unsigned short LoadSReg() {
    return regs.Read(inst.getModRmReg() + Instruction::RegEs);
  }

  // Store value into register
  void StoreR8(unsigned char value) {
    regs.Write(inst.getModRmReg() + Instruction::RegAl, value);
  }
  void StoreR16(unsigned short value) {
    regs.Write(inst.getModRmReg() + Instruction::RegAx, value);
  }
  void StoreR32(unsigned int value) {
    regs.Write(inst.getModRmReg() + Instruction::RegEax, value);
  }
  void StoreSReg(unsigned short value) {
    regs.Write(inst.getModRmReg() + Instruction::RegEs, value);
  }

  // Load value from index register
  unsigned char LoadIR8() {
    return regs.Read(inst.getOpIndex() + Instruction::RegAl);
  }
  unsigned short LoadIR16() {
    return regs.Read(inst.getOpIndex() + Instruction::RegAx);
  }
  unsigned int LoadIR32() {
    return regs.Read(inst.getOpIndex() + Instruction::RegEax);
  }

  // Store value into index register
  void StoreIR8(unsigned char value) {
    regs.Write(inst.getOpIndex() + Instruction::RegAl, value);
  }
  void StoreIR16(unsigned short value) {
    regs.Write(inst.getOpIndex() + Instruction::RegAx, value);
  }
  void StoreIR32(unsigned int value) {
...
#undef DEFSYSCALL
 ...
 public:
  ...
  /// Set uinst_active
  void setUinstActive(bool uinst_active) { this->uinst_active = uinst_active; }

  /// Add a new memory micro-instruction to the list only if we're running
  /// in timing simulation mode.
  void newMemoryUinst(Uinst::Opcode opcode, unsigned address, int size,
                      int idep0, int idep1, int idep2, int odep0, int odep1,
                      int odep2, int odep3);

  /// Add a new micro-instruction to the list only if we're running
  /// in timing simulation mode, omitting the \a address and \a size
  /// arguments.
  void newUinst(Uinst::Opcode opcode, int idep0, int idep1, int idep2,
                int odep0, int odep1, int odep2, int odep3) {
    newMemoryUinst(opcode, 0, 0, idep0, idep1, idep2, odep0, odep1, odep2,
                   odep3);
  }
