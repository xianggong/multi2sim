...
unsigned Context::LoadSegments(ELFReader::File* binary) {
  ...
  for (auto& program_header : binary->getProgramHeaders()) {
    ...
    if (program_header->getType() != 1) continue;
    ...
    if (program_header->getFlags() & PF_W) perm |= mem::Memory::AccessWrite;

    // Check if segment has execution permission
    if (program_header->getFlags() & PF_X) perm |= mem::Memory::AccessExec;

    // Map segment in memory
    memory->Map(program_header->getVaddr(), program_header->getMemsz(), perm);

    // Load segment
    memory->Init(program_header->getVaddr(), program_header->getFilesz(),
                 program_header->getBuffer());

    // Record highest address
    top =
        std::max(top, program_header->getVaddr() + program_header->getMemsz());
  }
...
void Context::LoadInterpreter() {
  ...
  for (auto& program_header : loader->binary->getProgramHeaders()) {
    ...
    if (program_header->getType() != 3) continue;
  ...
  if (!f) throw Error(loader->interpreter + ": Invalid interpreter");
...
void Context::LoadAuxiliaryVectorEntry(unsigned& sp, unsigned type,
                                       unsigned value) {
  // Write values
...
unsigned Context::LoadAuxiliaryVector(unsigned where) {
  ...
  // AT_PHDR
  if (phdt_base) LoadAuxiliaryVectorEntry(sp, 3, phdt_base);
...
void Context::LoadBinary() {
  ...
  if (!loader->stdout_file_name.empty()) {
    ...
    if (f < 0)
      misc::fatal("%s: cannot open stdout", loader->stdout_file_name.c_str());
