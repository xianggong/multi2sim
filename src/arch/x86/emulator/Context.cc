...
const misc::StringMap Context::StateMap = {
    {"running", StateRunning},     {"specmode", StateSpecMode},
    {"suspended", StateSuspended}, {"finished", StateFinished},
    {"exclusive", StateExclusive}, {"locked", StateLocked},
    {"handler", StateHandler},     {"sigsuspend", StateSigsuspend},
    {"nanosleep", StateNanosleep}, {"poll", StatePoll},
    {"read", StateRead},           {"write", StateWrite},
    {"waitpid", StateWaitpid},     {"zombie", StateZombie},
    {"futex", StateFutex},         {"alloc", StateAlloc},
    {"callback", StateCallback},   {"mapped", StateMapped}};
...
Context::Context()
  ...
  thread_affinity =
      misc::new_unique<misc::Bitmap>(Cpu::getNumCores() * Cpu::getNumThreads());
  thread_affinity->Set();
...
void Context::UpdateState(unsigned state) {
  ...
  if (diff & ~StateSpecMode) emulator->schedule_signal = true;

  // Update state
  this->state = state;
  if (this->state & StateFinished)
    this->state = StateFinished | (state & StateAlloc) | (state & StateMapped);
  if (this->state & StateZombie)
    this->state = StateZombie | (state & StateAlloc) | (state & StateMapped);
  if (!(this->state & StateSuspended) && !(this->state & StateFinished) &&
      !(this->state & StateZombie) && !(this->state & StateLocked))
  ...
  if (Emulator::context_debug && (diff & ~StateSpecMode)) {
    Emulator::context_debug << misc::fmt(
        ...
        emulator->getNumInstructions(), StateMap.MapFlags(this->state).c_str());
  }
  ...
  while (count) {
    ...
    for (auto it = emulator->getSuspendedContextsBegin(),
      ...
      if (!context->getState(StateFutex) || context->wakeup_futex != futex)
        continue;
      if (!(context->wakeup_futex_bitset & bitset)) continue;
      if (!wakeup_context ||
          context->wakeup_futex_sleep < wakeup_context->wakeup_futex_sleep)
...
void Context::ExitRobustList() {
  ...
  for (;;) {
    ...
    if (!next || next == robust_list_head) break;
    lock_entry = next;
  }
}
...
std::string Context::OpenProcSelfMaps() {
  ...
  for (;;) {
    ...
    int perm =
        page->getPerm() & (mem::Memory::AccessRead | mem::Memory::AccessWrite |
    ...
    fprintf(f, "%08x-%08x %c%c%c%c 00000000 00:00\n", start,
            ...
            perm & mem::Memory::AccessExec ? 'x' : '-', 'p');
  }
...
void Context::Load(const std::vector<std::string>& args,
  ...
  if (!args.size() || args[0].empty())
    throw misc::Panic(
        "Function invoked with no program name, or "
        "with an empty program.");

  // Program must not have been loaded before
  if (loader.get() || memory.get())
    throw misc::Panic(
        misc::fmt("[%s] Program already loaded", args[0].c_str()));
  ...
  for (auto& var : env) loader->env.emplace_back(var);
...
void Context::HostThreadSuspend() {
  ...
  if (getState(StateNanosleep)) {
    ...
    long long timeout = syscall_nanosleep_wakeup_time > now
                            ? syscall_nanosleep_wakeup_time - now
                            : 0;
    usleep(timeout);
  }
  ...
  if (getState(StateRead)) {
    ...
    if (!desc)
      throw misc::Panic(
          misc::fmt("Invalid file descriptor "
                    "(%d)",
                    syscall_read_fd));
  ...
  if (getState(StateWrite)) {
    ...
    comm::FileDescriptor* desc =
        file_table->getFileDescriptor(syscall_write_fd);
    if (!desc)
      throw misc::Panic(
          misc::fmt("Invalid file descriptor "
                    "(%d)",
...
bool Context::isSuspended() { return getState(StateSuspended); }
...
void Context::HostThreadTimerCancelUnsafe() {
  if (!host_thread_timer_active) return;
  if (pthread_cancel(host_thread_timer))
    throw misc::Panic(
        misc::fmt("[%s] Error canceling "
                  "host thread",
                  getName().c_str()));
...
void Context::Execute() {
  ...
  if (inst.getOpcode() == Instruction::OpcodeInvalid && !spec_mode) {
    inst.Dump(std::cout);
    throw Error(
        misc::fmt("Unsupported instruction "
                  "(%02x %02x %02x %02x...)\n",
  ...
  if (emulator->isa_debug)
    emulator->isa_debug << misc::fmt("%d %8lld %x: ", getId(),
                                     ...
                                     current_eip)
                        << inst << misc::fmt("  (%d bytes)", inst.getSize());
  ...
  if (inst.getOpcode()) {
    try {
      ...
      if (!spec_mode) {
        ...
        e.AppendPrefix(misc::fmt("eip 0x%x", regs.getEip()));
        throw e;
      }
    } catch (misc::Panic& e) {
      // Ignore in speculative mode
      if (!spec_mode) throw e;
    }
  }

  // Debug
  emulator->isa_debug << '\n';
  if (emulator->call_debug) DebugCallInst();
...
void Context::FinishGroup(int exit_code) {
  ...
  if (getState(StateFinished) || getState(StateZombie)) return;

  // Finish all contexts in the group
  for (auto it = emulator->getContextsBegin(), e = emulator->getContextsEnd();
       it != e; ++it) {
    Context* context = it->get();
    if (context->group_parent != this && context != this) continue;

    if (context->getState(StateZombie)) context->setState(StateFinished);
    if (context->getState(StateHandler)) context->ReturnFromSignalHandler();
...
void Context::Finish(int exit_code) {
  ...
  if (getState(StateHandler)) ReturnFromSignalHandler();
...
Context* Context::getZombie(int pid) {
  for (auto it = emulator->getZombieContextsBegin(),
       ...
       it != e; ++it) {
    Context* context = *it;
    if (context->parent != this) continue;
    if (context->getId() == pid || pid == -1) return context;
  }
  return nullptr;
}
