...
misc::StringMap Cpu::recover_kind_map = {{"Writeback", RecoverKindWriteback},
                                         {"Commit", RecoverKindCommit}};

misc::StringMap Cpu::fetch_kind_map = {{"Shared", FetchKindShared},
                                       {"TimeSlice", FetchKindTimeslice}};

misc::StringMap Cpu::dispatch_kind_map = {
    {"Shared", DispatchKindShared}, {"TimeSlice", DispatchKindTimeslice},
};

misc::StringMap Cpu::issue_kind_map = {
    {"Shared", IssueKindShared}, {"TimeSlice", IssueKindTimeslice},
};

misc::StringMap Cpu::commit_kind_map = {
    {"Shared", CommitKindShared}, {"TimeSlice", CommitKindTimeslice},
};

misc::StringMap Cpu::reorder_buffer_kind_map = {
    {"Private", ReorderBufferKindPrivate}, {"Shared", ReorderBufferKindShared},
};
...
Cpu::Cpu(Timing* timing) : timing(timing) {
  ...
  event_memory_access_start = esim_engine->RegisterEvent(
      "memory_access_start", MemoryAccessHandler, timing->getFrequencyDomain());
  event_memory_access_end = esim_engine->RegisterEvent(
      "memory_access_end", MemoryAccessHandler, timing->getFrequencyDomain());
...
void Cpu::ParseConfiguration(misc::IniFile* ini_file) {
  ...
  recover_kind = (RecoverKind)ini_file->ReadEnum(
      section, "RecoverKind", recover_kind_map, RecoverKindWriteback);
  recover_penalty = ini_file->ReadInt(section, "RecoverPenalty", 0);

  // Section '[ Pipeline ]'
  section = "Pipeline";
  fetch_kind = (FetchKind)ini_file->ReadEnum(
      section, "FetchKind", fetch_kind_map, FetchKindTimeslice);
  decode_width = ini_file->ReadInt(section, "DecodeWidth", 4);
  dispatch_kind = (DispatchKind)ini_file->ReadEnum(
      section, "DispatchKind", dispatch_kind_map, DispatchKindTimeslice);
  dispatch_width = ini_file->ReadInt(section, "DispatchWidth", 4);
  issue_kind = (IssueKind)ini_file->ReadEnum(
      section, "IssueKind", issue_kind_map, IssueKindTimeslice);
  issue_width = ini_file->ReadInt(section, "IssueWidth", 4);
  commit_kind = (CommitKind)ini_file->ReadEnum(
      section, "CommitKind", commit_kind_map, CommitKindShared);
  commit_width = ini_file->ReadInt(section, "CommitWidth", 4);
  occupancy_stats = ini_file->ReadBool(section, "OccupancyStats", false);

  // Section '[ Queues ]'
  section = "Queues";
  reorder_buffer_kind = (ReorderBufferKind)ini_file->ReadEnum(
      section, "RobKind", reorder_buffer_kind_map, ReorderBufferKindPrivate);
  reorder_buffer_size = ini_file->ReadInt(section, "RobSize", 64);
  fetch_queue_size = ini_file->ReadInt(section, "FetchQueueSize", 64);
  instruction_queue_kind = (InstructionQueueKind)ini_file->ReadEnum(
      section, "IqKind", instruction_queue_kind_map,
      InstructionQueueKindPrivate);
  instruction_queue_size = ini_file->ReadInt(section, "IqSize", 40);

  load_store_queue_kind = (LoadStoreQueueKind)ini_file->ReadEnum(
      section, "LsqKind", load_store_queue_kind_map, LoadStoreQueueKindPrivate);
...
void Cpu::MemoryAccess(mem::Module* module, mem::Module::AccessType access_type,
...
void Cpu::MemoryAccessHandler(esim::Event* event, esim::Frame* esim_frame) {
  ...
  if (event == event_memory_access_start) {
    ...
    frame->uop->memory_access = module->Access(
        frame->access_type, frame->address, nullptr, event_memory_access_end);
...
long long Cpu::getCycle() const { return timing->getCycle(); }
...
void Cpu::EmptyTraceList() {
  while (trace_list.size()) {
    ...
    Timing::trace << misc::fmt(
        ...
        "id=%lld "
        "core=%d\n",
        uop->getIdInCore(), uop->getCore()->getId());
  }
}
}
