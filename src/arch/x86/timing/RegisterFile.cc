...
const int RegisterFile::MinFloatingPointSize =
    Uinst::DepFpCount + Uinst::MaxODeps;
const int RegisterFile::MinXmmSize = Uinst::DepXmmCount + Uinst::MaxODeps;

misc::StringMap RegisterFile::KindMap = {{"Shared", KindShared},
                                         {"Private", KindPrivate}};
...
RegisterFile::RegisterFile(Thread* thread) : thread(thread) {
  // Core that the register file belongs to
  core = thread->getCore();

  // Integer register file
  integer_registers =
      misc::new_unique_array<PhysicalRegister>(integer_local_size);

  // Free list
  num_free_integer_registers = integer_local_size;
  free_integer_registers = misc::new_unique_array<int>(integer_local_size);
  for (int physical_register = 0; physical_register < integer_local_size;
       physical_register++)
    free_integer_registers[physical_register] = physical_register;

  // Floating-point register file
  floating_point_registers =
      misc::new_unique_array<PhysicalRegister>(floating_point_local_size);

  // Free list
  num_free_floating_point_registers = floating_point_local_size;
  free_floating_point_registers =
      misc::new_unique_array<int>(floating_point_local_size);

  // Initialize free list
  for (int physical_register = 0; physical_register < floating_point_local_size;
       physical_register++)
  ...
  // Initialize free list
  for (int physical_register = 0; physical_register < xmm_local_size;
       physical_register++)
...
void RegisterFile::Dump(std::ostream& os) const {
  ...
  os << misc::fmt("\t%d occupied, %d free, %d total\n",
                  floating_point_local_size - num_free_floating_point_registers,
                  num_free_floating_point_registers, floating_point_local_size);
...
void RegisterFile::ParseConfiguration(misc::IniFile* ini_file) {
  ...
  // Read parameters
  kind = (Kind)ini_file->ReadEnum(section, "RfKind", KindMap, KindPrivate);
  ...
  if (floating_point_size < MinFloatingPointSize)
    throw Error(
        misc::fmt("rf_fp_size must be at least %d", MinFloatingPointSize));
...
int RegisterFile::RequestIntegerRegister() {
  ...
  int physical_register =
      free_integer_registers[num_free_integer_registers - 1];
...
int RegisterFile::RequestFloatingPointRegister() {
  ...
  int physical_register =
      free_floating_point_registers[num_free_floating_point_registers - 1];
  ...
  // Debug
  debug << misc::fmt(
      "  Floating-point register %d allocated, "
      "%d available\n",
      physical_register, num_free_floating_point_registers);
...
int RegisterFile::RequestXmmRegister() {
  ...
  debug << misc::fmt("  XMM register %d allocated, %d available\n",
                     physical_register, num_free_xmm_registers);
...
bool RegisterFile::canRename(Uop* uop) {
  ...
  if (kind == KindPrivate) {
    ...
    if (num_occupied_integer_registers + uop->getNumIntegerOutputs() >
        integer_local_size)
      return false;
    ...
    if (num_occupied_xmm_registers + uop->getNumXmmOutputs() > xmm_local_size)
      return false;
  } else {
    // Not enough integer registers
    if (core->getNumOccupiedIntegerRegisters() + uop->getNumIntegerOutputs() >
        integer_local_size)
      return false;
    ...
    // Not enough XMM registers
    if (core->getNumOccupiedXmmRegisters() + uop->getNumXmmOutputs() >
        xmm_local_size)
      return false;
  }
...
void RegisterFile::Rename(Uop* uop) {
  ...
  for (int dep = 0; dep < Uinst::MaxIDeps; dep++) {
    ...
    if (Uinst::isIntegerDependency(logical_register)) {
      // Rename register
      int physical_register =
          integer_rat[logical_register - Uinst::DepIntFirst];
      uop->setInput(dep, physical_register);

      // Debug
      debug << "  Input " << Uinst::dep_map[logical_register]
            << " -> Integer regsiter " << physical_register << '\n';
      ...
      int stack_register =
          (logical_register - Uinst::DepFpFirst + floating_point_top) % 8 +
          Uinst::DepFpFirst;
      ...
      // Rename register
      int physical_register =
          floating_point_rat[stack_register - Uinst::DepFpFirst];
      uop->setInput(dep, physical_register);

      // Debug
      debug << "  Input " << Uinst::dep_map[logical_register]
            << " -> Floating-point regsiter " << physical_register << '\n';
      ...
      // Debug
      debug << "  Input " << Uinst::dep_map[logical_register]
            << " -> XMM regsiter " << physical_register << '\n';
  ...
  for (int dep = 0; dep < Uinst::MaxODeps; dep++) {
    ...
    if (Uinst::isFlagDependency(logical_register)) {
      ...
      debug << "  Output " << Uinst::dep_map[logical_register]
            << " -> Integer register " << physical_register << '\n';
      ...
      // Debug
      debug << "  Output " << Uinst::dep_map[logical_register]
            << " -> Floating-point register " << physical_register << '\n';
      ...
      int old_physical_register =
          xmm_rat[logical_register - Uinst::DepXmmFirst];
      ...
      // Debug
      debug << "  Output " << Uinst::dep_map[logical_register]
            << " -> XMM register " << physical_register << '\n';
  ...
  if (flag_count > 0) {
    ...
    for (int dep = 0; dep < Uinst::MaxODeps; dep++) {
      ...
      int logical_register = uop->getUinst()->getODep(dep);
      if (!Uinst::isFlagDependency(logical_register)) continue;
      ...
      debug << "  Output flag " << Uinst::dep_map[logical_register]
            << " -> Integer register " << flag_physical_register << '\n';
    }
  }
}
...
void RegisterFile::UndoUop(Uop* uop) {
  ...
  for (int dep = Uinst::MaxODeps - 1; dep >= 0; dep--) {
    ...
    if (Uinst::isIntegerDependency(logical_register)) {
      ...
      // Debug
      debug << "  Output " << Uinst::dep_map[logical_register]
            << " -> From integer register " << physical_register << " back to "
            << old_physical_register << '\n';
    } else if (Uinst::isFloatingPointDependency(logical_register)) {
      // Convert to top-of-stack relative
      int stack_register =
      ...
      if (!floating_point_registers[physical_register].busy) {
        ...
        // Add to free list
        free_floating_point_registers[num_free_floating_point_registers] =
            physical_register;
        num_free_floating_point_registers++;
      ...
      // Debug
      debug << "  Output " << Uinst::dep_map[logical_register]
            << " -> From floating-point register " << physical_register
            << " back to " << old_physical_register << '\n';
    } else if (Uinst::isXmmDependency(logical_register)) {
      // Decrease busy counter and free if 0.
      ...
      // Debug
      debug << "  Output " << Uinst::dep_map[logical_register]
            << " -> From XMM register " << physical_register << " back to "
            << old_physical_register << '\n';
    } else {
  ...
  for (int dep = 0; dep < Uinst::MaxODeps; dep++) {
    ...
    if (Uinst::isIntegerDependency(logical_register)) {
      ...
      if (!integer_registers[old_physical_register].busy) {
        ...
        // Add to free list
        free_integer_registers[num_free_integer_registers] =
            old_physical_register;
        num_free_integer_registers++;
...
void RegisterFile::CheckRegisterFile() {
  ...
  for (int logical_register = Uinst::DepIntFirst;
       logical_register <= Uinst::DepIntLast; logical_register++) {
  ...
  for (int logical_register = Uinst::DepFpFirst;
       logical_register <= Uinst::DepFpLast; logical_register++) {
    int physical_register =
        floating_point_rat[logical_register - Uinst::DepFpFirst];
    assert(floating_point_registers[physical_register].busy);
  }

  // XMM registers
  for (int logical_register = Uinst::DepXmmFirst;
       logical_register <= Uinst::DepXmmLast; logical_register++) {
    int physical_register = xmm_rat[logical_register - Uinst::DepXmmFirst];
    assert(xmm_registers[physical_register].busy);
  }
}
