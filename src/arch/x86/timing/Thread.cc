...
Thread::Thread(Core* core, int id_in_core)
    : core(core), id_in_core(id_in_core) {
  // Assign name
  ...
  branch_predictor =
      misc::new_unique<BranchPredictor>(name + ".BranchPredictor");

  // Initialize trace cache
  if (TraceCache::isPresent())
    trace_cache = misc::new_unique<TraceCache>(name + ".TraceCache");
...
void Thread::Dump(std::ostream& os) const {
  // Title
  std::string title =
      misc::fmt("Core %d - Thread %d", id_in_core, core->getId());
  os << title << '\n';
  os << std::string(title.length(), '=') << "\n\n";

  // Register file
  if (register_file) os << *register_file;
...
void Thread::InsertInFetchQueue(std::shared_ptr<Uop> uop) {
  ...
  if (uop->from_trace_cache) {
    ...
    if (uop->mop_index == 0) fetch_queue_occupancy += uop->mop_size;
  }
}
...
void Thread::ExtractFromFetchQueue(Uop* uop) {
  ...
  assert(uop == fetch_queue.front().get() || uop == fetch_queue.back().get());
...
bool Thread::canInsertInReorderBuffer() {
  switch (Cpu::getReorderBufferKind()) {
    case Cpu::ReorderBufferKindPrivate:
      ...
      return (int)reorder_buffer.size() < Cpu::getReorderBufferSize();
    ...
    case Cpu::ReorderBufferKindShared:
      ...
      return core->getReorderBufferOccupancy() < Cpu::getReorderBufferSize();
...
void Thread::InsertInReorderBuffer(std::shared_ptr<Uop> uop) {
  ...
  uop->reorder_buffer_iterator =
      reorder_buffer.insert(reorder_buffer.end(), uop);
  ...
  for (auto& uop : reorder_buffer) {
    ...
    if (uop->dispatched)
      os << misc::fmt("\t\tDispatched in cycle %lld (%lld ago)\n",
                      uop->dispatch_when, cycle - uop->dispatch_when);
    else
      os << "\t\tNot dispatched\n";
    ...
    if (uop->completed)
      os << misc::fmt("\t\tCompleted in cycle %lld (%lld ago)\n",
                      uop->complete_when, cycle - uop->complete_when);
    else
      os << "\t\tNot completed\n";
  }

  // Empty list
  if (reorder_buffer.empty()) os << "-Empty-\n";
...
bool Thread::canInsertInInstructionQueue() {
  switch (Cpu::getInstructionQueueKind()) {
    case Cpu::InstructionQueueKindPrivate:
      ...
      return (int)instruction_queue.size() < Cpu::getInstructionQueueSize();
...
void Thread::InsertInInstructionQueue(std::shared_ptr<Uop> uop) {
  ...
  uop->instruction_queue_iterator =
      instruction_queue.insert(instruction_queue.end(), uop);
...
void Thread::DumpInstructionQueue(std::ostream& os) const {
  ...
  // Empty list
  if (instruction_queue.empty()) os << "-Empty-\n";
...
bool Thread::canInsertInLoadStoreQueue() {
  switch (Cpu::getLoadStoreQueueKind()) {
    ...
    case Cpu::LoadStoreQueueKindShared:
      ...
      return core->getLoadStoreQueueOccupancy() < Cpu::getLoadStoreQueueSize();
