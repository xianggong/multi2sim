...
Thread::DispatchStall Thread::canDispatch() {
  ...
  if (!canInsertInReorderBuffer()) return DispatchStallReorderBuffer;
  ...
  if (!register_file->canRename(uop)) return DispatchStallRename;
...
int Thread::Dispatch(int quantum) {
  ...
  while (quantum) {
    ...
    core->incDispatchStall(
        uop->speculative_mode ? DispatchStallSpeculative : DispatchStallUsed,
        1);
    ...
    // the trace cache
    if (uop->from_trace_cache) trace_cache->incNumDispatchedUinsts();
