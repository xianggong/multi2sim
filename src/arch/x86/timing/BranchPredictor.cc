...
misc::StringMap BranchPredictor::KindMap = {
    {"Perfect", KindPerfect},   {"Taken", KindTaken},
    {"NotTaken", KindNottaken}, {"Bimodal", KindBimod},
    {"TwoLevel", KindTwoLevel}, {"Combined", KindCombined}};

BranchPredictor::BranchPredictor(const std::string& name) : name(name) {
  // Initialize
  ...
  if (kind == KindBimod || kind == KindCombined) {
    ...
    for (int i = 0; i < bimod_size; i++) bimod[i] = (char)2;
  }
  ...
  if (kind == KindTwoLevel || kind == KindCombined) {
    ...
    two_level_pht =
        misc::new_unique_array<char>(two_level_l2_size * two_level_l2_height);
  ...
  if (kind == KindCombined) {
    ...
    for (int i = 0; i < choice_size; i++) choice[i] = 2;
  }
...
void BranchPredictor::ParseConfiguration(misc::IniFile* ini_file) {
  ...
  kind = (Kind)ini_file->ReadEnum(section, "Kind", KindMap, KindTwoLevel);
  ...
  two_level_history_size =
      ini_file->ReadInt(section, "TwoLevel.HistorySize", 8);
...
BranchPredictor::Prediction BranchPredictor::Lookup(Uop* uop) {
  ...
  // Taken predictor
  if (kind == KindTaken) uop->prediction = PredictionTaken;

  // Not-taken predictor
  if (kind == KindNottaken) uop->prediction = PredictionNotTaken;

  // Bimodal predictor
  if (kind == KindBimod || kind == KindCombined) {
    int bimod_index = uop->eip & (bimod_size - 1);
    Prediction bimod_prediction =
        bimod[bimod_index] > 1 ? PredictionTaken : PredictionNotTaken;
  ...
  if (kind == KindTwoLevel || kind == KindCombined) {
    ...
    Prediction two_level_prediction =
        two_level_pht[two_level_pht_row * two_level_l2_size +
                      two_level_pht_col] > 1
            ? PredictionTaken
            : PredictionNotTaken;
  ...
  // Return prediction
  assert(uop->prediction == PredictionTaken ||
...
void BranchPredictor::Update(Uop* uop) {
  ...
  // Stats
  accesses++;
  if (uop->neip == uop->predicted_neip) hits++;
  ...
  if (kind == KindPerfect) return;
  if (uop->getFlags() & Uinst::FlagUncond) return;
  ...
  if (kind == KindTwoLevel ||
      (kind == KindCombined && uop->choice_prediction == PredictionTaken)) {
    ...
    *bht_ptr = ((*bht_ptr << 1) | taken) & (two_level_l2_height - 1);
    ...
    pht_ptr = &two_level_pht[uop->two_level_pht_row * two_level_l2_size +
                             uop->two_level_pht_col];
    if (taken)
  ...
  // predictions differ.
  if (kind == KindCombined &&
      uop->bimod_prediction != uop->two_level_prediction) {
...
unsigned int BranchPredictor::LookupBtb(Uop* uop) {
  ...
  if (kind == KindPerfect) return uop->neip;

  // Internal branch (string operations) always predicted to jump to itself
  if (uop->getUinst()->getOpcode() == Uinst::OpcodeIbranch) return uop->eip;
...
void BranchPredictor::UpdateBtb(Uop* uop) {
  ...
  if (found) {
    for (int way = 0; way < btb_num_ways; way++) {
      entry = &btb[set * btb_num_ways + way];
      if (entry->counter > found_entry->counter) entry->counter--;
    }
