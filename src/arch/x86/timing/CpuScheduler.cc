...
void Cpu::AllocateContext(Context* context) {
  ...
  Emulator::context_debug << misc::fmt(
      ...
      getCycle(), context->getId(), core->getId(), thread->getIdInCore());
  ...
  Timing::trace << misc::fmt(
      ...
      "thread=%d "
      "ppid=%d\n",
      context->getId(), core->getId(), thread->getIdInCore(),
      context->getParentId());
}
...
void Cpu::MapContext(Context* context) {
  ...
  for (int i = 0; i < getNumCores(); i++) {
    ...
    for (int j = 0; j < core->getNumThreads(); j++) {
      ...
      if (!context->thread_affinity->Test(thread->getIdInCpu())) continue;

      // Check if this thread is better
      if (!found_thread ||
          thread->getNumMappedContexts() < found_thread->getNumMappedContexts())
        found_thread = thread;
    }
  }
  ...
  for (int i = 0; i < getNumCores(); i++) {
    ...
    for (int j = 0; j < core->getNumThreads(); j++) {
      ...
      if (context && !context->evict_signal &&
          context->allocate_cycle < min_context_allocate_cycle)
        min_context_allocate_cycle = context->allocate_cycle;
    }
  }
}
...
void Cpu::Schedule() {
  ...
  bool quantum_expired =
      getCycle() >= min_context_allocate_cycle + context_quantum;
  ...
  if (!quantum_expired && !emulator->schedule_signal) return;

  // Debug
  if (Emulator::context_debug) {
    Emulator::context_debug << misc::fmt("@%lld Schedule: ", getCycle());
    if (quantum_expired)
      Emulator::context_debug << misc::fmt(
          "quantum expired "
          "(set at %lld)\n",
  ...
  for (auto it = emulator->getRunningContextsBegin(),
    ...
    // Check if not mapped
    if (!context->getState(Context::StateMapped)) MapContext(context);
  }
