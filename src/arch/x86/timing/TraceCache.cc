...
void TraceCache::ParseConfiguration(misc::IniFile* ini_file) {
  ...
  if ((num_sets & (num_sets - 1)) || !num_sets)
    throw Error(misc::fmt("%s: 'Sets' must be a power of 2 greater than 0",
                          section.c_str()));
  if ((num_ways & (num_ways - 1)) || !num_ways)
    throw Error(misc::fmt("%s: 'Assoc' must be a power of 2 greater than 0",
                          section.c_str()));
  if (!trace_size)
    throw Error(
        misc::fmt("%s: Invalid value for 'TraceSize'", section.c_str()));
  if (!max_branches)
    throw Error(
        misc::fmt("%s: Invalid value for 'BranchMax'", section.c_str()));
  if (max_branches > trace_size)
    throw Error(
        misc::fmt("%s: 'BranchMax' must be equal or less than 'TraceSize'",
                  section.c_str()));
  if (max_branches > 31)
    throw Error(
        misc::fmt("%s: Maximum value for 'BranchMax' is 31", section.c_str()));
}
...
TraceCache::TraceCache(const std::string& name) : name(name) {
  // Initialize
...
void TraceCache::DumpReport(std::ostream& os) {
  ...
  os << misc::fmt(
      ...
      trace_length_count ? (double)trace_length_acc / trace_length_count : 0);
...
void TraceCache::RecordUop(Uop* uop) {
  ...
  if (temp->uop_count + uop->mop_count > trace_size) Flush();
  ...
  if (uop->mop_count > trace_size) return;

  // First instruction. Store trace tag.
  if (!temp->uop_count) temp->tag = uop->eip;
  ...
  if (uop->getFlags() & Uinst::FlagCtrl) {
    ...
    if (temp->branch_count == max_branches) Flush();
  }
}

bool TraceCache::Lookup(unsigned int eip, int pred, Entry*& found_entry,
                        unsigned int& neip) {
  // Debug
