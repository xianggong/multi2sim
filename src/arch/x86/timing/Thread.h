...
class Thread {
 ...
 public:
  ...
  bool isPipelineEmpty() const {
    return fetch_queue.empty() && uop_queue.empty() && reorder_buffer.empty();
  }
  ...
  enum DispatchStall {
    ...
    DispatchStallInstructionQueue,  // Stall due to no space in the instruction
                                    // queue
    DispatchStallLoadStoreQueue,    // Stall due to no space in the load-store
                                    // queue
