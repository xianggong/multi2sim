...
Thread::FetchStall Thread::canFetch() {
  ...
  if (!context) return FetchStallContext;

  // The context must be running
  if (!context->getState(Context::StateRunning)) return FetchStallSuspended;

  // The current context must not have been sent an eviction signal
  if (context->evict_signal) return FetchStallContext;
  ...
  unsigned block_address =
      fetch_neip & ~(instruction_module->getBlockSize() - 1);
  if (block_address != fetch_block_address) {
    ...
    unsigned physical_address =
        mmu->TranslateVirtualAddress(mmu_space, fetch_neip);
...
Uop* Thread::FetchInstruction(bool fetch_from_trace_cache) {
  ...
  while (context->getNumUinsts()) {
    ...
    // Create uop
    auto uop = misc::new_shared<Uop>(this, context, uinst);
    ...
    uop->first_speculative_mode =
        uinst_index == 0 && !previous_speculative_mode && speculative_mode;
    ...
    if (uop->getFlags() & Uinst::FlagMem) {
      ...
      uop->physical_address =
          mmu->TranslateVirtualAddress(mmu_space, uinst->getAddress());
    }
    ...
    if (Timing::trace) {
      ...
      // Speculative mode
      if (uop->speculative_mode) Timing::trace << " spec=\"t\"";

      // First instruction in speculative mode
      if (uop->first_speculative_mode) Timing::trace << " first_spec=\"t\"";

      // Macro-instruction disassembly
      if (!uinst_index)
        Timing::trace << " asm=\"" << *context->getInstruction() << "\"";

      // Micro-instruction disassembly
      Timing::trace << " uasm=\"" << *uinst << "\"";
    ...
    if (!ret_uop || (uop->getFlags() & Uinst::FlagCtrl)) ret_uop = uop.get();
    ...
    num_fetched_uinsts++;
    if (fetch_from_trace_cache) trace_cache->incNumFetchedUinsts();
...
bool Thread::FetchFromTraceCache() {
  ...
  if (trace_cache_queue_occupancy >= TraceCache::getQueueSize()) return false;

  // Access BTB, branch predictor, and trace cache
  unsigned eip_branch = branch_predictor->getNextBranch(
      fetch_neip, instruction_module->getBlockSize());
  int prediction = eip_branch
                       ? branch_predictor->LookupMultiple(
                             eip_branch, TraceCache::getMaxBranches())
                       : 0;
  TraceCache::Entry* entry;
  unsigned neip;
  bool hit = trace_cache->Lookup(fetch_neip, prediction, entry, neip);
  if (!hit) return 0;
  ...
  for (int i = 0; i < entry->getNumMacroInstructions(); i++) {
    ...
    if (!context->getState(Context::StateRunning)) break;
...
void Thread::Fetch() {
  ...
  if (TraceCache::isPresent() && FetchFromTraceCache()) return;
  ...
  if (block_address != fetch_block_address) {
    ...
    fetch_access =
        instruction_module->Access(mem::Module::AccessLoad, physical_address);
  ...
  while ((fetch_neip & ~(instruction_module->getBlockSize() - 1)) ==
    ...
    if (fetch_queue_occupancy >= Cpu::getFetchQueueSize()) break;
    ...
    if (!context->getInstruction()->getSize()) break;
    ...
    if (uop->getFlags() & Uinst::FlagCtrl) {
      ...
      BranchPredictor::Prediction prediction = branch_predictor->Lookup(uop);
      bool taken = prediction == BranchPredictor::PredictionTaken && target;
