...
void Alu::ParseConfiguration(misc::IniFile* ini_file) {
  ...
  for (int i = 1; i < FunctionalUnit::TypeCount; i++) {
    ...
    // Number of instances
    configuration[i][0] =
        ini_file->ReadInt(section, name + ".Count", configuration[i][0]);

    // Get total operation latency
    configuration[i][1] =
        ini_file->ReadInt(section, name + ".OpLat", configuration[i][1]);

    // Get issue latency
    configuration[i][2] =
        ini_file->ReadInt(section, name + ".IssueLat", configuration[i][2]);
  }
}
...
Alu::Alu() {
  ...
  for (int type = 1; type < FunctionalUnit::TypeCount; type++)
    functional_units[type] = misc::new_unique<FunctionalUnit>(
        (FunctionalUnit::Type)type, FunctionalUnit::type_map[type],
        configuration[type][0], configuration[type][1], configuration[type][2]);
}
...
int Alu::Reserve(Uop* uop) {
  ...
  if (!uop->first_alu_cycle) uop->first_alu_cycle = cycle;
  ...
  if (type == FunctionalUnit::TypeNone) return 1;
