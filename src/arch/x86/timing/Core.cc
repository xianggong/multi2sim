...
Core::Core(Cpu* cpu, int id) : cpu(cpu), id(id) {
  // Assign name
...
void Core::Dump(std::ostream& os) const {
  // Dump all threads
  for (auto& thread : threads) os << *thread;
}
...
void Core::Fetch() {
  ...
  switch (Cpu::getFetchKind()) {
    ...
    case Cpu::FetchKindTimeslice: {
      ...
      for (int i = 0; i < Cpu::getNumThreads(); i++) {
        // Next thread
        current_fetch_thread =
            (current_fetch_thread + 1) % Cpu::getNumThreads();
...
void Core::Decode() {
  ...
  for (auto& thread : threads) thread->Decode();
}
...
void Core::Dispatch() {
  ...
  switch (Cpu::getDispatchKind()) {
    case Cpu::DispatchKindShared: {
      do {
        // Next thread
        current_dispatch_thread =
            (current_dispatch_thread + 1) % Cpu::getNumThreads();
...
void Core::Issue() {
  switch (Cpu::getIssueKind()) {
    case Cpu::IssueKindShared: {
      ...
      do {
        // Next thread
        current_issue_thread =
            (current_issue_thread + 1) % Cpu::getNumThreads();
...
void Core::Writeback() {
  ...
  for (;;) {
    ...
    if (uop->complete_when > cpu->getCycle()) break;
...
void Core::Commit() {
  ...
  switch (Cpu::getCommitKind()) {
    case Cpu::CommitKindShared: {
      ...
      while (quantum && skip) {
        // Check next thread
        current_commit_thread =
            (current_commit_thread + 1) % Cpu::getNumThreads();
    ...
    case Cpu::CommitKindTimeslice: {
      ...
      while (true) {
        ...
        if (next_thread == current_commit_thread) break;

        // Found a thread to commit instructions from
        if (threads[next_thread]->canCommit()) break;
      }
      ...
      // Commit if we can
      if (thread->canCommit()) thread->Commit(Cpu::getCommitWidth());
