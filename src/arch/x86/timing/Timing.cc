...
const std::string Timing::help_message =
    ...
    "      Normally the Cpu uses cohnerency-enabled Store commands.  Setting "
    "this to True\n"
    "      causes the Cpu to issue NCStore commands to reduce protocol "
    "overhead.\n"
...
Timing::Timing() : comm::Timing("x86") {
  ...
  trace.Header(
      misc::fmt("x86.init version=\"%d.%d\" "
                "num_cores=%d num_threads=%d\n",
                trace_version_major, trace_version_minor, cpu->getNumCores(),
                cpu->getNumThreads()));
}
...
bool Timing::Run() {
  ...
  if (emulator->getNumContexts() == 0) return false;

  // Fast-forward simulation
  if (Cpu::getNumFastForwardInstructions() &&
      emulator->getNumInstructions() < Cpu::getNumFastForwardInstructions())
    FastForward();
  ...
  if (Emulator::getMaxInstructions() &&
      cpu->getNumCommittedInstructions() >=
          Emulator::getMaxInstructions() - Cpu::getNumFastForwardInstructions())
...
void Timing::ParseMemoryConfigurationEntry(misc::IniFile* ini_file,
  ...
  if (!(unified_present ^ data_inst_present))
    throw Error(
        misc::fmt("%s: section [%s]: invalid combination of "
  ...
  if (core_index >= Cpu::getNumCores() ||
    ...
    misc::Warning(
        "%s: section [%s] ignored, referring to x86 Core "
        "%d, Thread %d.\n"
        ...
        "is not desired.",
        ini_file->getPath().c_str(), section.c_str(), core_index, thread_index);
    return;
  }
  ...
  if (thread->data_module || thread->instruction_module) {
    ...
    throw Error(
        misc::fmt("%s: section [%s]: entry from Core %d, "
                  ...
                  "to avoid duplicates.",
                  ini_file->getPath().c_str(), section.c_str(), core_index,
                  thread_index));
  }
  ...
  if (data_inst_present) {
    ...
    data_module_name = instruction_module_name =
  ...
  if (!data_module)
    throw Error(
        misc::fmt("%s: section [%s]: '%s' is not a valid "
                  "module name.\n"
  ...
  mem::Module* instruction_module =
      memory_system->getModule(instruction_module_name);
  ...
  // Debug
  mem::System::debug << "\tx86 Core " << core_index << ", "
                                                       "Thread "
                     << thread_index << '\n'
                     << "\t\tEntry for instructions -> "
                     << instruction_module->getName() << '\n'
                     << "\t\tEntry for data -> " << data_module->getName()
                     << '\n'
                     << '\n';
}
...
void Timing::CheckMemoryConfiguration(misc::IniFile* ini_file) {
  ...
  for (int i = 0; i < cpu->getNumCores(); i++) {
    ...
    for (int j = 0; j < core->getNumThreads(); j++) {
      ...
      if (!thread->data_module || !thread->instruction_module)
        throw Error(
            misc::fmt("%s: x86 Core %d, Thread %d "
...
void Timing::RegisterOptions() {
  ...
  command_line->RegisterString(
      ...
      "processor cores and threads. Type 'm2s --x86-help' for details on the "
      "file "
      "format.");

  // Option --x86-report <file>
  command_line->RegisterString(
      "--x86-report <file>", report_file,
      "File to dump a report of the x86 Cpu pipeline, including statistics "
      "such "
      "as the number of instructions handled in every pipeline stage, "
      "read/write "
      "accesses performed on pipeline queues, etc. This option is only valid "
      "for "
  ...
  command_line->RegisterInt64(
      "--x86-max-cycles <cycles>", Cpu::max_cycles,
...
void Timing::ProcessOptions() {
  ...
  if (!report_file.empty()) {
    ...
    if (!os.good())
      throw Error(
          misc::fmt("%s: Cannot open report file", report_file.c_str()));
  }
...
void Timing::DumpSummary(std::ostream& os) const {
  ...
  double instructions_per_cycle =
      cpu->getCycle()
          ? (double)cpu->getNumCommittedInstructions() / cpu->getCycle()
          : 0.0;
  ...
  double uinsts_per_cycle =
      cpu->getCycle() ? (double)cpu->getNumCommittedUinsts() / cpu->getCycle()
                      : 0.0;
...
void Timing::DumpUopReport(std::ostream& os, const long long* uop_stats,
  ...
  for (int i = 0; i < Uinst::OpcodeCount; i++) {
    ...
    if (flags & Uinst::FlagInt) uinst_int_count += uop_stats[i];
    if (flags & Uinst::FlagLogic) uinst_logic_count += uop_stats[i];
    if (flags & Uinst::FlagFp) uinst_fp_count += uop_stats[i];
    if (flags & Uinst::FlagMem) uinst_mem_count += uop_stats[i];
    if (flags & Uinst::FlagCtrl) uinst_ctrl_count += uop_stats[i];
  ...
  os << misc::fmt("%s.IPC = %.4g\n", prefix.c_str(),
                  getCycle() ? (double)uinst_total / getCycle() : 0.0);
  os << misc::fmt("%s.DutyCycle = %.4g\n", prefix.c_str(),
                  getCycle() && peak_ipc
                      ? (double)uinst_total / getCycle() / peak_ipc
                      : 0.0);
  os << '\n';
}
...
void Timing::DumpReport() const {
  ...
  std::ofstream os(report_file);
  if (!os.good())
    throw Error(misc::fmt("%s: Cannot open x86 report", report_file.c_str()));
  ...
  os << misc::fmt("Time = %.2f\n", (double)now / 1e6);
  os << misc::fmt("CyclesPerSecond = %.0f\n",
                  now ? (double)getCycle() / now * 1e6 : 0.0);
  os << '\n';

  // Dispatch stage
  os << "; Dispatch stage\n";
  DumpUopReport(os, cpu->getNumDispatchedUinstArray(), "Dispatch",
                Cpu::getDispatchWidth());

  // Issue stage
  os << "; Issue stage\n";
  DumpUopReport(os, cpu->getNumIssuedUinstArray(), "Issue",
                Cpu::getIssueWidth());

  // Commit stage
  os << "; Commit stage\n";
  DumpUopReport(os, cpu->getNumCommittedUinstArray(), "Commit",
                Cpu::getCommitWidth());
  ...
  for (int i = 0; i < Cpu::getNumCores(); i++) {
    ...
    // Dispatch stage
    os << "; Dispatch stage\n";
    DumpUopReport(os, core->getNumDispatchedUinstArray(), "Dispatch",
                  Cpu::getDispatchWidth());

    // Issue stage
    os << "; Issue stage\n";
    DumpUopReport(os, core->getNumIssuedUinstArray(), "Issue",
                  Cpu::getIssueWidth());

    // Commit stage
    os << "; Commit stage\n";
    DumpUopReport(os, core->getNumCommittedUinstArray(), "Commit",
                  Cpu::getCommitWidth());
    ...
    for (int j = 0; j < Cpu::getNumThreads(); j++) {
      ...
      // Dispatch stage
      os << "; Dispatch stage\n";
      DumpUopReport(os, thread->getNumDispatchedUinstArray(), "Dispatch",
                    Cpu::getDispatchWidth());

      // Issue stage
      os << "; Issue stage\n";
      DumpUopReport(os, thread->getNumIssuedUinstArray(), "Issue",
                    Cpu::getIssueWidth());

      // Commit stage
      os << "; Commit stage\n";
      DumpUopReport(os, thread->getNumCommittedUinstArray(), "Commit",
                    Cpu::getCommitWidth());
      ...
      if (TraceCache::isPresent() && trace_cache) trace_cache->DumpReport(os);
    }
  }
}
