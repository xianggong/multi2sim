...
class Disassembler : public comm::Disassembler {
  ...
  void InitTable(Instruction::Opcode opcode, const char* name,
                 const char* fmt_str, Args&&... args) {
    int argv[sizeof...(args)];
    InitTableWithArray(opcode, name, fmt_str, 0, argv, args...);
  }

  template <typename... Args>
  void InitTableWithArray(Instruction::Opcode opcode, const char* name,
                          const char* fmt_str, int argc, int argv[], int arg,
                          Args&&... args) {
    argv[argc] = arg;
