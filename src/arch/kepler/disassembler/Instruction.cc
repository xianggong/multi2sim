...
misc::StringMap inst_sat_map = {{"", 0}, {".SAT", 1}};

misc::StringMap inst_x_map = {{"", 0}, {".X", 1}};

misc::StringMap inst_cc2_map = {
    {"CC.F", 0},        {"CC.LT", 1},       {"CC.EQ", 2},
    {"CC.LE", 3},       {"CC.GT", 4},       {"CC.NE", 5},
    {"CC.GE", 6},       {"CC.NUM", 7},      {"CC.NAN", 8},
    {"CC.LTU", 9},      {"CC.EQU", 10},     {"CC.LEU", 11},
    {"CC.GTU", 12},     {"CC.NEU", 13},     {"CC.GEU", 14},
    {"", 15},           {"CC.OFF", 16},     {"CC.LO", 17},
    {"CC.SFF", 18},     {"CC.LS", 19},      {"CC.HI", 20},
    {"CC.SFT", 21},     {"CC.HS", 22},      {"CC.OFT", 23},
    {"CC.CSM_TA", 24},  {"CC.CSM_TR", 25},  {"CC.CSM_MX", 26},
    {"CC.FCSM_TA", 27}, {"CC.FCSM_TR", 28}, {"CC.FCSM_MX", 29},
    {"CC.RLE", 30},     {"CC.RGT", 31}};

misc::StringMap inst_cc_map = {{"", 0}, {".CC", 1}};

misc::StringMap inst_hi_map = {{"", 0}, {".HI", 1}};

misc::StringMap inst_keeprefcount_map = {{"", 0}, {".KEEPREFCOUNT", 1}};

misc::StringMap inst_s_map = {{"", 0}, {".S", 1}};

misc::StringMap inst_and_map = {
    {".AND", 0}, {".OR", 1}, {".XOR", 2}, {".INVALIDBOP3", 3}};

misc::StringMap inst_cv_map = {{"", 0}, {".CG", 1}, {".CS", 2}, {".CV", 3}};

misc::StringMap inst_wt_map = {{"", 0}, {".CG", 1}, {".CS", 2}, {".WT", 3}};

misc::StringMap inst_po_map = {{"", 0}, {".PO", 1}};

misc::StringMap inst_u1_map = {{".U32.U32", 0}, {".U32.S32", 1}};

misc::StringMap inst_f_map = {{".F", 0},  {".LT", 1}, {".EQ", 2}, {".LE", 3},
                              {".GT", 4}, {".NE", 5}, {".GE", 6}, {".T", 7}};

misc::StringMap inst_u_map = {{"", 0}, {".U", 1}};

misc::StringMap inst_lmt_map = {{"", 0}, {".LMT", 1}};

misc::StringMap inst_e_map = {{"", 0}, {".E", 1}};

misc::StringMap inst_u32_map = {{".U32", 0}, {"", 1}};

misc::StringMap inst_rm_map = {{"", 0}, {".RM", 1}, {".RP", 2}, {".RZ", 3}};

misc::StringMap inst_us_map = {{".U32", 0}, {".S32", 1}};

misc::StringMap inst_u8_map = {{".U8", 0},  {".S8", 1},   {".U16", 2},
                               {".S16", 3}, {"", 4},      {".64", 5},
                               {".128", 6}, {".U.128", 7}};
...
Instruction::Instruction() { disassembler = Disassembler::getInstance(); }
...
void Instruction::DumpHex(std::ostream& os) const {
  os << misc::fmt("\n\t/*%04x*/     /*0x%08x%08x*/ \t", address,
                  bytes.as_uint[0], bytes.as_uint[1]);
}
...
void Instruction::DumpSRCB(std::ostream& os, int high0, int low0, int high1,
  ...
  if (value0 == 0)
    os << misc::fmt("c [0x%x] [0x%llx]", value1, valueConst);
  else if (value0 == 1) {
    if (value4 == 255)
      os << "RZ";
    else
...
void Instruction::DumpOffset(std::ostream& os, int high, int low) const {
  int value = misc::getBits64(bytes.as_dword, high, low);
  if (value) os << misc::fmt(" 0x%x", value);
}
...
void Instruction::Dump(std::ostream& os) const {
  ...
  while (*fmt_str) {
    ...
    if (comm::Disassembler::isToken(fmt_str, "tgt", length)) {
      ...
      throw misc::Panic(misc::fmt("%s: Unrecognized token", fmt_str));
    }
