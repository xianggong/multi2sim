...
void Grid::SetupSize(unsigned* thread_block_count,
  ...
  for (int i = 0; i < 3; i++) thread_block_count3[i] = thread_block_count[i];
  this->thread_block_count =
      thread_block_count3[0] * thread_block_count3[1] * thread_block_count3[2];

  // Setup thread-block sizes
  for (int i = 0; i < 3; i++) thread_block_size3[i] = thread_block_size[i];
  this->thread_block_size =
      thread_block_size3[0] * thread_block_size3[1] * thread_block_size3[2];

  // Calculate thread counts
  for (int i = 0; i < 3; i++)
    thread_count3[i] = thread_block_count[i] * thread_block_size[i];
  thread_count = thread_count3[0] * thread_count3[1] * thread_count3[2];
...
void Grid::GridSetupConstantMemory() {
  ...
  emulator->WriteConstantMemory(0x100000, sizeof(unsigned), (const char*)&v);
}

void Grid::WaitingToRunning(int thread_block_id, unsigned* id_3d) {
  running_thread_blocks.push_back(std::unique_ptr<ThreadBlock>(
      new ThreadBlock(this, thread_block_id, id_3d)));
...
void Grid::PopRunningThreadBlock() { running_thread_blocks.pop_front(); }
