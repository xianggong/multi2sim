...
ThreadBlock::ThreadBlock(Grid* grid, int id, unsigned* id_3d) {
  ...
  this->id = id;
  this->grid = grid;
  for (int i = 0; i < 3; i++) this->id_3d[i] = id_3d[i];

  // Create warps
  warp_count = (grid->getThreadBlockSize() + warp_size - 1) / warp_size;
  for (int i = 0; i < warp_count; ++i)
    warps.push_back(std::unique_ptr<Warp>(new Warp(this, i)));

  // Create threads
  thread_count = grid->getThreadBlockSize();
  for (int i = 0; i < thread_count; ++i)
    threads.push_back(
        std::unique_ptr<Thread>(new Thread(warps[i / warp_size].get(), i)));
  ...
  warps[warp_count - 1]->setThreadBegin(threads.begin() +
                                        warp_size * (warp_count - 1));
  warps[warp_count - 1]->setThreadEnd(threads.end());
  ...
  shared_memory_top_generic_address = shared_memory_top_address +
...
unsigned ThreadBlock::getWarpCount() const {
  return (grid->getThreadBlockSize() + warp_size - 1) / warp_size;
}
