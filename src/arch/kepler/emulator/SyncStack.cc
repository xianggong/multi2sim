...
unsigned SyncStack::getTempMaskBitCount() const {
  ...
  for (unsigned i = 0; i < thread_count; i++) num += (temp_mask >> i) & 1u;
...
unsigned SyncStack::getActiveMaskBitCount() const {
  ...
  for (unsigned i = 0; i < thread_count; i++) num += (active_mask >> i) & 1u;
...
void SyncStack::push(unsigned reconv_pc, unsigned active_mask,
                     SyncStackEntryType entry_type) {
  std::list<std::list<std::unique_ptr<SyncStackEntry>>::iterator> no_bra;

  SyncStackEntry* entry =
      new SyncStackEntry(reconv_pc, active_mask, entry_type);
...
void SyncStack::push() { push(0x0, 0x0, SyncStackEntryNotControl); }
...
bool SyncStack::checkBRK(unsigned& address) {
  ...
  if (!find_PBK) misc::Panic("No PBK is found in sync stack.\n");
...
void SyncStack::Dump(std::ostream& os) const {
  ...
  for (auto& pair : hash_table) {
    ...
    os << pair.second->get()->getActiveThreadMask() << "		"
       << pair.second->get()->getEntryType() << std::endl;
  }
