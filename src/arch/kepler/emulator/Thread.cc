...
Thread::Thread(Warp* warp, int id) {
  ...
  local_memory_top_generic_address = local_memory_top_address +
...
#undef DEFINST
  ...
  for (int i = 0; i < 256; ++i) WriteGPR(i, 0);
  ...
  for (int i = 0; i < 82; ++i) WriteSpecialRegister(i, 0);
  ...
  */
  ...
  WriteSpecialRegister(
      34, (id / grid->getThreadBlockSize3(0)) % grid->getThreadBlockSize3(1));
  WriteSpecialRegister(
      35, id / (grid->getThreadBlockSize3(0) * grid->getThreadBlockSize3(1)));
  WriteSpecialRegister(37,
                       thread_block->getId() % grid->getThreadBlockCount3(0));
  WriteSpecialRegister(38,
                       (thread_block->getId() / grid->getThreadBlockCount3(0)) %
                           grid->getThreadBlockCount3(1));
  WriteSpecialRegister(
      39, thread_block->getId() /
              (grid->getThreadBlockCount3(0) * grid->getThreadBlockCount3(1)));
  ...
  for (int i = 0; i < 7; ++i) WritePredicate(i, 0);
  WritePredicate(7, 1);
...
void Thread::ExecuteSpecial() { ExecuteInst_Special(); }

void Thread::ISAUnimplemented(Instruction* inst) {
  throw misc::Panic(
      misc::fmt("%s: Unimplemented Kepler "
                "instruction\n"
                "\n\t"
