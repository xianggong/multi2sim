...
void Thread::ExecuteInst_Special() {
  ...
  if ((id_in_warp == 0) && warp->getPC()) {
    unsigned temp_am;
    if (stack->pop(warp->getPC(), temp_am)) stack->setActiveMask(temp_am);
  }
...
void Thread::ExecuteInst_ISAD_A(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    if (format.u_s == 1)  // sign mode
    {
      ...
      src1_signed = src1;
      src2_signed = src2;
      dst = src3 + ((src1_signed > src2_signed) ? src1_signed - src2_signed
...
void Thread::ExecuteInst_BFI_B(Instruction* inst) { ISAUnimplemented(inst); }
...
void Thread::ExecuteInst_BFE_A(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    if (((format.op1 >> 5) & 1) == 1) src2 = src2 | 0xfff8000;
    ...
    unsigned temp;
    temp = src1;
    if (format.bit_reverse == 1) temp ^= 0xffffffff;
    if (size == 0)
      temp = 0;
...
void Thread::ExecuteInst_IMAD(Instruction* inst) {
  ...
  if (getenv("M2S_KPL_ISA_DEBUG")) {
    std::cerr << "Warp id " << std::hex << getWarpId() << " IMAD op0 "
              << format.op0;
    std::cerr << " dst " << format.dst << " mod0 " << format.mod0 << " s "
              << format.s << " srcB " << format.srcB

              << " mod1 " << format.mod1 << " op1 " << format.op1
              << " srcB_mod " << format.srcB_mod << std::endl;
  }
}
...
void Thread::ExecuteInst_IADD_A(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    if (format.x == 1) lsb = ReadCC_CF();  // Illegal to combine .PO and .X
  ...
  Emulator::isa_debug << misc::fmt(
      "At instruction %s:\tthe current PC is "
      "= %x\n",
      inst->getName(), warp->getPC());
  Emulator::isa_debug << misc::fmt(
      "At instruction %s:\tthe thread ID is "
      "= %u\n",
      inst->getName(), id_in_warp);
  Emulator::isa_debug << misc::fmt(
      "At instruction %s:\tinput: the src1 is = %d\n", inst->getName(), src1);
  Emulator::isa_debug << misc::fmt(
      "At instruction %s:\tinput: the src2 is = %d\n", inst->getName(), src2);
  Emulator::isa_debug << misc::fmt(
      "At instruction %s:\toutput: the result is "
      "= %d\n",
      inst->getName(), dst);
  Emulator::isa_debug << misc::fmt("\n");
}
...
void Thread::ExecuteInst_ISET_A(Instruction* inst) { ISAUnimplemented(inst); }
  ...
  if (active == 1 && pred == 1) {
    ...
    if (format.x == 1)
      ...
      if (cmp_op == 0)
        ...
        cmp_result = format.u_s ? ((signed)src1 < (signed)src2) : (src1 < src2);
      else if (cmp_op == 2)
        cmp_result =
            format.u_s ? ((signed)src1 == (signed)src2) : (src1 == src2);
      else if (cmp_op == 3)
        cmp_result =
            format.u_s ? ((signed)src1 <= (signed)src2) : (src1 <= src2);
      else if (cmp_op == 4)
    ...
    if (format.cc == 1) ISAUnsupportedFeature(inst);
...
void Thread::ExecuteInst_ISETP_A(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    if (((format.mod1 >> 3) & 0x1)) pred_3 = !pred_3;
    ...
    if (cmp_op == 1)
      ...
      std::cerr << __func__ << ": compare operation (" << cmp_op
                << ") not implemented\n";
    ...
    if (bool_op == 0) {
      ...
      std::cerr << __func__ << ": bitwise operation " << bool_op
                << " not implemented\n";

    /* Write */
    pred_id_1 = (format.dst >> 3) & 0x7;
    pred_id_2 = format.dst & 0x7;
    if (pred_id_1 != 7) WritePredicate(pred_id_1, pred_1);
    if (pred_id_2 != 7) WritePredicate(pred_id_2, pred_2);
  }
  ...
  if (getenv("M2S_KPL_ISA_DEBUG")) {
    std::cerr << "Warp id " << std::hex << getWarpId() << " ISETP op0 "
              << format.op0;
    std::cerr << " dst " << format.dst << " mod0 " << format.mod0 << " s "
              << format.s << " srcB " << format.srcB << " mod1 " << format.mod1
              << " op1 " << format.op1 << " srcB_mod " << format.srcB_mod
              << " cmp " << cmp_op << " bool " << bool_op << std::endl;
  }
}
...
void Thread::ExecuteInst_LOP_A(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    // Execute
    if (format.src1_negate == 1) src1 = ~src1;
    if (format.src2_negate == 1) src2 = ~src2;
    if (format.lop == 0)
      dst = src1 & src2;
...
void Thread::ExecuteInst_EXIT(Instruction* inst) {
  ...
  if (id_in_warp == 0) stack->resetTempMask();
  ...
  if (id_in_warp == warp->getThreadCount() - 1) {
    ...
    else if (warp->getFinishedThreadCount() > warp->getThreadCount())
...
void Thread::ExecuteInst_BRA(Instruction* inst) {
  ...
  int offset = format.offset >> 23 ? format.offset | 0xff000000 : format.offset;
    ...
    */
    ...
    // predicated BRA or non-predicated BRA
      ...
      if (branch_direction == 0) {
        ...
        stack->push(pc + inst_size + offset, stack->getTempMask(),
                    SyncStackEntryBRA);

        stack->setActiveMask(stack->getActiveMask() & ~stack->getTempMask());

        assert(active_thread == stack->getActiveMaskBitCount() + taken_thread);
      ...
      // is the target pc or next pc.
        ...
        // taken_thread < active_thread
        {
          ...
          if (stack->popTillTarget(target_pc, pc))
            throw misc::Panic(
                "Stack entries found between pc to"
...
void Thread::ExecuteInst_MOV_B(Instruction* inst) {
  ...
  if (getenv("M2S_KPL_ISA_DEBUG")) {
    std::cerr << "Warp id " << std::hex << getWarpId() << " MOV_B op0 "
              << format.op0;
    std::cerr << " dst " << format.dst << " mod0 " << format.mod0 << " s "
              << format.s << " srcB " << format.srcB << " mod1 " << format.mod1
              << " op1 " << format.op1 << " srcB_mod " << format.srcB_mod
...
void Thread::ExecuteInst_I2F_A(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    // Negate
    if (format.src_negate == 1) src = -src;

    // Absolute Value
    if (format.src_abs == 1) src = abs(src);
...
void Thread::ExecuteInst_F2I_A(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    // Absolute Value
    if (format.src_abs == 1) src = fabsf(src);
...
void Thread::ExecuteInst_F2F_A(Instruction* inst) { ISAUnimplemented(inst); }
...
void Thread::ExecuteInst_F2F_B(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    if ((format.s_fmt == 2) && (format.d_fmt != 3) && (format.ftz == 1)) {
      if (std::fpclassify(src) == FP_SUBNORMAL) src = 0.0f;
    }
...
void Thread::ExecuteInst_LD(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    {
      unsigned shared_memory_addr =
          addr - emulator->getGlobalMemoryTotalSize() -
          thread_block->getId() * thread_block->getSharedMemorySize();
    ...
    if (data_type > 4) WriteGPR(dst_id + 1, dst[1]);
    if (data_type > 5) {
  ...
  if (getenv("M2S_KPL_ISA_DEBUG")) {
    std::cerr << "Warp id " << std::hex << getWarpId() << " LD op0 "
              << format.op0;
    std::cerr << " dst " << format.dst << " mod0 " << format.mod0 << " s "
              << format.s << " srcB " << format.srcB << " mod1 " << format.mod1
              << " op1 " << format.op1 << " srcB_mod " << format.srcB_mod
...
void Thread::ExecuteInst_LDS(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    if (id_in_warp == 0 && getenv("M2S_KPL_ISA_DEBUG")) {
      ...
      std::cerr << " dst " << std::hex << format.dst << " mod0 " << format.mod0
                << " s " << format.s << " srcB " << format.srcB << " mod1 "
                << format.mod1 << " op1 " << format.op1 << " srcB_mod "
                << format.srcB_mod << std::endl;
    }
...
void Thread::ExecuteInst_LDC(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    if (format.u_or_s == 4)  // default case, unsigned 32
    {
      ...
      throw misc::Panic(
          "Unsupported feature in Kepler LDC instruction\n"
  ...
  if (getenv("M2S_KPL_ISA_DEBUG")) {
    std::cerr << "Warp id " << std::hex << getWarpId() << " LDC op0 "
              << format.op0;
    std::cerr << " dst " << format.dst << " src1 " << format.src1 << " pred "
              << format.pred << " S " << format.s << " src2_2 " << format.src2_2
              << " src2_1 " << format.src2_1 << " IS " << format.is << " u_or_s"
...
void Thread::ExecuteInst_ST(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    if (data_type > 4) src[1] = ReadGPR(src_id + 1);
    if (data_type > 5)  // Really? FIXME
    {
  ...
  if (getenv("M2S_KPL_ISA_DEBUG")) {
    std::cerr << "Warp id " << std::hex << getWarpId() << " ST op0 "
              << format.op0;
    std::cerr << " dst " << format.dst << " mod0 " << format.mod0 << " s "
              << format.s << " srcB " << format.srcB << " mod1 " << format.mod1
              << " op1 " << format.op1 << " srcB_mod " << format.srcB_mod
...
void Thread::ExecuteInst_STS(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    if (((format.mod1 >> 9) & 0x7) == 5) {
      ...
      thread_block->WriteToSharedMemory(dst + sizeof(int), sizeof(int),
                                        (char*)&src);
    }
...
void Thread::ExecuteInst_DADD(Instruction* inst) { ISAUnimplemented(inst); }
...
void Thread::ExecuteInst_FMUL(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    if (((format.mod1 >> 3) & 0x1) == 1) src1 = fabsf(src1);
    if (((format.mod1 >> 5) & 0x1) == 1) src1 = -src1;
    ...
    if (((format.mod1 >> 2) & 0x1) == 1) src2 = fabsf(src2);
    if (((format.mod1 >> 4) & 0x1) == 1) src2 = -src2;
...
/* void Thread::ExecuteInst_FADD(Instruction *inst)
{
        ...
        // Determine whether the warp reaches reconvergence pc.
        // If it is, pop the synchronization stack top and restore the active
mask
        // Only effect on thread 0 in warp
        if ((id_in_warp == 0) && warp->getPC())
        {
        ...
        if (active == 1 && pred == 1)
        {
                ...
                if (format.srcB_mod == 0)
                {
                        emulator->ReadConstantMemory(src_id << 2, 4,
(char*)&src2);
                }
                else if (format.srcB_mod == 1 || format.srcB_mod == 2)
                        src2 = ReadFloatGPR(src_id);
...
void Thread::ExecuteInst_FADD_B(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    // Src1 abs
    if (format.src1_abs == 1) src1 = fabsf(src1);

    // Src1 negate
    if (format.src1_negate == 1) src1 = -src1;

    // Src2 abs
    if (format.src2_abs == 1) src2 = fabsf(src2);

    // Src2 negate
    if (format.src2_negate == 1) src2 = -src2;

    if (format.ftz == 1) {
      if (std::fpclassify(src1) == FP_SUBNORMAL) src1 = 0.0f;
      if (std::fpclassify(src1) == FP_SUBNORMAL) src2 = 0.0f;
    }
    ...
    // Ftz for dst
    if (format.ftz == 1) {
      if (std::fpclassify(dst) == FP_SUBNORMAL) dst = 0.0f;
    }
    ...
    */
    ...
    // Saturate mode
    if (format.sat == 1) this->ISAUnsupportedFeature(inst);

    // Conditional code
    if (format.cc == 1) this->ISAUnsupportedFeature(inst);
...
void Thread::ExecuteInst_FFMA_A(Instruction* inst) { ISAUnimplemented(inst); }
...
void Thread::ExecuteInst_FFMA_B(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    if (format.fmz == 1) ISAUnsupportedFeature(inst);
...
*/
    // Saturate
    if (format.sat == 1) ISAUnsupportedFeature(inst);
...
void Thread::ExecuteInst_FSET_A(Instruction* inst) { ISAUnimplemented(inst); }
...
void Thread::ExecuteInst_FSET_B(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    if (cmp_op == 0)
      ...
      cmp_result =
          ((src1 < src2) && (!std::isnan(src1)) && (!std::isnan(src2)));
    else if (cmp_op == 2)
      cmp_result =
          ((src1 == src2) && (!std::isnan(src1)) && (!std::isnan(src2)));
    else if (cmp_op == 3)
...
void Thread::ExecuteInst_S2R(Instruction* inst) {
  ...
  if (getenv("M2S_KPL_ISA_DEBUG")) {
    std::cerr << "Warp id " << std::hex << getWarpId() << " S2R op0 "
              << format.op0;
    std::cerr << " dst " << format.dst << " mod0 " << format.mod0 << " s "
              << format.s << " srcB " << format.srcB << " mod1 " << format.mod1
              << " op1 " << format.op1 << " srcB_mod " << format.srcB_mod
...
void Thread::ExecuteInst_SHF(Instruction* inst) { ISAUnimplemented(inst); }
...
void Thread::ExecuteInst_BAR(Instruction* inst) {
  ...
  if (id_in_warp == warp->getThreadCount() - 1) {
    ...
    std::cout << " to " << thread_block->getWarpsAtBarrier()
                    << "warp count is " << thread_block->getWarpCount()<<
    std::endl;
                    */

    if (thread_block->getNumWarpsAtBarrier() == thread_block->getWarpCount()) {
...
void Thread::ExecuteInst_JMX(Instruction* inst) { ISAUnimplemented(inst); }

void Thread::ExecuteInst_JMP(Instruction* inst) { ISAUnimplemented(inst); }

void Thread::ExecuteInst_JCAL(Instruction* inst) { ISAUnimplemented(inst); }

void Thread::ExecuteInst_BRX(Instruction* inst) { ISAUnimplemented(inst); }
...
void Thread::ExecuteInst_CAL(Instruction* inst) {
  ...
  if (id_in_warp == warp->getThreadCount() - 1) {
    if (stack->getActiveMask()) {
      ...
      warp->getReturnAddressStack()->get()->push(
          warp->getPC() + 8, stack->getActiveMask(), sync_stack);
  ...
  if (getenv("M2S_KPL_ISA_DEBUG")) {
    std::cerr << "Warp id " << std::hex << getWarpId() << " CAL op0 "
              << format.op0 << " offset " << format.offset << " op1 "
              << format.op1 << std::endl;
  }
}

void Thread::ExecuteInst_PRET(Instruction* inst) { ISAUnimplemented(inst); }

void Thread::ExecuteInst_PLONGJMP(Instruction* inst) { ISAUnimplemented(inst); }
...
void Thread::ExecuteInst_PBK(Instruction* inst) {
  ...
  if (id_in_warp == warp->getThreadCount() - 1) {
    // Check this
    if (format.constant) {
      // emulator->ReadConstMem(format.offset << 2, sizeof(address), (char*)
      // &address);
      throw misc::Panic(
          "getting address from constant memory is "
          "not supported.\n");
    } else {
      // Get operand value
      if (format.offset >> 23 == 0)
        ...
        throw misc::Panic(
            "Negative PKB address offset."
            "Not supported.");
    }

    // Push the address and current active mask into stack
    stack->push(address, stack->getActiveMask(), SyncStackEntryPBK);
  }
  ...
  if (id_in_warp == warp->getThreadCount() - 1) {
    ...
    if (format.constant) {
      ...
      if (format.offset >> 23 == 0)
        ...
        // means offset is negative value
        throw misc::Panic(
            "Negative PCNT address offset."
            "Not supported.");
    }

    // Push the address and current active mask into stack
    stack->push(address, 0, SyncStackEntryPCNT);
  }
...
void Thread::ExecuteInst_LONGJMP(Instruction* inst) { ISAUnimplemented(inst); }
...
*/
  ...
  if (getenv("M2S_KPL_ISA_DEBUG")) {
    std::cerr << "RET Warp id " << std::hex << getWarpId() << " op0 "
              << format.op0 << " offset " << format.mod << " mod " << format.op1
              << std::endl;
    std::cerr << "From " << std::hex << warp->getPC() << " to " << return_addr
              << std::endl;
  }
...
void Thread::ExecuteInst_KIL(Instruction* inst) { ISAUnimplemented(inst); }
...
void Thread::ExecuteInst_RTT(Instruction* inst) { ISAUnimplemented(inst); }

void Thread::ExecuteInst_SAM(Instruction* inst) { ISAUnimplemented(inst); }

void Thread::ExecuteInst_RAM(Instruction* inst) { ISAUnimplemented(inst); }

void Thread::ExecuteInst_IDE(Instruction* inst) { ISAUnimplemented(inst); }
...
void Thread::ExecuteInst_FADD32I(Instruction* inst) { ISAUnimplemented(inst); }

void Thread::ExecuteInst_FFMA32I(Instruction* inst) { ISAUnimplemented(inst); }

void Thread::ExecuteInst_IMAD32I(Instruction* inst) { ISAUnimplemented(inst); }
...
void Thread::ExecuteInst_SHR_A(Instruction* inst) {
  ...
  if (active == 1 && pred == 1) {
    ...
    if (format.bit_reverse == 1) src1 ^= 0xffffffff;
