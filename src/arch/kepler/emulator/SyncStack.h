...
class SyncStack {
  class SyncStackEntry {
   ...
   public:
    ...
    void setActiveThreadMask(unsigned am) { active_thread_mask = am; }
  ...
  std::unordered_multimap<unsigned,
                          std::list<std::unique_ptr<SyncStackEntry>>::iterator>
      hash_table;
 ...
 public:
  SyncStack(unsigned thread_count) {
    ...
    temp_mask = 0;
    active_mask =
        (thread_count < 32) ? ((1u << thread_count) - 1) : unsigned(-1);
  }
  ...
  void setTempMaskBit(unsigned num) { temp_mask |= 1u << num; }

  /// Initial (or reset) temp_entry
  void resetTempMask() { temp_mask = 0u; }
  ...
  /// into stack
  void push(unsigned reconv_pc, unsigned active_mask,
            SyncStackEntryType entry_type);
