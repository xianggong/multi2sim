...
/// \return
///	The function does not have any return value.
int Driver::CallLaunchKernel(comm::Context* context, mem::Memory* memory,
                             unsigned args_ptr) {
  Kepler::Emulator* kpl_emu = Kepler::Emulator::getInstance();
  ...
  if (function == nullptr)
    throw Driver::Error(misc::fmt("Invalid function ID (%d)", function_id));
  ...
  for (int i = 0; i < function->getNumArguments(); ++i) {
    ...
    // std::cout<<"in function	"<<__FUNCTION__<<"the const
    // is"<<temp<<std::endl;
    offset += 0x4;
  }
...
int Driver::CallMemGetInfo(comm::Context* context, mem::Memory* memory,
                           unsigned args_ptr) {
...
/// \return
///	The function always returns 0
int Driver::CallModuleLoad(comm::Context* context, mem::Memory* memory,
                           unsigned args_ptr) {
  // Arguments
  unsigned path_ptr;
...
///
/// \return
///	The return value is a valid function id on success otherwise an error
/// will
///	be thrown
int Driver::CallModuleGetFunction(comm::Context* context, mem::Memory* memory,
                                  unsigned args_ptr) {
  // Read module id
  unsigned module_id;
