...
class Module {
 public:
  /// Module types
  enum Type { TypeInvalid = 0, TypeCache, TypeMainMemory, TypeLocalMemory };

  /// Address range
  enum RangeType { RangeInvalid = 0, RangeBounds, RangeInterleaved };

  /// Module access types
  enum AccessType { AccessInvalid = 0, AccessLoad, AccessStore, AccessNCStore };
 ...
 public:
  ...
  /// Constructor
  Module(const std::string& name, Type type, int num_ports, int block_size,
         int data_latency);

  /// Set the directory properties. This does not instantiate the
  /// directory, it just saves its properties internally.
  void setDirectoryProperties(int directory_num_sets, int directory_num_ways,
  ...
  void InitializeDirectory(int num_sets, int num_ways, int num_sub_blocks,
                           int num_nodes) {
    assert(!directory.get());
    directory = misc::new_unique<Directory>(name, num_sets, num_ways,
                                            num_sub_blocks, num_nodes);
  }
  ...
  void setCache(unsigned num_sets, unsigned num_ways, unsigned block_size,
                Cache::ReplacementPolicy replacement_policy,
                Cache::WritePolicy write_policy) {
    assert(!cache.get());
    cache = misc::new_unique<Cache>(name, num_sets, num_ways, block_size,
                                    replacement_policy, write_policy);
  }
  ...
  void addLowModule(Module* low_module) { low_modules.push_back(low_module); }
  ...
  ///	access.
  ///
  long long Access(AccessType access_type, unsigned address,
                   int* witness = nullptr, esim::Event* return_event = nullptr);
  ...
  /// nullptr.
  Frame* canCoalesce(AccessType access_type, unsigned address,
                     Frame* older_than_frame = nullptr);
  ...
  ///
  bool FindBlock(unsigned address, int& set, int& way, int& tag,
  ...
  std::list<Frame*>::iterator getAccessListBegin() { return accesses.begin(); }

  /// Return a past-the-end iterator for the list of in-flight accesses.
  std::list<Frame*>::iterator getAccessListEnd() { return accesses.end(); }
  ...
  void incRetryDirectoryEntryConflicts() {
