...
Mmu::Space::Space(const std::string& name, Mmu* mmu) : name(name), mmu(mmu) {
  // Debug
...
Mmu::Mmu(const std::string& name) : name(name) {
  // Debug
...
unsigned Mmu::TranslateVirtualAddress(Space* space, unsigned virtual_address) {
  ...
  if (page == nullptr) {
    // Create new page
    pages.emplace_back(new Page(space, virtual_tag, top_physical_address));
    ...
    if (debug)
      debug << misc::fmt(
          ...
          "Physical 0x%x\n",
          name.c_str(), space->getName().c_str(), virtual_tag,
  ...
  if (debug)
    debug << misc::fmt(
        "[MMU %s] Space %s, Virtual 0x%x => "
        "Physical 0x%x\n",
        name.c_str(), space->getName().c_str(), virtual_address,
        physical_address);
...
bool Mmu::TranslatePhysicalAddress(unsigned physical_address, Space*& space,
  ...
  if (debug)
    debug << misc::fmt(
        "[MMU %s] Physical 0x%x => "
        "Space %s, Virtual 0x%x\n",
        name.c_str(), physical_address, space->getName().c_str(),
        virtual_address);
