...
void System::EventLoadHandler(esim::Event* event, esim::Frame* esim_frame) {
  ...
  if (sanity_check_interval &&
      esim_engine->getTime() > last_sanity_check * sanity_check_interval) {
  ...
  if (event == event_load) {
    ...
    trace << misc::fmt(
        ...
        "state=\"%s:load\" "
        "addr=0x%x\n",
        frame->getId(), module->getName().c_str(), frame->getAddress());
    ...
    // Coalesce access
    Frame* master_frame =
        module->canCoalesce(Module::AccessLoad, frame->getAddress(), frame);
    if (master_frame) {
  ...
  if (event == event_load_lock) {
    debug << misc::fmt("  %lld A-%lld 0x%x %s load lock\n",
                       esim_engine->getTime(), frame->getId(),
                       frame->getAddress(), module->getName().c_str());
    trace << misc::fmt(
        "mem.access "
        "name=\"A-%lld\" "
        "state=\"%s:load_lock\"\n",
        frame->getId(), module->getName().c_str());
    ...
    older_frame = module->getInFlightAddress(frame->getAddress(), frame);
    if (older_frame) {
    ...
    auto new_frame =
        misc::new_shared<Frame>(frame->getId(), module, frame->getAddress());
    new_frame->request_direction = Frame::RequestDirectionUpDown;
    new_frame->blocking = true;
    new_frame->read = true;
    new_frame->retry = frame->retry;
    esim_engine->Call(event_find_and_lock, new_frame, event_load_action);
    return;
  }
  ...
  if (event == event_load_action) {
    ...
    // Miss
    auto new_frame =
        misc::new_shared<Frame>(frame->getId(), module, frame->tag);
    new_frame->target_module = module->getLowModuleServingAddress(frame->tag);
    new_frame->request_direction = Frame::RequestDirectionUpDown;
    esim_engine->Call(event_read_request, new_frame, event_load_miss);
  ...
  if (event == event_load_miss) {
    ...
    if (frame->error) {
      ...
      directory->UnlockEntry(frame->set, frame->way, frame->getId());
  ...
  if (event == event_load_unlock) {
    ...
    debug << misc::fmt(
        "  %lld A-%lld 0x%x %s "
        "load unlock\n",
        esim_engine->getTime(), frame->getId(), frame->getAddress(),
        module->getName().c_str());
    trace << misc::fmt(
        "mem.access "
        "name=\"A-%lld\" "
    ...
    esim_engine->Next(event_load_finish, module->getDataLatency());
    return;
  }
  ...
  if (event == event_load_finish) {
    ...
    if (frame->witness) (*frame->witness)++;
...
void System::EventStoreHandler(esim::Event* event, esim::Frame* esim_frame) {
  // Get useful objects
  esim::Engine* esim_engine = esim::Engine::getInstance();
  Frame* frame = misc::cast<Frame*>(esim_frame);
  Module* module = frame->getModule();
  ...
  if (event == event_store) {
    ...
    // Coalesce access
    Frame* master_frame =
        module->canCoalesce(Module::AccessStore, frame->getAddress(), frame);
    if (master_frame) {
      // Coalesce
  ...
  if (event == event_store_lock) {
    ...
    esim_engine->Call(event_find_and_lock, new_frame, event_store_action);
    return;
  }
  ...
  if (event == event_store_action) {
    ...
    esim_engine->Call(event_write_request, new_frame, event_store_unlock);
    return;
  }
  ...
  if (event == event_store_unlock) {
    ...
    // Update tag/state
    cache->setBlock(frame->set, frame->way, frame->tag, Cache::BlockModified);
    ...
    module->incDataAccesses();
    esim_engine->Next(event_store_finish, module->getDataLatency());
    return;
  }
...
void System::EventNCStoreHandler(esim::Event* event, esim::Frame* esim_frame) {
  // Get useful objects
  esim::Engine* esim_engine = esim::Engine::getInstance();
  Frame* frame = misc::cast<Frame*>(esim_frame);
  Module* module = frame->getModule();
  ...
  if (event == event_nc_store) {
    ...
    // Coalesce access
    Frame* master_frame =
        module->canCoalesce(Module::AccessNCStore, frame->getAddress(), frame);
    if (master_frame) {
  ...
  if (event == event_nc_store_lock) {
    ...
    new_frame->retry = frame->retry;
    esim_engine->Call(event_find_and_lock, new_frame, event_nc_store_writeback);
    return;
  }
  ...
  if (event == event_nc_store_writeback) {
    ...
    if (frame->state == Cache::BlockModified ||
      ...
      // Call 'evict'
      auto new_frame = misc::new_shared<Frame>(frame->getId(), module, 0);
      new_frame->set = frame->set;
      new_frame->way = frame->way;
      esim_engine->Call(event_evict, new_frame, event_nc_store_action);
      return;
    }
  ...
  if (event == event_nc_store_action) {
    ...
    switch (frame->state) {
      ...
      case Cache::BlockExclusive: {
        ...
        new_frame->target_module =
            module->getLowModuleServingAddress(frame->tag);
        esim_engine->Call(event_message, new_frame, event_nc_store_miss);
        break;
      }
      ...
      case Cache::BlockInvalid: {
        ...
        new_frame->target_module =
            module->getLowModuleServingAddress(frame->tag);
        new_frame->request_direction = Frame::RequestDirectionUpDown;
        esim_engine->Call(event_read_request, new_frame, event_nc_store_miss);
        break;
      }
  ...
  if (event == event_nc_store_unlock) {
    ...
    esim_engine->Next(event_nc_store_finish, module->getDataLatency());
    return;
  }
...
void System::EventFindAndLockHandler(esim::Event* event,
  ...
  if (event == event_find_and_lock) {
    debug << misc::fmt(
        ...
        esim_engine->getTime(), frame->getId(), frame->getAddress(),
        module->getName().c_str(), frame->blocking);
    trace << misc::fmt(
        "mem.access "
        "name=\"A-%lld\" "
  ...
  if (event == event_find_and_lock_port) {
    ...
    // Statistics
    module->incAccesses();
    if (frame->retry) module->incRetryAccesses();
    ...
    // Look for block
    frame->hit = module->FindBlock(frame->getAddress(), frame->set, frame->way,
                                   frame->tag, frame->state);
    if (frame->hit) {
      debug << misc::fmt(
          "    A-%lld 0x%x %s "
          "hit: set=%d, way=%d, "
          "state=%s\n",
          frame->getId(), frame->tag, module->getName().c_str(), frame->set,
          frame->way, Cache::BlockStateMap[frame->state]);
    }
    ...
    if (frame->write && frame->hit && frame->witness) (*frame->witness)++;
    ...
    if (directory->isEntryLocked(frame->set, frame->way) && !frame->blocking) {
      // Debug
      debug << misc::fmt(
          "    A-%lld 0x%x %s block locked at "
          "set=%d, "
          "way=%d "
          "by A-%lld - aborting\n",
          frame->getId(), frame->tag, module->getName().c_str(), frame->set,
          frame->way, directory->getEntryAccessId(frame->set, frame->way));
      ...
      if (frame->retry) module->incRetryDirectoryEntryConflicts();
    ...
    // retried.
    if (!directory->LockEntry(frame->set, frame->way, event_find_and_lock,
                              frame->getId())) {
      // Debug
      debug << misc::fmt(
          "    A-%lld 0x%x %s block locked at "
    ...
    if (!frame->hit) {
      ...
      assert(frame->state ||
             !directory->isBlockSharedOrOwned(frame->set, frame->way));

      // Debug
      debug << misc::fmt(
          "    A-%lld 0x%x %s miss -> lru: "
          "set=%d, "
          "way=%d, "
          "state=%s\n",
          frame->getId(), frame->tag, module->getName().c_str(), frame->set,
  ...
  if (event == event_find_and_lock_action) {
    ...
    debug << misc::fmt("  %lld A-%lld 0x%x %s find_and_lock_action\n",
                       esim_engine->getTime(), frame->getId(), frame->tag,
                       module->getName().c_str());
    trace << misc::fmt(
        "mem.access "
        "name=\"A-%lld\" "
    ...
    if (!frame->hit && frame->state) {
      ...
      new_frame->set = frame->set;
      new_frame->way = frame->way;
      esim_engine->Call(event_evict, new_frame, event_find_and_lock_finish);
      return;
    }
  ...
  if (event == event_find_and_lock_finish) {
    ...
    debug << misc::fmt(
        ...
        esim_engine->getTime(), frame->getId(), frame->tag,
        module->getName().c_str(), frame->error);
    trace << misc::fmt(
        "mem.access name=\"A-%lld\" "
    ...
    if (frame->eviction) {
      ...
      cache->setBlock(frame->set, frame->way, 0, Cache::BlockInvalid);
    }

    // If this is a main memory, the block is here. A previous miss
    // was just a miss in the directory.
    if (module->getType() == Module::TypeMainMemory && !frame->state) {
      frame->state = Cache::BlockExclusive;
      cache->setBlock(frame->set, frame->way, frame->tag, frame->state);
    }
...
void System::EventEvictHandler(esim::Event* event, esim::Frame* esim_frame) {
  // Engine, frame, module
  esim::Engine* esim_engine = esim::Engine::getInstance();
  Frame* frame = misc::cast<Frame*>(esim_frame);
  Frame* parent_frame = misc::cast<Frame*>(esim_engine->getParentFrame());
  ...
  if (event == event_evict) {
    ...
    // Debug and trace
    debug << misc::fmt(
        "  %lld A-%lld 0x%x %s evict "
        "(set=%d, way=%d, state=%s)\n",
        esim_engine->getTime(), frame->getId(), frame->tag,
        module->getName().c_str(), frame->set, frame->way,
    ...
    esim_engine->Call(event_invalidate, new_frame, event_evict_invalid);
    return;
  }
  ...
  if (event == event_evict_invalid) {
    ...
    if (module->getType() == Module::TypeMainMemory) {
      cache->setBlock(frame->src_set, frame->src_way, 0, Cache::BlockInvalid);
  ...
  if (event == event_evict_action) {
    ...
    frame->message = network->TrySend(source_node, low_node, message_size,
                                      event_evict_receive, event);
    if (frame->message)
      net::System::trace << misc::fmt(
          "net.msg_access "
          "net=\"%s\" "
          "name=\"M-%lld\" "
          "access=\"A-%lld\"\n",
          network->getName().c_str(), frame->message->getId(), frame->getId());
    return;
  }
  ...
  if (event == event_evict_receive) {
    ...
    trace << misc::fmt(
        ...
        frame->getId(), target_module->getName().c_str());
    ...
    // Call find-and-lock
    auto new_frame =
        misc::new_shared<Frame>(frame->getId(), target_module, frame->src_tag);
    ...
    if (frame->state == Cache::BlockNonCoherent)
      esim_engine->Call(event_find_and_lock, new_frame,
                        event_evict_process_noncoherent);
    else
      esim_engine->Call(event_find_and_lock, new_frame, event_evict_process);
    return;
  }
  ...
  if (event == event_evict_process) {
    ...
    if (frame->reply == Frame::ReplyAck) {
      ...
      if (frame->state == Cache::BlockExclusive) {
        ...
        target_cache->setBlock(frame->set, frame->way, frame->tag,
                               Cache::BlockModified);
    ...
    for (int z = 0; z < directory->getNumSubBlocks(); z++) {
      ...
      unsigned directory_entry_tag =
          frame->tag + z * target_module->getSubBlockSize();
      assert(directory_entry_tag <
             frame->tag + (unsigned)target_module->getBlockSize());
      if (directory_entry_tag < (unsigned)frame->src_tag ||
          directory_entry_tag >=
              frame->src_tag + (unsigned)module->getBlockSize())
        continue;

      Directory::Entry* directory_entry =
          directory->getEntry(frame->set, frame->way, z);
      directory->clearSharer(frame->set, frame->way, z,
                             module->getLowNetworkNode()->getIndex());
      if (directory_entry->getOwner() ==
          module->getLowNetworkNode()->getIndex())
        directory->setOwner(frame->set, frame->way, z, Directory::NoOwner);
    }
    ...
    esim_engine->Next(event_evict_reply, target_module->getDataLatency());
    return;
  }
  ...
  if (event == event_evict_process_noncoherent) {
    ...
    for (int z = 0; z < directory->getNumSubBlocks(); z++) {
      ...
      Directory::Entry* entry = directory->getEntry(frame->set, frame->way, z);
      int index = module->getLowNetworkNode()->getIndex();
      directory->clearSharer(frame->set, frame->way, z, index);
  ...
  if (event == event_evict_reply) {
    ...
    frame->message = network->TrySend(source_node, destination_node, 8,
                                      event_evict_reply_receive, event);
    if (frame->message)
      net::System::trace << misc::fmt(
          "net.msg_access "
          "net=\"%s\" "
...
void System::EventWriteRequestHandler(esim::Event* event,
  ...
  if (event == event_write_request) {
    ...
    assert(target_module->getLowModuleServingAddress(frame->getAddress()) ==
               module ||
    ...
    // Send message
    frame->message = network->TrySend(source_node, destination_node, 8,
                                      event_write_request_receive, event);
    if (frame->message)
      net::System::trace << misc::fmt(
          "net.msg_access "
          "net=\"%s\" "
  ...
  if (event == event_write_request_receive) {
    ...
    // Call 'find-and-lock'
    auto new_frame = misc::new_shared<Frame>(frame->getId(), target_module,
                                             frame->getAddress());
    new_frame->blocking =
        frame->request_direction == Frame::RequestDirectionDownUp;
  ...
  if (event == event_write_request_action) {
    ...
    esim_engine->Call(event_invalidate, new_frame,
  ...
  if (event == event_write_request_updown) {
    ...
    switch (frame->state) {
      ...
      case Cache::BlockNonCoherent: {
        auto new_frame =
            misc::new_shared<Frame>(frame->getId(), target_module, frame->tag);
        new_frame->target_module =
            target_module->getLowModuleServingAddress(frame->tag);
        new_frame->request_direction = Frame::RequestDirectionUpDown;
        if (frame->state == Cache::BlockInvalid) {
          new_frame->reply_size = target_module->getBlockSize() + 8;
          new_frame->setReplyIfHigher(Frame::ReplyAckData);
        } else {
          new_frame->reply_size = 8;
          new_frame->setReplyIfHigher(Frame::ReplyAck);
        }
        esim_engine->Call(event_write_request, new_frame,
  ...
  if (event == event_write_request_updown_finish) {
    ...
    if (frame->error) {
      ...
      // Unlock directory entry
      target_directory->UnlockEntry(frame->set, frame->way, frame->getId());
    ...
    for (int z = 0; z < target_directory->getNumSubBlocks(); z++) {
      ...
      if (directory_entry_tag > frame->getAddress() ||
          directory_entry_tag + (unsigned)module->getSubBlockSize() <=
              frame->getAddress())
        continue;
      ...
      Directory::Entry* entry =
          target_directory->getEntry(frame->set, frame->way, z);
      target_directory->setSharer(frame->set, frame->way, z, index);
      target_directory->setOwner(frame->set, frame->way, z, index);
  ...
  if (event == event_write_request_downup_finish) {
    ...
    // Set state to I
    target_cache->setBlock(frame->set, frame->way, 0, Cache::BlockInvalid);
  ...
  if (event == event_write_request_reply) {
    ...
    debug << misc::fmt(
        ...
        esim_engine->getTime(), frame->getId(), frame->tag,
        target_module->getName().c_str(), frame->reply_size);
    trace << misc::fmt(
        "mem.access "
        "name=\"A-%lld\" "
    ...
    // Send message
    frame->message =
        network->TrySend(source_node, destination_node, frame->reply_size,
                         event_write_request_finish, event);
    if (frame->message)
      net::System::trace << misc::fmt(
          "net.msg_access "
          "net=\"%s\" "
...
void System::EventReadRequestHandler(esim::Event* event,
  ...
  if (event == event_read_request) {
    ...
    // Send message
    frame->message = network->TrySend(source_node, destination_node, 8,
                                      event_read_request_receive, event);
    if (frame->message)
      net::System::trace << misc::fmt(
          "net.msg_access "
          "net=\"%s\" "
  ...
  if (event == event_read_request_receive) {
    ...
    debug << misc::fmt("  %lld A-%lld 0x%x %s read_request_receive\n",
                       esim_engine->getTime(), frame->getId(),
                       frame->getAddress(), target_module->getName().c_str());
    trace << misc::fmt(
        "mem.access "
        "name=\"A-%lld\" "
  ...
  if (event == event_read_request_updown) {
    ...
    if (frame->state) {
      ...
      for (int z = 0; z < directory->getNumSubBlocks(); z++) {
        ...
        // No owner, skip
        if (directory_entry->getOwner() == Directory::NoOwner) continue;
        ...
        if (directory_entry_tag % owner_module->getBlockSize()) continue;
        ...
        new_frame->request_direction = Frame::RequestDirectionDownUp;
        esim_engine->Call(event_read_request, new_frame,
  ...
  if (event == event_read_request_updown_miss) {
    ...
    // Also set the tag of the block.
    Cache* target_cache = target_module->getCache();
    target_cache->setBlock(
        frame->set, frame->way, frame->tag,
        frame->shared ? Cache::BlockShared : Cache::BlockExclusive);
  ...
  if (event == event_read_request_updown_finish) {
    ...
    for (int z = 0; z < directory->getNumSubBlocks(); z++) {
      ...
      if (directory_entry_tag < frame->getAddress() ||
          directory_entry_tag >=
              frame->getAddress() + (unsigned)module->getBlockSize())
        continue;

      Directory::Entry* entry = directory->getEntry(frame->set, frame->way, z);
      directory->setSharer(frame->set, frame->way, z,
                           module->getLowNetworkNode()->getIndex());
    ...
    if (!shared) {
      for (int z = 0; z < directory->getNumSubBlocks(); z++) {
        ...
        // Set owner
        directory->setOwner(frame->set, frame->way, z,
                            module->getLowNetworkNode()->getIndex());
      }
    }
  ...
  if (event == event_read_request_downup) {
    ...
    for (int z = 0; z < target_directory->getNumSubBlocks(); z++) {
      unsigned directory_entry_tag =
          frame->tag + z * (unsigned)target_module->getSubBlockSize();
      assert(directory_entry_tag <
             frame->tag + (unsigned)target_module->getBlockSize());
      Directory::Entry* entry =
          target_directory->getEntry(frame->set, frame->way, z);
      ...
      // No owner
      if (entry->getOwner() == Directory::NoOwner) continue;
      ...
      if (directory_entry_tag % owner->getBlockSize()) continue;
  ...
  if (event == event_read_request_downup_finish) {
    ...
    switch (frame->reply) {
      case Frame::ReplyAckData: {
        ...
        // sub-blocks to 0.
        for (int z = 0; z < target_directory->getNumSubBlocks(); z++)
          target_directory->setOwner(frame->set, frame->way, z,
                                     Directory::NoOwner);
  ...
  if (event == event_read_request_reply) {
    ...
    frame->message =
        network->TrySend(source_node, destination_node, frame->reply_size,
                         event_read_request_finish, event);
    if (frame->message)
      net::System::trace << misc::fmt(
          "net.msg_access "
          "net=\"%s\" "
...
void System::EventInvalidateHandler(esim::Event* event,
  ...
  if (event == event_invalidate) {
    ...
    // 'except_module'.
    for (int z = 0; z < directory->getNumSubBlocks(); z++) {
      unsigned directory_entry_tag = frame->tag + z * module->getSubBlockSize();
      assert(directory_entry_tag <
             frame->tag + (unsigned)module->getBlockSize());

      // Skip other sub-blocks
      if (frame->partial_invalidation &&
          (frame->getAddress() < directory_entry_tag ||
           frame->getAddress() >=
               directory_entry_tag + module->getSubBlockSize()))
        continue;
      ...
      for (int i = 0; i < directory->getNumNodes(); i++) {
        ...
        if (directory_entry_tag % sharer->getBlockSize()) continue;

        // One more pending request
        frame->pending++;

        // Send write request upwards if beginning of block
        auto new_frame = misc::new_shared<Frame>(frame->getId(), module,
                                                 directory_entry_tag);
...
void System::EventMessageHandler(esim::Event* event, esim::Frame* esim_frame) {
  // Get useful objects
  esim::Engine* esim_engine = esim::Engine::getInstance();
  Frame* frame = misc::cast<Frame*>(esim_frame);
  Frame* parent_frame = misc::cast<Frame*>(esim_engine->getParentFrame());
  ...
  if (event == event_message) {
    ...
    net::Network* network = module->getLowNetwork();
    net::EndNode* source_node = module->getLowNetworkNode();
    net::EndNode* destination_node = target_module->getHighNetworkNode();

    // Send message
    frame->message = network->TrySend(source_node, destination_node, 8,
                                      event_message_receive, event);
  ...
  if (event == event_message_receive) {
    ...
    // Schedule event
    esim_engine->Call(event_find_and_lock, new_frame, event_message_action);
  ...
  if (event == event_message_action) {
    ...
    if (frame->message_type == Frame::MessageClearOwner) {
      ...
      for (int z = 0; z < target_module->getDirectorySize(); z++) {
        ...
        if (int(frame->getAddress()) ==
            frame->tag + z * target_module->getNumSubBlocks()) {
          // Clear the owner
  ...
  if (event == event_message_reply) {
    ...
    debug << misc::fmt(
        ...
        esim_engine->getTime(), frame->getId(), frame->tag,
        module->getName().c_str(), frame->reply_size);
...
void System::EventFlushHandler(esim::Event* event, esim::Frame* esim_frame) {
  // Get objects
  esim::Engine* esim_engine = esim::Engine::getInstance();
  Frame* frame = misc::cast<Frame*>(esim_frame);
  Module* module = frame->getModule();
  ...
  if (event == event_flush) {
    ...
    if (module->getType() == Module::TypeCache) module->RecursiveFlush();
...
void System::EventLocalLoadHandler(esim::Event* event,
  ...
  if (event == event_local_load_lock) {
    ...
    new_frame->read = true;
    new_frame->retry = frame->retry;
    esim_engine->Call(event_local_find_and_lock, new_frame,
