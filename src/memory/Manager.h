...
class Manager {
 protected:
  ...
  class Chunk {
   ...
   public:
    /// Constructor
    Chunk(unsigned address, unsigned size, bool is_allocated = false)
        : address(address), size(size), is_allocated(is_allocated) {}
    ...
    void setAllocated(bool is_allocated) { this->is_allocated = is_allocated; }

    /// Return is_allocated field
    bool isAllocated() const { return is_allocated; }

    /// Return the iterator in the chunks map
    std::map<unsigned, std::unique_ptr<Chunk>>::iterator getChunksIterator()
        const {
      return it_chunks;
    }

    /// set the iterator in the chunks map
    void setChunksIterator(
        std::map<unsigned, std::unique_ptr<Chunk>>::iterator it) {
      it_chunks = it;
    }

    /// Return the iterator in the holes map
    std::multimap<unsigned, Chunk*>::iterator getHolesIterator() const {
      assert(!is_allocated);
      return it_holes;
    }

    /// Set the iterator in the chunks map
    void setHolesIterator(std::multimap<unsigned, Chunk*>::iterator it) {
      it_holes = it;
    }

    /// Dump the chunk information
    void Dump(std::ostream& os = std::cout) const {
      std::string chunk_allocation_status = isAllocated() ? "Occupied" : "Free";
      os << misc::fmt("    0x%x - 0x%0x (size %d) --- %s\n", getAddress(),
                      getEndAddress(), getSize(),
  ...
  bool canHoleContain(Chunk* hole, unsigned size, unsigned alignment) const;
  ...
  unsigned getNextAlignedAddress(unsigned address, unsigned alignment) const;
