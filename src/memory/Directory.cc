...
Directory::Directory(const std::string& name, int num_sets, int num_ways,
                     int num_sub_blocks, int num_nodes)
    : name(name),
      num_sets(num_sets),
      num_ways(num_ways),
  ...
  // Initialize entries
  entries = misc::new_unique_array<Entry>(num_sets * num_ways * num_sub_blocks);
...
void Directory::setOwner(int set_id, int way_id, int sub_block_id, int owner) {
  ...
  System::trace << misc::fmt(
      "mem.set_owner dir=\"%s\" "
      "x=%d y=%d z=%d owner=%d\n",
      name.c_str(), set_id, way_id, sub_block_id, owner);
...
void Directory::setSharer(int set_id, int way_id, int sub_block_id,
                          int node_id) {
  // Sanity
  ...
  int bit_id = set_id * num_ways * num_sub_blocks * num_nodes +
               way_id * num_sub_blocks * num_nodes + sub_block_id * num_nodes +
               node_id;

  // Check if already set
  if (sharers[bit_id]) return;
  ...
  // Trace
  System::trace << misc::fmt(
      "mem.set_sharer dir=\"%s\" "
      "x=%d y=%d z=%d sharer=%d\n",
      name.c_str(), set_id, way_id, sub_block_id, node_id);
...
void Directory::clearSharer(int set_id, int way_id, int sub_block_id,
                            int node_id) {
  // Sanity
  assert(misc::inRange(set_id, 0, num_sets - 1));
  assert(misc::inRange(way_id, 0, num_ways - 1));
...
void Directory::clearAllSharers(int set_id, int way_id, int sub_block_id) {
  ...
  Entry* entry = getEntry(set_id, way_id, sub_block_id);
  if (entry->getNumSharers() == 0) return;
  ...
  for (int i = 0; i < num_nodes; i++) sharers.Set(bit_id + i, false);

  // Trace
  System::trace << misc::fmt(
      "mem.clear_all_sharers dir=\"%s\" "
      "x=%d y=%d z=%d\n",
      name.c_str(), set_id, way_id, sub_block_id);
...
bool Directory::isSharer(int set_id, int way_id, int sub_block_id,
                         int node_id) {
  // Sanity
  assert(misc::inRange(set_id, 0, num_sets - 1));
  assert(misc::inRange(way_id, 0, num_ways - 1));
...
bool Directory::isBlockSharedOrOwned(int set_id, int way_id) {
  ...
  for (int sub_block_id = 0; sub_block_id < num_sub_blocks; sub_block_id++) {
    Entry* entry = getEntry(set_id, way_id, sub_block_id);
    if (entry->getNumSharers() > 0 || entry->getOwner() != NoOwner) return true;
  }
...
void Directory::DumpSharers(int set_id, int way_id, int sub_block_id,
  ...
  for (int i = 0; i < num_nodes; i++)
    if (isSharer(set_id, way_id, sub_block_id, i)) os << misc::fmt("%d ", i);
  os << "}\n";
}

bool Directory::LockEntry(int set_id, int way_id, esim::Event* event,
                          long long access_id) {
  // Get lock
  assert(access_id > 0);
  assert(misc::inRange(set_id, 0, num_sets - 1));
  ...
  System::trace << misc::fmt(
      ...
      "set=%d "
      "way=%d\n",
      name.c_str(), access_id, set_id, way_id);
