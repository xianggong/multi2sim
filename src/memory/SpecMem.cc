...
SpecMem::Page* SpecMem::getPage(unsigned address) {
  ...
  while (page && page->address != address) page = page->next;
...
void SpecMem::AccessAligned(unsigned address, int size, char* buffer,
  ...
  if (!page) {
    ...
    if (access == Memory::AccessRead) {
      ...
      // will be ignored.
      if (num_pages >= MaxPages) return;

      // Create page
      page = newPage(address);
    } else {
      misc::panic("%s: invalid access type (%d)", __FUNCTION__, access);
    }
  }
...
SpecMem::SpecMem(Memory* memory) {
  ...
  this->memory = memory;
  num_pages = 0;
  for (unsigned i = 0; i < PageTableSize; i++) pages[i] = nullptr;
}
