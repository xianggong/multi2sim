...
Manager::Manager(Memory* memory) { this->memory = memory; }
...
void Manager::RegisterOptions() {
  ...
  command_line->RegisterString(
      "--mem-manager-debug <file>", debug_file,
...
unsigned Manager::Allocate(unsigned size, unsigned alignment) {
  ...
  for (auto it = holes.lower_bound(size); it != holes.end(); it++) {
    if (canHoleContain(it->second, size, alignment)) {
      ...
      unsigned address = AllocateIn(it->second, size, alignment);
      if (debug) {
        Dump(debug);
      }
      return address;
...
bool Manager::canHoleContain(Chunk* hole, unsigned size,
  ...
  unsigned aligned_address =
      getNextAlignedAddress(hole->getAddress(), alignment);
...
unsigned Manager::AllocateLarge(unsigned size) {
  ...
  unsigned int page_aligned_size =
      ceil((double)size / Memory::PageSize) * Memory::PageSize;
...
void Manager::Free(unsigned address) {
  ...
  if (it == chunks.end()) {
    throw Error(
        "Trying to free a pointer not created by Allocate "
...
void Manager::MergeHoles(Chunk* hole) {
  ...
  it++;
  if (it != chunks.end() && isInSamePage(addr, it->second->getAddress()) &&
  ...
  if (it != chunks.begin()) {
    ...
    if (isInSamePage(addr, chunk2->getAddress()) && (!chunk2->isAllocated())) {
...
Manager::Chunk* Manager::RequestOnePage(unsigned base_address) {
  ...
  if (addr == (unsigned)-1) throw misc::Error("Guest program out of memory.");
...
Manager::Chunk* Manager::CreateHole(unsigned addr, unsigned size) {
  ...
  auto pair = chunks.insert(std::make_pair(addr, std::unique_ptr<Chunk>(hole)));
...
Manager::Chunk* Manager::CreatePointer(unsigned addr, unsigned size) {
  ...
  // Insert it into the chunks map
  auto pair =
      chunks.insert(std::make_pair(addr, std::unique_ptr<Chunk>(pointer)));
...
bool Manager::isValidAddress(unsigned address) {
  ...
  if (it->second->getAddress() <= address &&
      it->second->getEndAddress() >= address && it->second->isAllocated())
    return true;
  else
    return false;
}
