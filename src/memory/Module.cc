...
Module::Module(const std::string& name, Type type, int num_ports,
               int block_size, int data_latency)
    : name(name),
      type(type),
...
bool Module::ServesAddress(unsigned address) const {
  ...
  if (range_type == RangeBounds)
    return address >= range.bounds.low && address <= range.bounds.high;

  // Interleaved addresses
  if (range_type == RangeInterleaved)
    return (address / range.interleaved.div) % range.interleaved.mod ==
...
Module* Module::getLowModuleServingAddress(unsigned address) const {
  ...
  for (Module* low_module : low_modules) {
    ...
    if (!low_module->ServesAddress(address)) continue;

    // Address served by more than one module
    if (server_module)
      throw Error(
          misc::fmt("%s: low modules '%s' "
                    "and '%s' both serve address 0x%x",
                    name.c_str(), server_module->getName().c_str(),
                    low_module->getName().c_str(), address));
  ...
  if (!server_module)
    throw Error(
        misc::fmt("Module %s: no lower module "
                  "serves address 0x%x",
                  name.c_str(), address));
...
Module* Module::getOwner(int set_id, int way_id, int sub_block_id) const {
  ...
  Directory::Entry* entry = directory->getEntry(set_id, way_id, sub_block_id);
  ...
  // No owner
  if (owner == Directory::NoOwner) return nullptr;
...
bool Module::canAccess(int address) const {
  ...
  if (num_locked_ports == num_ports) return false;

  // If no MSHR is given, module can be accessed
  if (!mshr_size) return true;

  // Module can be accessed if number of non-coalesced in-flight accesses
  // is smaller than the MSHR size.
  int num_non_coalesced_accesses = accesses.size() - num_coalesced_accesses;
  return num_non_coalesced_accesses < mshr_size;
}

long long Module::Access(AccessType access_type, unsigned address, int* witness,
                         esim::Event* return_event) {
  // Create a new event frame
  auto frame = misc::new_shared<Frame>(Frame::getNewId(), this, address);
...
void Module::StartAccess(Frame* frame, AccessType access_type) {
  ...
  frame->accesses_iterator = accesses.insert(accesses.end(), frame);

  // Insert in write access list
  if (access_type == AccessStore)
    frame->write_accesses_iterator =
        write_accesses.insert(write_accesses.end(), frame);
...
void Module::FinishAccess(Frame* frame) {
  ...
  if (!found) throw misc::Panic("Frame not found");
  ...
  for (auto it = accesses.begin(); it != accesses.end(); it++) {
    Frame* list_frame = *it;
    if (list_frame->master_frame == frame) list_frame->master_frame = nullptr;
  }
...
Frame* Module::getInFlightAddress(unsigned address, Frame* older_than_frame) {
  // Look for address
  ...
  for (auto it = range.first; it != range.second; ++it) {
    ...
    if (older_than_frame && frame->getId() >= older_than_frame->getId())
      continue;

    // Block address matches
    assert(frame->getAddress() >> log_block_size == address >> log_block_size);
    return frame;
  }
...
Frame* Module::getInFlightWrite(Frame* older_than_frame) {
  ...
  if (!older_than_frame)
    return write_accesses.size() ? write_accesses.back() : nullptr;
  ...
  while (it != accesses.begin()) {
    ...
    Frame* frame = *it;
    if (frame->access_type == AccessStore) return frame;
  }
...
void Module::Dump(std::ostream& os) const {
  ...
  for (unsigned set = 0; set < sets; set++) {
    for (unsigned way = 0; way < ways; way++) {
      ...
      if (!block->getState()) continue;

      // Start the element size with the tag of the block
      if (element_size < std::to_string(block->getTag()).length())
        element_size = std::to_string(block->getTag()).length();
      ...
      // directory entries.
      for (int sub_block_id = 0; sub_block_id < sub_blocks; sub_block_id++) {
        ...
        for (auto& high_module : high_modules) {
          ...
          if (directory->isSharer((int)set, (int)way, sub_block_id, index)) {
            if (entry_text_size < high_module->getName().length())
              entry_text_size = high_module->getName().length();
            sharers++;
          }
        }
        ...
        if (max_sharers[set] < sharers) max_sharers[set] = sharers;
      }

      // Check if this string is bigger than the previous
      // maximum value
      if (element_size < entry_text_size) element_size = entry_text_size;
    }
  }
  ...
  os << "| Module " << name.c_str()
     << std::string((ways * sub_blocks + 2) * element_size - 10 - name.length(),
                    ' ')
     << "|"
     << "\n";
  os << std::string((ways * sub_blocks + 2) * element_size, '=') << "\n";
  ...
  for (unsigned i = 0; i < ways; i++) {
    ...
    os << "|" << std::string(pad_size / 2, ' ') << way_id.c_str()
       << std::string(
              (sub_blocks * element_size) - 2 - way_id.length() - pad_size / 2,
              ' ')
       << "|";
  }
  ...
  for (unsigned set = 0; set < sets; set++) {
    ...
    // Print the set index
    os << "| " << std::left << std::setw(element_size - 3) << std::setfill(' ')
       << set_id.c_str() << "|";

    // The first row is the state (which is 5 letters)
    pad_size = element_size - 2 - 5;
    os << "|" << std::string(pad_size / 2, ' ') << "State"
       << std::string(element_size - 7 - pad_size / 2, ' ') << "|";
    ...
    for (unsigned way = 0; way < ways; way++) {
      ...
      os << "|" << std::string(pad_size / 2, ' ')
         << Cache::BlockStateMap[block->getState()]
         << std::string(sub_blocks * element_size - 3 - pad_size / 2, ' ')
         << "|";
    }
    ...
    os << "\n"
       ...
       << std::string(element_size * (ways * sub_blocks + 1), '-') << "\n";
    ...
    os << "|" << std::string(pad_size / 2, ' ') << "Tag"
       << std::string(element_size - 5 - pad_size / 2, ' ') << "|";
    ...
    for (unsigned way = 0; way < ways; way++) {
      ...
      if (!block->getState()) {
        ...
        // Print the tag
        pad_size = sub_blocks * element_size - 2 - tag.length();
        os << "|" << std::string(pad_size / 2, ' ') << tag.c_str()
           << std::string(
                  sub_blocks * element_size - 2 - tag.length() - pad_size / 2,
                  ' ')
           << "|";
      }
    }
    ...
    os << "\n"
       << "|" << std::string(element_size - 1, ' ')
       << std::string(element_size * (sub_blocks * ways + 1), '-') << "\n";

    // Start printing sharers of the sub-blocks one sharer
    // per line
    std::vector<int> printed_counter;
    printed_counter.resize(cache->getNumWays() * directory->getNumSubBlocks());

    // Minimum number of rows for sharer is 1
    int sharer_rows = 1;
    if (sharer_rows < max_sharers[set]) sharer_rows = max_sharers[set];
    for (int sharer_row = 0; sharer_row < sharer_rows; sharer_row++) {
      ...
      if (sharer_row == 0) {
        ...
        os << "|" << std::string(pad_size / 2, ' ') << "Sharers"
           << std::string(element_size - 9 - pad_size / 2, ' ') << "|";
      } else {
        os << "|" << std::string(element_size - 2, ' ') << "|";
      }
      ...
      // For each entry
      for (unsigned way = 0; way < ways; way++) {
        for (int sub_block = 0; sub_block < sub_blocks; sub_block++) {
          ...
          for (auto& high_module : high_modules) {
            ...
            int printed =
                printed_counter[way * directory->getNumSubBlocks() + sub_block];

            // Find a sharer module
            int index = getSharerIndex(high_module);
            if (directory->isSharer((int)set, (int)way, sub_block, index)) {
              ...
              if (printed > counting) {
                ...
                // printed
                // counter
                printed_counter[way * directory->getNumSubBlocks() +
                                sub_block]++;

                // Print
                int length = high_module->getName().length();
                pad_size = element_size - 2 - length;
                os << "|" << std::string(pad_size / 2, ' ')
                   << high_module->getName()
                   << std::string(element_size - 2 - length - pad_size / 2, ' ')
                   << "|";
                break;
              }
            }
    ...
    pad_size = element_size - 2 - 5;
    os << "|" << std::string(pad_size / 2, ' ') << "Owner"
       << std::string(element_size - 7 - pad_size / 2, ' ') << "|";
    ...
    for (unsigned way = 0; way < ways; way++) {
      for (int sub_block = 0; sub_block < sub_blocks; sub_block++) {
        ...
        if (!owner) {
          ...
          pad_size = element_size - 2 - length;
          os << "|" << std::string(pad_size / 2, ' ') << owner->getName()
             << std::string(element_size - 2 - length - pad_size / 2, ' ')
             << "|";
        }
      }
    ...
    // Set separator
    os << "\n"
       << "|" << std::string((2 + ways * sub_blocks) * element_size - 2, '=')
       << "|"
       << "\n";
  }
}
...
void Module::DumpReport(std::ostream& os) const {
  ...
  if (type == TypeCache) {
    ...
    os << "ReplacementPolicy = "
       << cache->ReplacementPolicyMap.MapValue(cache->getReplacementPolicy())
       << "\n";
    os << "WritePolicy = "
       << cache->WritePolicyMap.MapValue(cache->getWritePolicy()) << "\n";
  }
  ...
  os << misc::fmt(
      "CoalescedAccesses = %lld\n",
      num_coalesced_reads + num_coalesced_writes + num_coalesced_nc_writes);
  os << misc::fmt("RetriedAccesses = %lld\n", num_retry_accesses);
  os << misc::fmt("Evictions = %lld\n", num_evictions);

  // Statistics - Hits and misses
  long long int num_hits = num_read_hits + num_write_hits + num_nc_write_hits;
  os << misc::fmt("Hits = %lld\n", num_hits);
  os << misc::fmt("Misses = %lld\n", num_accesses - num_hits);
  os << misc::fmt("HitRatio = %.4g\n",
                  num_accesses ? (double)num_hits / num_accesses : 0.0);
  os << "\n";
...
void Module::DumpInFlightAddresses(std::ostream& os) {
  ...
  for (auto& pair : in_flight_block_addresses) {
    ...
    os << misc::fmt(
        "\tkey (block_address) = 0x%x: "
        "id = %lld, "
        "address = 0x%x, "
        "block_address = 0x%x\n",
        block_address, frame->getId(), frame->getAddress(),
...
Frame* Module::canCoalesce(AccessType access_type, unsigned address,
  ...
  // null.
  auto tail =
      older_than_frame ? older_than_frame->accesses_iterator : accesses.end();
  ...
  switch (access_type) {
    case AccessLoad: {
      ...
      while (true) {
        ...
        Frame* frame = *it;
        if (frame->access_type != AccessLoad) return nullptr;

        // Same block address, coalesce
        if (frame->getAddress() >> log_block_size ==
            address >> log_block_size) {
          assert(!frame->master_frame || !frame->master_frame->master_frame);
          return frame->master_frame ? frame->master_frame : frame;
        }

        // Done when head reached
        if (it == accesses.begin()) break;
    ...
    case AccessStore: {
      ...
      Frame* frame = *tail;
      if (frame->access_type != AccessStore) return nullptr;

      // Only if it is an access to the same block
      if (frame->getAddress() >> log_block_size != address >> log_block_size)
        return nullptr;

      // Only if previous write has not started yet
      if (frame->port_locked) return nullptr;
    ...
    case AccessNCStore: {
      ...
      Frame* frame = *tail;
      if (frame->access_type != AccessNCStore) return nullptr;
...
void Module::Coalesce(Frame* master_frame, Frame* frame) {
  ...
  System::debug << misc::fmt(
      ...
      "on %s for 0x%x\n",
      frame->getId(), master_frame->getId(), name.c_str(), frame->getAddress());
...
void Module::LockPort(Frame* frame, esim::Event* event) {
  ...
  System::debug << misc::fmt(
      ...
      frame->getId(), port_index, name.c_str());
...
bool Module::FindBlock(unsigned address, int& set, int& way, int& tag,
  ...
  if (range_type == RangeInterleaved) {
    ...
    set =
        ((tag >> cache->getLogBlockSize()) / num_modules) % cache->getNumSets();
  } else if (range_type == RangeBounds) {
    set = (tag >> cache->getLogBlockSize()) % cache->getNumSets();
  } else {
    throw misc::Panic("Invalid range type");
  }
  ...
  for (way = 0; way < (int)cache->getNumWays(); way++) {
    ...
    if (block->getTag() == (unsigned)tag && state) return true;
...
void Module::Flush(int* witness) {
  ...
  // Create a new event frame
  auto new_frame = misc::new_shared<Frame>(Frame::getNewId(), this, 0);
...
void Module::FlushCache() {
  ...
  for (int set = 0; set < directory_num_sets; set++) {
    for (int way = 0; way < directory_num_ways; way++) {
      ...
      if (state == Cache::BlockState::BlockInvalid) continue;
      ...
      // Create new frame
      auto new_frame =
          misc::new_shared<Frame>(frame->getId(), this, frame->tag);
      new_frame->set = set;
      new_frame->way = way;
...
void Module::UpdateStats(Frame* frame) {
  ...
  if (frame->request_direction == Frame::RequestDirectionUpDown) {
    if (frame->read) {
      num_reads++;
      if (frame->retry) num_retry_reads++;
      if (frame->blocking)
        num_blocking_reads++;
      else
        num_non_blocking_reads++;
      if (frame->hit) {
        num_read_hits++;
        if (frame->retry) num_retry_read_hits++;
      } else {
        num_read_misses++;
        if (frame->retry) num_retry_read_misses++;
      }

    } else if (frame->nc_write)  // Must go after read
    {
      num_nc_writes++;
      if (frame->retry) num_retry_nc_writes++;
      if (frame->blocking)
        num_blocking_nc_writes++;
      else
        num_non_blocking_nc_writes++;
      if (frame->hit) {
        num_nc_write_hits++;
        if (frame->retry) num_retry_nc_write_hits++;
      } else {
        num_nc_write_misses++;
        if (frame->retry) num_retry_nc_write_misses++;
      }
    } else if (frame->write) {
      num_writes++;
      if (frame->retry) num_retry_writes++;
      if (frame->blocking)
        num_blocking_writes++;
      else
        num_non_blocking_writes++;
      if (frame->hit) {
        num_write_hits++;
        if (frame->retry) num_retry_write_hits++;
      } else {
        num_write_misses++;
        if (frame->retry) num_retry_write_misses++;
      }
    } else {
    ...
    if (frame->write) {
      num_write_probes++;
      if (frame->retry) num_retry_write_probes++;
    } else if (frame->read) {
      num_read_probes++;
      if (frame->retry) num_retry_read_probes++;
    } else {
      throw misc::Panic("Invalid memory operation type");
    }
  } else {
