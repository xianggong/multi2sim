...
class Directory {
 public:
  ...
  class Entry {
   ...
   public:
    ...
    void setNumSharers(int num_sharers) { this->num_sharers = num_sharers; }
  };
 ...
 public:
  ...
  ///
  Directory(const std::string& name, int num_sets, int num_ways,
  ...
  Entry* getEntry(int set_id, int way_id, int sub_block_id) {
    ...
    return &entries.get()[set_id * num_ways * num_sub_blocks +
                          way_id * num_sub_blocks + sub_block_id];
  }
  ...
  ///
  bool LockEntry(int set_id, int way_id, esim::Event* event,
                 long long access_id);
