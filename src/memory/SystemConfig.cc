  ...
  if (!esim::Engine::isValidFrequency(frequency))
    throw Error(
        misc::fmt("%s: The value for 'Frequency' "
                  ...
                  ini_file->getPath().c_str(), err_config_note));
...
void System::ConfigReadNetworks(misc::IniFile* ini_file) {
  ...
  for (auto it = ini_file->sections_begin(), e = ini_file->sections_end();
       it != e; ++it) {
    // Check if this is the name of a network
    std::string name = *it;
    if (strncasecmp(name.c_str(), "Network ", 8)) continue;
    name.erase(0, 8);
...
void System::ConfigInsertModuleInInternalNetwork(
    misc::IniFile* ini_file, Module* module, const std::string& network_name,
    const std::string& network_node_name, net::Network*& network,
    net::EndNode*& network_node) {
  // No network specified
  network = nullptr;
  network_node = nullptr;
  if (network_name.empty()) return;
  ...
  if (!network) {
    ConfigInsertModuleInExternalNetwork(ini_file, module, network_name,
  ...
  if (!network_node_name.empty())
    throw Error(
        misc::fmt("%s: %s: network node name should be "
                  "empty.\n%s",
                  ini_file->getPath().c_str(), module->getName().c_str(),
                  err_config_note));

  // Network should not have this module already
  if (network->getNodeByUserData(module))
    throw Error(
        misc::fmt("%s: network '%s' already contains "
                  "module '%s'.\n%s",
                  ini_file->getPath().c_str(), network->getName().c_str(),
                  module->getName().c_str(), err_config_note));

  // Read buffer sizes from network
  int default_input_buffer_size =
      ini_file->ReadInt(section, "DefaultInputBufferSize");
  int default_output_buffer_size =
      ini_file->ReadInt(section, "DefaultOutputBufferSize");
  if (!default_input_buffer_size)
    throw Error(
        misc::fmt("%s: network %s: variable "
  ...
  if (default_input_buffer_size < module->getBlockSize() + 8)
    throw Error(
        misc::fmt("%s: network %s: minimum input buffer "
                  "size is %d for cache '%s'.\n%s",
                  ini_file->getPath().c_str(), network->getName().c_str(),
                  module->getBlockSize() + 8, module->getName().c_str(),
                  err_config_note));
  if (default_output_buffer_size < module->getBlockSize() + 8)
    throw Error(
        misc::fmt("%s: network %s: minimum output buffer "
                  "size is %d for cache '%s'.\n%s",
                  ini_file->getPath().c_str(), network->getName().c_str(),
                  module->getBlockSize() + 8, module->getName().c_str(),
                  err_config_note));
  ...
  // Create node
  network_node =
      network->addEndNode(default_input_buffer_size, default_output_buffer_size,
...
void System::ConfigInsertModuleInExternalNetwork(
  ...
  if (!network)
    throw Error(misc::fmt("%s: %s: invalid network name.\n%s%s",
                          ini_file->getPath().c_str(), network_name.c_str(),
                          err_config_note, err_config_net));

  // Node name must be specified
  if (network_node_name.empty())
    throw Error(
        misc::fmt("%s: %s: network node name required for "
                  "external network.\n%s%s",
                  ini_file->getPath().c_str(), module->getName().c_str(),
                  err_config_note, err_config_net));
  ...
  // Get node
  network_node =
      dynamic_cast<net::EndNode*>(network->getNodeByName(network_node_name));
  if (!network_node)
    throw Error(
        misc::fmt("%s: network %s: node %s: invalid node "
                  "name.\n%s%s",
                  ini_file->getPath().c_str(), network_name.c_str(),
                  network_node_name.c_str(), err_config_note, err_config_net));

  // No module must have been assigned previously to this node
  if (network_node->getUserData())
    throw Error(
        misc::fmt("%s: network %s: node '%s' already "
                  "assigned.\n%s",
                  ini_file->getPath().c_str(), network_name.c_str(),
                  network_node_name.c_str(), err_config_note));

  // Network should not have this module already
  if (network->getNodeByUserData(module))
    throw Error(
        misc::fmt("%s: network %s: module '%s' is already "
                  "present.\n%s",
                  ini_file->getPath().c_str(), network_name.c_str(),
                  module->getName().c_str(), err_config_note));
...
Module* System::ConfigReadCache(misc::IniFile* ini_file,
  ...
  // Cache parameters
  std::string geometry_section =
      "CacheGeometry " + ini_file->ReadString(section, "Geometry");
  ...
  int directory_latency =
      ini_file->ReadInt(geometry_section, "DirectoryLatency", 0);
  ...
  Cache::ReplacementPolicy replacement_policy =
      (Cache::ReplacementPolicy)Cache::ReplacementPolicyMap.MapString(
          replacement_policy_str);
  if (!replacement_policy)
    throw Error(
        misc::fmt("%s: Cache %s: %s: "
                  "Invalid block replacement policy.\n%s",
                  ini_file->getPath().c_str(), module_name.c_str(),
                  replacement_policy_str.c_str(), err_config_note));

  // Check write policy
  Cache::WritePolicy write_policy =
      (Cache::WritePolicy)Cache::WritePolicyMap.MapString(write_policy_str);
  if (!write_policy)
    throw Error(
        misc::fmt("%s: Cache %s: %s: "
                  "Invalid write policy.\n%s",
                  ini_file->getPath().c_str(), module_name.c_str(),
                  write_policy_str.c_str(), err_config_note));
  if (write_policy == Cache::WriteThrough)
    misc::Warning(
        "%s: Cache %s: %s: Write policy "
        "not yet implemented, "
  ...
  if (num_sets < 1 || (num_sets & (num_sets - 1)))
    throw Error(misc::fmt(
        "%s: cache %s: number of sets must be a "
        "power of two greater than 1.\n%s",
        ini_file->getPath().c_str(), module_name.c_str(), err_config_note));
  if (num_ways < 1 || (num_ways & (num_ways - 1)))
    throw Error(misc::fmt(
        "%s: cache %s: associativity must be a "
        "power of two and > 1.\n%s",
        ini_file->getPath().c_str(), module_name.c_str(), err_config_note));
  if (block_size < 4 || (block_size & (block_size - 1)))
    throw Error(misc::fmt(
        "%s: cache %s: block size must be power "
        "of two and at least 4.\n%s",
        ini_file->getPath().c_str(), module_name.c_str(), err_config_note));
  if (directory_latency < 0)
    throw Error(misc::fmt(
        "%s: cache %s: invalid value for "
  ...
  // Create module
  Module* module =
      addModule(module_name, Module::TypeCache, num_ports, block_size, latency);
  ...
  std::string network_node_name =
      ini_file->ReadString(section, "HighNetworkNode");
  net::Network* network;
  net::EndNode* network_node;
  ConfigInsertModuleInInternalNetwork(ini_file, module, network_name,
                                      network_node_name, network, network_node);
  ...
  // Create cache
  module->setCache(num_sets, num_ways, block_size, replacement_policy,
                   write_policy);
...
Module* System::ConfigReadMainMemory(misc::IniFile* ini_file,
  ...
  if (block_size < 1 || (block_size & (block_size - 1)))
    throw Error(misc::fmt(
        "%s: %s: block size must be power of "
        "two.\n%s",
        ini_file->getPath().c_str(), module_name.c_str(), err_config_note));
  if (latency < 0)
    throw Error(misc::fmt(
        "%s: %s: invalid value for variable "
        "'Latency'.\n%s",
        ini_file->getPath().c_str(), module_name.c_str(), err_config_note));
  if (num_ports < 1)
    throw Error(misc::fmt(
  ...
  if (directory_size < 1 || (directory_size & (directory_size - 1)))
    throw Error(misc::fmt(
        "%s: %s: directory size must be a power "
        "of two.\n%s",
        ini_file->getPath().c_str(), module_name.c_str(), err_config_note));
  if (directory_num_ways < 1 || (directory_num_ways & (directory_num_ways - 1)))
    throw Error(misc::fmt(
        "%s: %s: directory associativity must be "
        "a power of two.\n%s",
        ini_file->getPath().c_str(), module_name.c_str(), err_config_note));
  ...
  // Create module
  Module* module = addModule(module_name, Module::TypeMainMemory, num_ports,
                             block_size, latency);

  // Initialize module
  int directory_num_sets = directory_size / directory_num_ways;
  module->setDirectoryProperties(directory_num_sets, directory_num_ways,
                                 directory_latency);
  ...
  // Create cache
  module->setCache(directory_num_sets, directory_num_ways, block_size,
                   Cache::ReplacementLRU, Cache::WriteBack);

  // Done
  return module;
}

void System::ConfigInvalidAddressRange(misc::IniFile* ini_file,
                                       Module* module) {
...
void System::ConfigReadModuleAddressRange(misc::IniFile* ini_file,
  ...
  if (tokens.empty()) ConfigInvalidAddressRange(ini_file, module);

  // First token - ADDR or BOUNDS
  if (!misc::StringCaseCompare(tokens[0], "BOUNDS")) {
    // Format is: BOUNDS <low> <high>
    if (tokens.size() != 3) ConfigInvalidAddressRange(ini_file, module);
    ...
    if (error)
      throw Error(
          misc::fmt("%s: %s: invalid value '%s' in "
                    "'AddressRange'",
                    ini_file->getPath().c_str(), module->getName().c_str(),
                    tokens[1].c_str()));
    if (low % module->getBlockSize())
      throw Error(
          misc::fmt("%s: %s: low address bound must "
    ...
    if ((high + 1) % module->getBlockSize())
      throw Error(
          misc::fmt("%s: %s: high address bound must "
    ...
    if (tokens.size() != 7) ConfigInvalidAddressRange(ini_file, module);
    ...
    if (error)
      throw Error(
          misc::fmt("%s: %s: invalid value '%s' in "
                    ...
                    ini_file->getPath().c_str(), module->getName().c_str(),
                    tokens[2].c_str()));
    if (div < 1) ConfigInvalidAddressRange(ini_file, module);
    if (div % module->getBlockSize())
      throw Error(
          misc::fmt("%s: %s: value for <div> must be "
    ...
    if (error)
      throw Error(
          misc::fmt("%s: %s: invalid value '%s' in "
                    ...
                    ini_file->getPath().c_str(), module->getName().c_str(),
                    tokens[4].c_str()));
    if (mod < 1) ConfigInvalidAddressRange(ini_file, module);
    ...
    if (error)
      throw Error(
          misc::fmt("%s: %s: invalid value '%s' in "
                    ...
                    ini_file->getPath().c_str(), module->getName().c_str(),
                    tokens[6].c_str()));
    if (eq >= mod) ConfigInvalidAddressRange(ini_file, module);
...
void System::ConfigReadModules(misc::IniFile* ini_file) {
  ...
  for (auto it = ini_file->sections_begin(), e = ini_file->sections_end();
    ...
    if (strncasecmp(section.c_str(), "Module ", 7)) continue;
  ...
  for (auto& module : modules) {
    ...
    if (!ini_file->Exists(section))
      throw misc::Panic(ini_file->getPath() + ": section '" + section +
                        "' does not exist");
...
void System::ConfigCheckRouteToMainMemory(misc::IniFile* ini_file,
                                          Module* module, int block_size,
                                          int level) {
  // Maximum levels
  if (level > 10)
    throw Error(misc::fmt(
        "%s: %s: too many cache levels.\n%s%s", ini_file->getPath().c_str(),
        module->getName().c_str(), err_levels, err_config_note));

  // Check block size
  if (module->getBlockSize() < block_size)
    throw Error(misc::fmt(
        "%s: %s: decreasing block size.\n%s%s", ini_file->getPath().c_str(),
        module->getName().c_str(), err_block_size, err_config_note));

  // Change current block size
  block_size = module->getBlockSize();

  // Dump current module
  debug << '\t' << std::string(level * 2, ' ') << module->getName() << '\n';

  // Check that cache has a way to main memory
  if (!module->getNumLowModules() && module->getType() == Module::TypeCache)
    throw Error(
        misc::fmt("%s: %s: main memory not accessible from "
                  "cache.\n%s",
                  ini_file->getPath().c_str(), module->getName().c_str(),
                  err_config_note));
  ...
  for (int i = 0; i < module->getNumLowModules(); i++) {
    ...
    ConfigCheckRouteToMainMemory(ini_file, low_module, block_size, level + 1);
  }
}
...
void System::ConfigReadLowModules(misc::IniFile* ini_file) {
  ...
  for (auto& module : modules) {
    ...
    if (module->getType() != Module::TypeCache) continue;
    ...
    std::string low_module_names = ini_file->ReadString(section, "LowModules");
    if (low_module_names.empty())
      throw Error(misc::fmt(
          "%s: %s: missing or invalid "
          "value for 'LowModules'.\n%s",
          ini_file->getPath().c_str(), section.c_str(), err_config_note));
  ...
  for (auto& module : modules)
    ConfigCheckRouteToMainMemory(ini_file, module.get(), module->getBlockSize(),
                                 1);
  debug << '\n';
}
...
void System::ConfigReadEntries(misc::IniFile* ini_file) {
  ...
  for (auto it = ini_file->sections_begin(), e = ini_file->sections_end();
    ...
    std::string section = *it;
    if (strncasecmp(section.c_str(), "Entry ", 6)) continue;
    ...
    if (!arch) {
      ...
      throw Error(
          misc::fmt("%s: section [%s]: '%s' is an "
                    "invalid value for 'Arch'.\n"
                    "\tPossible values are %s.\n%s",
                    ini_file->getPath().c_str(), section.c_str(),
                    arch_name.c_str(), names.c_str(), err_config_note));
    }
  ...
  for (auto it = arch_pool->getTimingBegin(), e = arch_pool->getTimingEnd();
       it != e; ++it) {
    comm::Arch* arch = *it;
...
void System::ConfigCreateSwitches(misc::IniFile* ini_file) {
  ...
  for (auto& network : networks) {
    ...
    if (default_bandwidth < 1)
      throw Error(
          misc::fmt("%s: %s: invalid or missing "
    ...
    // Create switch
    net::Node* network_switch = network->addSwitch(default_input_buffer_size,
                                                   default_output_buffer_size,
                                                   default_bandwidth, "Switch");
    ...
    for (int i = 0; i < network->getNumNodes(); i++) {
      ...
      if (network_node != network_switch) {
        std::string link_name =
            network_switch->getName() + "<->" + network_node->getName();
        network->addBidirectionalLink(
            link_name, network_node, network_switch, default_bandwidth,
            default_output_buffer_size, default_input_buffer_size, 1);
  ...
  for (auto& module : modules) {
    ...
    for (int i = 0; i < module->getNumLowModules(); i++) {
      ...
      if (module->getLowNetwork() != low_module->getHighNetwork())
        throw Error(
            misc::fmt("%s: %s: low node '%s' "
                      "is not in the same network.\n%s",
                      ini_file->getPath().c_str(), module->getName().c_str(),
                      low_module->getName().c_str(), err_config_note));

      // Check that there is a route
      net::Network* network = module->getLowNetwork();
      net::RoutingTable* routing_table = network->getRoutingTable();
      net::RoutingTable::Entry* entry = routing_table->Lookup(
          module->getLowNetworkNode(), low_module->getHighNetworkNode());
      if (!entry->getBuffer())
        throw Error(
            misc::fmt("%s: %s: network does not "
                      "connect '%s' with '%s'. %s",
                      ini_file->getPath().c_str(), network->getName().c_str(),
                      module->getName().c_str(), low_module->getName().c_str(),
                      err_connect));
    }
    ...
    for (int i = 0; i < module->getNumHighModules(); i++) {
      ...
      if (module->getHighNetwork() != high_module->getLowNetwork())
        throw Error(
            misc::fmt("%s: %s: high node '%s' "
                      "is not in the same network.\n%s",
                      ini_file->getPath().c_str(), module->getName().c_str(),
                      high_module->getName().c_str(), err_config_note));

      // Check that there is a route
      net::Network* network = module->getHighNetwork();
      net::RoutingTable* routing_table = network->getRoutingTable();
      net::RoutingTable::Entry* entry = routing_table->Lookup(
          module->getHighNetworkNode(), high_module->getLowNetworkNode());
      if (!entry->getBuffer())
        throw Error(
            misc::fmt("%s: %s: network does not "
                      "connect '%s' with '%s'. %s",
                      ini_file->getPath().c_str(), network->getName().c_str(),
                      module->getName().c_str(), high_module->getName().c_str(),
                      err_connect));
    }
...
void System::ConfigCalculateSubBlockSizes() {
  ...
  for (auto& module : modules) {
    ...
    for (int i = 0; i < module->getNumHighModules(); i++) {
      Module* high_module = module->getHighModule(i);
      sub_block_size = std::min(sub_block_size, high_module->getBlockSize());
    }
    ...
    int num_nodes = high_network && high_network->getNumNodes()
                        ? high_network->getNumNodes()
                        : 1;

    // Create directory
    module->InitializeDirectory(module->getDirectoryNumSets(),
                                module->getDirectoryNumWays(),
                                module->getNumSubBlocks(), num_nodes);
    Directory* directory = module->getDirectory();
    debug << misc::fmt(
        "\t%s - %dx%dx%d (%dx%dx%d effective) - "
        "%d entries, %d sub-blocks\n",
        module->getName().c_str(), directory->getNumSets(),
        directory->getNumWays(), num_nodes, directory->getNumSets(),
        directory->getNumWays(), module->getNumHighModules(),
        module->getDirectorySize(), module->getNumSubBlocks());
  }
...
void System::ConfigSetModuleLevel(Module* module, int level) {
  ...
  if (module->getLevel() >= level) return;
...
void System::ConfigTrace() {
  ...
  for (auto& mod : modules) {
    ...
    // Trace Header
    trace.Header(misc::fmt(
        "mem.new_mod name=\"%s\" num_sets=%u assoc=%u "
