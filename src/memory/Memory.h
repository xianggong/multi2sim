...
class Memory {
 public:
  ...
  class Page {
   ...
   public:
    /// Constructor
    Page(unsigned tag, unsigned perm) : tag(tag), perm(perm) {
    ...
    void AllocateData() {
      if (data == nullptr) data = misc::new_unique_array<char>(PageSize);
    }
 ...
 public:
  ...
  ///	argument \a access.
  void Access(unsigned address, unsigned size, char* buffer, AccessType access);
  ...
  void growHeapBreak(unsigned heap_break) {
    if (this->heap_break < heap_break) this->heap_break = heap_break;
  }
