...
void WelcomeMessage(std::ostream& os) {
  ...
  unsigned id =
      (tv.tv_sec * 1000000000 + tv.tv_usec) % (max_id - min_id + 1) + min_id;
  ...
  os << "; Please use command 'm2s --help' for a list of command-line "
        "options.\n";
...
void LoadProgram(const std::vector<std::string>& arguments,
  ...
  switch (elf_file.getMachine()) {
    ...
    default:
      ...
      throw misc::Error(
          misc::fmt("%s: unsupported ELF architecture", exe.c_str()));
  }

  // Load the program in selected emulator
  emulator->LoadProgram(arguments, environment, current_directory,
...
void LoadPrograms() {
  ...
  if (m2s_context_config.empty()) return;
  ...
  for (int index = 0;; index++) {
    ...
    if (!ini_file.Exists(section)) break;
    ...
    // Load program
    LoadProgram(arguments, environment, current_directory, stdin_file_name,
                stdout_file_name);
  }
}
...
void RegisterOptions() {
  ...
  command_line->RegisterString(
      "--call-debug <file>", m2s_debug_callstack,
  ...
  command_line->RegisterString(
      "--ctx-config <file>", m2s_context_config,
  ...
  // Debug information
  command_line->RegisterString(
      "--cuda-debug <file>", m2s_cuda_debug,
  ...
  // Debug information
  command_line->RegisterString(
      "--opencl-debug <file>", m2s_opencl_debug,
  ...
  // Device list
  command_line->RegisterString(
      "--opencl-devices <list>", m2s_opencl_devices,
  ...
  // Binary
  command_line->RegisterString(
      "--opencl-binary <file>", m2s_opencl_binary,
...
void ProcessOptions() {
  ...
  // Event-driven simulator debugger
  if (!m2s_debug_esim.empty()) esim::Engine::setDebugPath(m2s_debug_esim);
  ...
  // Visualization
  if (!m2s_visual_file.empty()) visual_run(m2s_visual_file.c_str());
}
...
void MainLoop() {
  ...
  while (!esim->hasFinished()) {
    ...
    arch_pool->Run(num_active_emulators, num_active_timing_simulators);
    ...
    if (num_active_timing_simulators) esim->ProcessEvents();
    ...
    // of system calls.
    m2s_loop_iterations++;
    if (m2s_max_time > 0 && !(m2s_loop_iterations & ((1 << 17) - 1)) &&
...
void DumpStatisticsSummary(std::ostream& os = std::cerr) {
  ...
  if (m2s_loop_iterations < 2) return;
