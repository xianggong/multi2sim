...
Bank::Bank(int id, Rank* parent, int num_rows, int num_columns, int num_bits)
    : id(id),
      rank(parent),
      num_rows(num_rows),
...
void Bank::ProcessRequest(std::shared_ptr<Request> request) {
  ...
  if (isPrechargedFuture()) {
    // Create the command.
    auto activate_command =
        std::make_shared<Command>(request, CommandActivate, cycle, this);
  ...
  // be open. (Row miss)
    ...
    // Create the commands.
    auto precharge_command =
        std::make_shared<Command>(request, CommandPrecharge, cycle, this);
    auto activate_command =
        std::make_shared<Command>(request, CommandActivate, cycle, this);
  ...
  if (request->getType() == RequestRead)
    access_command =
        std::make_shared<Command>(request, CommandRead, cycle, this);
  else if (request->getType() == RequestWrite)
    access_command =
        std::make_shared<Command>(request, CommandWrite, cycle, this);
  else
  ...
  System::debug << misc::fmt(
      "[%lld] Processed request for 0x%llx in "
      "bank %d\n",
      cycle, address->getEncoded(), id);
}
...
void Bank::RunFrontCommand() {
  ...
  System::activity << misc::fmt(
      "[%lld] [%d : %d] Running command #%d "
      "%s for 0x%llx\n",
      cycle, rank->getId(), id, command->getId(),
