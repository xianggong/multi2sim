...
Bank* OldestFirst::FindNext() {
  ...
  for (int i = 0; i < channel->getNumBanksTotal(); i++) {
    ...
    Bank* bank = channel->getRank(i / num_banks)->getBank(i % num_banks);
    ...
    // created.
    long long current_bank_cycle = bank->getFrontCommandCycleCreated();
...
Bank* RankBankRoundRobin::FindNext() {
  ...
  for (int i = 0; i < channel->getNumBanksTotal(); i++) {
    ...
    // to the first one.
    if (current_rank == channel->getNumRanks()) current_rank = 0;

    // Get a pointer to this bank.
    Bank* bank = channel->getRank(current_rank)->getBank(current_bank);
    ...
    System::debug << misc::fmt(
        "[%lld] Scheduler returns %d : %d "
        "for next command scheduling\n",
        cycle, current_rank, current_bank);
