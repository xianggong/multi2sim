...
Actions::Actions() {
  ...
  System::ACTION_REQUEST = esim->RegisterEvent(
      "ACTION_REQUEST", ActionRequestHandler, System::frequency_domain);
...
void Actions::ParseConfiguration(misc::IniFile& config) {
  ...
  while (1) {
    ...
    std::string current_action =
        config.ReadString("Actions", misc::fmt("Action[%d]", i));
...
void Actions::DoChecks() {
  ...
  for (unsigned e_num = 0; e_num < commands.size(); e_num++) {
    ...
    for (unsigned c_num = 0; c_num < checks.size(); c_num++) {
      ...
      if (checks[c_num].getType() != commands[e_num].getType()) continue;

      // Move to the next check if cycles don't match.
      if (checks[c_num].getCycle() != commands[e_num].getCycle()) continue;
  ...
  if (checks.size() > 0) {
    ...
    debug << misc::fmt(
        "Extra checks left over; %d commands "
        "weren't scheduled.\n",
        (int)checks.size());
