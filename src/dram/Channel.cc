...
Channel::Channel(int id, Controller* parent, int num_ranks, int num_banks,
                 int num_rows, int num_columns, int num_bits,
                 SchedulerType scheduler_type)
    : id(id), controller(parent), num_ranks(num_ranks), num_banks(num_banks) {
  // Create the ranks for this channel.
  for (int i = 0; i < num_ranks; i++)
    ranks.emplace_back(
        new Rank(i, this, num_banks, num_rows, num_columns, num_bits));

  switch (scheduler_type) {
    // Create a Rank Bank Round Robin scheduler.
    case SchedulerRankBankRoundRobin:
      scheduler = std::unique_ptr<Scheduler>(new RankBankRoundRobin(this));
      break;

    // Create an Oldest First scheduler.
    case SchedulerOldestFirst:
      scheduler = std::unique_ptr<Scheduler>(new OldestFirst(this));
      break;
  }
}
...
void Channel::RunScheduler() {
  ...
  System::debug << misc::fmt(
      "[%lld] Controller %d Channel %d running "
      "scheduler\n",
      ...
      switch (cmd_type) {
        ...
        case CommandPrecharge:
          ...
          if (rank_id == cmd_rank && bank_id == cmd_bank) {
            // A P s s
            ready = std::max(
                bank->getLastScheduledCommand(CommandActivate) +
                    controller->getTiming(TimingActivate, TimingPrecharge,
                                          TimingSame, TimingSame),
                ready);

            // R P s s
            ready =
                std::max(bank->getLastScheduledCommand(CommandRead) +
                             controller->getTiming(TimingRead, TimingPrecharge,
                                                   TimingSame, TimingSame),
                         ready);

            // W P s s
            ready =
                std::max(bank->getLastScheduledCommand(CommandWrite) +
                             controller->getTiming(TimingWrite, TimingPrecharge,
                                                   TimingSame, TimingSame),
                         ready);
          }
          break;
        ...
        case CommandActivate:
          ...
          if (rank_id == cmd_rank && bank_id == cmd_bank) {
            ...
            ready = std::max(
                bank->getLastScheduledCommand(CommandActivate) +
                    controller->getTiming(TimingActivate, TimingActivate,
                                          TimingSame, TimingSame),
                ready);

            // P A s s
            ready = std::max(
                bank->getLastScheduledCommand(CommandPrecharge) +
                    controller->getTiming(TimingPrecharge, TimingActivate,
                                          TimingSame, TimingSame),
                ready);
          }
        ...
        case CommandRead:
          ...
          if (rank_id == cmd_rank && bank_id == cmd_bank) {
            // A R s s
            ready =
                std::max(bank->getLastScheduledCommand(CommandActivate) +
                             controller->getTiming(TimingActivate, TimingRead,
                                                   TimingSame, TimingSame),
                         ready);
          }
          ...
          if (rank_id == cmd_rank) {
            ...
            ready = std::max(rank->getLastScheduledCommand(CommandRead) +
                                 controller->getTiming(TimingRead, TimingRead,
                                                       TimingSame, TimingAny),
                             ready);

            // W R s a
            ready = std::max(rank->getLastScheduledCommand(CommandWrite) +
                                 controller->getTiming(TimingWrite, TimingRead,
                                                       TimingSame, TimingAny),
                             ready);
          }
          ...
          if (rank_id != cmd_rank) {
            // R R d a
            ready =
                std::max(rank->getLastScheduledCommand(CommandRead) +
                             controller->getTiming(TimingRead, TimingRead,
                                                   TimingDifferent, TimingAny),
                         ready);

            // W R d a
            ready =
                std::max(rank->getLastScheduledCommand(CommandWrite) +
                             controller->getTiming(TimingRead, TimingRead,
                                                   TimingDifferent, TimingAny),
                         ready);
          }
        ...
        case CommandWrite:
          ...
          if (rank_id == cmd_rank && bank_id == cmd_bank) {
            // A W s s
            ready =
                std::max(rank->getLastScheduledCommand(CommandActivate) +
                             controller->getTiming(TimingActivate, TimingWrite,
                                                   TimingSame, TimingSame),
                         ready);
          }
          ...
          if (rank_id == cmd_rank) {
            // W W s a
            ready = std::max(rank->getLastScheduledCommand(CommandWrite) +
                                 controller->getTiming(TimingWrite, TimingWrite,
                                                       TimingSame, TimingAny),
                             ready);
          }
          ...
          // R W a a
          ready = std::max(rank->getLastScheduledCommand(CommandRead) +
                               controller->getTiming(TimingRead, TimingWrite,
                                                     TimingAny, TimingAny),
                           ready);
          break;
