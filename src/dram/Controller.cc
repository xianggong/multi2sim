...
misc::StringMap PagePolicyTypeMap{{"Open", PagePolicyOpen},
...
void Controller::ParseConfiguration(misc::IniFile* config,
  ...
  if (section_tokens.size() < 2)
    throw Error(
        misc::fmt("%s: MemoryController must have a name "
                  "in the form 'MemoryController <name>'.\n%s",
                  config->getPath().c_str(), System::err_config_note));
  name = section_tokens[1];

  // Load the page policy, defaulting to PagePolicyOpen
  page_policy = (PagePolicyType)config->ReadEnum(
      section, "PagePolicy", PagePolicyTypeMap, PagePolicyOpen);

  // Load the scheduling algorithm, defaulting to SchedulerOldestFirst.
  SchedulerType scheduler_type = (SchedulerType)config->ReadEnum(
      section, "SchedulingPolicy", SchedulerTypeMap, SchedulerOldestFirst);
  ...
  for (int i = 0; i < num_channels; i++)
    channels.emplace_back(new Channel(i, this, num_ranks, num_banks, num_rows,
                                      num_columns, num_bits, scheduler_type));
...
void Controller::ParseConfigurationTiming(misc::IniFile* ini_file,
  ...
  timings[TimingRead][TimingRead][TimingSame][TimingAny] =
      std::max(parameters.getTimeBurst(), parameters.getTimeCcd());
  // R R d a
...
void Controller::CreateRequestProcessor(int controller) {
  ...
  REQUEST_PROCESSORS[controller] = esim->RegisterEvent(
      ...
      Controller::RequestProcessorHandler, System::frequency_domain);
}
...
void Controller::CreateSchedulers(int num_channels) {
  ...
  for (int i = 0; i < num_channels; i++)
    SCHEDULERS[i] = esim->RegisterEvent(misc::fmt("%d_%d_SCHEDULER", id, i),
...
void Controller::RunRequestProcessor() {
  ...
  if (incoming_requests.size() == 0) return;
  ...
  if (incoming_requests.size() > 0) CallRequestProcessor();
}

void Controller::CommandReturnHandler(esim::Event* type, esim::Frame* frame) {
  // Get the command pointer out of the frame.
  CommandReturnFrame* command_frame = dynamic_cast<CommandReturnFrame*>(frame);
  ...
  System::activity << misc::fmt(
      "[%lld] [%d : %d] Finished command #%d "
      "%s for 0x%llx\n",
      cycle, command->getRankId(), command->getBankId(), command->getId(),
      command->getTypeString().c_str(), command->getAddress()->getEncoded());
}
