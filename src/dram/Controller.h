...
enum TimingLocation { TimingSame = 0, TimingDifferent, TimingAny = 0 };

/// Page Policy Types
enum PagePolicyType { PagePolicyOpen = 0, PagePolicyClosed };
...
class Controller {
 ...
 public:
  ...
  int getTiming(TimingCommand prev, TimingCommand next, TimingLocation rank,
                TimingLocation bank) const {
    return std::max(timings[prev][next][rank][bank], 1);
  }

  /// Returns the duration of a command of the specified type.
  int getCommandDuration(CommandType type) { return command_durations[type]; }
  ...
  esim::Event* getScheduler(int channel) { return SCHEDULERS[channel]; }
  ...
  friend std::ostream& operator<<(std::ostream& os, const Controller& object) {
    object.dump(os);
    return os;
  }
};
...
class CommandReturnFrame : public esim::Frame {
 ...
 public:
  CommandReturnFrame(std::shared_ptr<Command> command) : command(command) {}
