...
const std::string System::help_message =
    ...
    "\n"
    "Section [MemoryController <name>] defines a generic DRAM device. This "
    "section is\n"
    "used to define the modes of the DRAM, the size of the componants and the "
    "timings.\n"
    "The default configuration results in an 8GB DRAM device with typical "
    "timings for \n"
    "a DDR3 device running at 1600MHz.\n"
    "\n"
    "  PagePolicy = {Open|Closed} (Default = Open) \n"
    "      Policy that dictates whether the row of a bank remains open or "
    "closed.\n"
    "  SchedulingPolicy = {OldestFirst|BankRoundRobin} (Default = "
    "OldestFirst)\n"
...
void System::Destroy() { instance = nullptr; }

System::System() { debug << "Dram simulator initialized\n"; }
...
int System::getCapacity() {
  ...
  for (auto const& controller : controllers) {
    num_channels = std::max(num_channels, controller->getNumChannels());
    ...
    num_columns = std::max(column_size, controller->getNumColumns());
  }

  return num_controllers * num_channels * num_ranks * num_banks * num_rows *
         num_columns;
}
...
void System::RegisterOptions() {
  /*
          ...
          command_line->RegisterBool("--dram-help",
                          help,
                          "Print help message describing the network
     configuration"
...
void System::ParseConfiguration(misc::IniFile* ini_file) {
  ...
  frequency_domain =
      esim->RegisterFrequencyDomain("frequency_domain", frequency);

  // Create events used by the entire system
  event_command_return = esim->RegisterEvent(
      "command_return", Controller::CommandReturnHandler, frequency_domain);
  ...
  for (int i = 0; i < ini_file->getNumSections(); i++) {
    ...
    if (misc::StringPrefix(section_name, "MemoryController")) {
      controllers.emplace_back(
          new Controller(num_controller, ini_file, section_name));
      num_controller++;
    }
  }
...
void System::GenerateAddressSizes() {
  ...
  for (auto const& controller : controllers) {
    logical_size = std::max(logical_size, Log2(controller->getNumChannels()));
    rank_size = std::max(rank_size, Log2(controller->getNumRanks()));
    bank_size = std::max(bank_size, Log2(controller->getNumBanks()));
    row_size = std::max(row_size, Log2(controller->getNumRows()));
    column_size = std::max(column_size, Log2(controller->getNumColumns()));
  }
}
...
void System::Dump(std::ostream& os) const {
  ...
  for (auto& controller : controllers) controller->dump(os);
}
